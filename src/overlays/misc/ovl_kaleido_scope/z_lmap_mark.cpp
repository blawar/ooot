...
void PauseMapMark_DrawForDungeon(GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_lmap_mark.c", 182);
	...
	while(true)
	{
		...
		if((mapMarkData->markType == PAUSE_MAP_MARK_BOSS) && (globalCtx->sceneNum >= SCENE_YDAN_BOSS) && (globalCtx->sceneNum <= SCENE_GANON_FINAL))
		{
			if(gBossMarkState == 0)
			{
		...
		for(i = 0; i < mapMarkData->count; i++)
		{
			...
			if(display)
			{
				...
				gDPPipeSync(POLY_OPA_DISP++);
				gDPLoadTextureBlock(
				    POLY_OPA_DISP++, markInfo->texture, markInfo->imageFormat, G_IM_SIZ_MARK, markInfo->textureWidth, markInfo->textureHeight, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
				    G_TX_NOLOD);

				Matrix_Push();
				Matrix_Translate(GREG(92) + markPoint->x, GREG(93) + markPoint->y, 0.0f, MTXMODE_APPLY);
				Matrix_Scale(scale, scale, scale, MTXMODE_APPLY);
				gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_lmap_mark.c", 272), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				Matrix_Pop();
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_lmap_mark.c", 286);
}
