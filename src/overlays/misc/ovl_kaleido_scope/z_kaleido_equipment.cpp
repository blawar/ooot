...
void KaleidoScope_DrawEquipmentImage(GlobalContext* globalCtx, void* source, u32 width, u32 height, Vtx* equipVtx)
{
	...
	gDPSetTileCustom(POLY_OPA_DISP++, G_IM_FMT_RGBA, G_IM_SIZ_16b, width, textureHeight, 0, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
...
void KaleidoScope_DrawPlayerWork(GlobalContext* globalCtx)
{
	...
	rot.y = 32300;
	rot.x = rot.z = 0;
	func_8009214C(globalCtx, pauseCtx->playerSegment, &pauseCtx->playerSkelAnime, &pos, &rot, scale, CUR_EQUIP_VALUE(EQUIP_SWORD), CUR_EQUIP_VALUE(EQUIP_TUNIC) - 1, CUR_EQUIP_VALUE(EQUIP_SHIELD), CUR_EQUIP_VALUE(EQUIP_BOOTS) - 1);
}
	...
	if((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (page == KaleidoScope_CurrentPage()))
	{
			...
			while(cursorMoveResult == 0)
			{
				if(pauseCtx->stickRelX < -30)
				{
					...
					else
					{
						...
						page->cursorPoint() = page->cursorX() + (page->cursorY() * 4);
			...
			while(cursorMoveResult == 0)
			{
				if(pauseCtx->stickRelY > 30)
				{
					if(page->cursorY() != 0)
					{
						...
						else if(gBitFlags[page->cursorPoint() - 1] & gSaveContext.inventory.equipment)
						{
							cursorMoveResult = 2;
						}
					}
		...
		if(page->cursorX() == 0)
		{
			...
			if(LINK_AGE_IN_YEARS == YEARS_CHILD)
			{
				...
				{
					cursorItem = ITEM_QUIVER_30 + sUpgradeItemOffsets[page->cursorY()] + CUR_UPG_VALUE(page->cursorY()) - 1;
		...
		if((pauseCtx->cursorSpecialPos == 0) && (cursorItem != PAUSE_ITEM_NONE) && (pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (page->cursorX() != 0))
		{
		...
		for(k = 0, temp = rowStart + 1, bit = rowStart, j = point; k < 3; k++, bit++, j += 4, temp++)
		{
			if((gBitFlags[bit] & gSaveContext.inventory.equipment) && (pauseCtx->cursorSpecialPos == 0))
			{
				if((gEquipAgeReqs[i][k + 1] == 9) || (gEquipAgeReqs[i][k + 1] == ((void)0, gSaveContext.linkAge)))
				{
					if(temp == cursorSlot)
					{
						equipVtx[j].v.ob[0] = equipVtx[j + 2].v.ob[0] = equipVtx[j].v.ob[0] - 2;
						equipVtx[j + 1].v.ob[0] = equipVtx[j + 3].v.ob[0] = equipVtx[j + 1].v.ob[0] + 4;
						equipVtx[j].v.ob[1] = equipVtx[j + 1].v.ob[1] = equipVtx[j].v.ob[1] + 2;
						equipVtx[j + 2].v.ob[1] = equipVtx[j + 3].v.ob[1] = equipVtx[j + 2].v.ob[1] - 4;
					}
				}
			}
		}
