...
void EnHeishi3_Reset(Actor* pthisx, GlobalContext* globalCtx);
...
void EnHeishi3_StandSentinelInGrounds(EnHeishi3* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.xzDistToPlayer < sightRange) && (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < 100.0f) && (sPlayerCaught == 0))
	{
		sPlayerCaught = 1;
...
void EnHeishi3_StandSentinelInCastle(EnHeishi3* pthis, GlobalContext* globalCtx)
{
	...
	SkelAnime_Update(&pthis->skelAnime);
	if((player->actor.world.pos.x < -190.0f) && (player->actor.world.pos.x > -380.0f) && (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < 100.0f) && (player->actor.world.pos.z < 1020.0f) && (player->actor.world.pos.z > 700.0f) &&
	   (sPlayerCaught == 0))
	{
		if(pthis->unk_278 == 1)
		{
...
{
	SkelAnime_Update(&pthis->skelAnime);
	if((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(globalCtx) && (pthis->respawnFlag == 0))
	{
...
s32 EnHeishi3_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnHeishi3* pthis = (EnHeishi3*)thisx;
