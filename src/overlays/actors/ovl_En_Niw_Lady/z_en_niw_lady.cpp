...
void func_80AB9F24(EnNiwLady* pthis, GlobalContext* globalCtx)
{
	...
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->objectAneIndex) && Object_IsLoaded(&globalCtx->objectCtx, pthis->objectOsAnimeIndex))
	{
...
void func_80ABA244(EnNiwLady* pthis, GlobalContext* globalCtx)
{
	...
	while(currentCucco != NULL)
	{
		...
		{
			if((fabsf(currentCucco->actor.world.pos.x - 330.0f) < 90.0f) && (fabsf(currentCucco->actor.world.pos.z - 1610.0f) < 190.0f))
			{
				if(pthis->unk_26C == 0)
				{
...
static s16 sTradeItemTextIds[] = {0x503E, 0x503F, 0x5047, 0x5040, 0x5042, 0x5043, 0x5044, 0x00CF, 0x5045, 0x5042, 0x5027};
...
s32 EnNiwLady_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnNiwLady* pthis = (EnNiwLady*)thisx;
	s32 pad;
