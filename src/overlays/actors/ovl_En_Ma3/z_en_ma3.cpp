...
ActorInit En_Ma3_InitVars = {
    ACTOR_EN_MA3, ACTORCAT_NPC, FLAGS, OBJECT_MA2, ACTOR_FACTORY(EnMa3), (ActorFunc)EnMa3_Init, (ActorFunc)EnMa3_Destroy, (ActorFunc)EnMa3_Update, (ActorFunc)EnMa3_Draw, (ActorFunc)EnMa3_Reset,
};
...
static struct_D_80AA1678 sAnimationInfo[] = {
    {&gMalonAdultIdleAnim, 1.0f, ANIMMODE_LOOP, 0.0f}, {&gMalonAdultIdleAnim, 1.0f, ANIMMODE_LOOP, -10.0f}, {&gMalonAdultStandStillAnim, 1.0f, ANIMMODE_LOOP, 0.0f},
    {&gMalonAdultSingAnim, 1.0f, ANIMMODE_LOOP, 0.0f}, {&gMalonAdultSingAnim, 1.0f, ANIMMODE_LOOP, -10.0f},
};

u16 func_80AA2AA0(GlobalContext* globalCtx, Actor* thisx)
{
	Player* player = GET_PLAYER(globalCtx);
	auto* timer1ValuePtr = &gSaveContext.timer1Value;
...
s32 EnMa3_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	...
	if((limbIndex == MALON_ADULT_LIMB_CHEST_AND_NECK) || (limbIndex == MALON_ADULT_LIMB_LEFT_SHOULDER) || (limbIndex == MALON_ADULT_LIMB_RIGHT_SHOULDER))
	{
...
void EnMa3_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_ma3.c", 927);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_ma3.c", 950);
}
...
void EnMa3_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_ma3.c", 978);
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnMa3_OverrideLimbDraw, EnMa3_PostLimbDraw, pthis);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_ma3.c", 1013);
}
