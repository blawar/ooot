...
void EnBomChu_Explode(EnBomChu* pthis, GlobalContext* globalCtx)
{
	...
	bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, BOMB_BODY);
	if(bomb != NULL)
	{
		bomb->timer = 0;
	}
...
void EnBomChu_Move(EnBomChu* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer == 0) || (pthis->collider.base.acFlags & AC_HIT) || ((pthis->collider.base.ocFlags1 & OC1_HIT) && (pthis->collider.base.oc->category != ACTORCAT_PLAYER)))
	{
		EnBomChu_Explode(pthis, globalCtx);
		return;
	}
	...
	if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &posA, &posB, &posUpDown, &polyUpDown, true, true, true, true, &bgIdUpDown) && !(func_80041DB8(&globalCtx->colCtx, polyUpDown, bgIdUpDown) & 0x30) && // && not crawl space?
	   !SurfaceType_IsIgnoredByProjectiles(&globalCtx->colCtx, polyUpDown, bgIdUpDown))
	{
		...
		if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &posA, &posB, &posSide, &polySide, true, true, true, true, &bgIdSide) && !(func_80041DB8(&globalCtx->colCtx, polySide, bgIdSide) & 0x30) &&
...
void EnBomChu_ModelToWorld(EnBomChu* pthis, Vec3f* posModel, Vec3f* dest)
{
	...
	dest->x = pthis->actor.world.pos.x + (pthis->axisLeft.x * x) + (pthis->axisUp.x * posModel->y) + (pthis->axisForwards.x * posModel->z);
	dest->y = pthis->actor.world.pos.y + (pthis->axisLeft.y * x) + (pthis->axisUp.y * posModel->y) + (pthis->axisForwards.y * posModel->z);
	dest->z = pthis->actor.world.pos.z + (pthis->axisLeft.z * x) + (pthis->axisUp.z * posModel->y) + (pthis->axisForwards.z * posModel->z);
}
...
void EnBomChu_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	{
		pthis->visualJitter = (5.0f + (Rand_ZeroOne() * 3.0f)) * Math_SinS(((Rand_ZeroOne() * (f32)0x200) + (f32)0x3000) * pthis->timer);
		...
		if(WaterBox_GetSurface1(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &waterY, &waterBox))
		{
