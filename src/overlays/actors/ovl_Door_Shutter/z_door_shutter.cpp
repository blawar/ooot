...
static ShutterObjectInfo sObjectInfo[] = {
    {OBJECT_GND, 4, 4},		{OBJECT_GOMA, 5, 5},	      {OBJECT_YDAN_OBJECTS, 0, 1},	{OBJECT_DDAN_OBJECTS, 2, 2},	{OBJECT_BDAN_OBJECTS, 3, 3}, {OBJECT_GAMEPLAY_KEEP, 8, 8},
    {OBJECT_BDOOR, 7, 7},	{OBJECT_GAMEPLAY_KEEP, 8, 8}, {OBJECT_HIDAN_OBJECTS, 9, 10},	{OBJECT_GANON_OBJECTS, 11, 11}, {OBJECT_JYA_DOOR, 6, 6},     {OBJECT_MIZU_OBJECTS, 12, 13},
    {OBJECT_HAKA_DOOR, 14, 15}, {OBJECT_ICE_OBJECTS, 16, 16}, {OBJECT_MENKURI_OBJECTS, 17, 17}, {OBJECT_DEMO_KEKKAI, 18, 18},	{OBJECT_OUKE_HAKA, 19, 19},
};
...
static ShutterSceneInfo sSceneInfo[] = {
    {SCENE_YDAN, 0x02},	      {SCENE_DDAN, 0x03},    {SCENE_DDAN_BOSS, 0x03}, {SCENE_BDAN, 0x04},      {SCENE_BMORI1, 0x05},	  {SCENE_HIDAN, 0x08}, {SCENE_GANON, 0x09},	{SCENE_GANON_BOSS, 0x09},   {SCENE_JYASINZOU, 0x0A},
    {SCENE_JYASINBOSS, 0x0A}, {SCENE_MIZUSIN, 0x0B}, {SCENE_HAKADAN, 0x0C},   {SCENE_HAKADANCH, 0x0C}, {SCENE_ICE_DOUKUTO, 0x0D}, {SCENE_MEN, 0x0E},   {SCENE_GANONTIKA, 0x0F}, {SCENE_HAKAANA_OUKE, 0x10}, {-1, 0x07},
};
...
static void* D_809982D4[] = {
    object_bdoor_Tex_0065C0, object_bdoor_Tex_0035C0, object_bdoor_Tex_0055C0, object_bdoor_Tex_0045C0, object_bdoor_Tex_000000, object_bdoor_Tex_0025C0, object_bdoor_Tex_0015C0,
};
...
s32 DoorShutter_SetupDoor(DoorShutter* pthis, GlobalContext* globalCtx)
{
	...
	if(doorType != SHUTTER_KEY_LOCKED)
	{
		...
		{
			if(doorType == SHUTTER_FRONT_SWITCH_BACK_CLEAR)
...
void DoorShutter_SetupType(DoorShutter* pthis, GlobalContext* globalCtx)
{
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->requiredObjBankIndex))
	{
		...
		if(pthis->doorType == SHUTTER_PG_BARS || pthis->doorType == SHUTTER_GOHMA_BLOCK)
		{
			...
			CollisionHeader_GetVirtual((pthis->doorType == SHUTTER_GOHMA_BLOCK) ? &gGohmaDoorCol : &gPhantomGanonBarsCol, &colHeader);
...
s32 func_80996D14(DoorShutter* pthis, GlobalContext* globalCtx)
{
	if(pthis->unk_16C != 3)
	{
		...
		if(Math_StepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y + 200.0f, pthis->dyna.actor.velocity.y))
		{
			return true;
		}
	}
	...
	if(DECR(pthis->unk_16E) == 0 && globalCtx->roomCtx.status == 0 && func_80996D14(pthis, globalCtx) != 0)
	{
		if(((pthis->doorType == SHUTTER_BOSS) ? 20.0f : 50.0f) < pthis->dyna.actor.xzDistToPlayer)
		{
			...
			else
			{
				...
				if((pthis->doorType == SHUTTER_FRONT_SWITCH || pthis->doorType == SHUTTER_FRONT_SWITCH_BACK_CLEAR) && !Flags_GetSwitch(globalCtx, pthis->dyna.actor.params & 0x3F))
				{
					Audio_PlayActorSound2(&pthis->dyna.actor, NA_SE_EV_BUYOSHUTTER_CLOSE);
				}
...
void func_80997220(DoorShutter* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.room >= 0)
	{
		...
		pthis->dyna.actor.room = globalCtx->transiActorCtx.list[(u16)pthis->dyna.actor.params >> 0xA].sides[(vec.z < 0.0f) ? 0 : 1].room;
...
void func_809973E8(DoorShutter* pthis, GlobalContext* globalCtx)
{
	...
	if(Math_StepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y, pthis->dyna.actor.velocity.y))
	{
		if(pthis->dyna.actor.velocity.y > 20.0f)
		{
			...
			Actor_SpawnFloorDustRing(globalCtx, &pthis->dyna.actor, &pthis->dyna.actor.world.pos, 45.0f, 0xA, 8.0f, 0x1F4, 0xA, 0);
		}
...
void func_809975C0(DoorShutter* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.bgCheckFlags & 1)
	{
		...
		if(!(gSaveContext.eventChkInf[7] & 1))
		{
			...
			Actor_SpawnFloorDustRing(globalCtx, &pthis->dyna.actor, &pthis->dyna.actor.world.pos, 70.0f, 20, 8.0f, 500, 10, 1);
		}
	}
}
...
void DoorShutter_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.objBankIndex == pthis->requiredObjBankIndex && (pthis->unk_16B == 0 || func_80997A34(pthis, globalCtx) != 0))
	{
		s32 pad[2];
