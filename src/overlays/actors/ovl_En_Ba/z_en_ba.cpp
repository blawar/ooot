...
void EnBa_Idle(EnBa* pthis, GlobalContext* globalCtx)
{
	...
	if(!(player->stateFlags1 & PLAYER_STATE1_26) && (pthis->actor.xzDistToPlayer <= 175.0f) && (pthis->actor.world.pos.y == pthis->actor.home.pos.y + 100.0f))
	{
...
void EnBa_SwingAtPlayer(EnBa* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.xzDistToPlayer <= 175.0f) || (pthis->unk31A != 0))
	{
		...
		else
		{
			...
			else if((pthis->actor.xzDistToPlayer > 175.0f) || (player->stateFlags1 & PLAYER_STATE1_26))
			{
				EnBa_SetupIdle(pthis);
			}
			else
	...
	if((pthis->actor.xzDistToPlayer > 175.0f) || (player->stateFlags1 & PLAYER_STATE1_26))
	{
		EnBa_SetupIdle(pthis);
	}
	else
...
void func_809B75A0(EnBa* pthis, GlobalContext* globalCtx2)
{
	...
	for(i = 7; i < 14; i++)
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BA, pthis->unk158[i].x, pthis->unk158[i].y, pthis->unk158[i].z, 0, 0, 0, EN_BA_DEAD_BLOB);
	}
