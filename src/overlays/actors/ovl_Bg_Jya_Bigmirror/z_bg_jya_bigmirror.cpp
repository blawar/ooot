...
void BgJyaBigmirror_SetRoomFlag(Actor* thisx, GlobalContext* globalCtx)
{
	...
	pthis->puzzleFlags &= ~(BIGMIR_PUZZLE_IN_STATUE_ROOM | BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM);
...
void BgJyaBigmirror_HandleCobra(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->puzzleFlags & (BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM))
	{
		for(i = 0; i < 2; i++)
		{
			...
			{
				curCobraInfo->cobra =
				    (BgJyaCobra*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BG_JYA_COBRA, curSpawnData->pos.x, curSpawnData->pos.y, curSpawnData->pos.z, 0, curCobraInfo->rotY, 0, curSpawnData->params);
...
void BgJyaBigmirror_HandleMirRay(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		lightBeamToggles[1] = lightBeamToggles[2] = pthis->puzzleFlags & (BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM);
		...
		for(i = 0; i < 3; i++)
		{
			if(lightBeamToggles[i])
			{
				...
				{
					pthis->lightBeams[i] = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_MIR_RAY, sMirRayPoss_24[i].x, sMirRayPoss_24[i].y, sMirRayPoss_24[i].z, 0, 0, 0, sMirRayParamss_24[i]);
...
void BgJyaBigmirror_DrawLightBeam(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_jya_bigmirror.c", 435);
	...
	func_800D1694(pthis->actor.world.pos.x, pthis->actor.world.pos.y + 40.0f, pthis->actor.world.pos.z, &pthis->actor.shape.rot);
	Matrix_Scale(0.1f, (pthis->liftHeight * -(1.0f / 1280.0f)) + (1779.4f / 1280.0f), 0.1f, MTXMODE_APPLY);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_jya_bigmirror.c", 457), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	if(lift != NULL)
	{
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_jya_bigmirror.c", 467), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, gBigMirror2DL);
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_jya_bigmirror.c", 476);
}
...
void BgJyaBigmirror_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->puzzleFlags & (BIGMIR_PUZZLE_IN_STATUE_ROOM | BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM)) && (pthis->puzzleFlags & BIGMIR_PUZZLE_COBRA2_SOLVED) && (pthis->puzzleFlags & BIGMIR_PUZZLE_COBRA1_SOLVED))
	{
