...
void BgJyaBigmirror_DrawLightBeam(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, oot::asset::gfx::load(symbol::gBigMirror1DL));
    ...
    if (lift != NULL) {
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_jya_bigmirror.c", 467),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_XLU_DISP++, oot::asset::gfx::load(symbol::gBigMirror2DL));
    }
...
void BgJyaBigmirror_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->puzzleFlags & BIGMIR_PUZZLE_IN_1ST_TOP_ROOM) {
        Gfx_DrawDListOpa(globalCtx, oot::asset::gfx::load(symbol::gBigMirror3DL));
        Gfx_DrawDListXlu(globalCtx, oot::asset::gfx::load(symbol::gBigMirror4DL));
    }
