...
void ArmsHook_Shoot(ArmsHook* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer != 0) && (pthis->collider.base.atFlags & AT_HIT) && (pthis->collider.info.atHitInfo->elemType != ELEMTYPE_UNK4))
	{
		...
		pthis->timer = 0;
		Audio_PlaySoundGeneral(NA_SE_IT_ARROW_STICK_CRE, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	...
	else
	{
		...
		if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &sp60, &pthis->unk_1E8, &sp78, &poly, true, true, true, true, &bgId) && !func_8002F9EC(globalCtx, &pthis->actor, poly, bgId, &sp78))
		{
			...
			if(SurfaceType_IsHookshotSurface(&globalCtx->colCtx, poly, bgId))
			{
				...
				func_80865044(pthis);
				Audio_PlaySoundGeneral(NA_SE_IT_HOOKSHOT_STICK_OBJ, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
			else
			{
