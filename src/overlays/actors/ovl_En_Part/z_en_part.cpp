...
void func_80ACE13C(EnPart* pthis, GlobalContext* globalCtx)
{
	...
	else if(pthis->timer <= 0)
	{
		switch(pthis->actor.params)
		{
			...
			case 10:
			case 14:
				EffectSsDeadDb_Spawn(globalCtx, &pthis->actor.world.pos, &zeroVec, &zeroVec, (s16)(pthis->actor.scale.y * 100.0f) * 40, 7, 255, 255, 255, 255, 0, 255, 0, 1, 9, true);
				break;
			case 3:
			case 11:
				EffectSsDeadDb_Spawn(globalCtx, &pthis->actor.world.pos, &zeroVec, &zeroVec, (s16)(pthis->actor.scale.y * 100.0f) * 40, 7, 255, 255, 255, 255, 0, 0, 255, 1, 9, true);
				break;
			case 4:
				for(i = 7; i >= 0; i--)
				{
					pos.x = pthis->actor.world.pos.x + Rand_CenteredFloat(60.0f);
					pos.y = pthis->actor.world.pos.y + pthis->actor.shape.yOffset * pthis->actor.scale.y + Rand_CenteredFloat(50.0f);
					pos.z = pthis->actor.world.pos.z + Rand_CenteredFloat(60.0f);
					velocity.y = Rand_ZeroOne() + 1.0f;
					EffectSsDtBubble_SpawnColorProfile(globalCtx, &pos, &velocity, &accel, Rand_S16Offset(80, 100), 25, 0, true);
				}
				break;
			case 5:
			case 6:
			...
			case 8:
				for(i = 4; i >= 0; i--)
				{
					...
					EffectSsDeadDb_Spawn(globalCtx, &pos, &zeroVec, &zeroVec, 40, 7, 255, 255, 255, 255, 0, 0, 255, 1, 9, true);
				}
				break;
		}
...
void func_80ACE5C8(EnPart* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(sqrt(pthis->actor.xyzDistToPlayerSq) <= 40.0f)
		{
			...
			func_8002F71C(globalCtx, pthis->actor.parent, (650.0f - pthis->actor.parent->xzDistToPlayer) * 0.04f + 4.0f, pthis->actor.parent->world.rot.y, 8.0f);
		...
		EffectSsHahen_Spawn(globalCtx, &pthis->actor.world.pos, &velocity, &accel, 20, (s32)((Rand_ZeroOne() * 5.0f + 12.0f) * 2), -1, 10, NULL);
