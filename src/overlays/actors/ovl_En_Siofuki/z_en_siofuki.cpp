...
void func_80AFBE8C(EnSiofuki* pthis, GlobalContext* globalCtx)
{
	...
	if((dX > (pthis->dyna.actor.scale.x * -346.0f)) && (dX < (pthis->dyna.actor.scale.x * 346.0f)) && (dZ > (pthis->dyna.actor.scale.z * -400.0f)) && (dZ < (pthis->dyna.actor.scale.z * 400.0f)) && (dY < 0.0f))
	{
...
void func_80AFC218(EnSiofuki* pthis, GlobalContext* globalCtx)
{
	...
	if(((((u16)pthis->dyna.actor.params >> 0xC) & 0xF) == EN_SIOFUKI_LOWERING) && Flags_GetTreasure(globalCtx, (u16)pthis->dyna.actor.params & 0x3F))
	{
		pthis->currentHeight = -45.0f;
		pthis->targetHeight = -45.0f;
		Flags_UnsetSwitch(globalCtx, ((u16)pthis->dyna.actor.params >> 6) & 0x3F);
...
void EnSiofuki_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_siofuki.c", 654);
	func_80093D84(globalCtx->gfxCtx);
	Matrix_Translate(0.0f, pthis->unk_170, 0.0f, MTXMODE_APPLY);
	Matrix_Scale(1.0f, 1.0f, 1.0f, MTXMODE_APPLY);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_siofuki.c", 662), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	x = gameplayFrames * 15;
	y = gameplayFrames * -15;
	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, x, y, 64, 64, 1, x, y, 64, 64));
	gSPDisplayList(POLY_XLU_DISP++, object_siofuki_DL_000B70);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_siofuki.c", 674);
