...
ActorInit Bg_Hidan_Rsekizou_InitVars = {
    ACTOR_BG_HIDAN_RSEKIZOU,	      ACTORCAT_BG, FLAGS, OBJECT_HIDAN_OBJECTS, ACTOR_FACTORY(BgHidanRsekizou), (ActorFunc)BgHidanRsekizou_Init, (ActorFunc)BgHidanRsekizou_Destroy, (ActorFunc)BgHidanRsekizou_Update, (ActorFunc)BgHidanRsekizou_Draw,
...
void BgHidanRsekizou_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(pthis->colliderItems); i++)
	{
		...
		sphere->dim.worldSphere.center.x = pthis->dyna.actor.home.pos.x + yawCosine * sphere->dim.modelSphere.center.x + yawSine * sphere->dim.modelSphere.center.z;
		sphere->dim.worldSphere.center.y = (s16)pthis->dyna.actor.home.pos.y + sphere->dim.modelSphere.center.y;
		sphere->dim.worldSphere.center.z = (pthis->dyna.actor.home.pos.z - yawSine * sphere->dim.modelSphere.center.x) + yawCosine * sphere->dim.modelSphere.center.z;
	}
...
Gfx* BgHidanRsekizou_DrawFireball(GlobalContext* globalCtx, BgHidanRsekizou* pthis, s16 frame, MtxF* mf, s32 a, Gfx* displayList)
{
	f32 coss;
	f32 sins;
	s32 temp;
