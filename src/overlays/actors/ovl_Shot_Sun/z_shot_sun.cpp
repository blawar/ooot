...
void ShotSun_SpawnFairy(ShotSun* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ELF, pthis->actor.home.pos.x, pthis->actor.home.pos.y, pthis->actor.home.pos.z, 0, 0, 0, fairyType);
...
void ShotSun_TriggerFairy(ShotSun* pthis, GlobalContext* globalCtx)
{
	if((func_8005B198() == pthis->actor.category) || (pthis->timer != 0))
	{
		...
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_KANKYO, pthis->actor.home.pos.x, pthis->actor.home.pos.y, pthis->actor.home.pos.z, 0, 0, 0, 0x11);
...
void func_80BADF0C(ShotSun* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		{
			if((params == 0x40 && globalCtx->msgCtx.lastPlayedSong == OCARINA_SONG_SUNS) || (params == 0x41 && globalCtx->msgCtx.lastPlayedSong == OCARINA_SONG_STORMS))
			{
