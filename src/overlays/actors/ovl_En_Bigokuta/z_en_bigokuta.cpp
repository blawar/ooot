...
static InitChainEntry sInitChain[] = {
    ICHAIN_F32(targetArrowOffset, 2000, ICHAIN_CONTINUE), ICHAIN_U8(targetMode, 2, ICHAIN_CONTINUE), ICHAIN_F32(gravity, -1, ICHAIN_CONTINUE), ICHAIN_S8(naviEnemyId, 0x59, ICHAIN_CONTINUE), ICHAIN_VEC3F_DIV1000(scale, 33, ICHAIN_STOP),
};
...
void EnBigokuta_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_bigokuta_Skel_006BC0, &object_bigokuta_Anim_0014B8, pthis->jointTable, pthis->morphTable, 20);
...
void func_809BD8DC(EnBigokuta* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_196 >= 0)
	{
		...
		{
			pthis->actor.world.pos.y = sinf((pthis->unk_196 - 7) * (M_PI / 14)) * 130.0f + (pthis->actor.home.pos.y + 70.0f);
		}
		if(pthis->unk_196 == 0)
		{
...
void func_809BDC08(EnBigokuta* pthis, GlobalContext* globalCtx)
{
	...
	if(fabsf(sp28.x) > 263.0f || ((sp28.z > 0.0f) && !Actor_IsFacingPlayer(&pthis->actor, 0x1B00) && !Player_IsFacingActor(&pthis->actor, 0x2000, globalCtx)))
	{
		phi_v1 -= 0x80;
		if(pthis->unk_196 != 0)
		{
...
void func_809BE26C(EnBigokuta* pthis, GlobalContext* globalCtx)
{
	...
	else if(SkelAnime_Update(&pthis->skelAnime))
	{
		...
		if(pthis->unk_198 == 6)
		{
			...
			func_8002829C(globalCtx, &effectPos, &sEffectPosAccel, &sEffectPosAccel, &sEffectPrimColor, &sEffectEnvColor, 1200, 20);
...
void func_809BE568(EnBigokuta* pthis)
{
	...
	pthis->collider.elements->dim.worldSphere.center.x = (pthis->collider.elements->dim.modelSphere.center.z * sin) + (pthis->actor.world.pos.x + (pthis->collider.elements->dim.modelSphere.center.x * cos));
	pthis->collider.elements->dim.worldSphere.center.z = (pthis->actor.world.pos.z + (pthis->collider.elements->dim.modelSphere.center.z * cos)) - (pthis->collider.elements->dim.modelSphere.center.x * sin);
	pthis->collider.elements->dim.worldSphere.center.y = pthis->collider.elements->dim.modelSphere.center.y + pthis->actor.world.pos.y;

	for(i = 0; i < ARRAY_COUNT(pthis->cylinder); i++)
	{
		pthis->cylinder[i].dim.pos.x = pthis->actor.world.pos.x + sCylinderInit[i].dim.pos.z * sin + sCylinderInit[i].dim.pos.x * cos;
		pthis->cylinder[i].dim.pos.z = pthis->actor.world.pos.z + sCylinderInit[i].dim.pos.z * cos - sCylinderInit[i].dim.pos.x * sin;
...
void func_809BE798(EnBigokuta* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->cylinder[0].base.atFlags & AT_HIT) || (pthis->cylinder[1].base.atFlags & AT_HIT) || (pthis->collider.base.atFlags & AT_HIT))
	{
...
s32 EnBigokuta_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnBigokuta* pthis = (EnBigokuta*)thisx;
	u8 intensity;
	f32 temp_f0;
...
void EnBigokuta_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actionFunc != func_809BE26C) || (pthis->unk_196 != 0) || (pthis->unk_198 != 0))
	{
		...
		if(pthis->unk_196 & 1)
		{
			if((pthis->actionFunc == func_809BE180 && pthis->unk_196 >= 8) || (pthis->actionFunc == func_809BE26C && pthis->unk_196 >= 10))
			{
		...
		SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnBigokuta_OverrideLimbDraw, NULL, pthis);
	}
	else
	{
...
void EnBigokuta_Reset(Actor* pthisx, GlobalContext* globalCtx)
{
	...
	sEffectEnvColor = {100, 255, 255, 255};
