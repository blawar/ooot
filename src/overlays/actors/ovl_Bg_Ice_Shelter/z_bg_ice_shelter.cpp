...
void func_80890874(BgIceShelter* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 moveFlag)
{
	...
	if(pthis->dyna.bgId == BG_ACTOR_MAX)
	{
		...
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_bg_ice_shelter.c", 362, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void func_80890B8C(BgIceShelter* pthis, GlobalContext* globalCtx, f32 chance, f32 scale)
{
	...
	for(i = 0; i < 2; i++)
	{
		...
		func_8002829C(globalCtx, &dustPos, &dustVel, &dustAccel, &sDustPrimColor, &sDustEnvColor, 450.0f * scale, (s16)((Rand_ZeroOne() * 40.0f) + 40.0f) * scale);
	}
}
...
void func_80890E00(BgIceShelter* pthis, GlobalContext* globalCtx, f32 chance, f32 arg3)
{
	...
	for(i = 0; i < 2; i++)
	{
		...
		func_8002829C(globalCtx, &dustPos, &dustVel, &dustAccel, &sDustPrimColor, &sDustEnvColor, 450, (Rand_ZeroOne() * 40.0f) + 40.0f);
	}
}
