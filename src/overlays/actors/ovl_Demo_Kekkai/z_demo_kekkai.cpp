...
void DemoKekkai_SpawnParticles(DemoKekkai* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 85; i++)
	{
		...
		EffectSsKiraKira_SpawnFocused(globalCtx, &pos, &vel_31, &accel_31, &lightYellow_31, &darkRed_31, 3000, (s32)Rand_ZeroFloat(40.0f) + 45);
	}
}

void DemoKekkai_TowerBarrier(DemoKekkai* pthis, GlobalContext* globalCtx)
{
	if((globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[0] != NULL) && (globalCtx->csCtx.npcActions[0]->action != 1) && (globalCtx->csCtx.npcActions[0]->action == 2))
	{
		if(!(pthis->sfxFlag & 1))
		{
			func_800F3F3C(0xC);
...
static CutsceneData* sSageCutscenes[] = {
    NULL, gWaterTrialSageCs, gLightTrialSageCs, gFireTrialSageCs, gShadowTrialSageCs, gSpiritTrialSageCs, gForestTrialSageCs,
};
...
void DemoKekkai_DrawTrialBarrier(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	u8 alphaIndex[102] = {
	    1, 1, 0, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 1, 0, 0, 0, 1, 2, 2, 2, 2, 2, 2, 1, 0, 0, 1, 2, 2, 1, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 1, 0, 0, 0,
	    1, 1, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 1, 0, 0, 0, 1, 1, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 0, 1, 0, 2, 2, 2, 2, 2, 2, 2, 1, 1, 2, 0, 0,
	};
	s32 colorIndex;
	DemoKekkai* pthis = (DemoKekkai*)thisx;
	u8 alphas[3];
