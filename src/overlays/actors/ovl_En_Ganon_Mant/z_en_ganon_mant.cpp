...
#define MAP_STRAND_TO_VTX(n)                                                                                                                                                                                                                                   \
	(n) + GANON_MANT_NUM_JOINTS * 0, (n) + GANON_MANT_NUM_JOINTS * 1, (n) + GANON_MANT_NUM_JOINTS * 2, (n) + GANON_MANT_NUM_JOINTS * 3, (n) + GANON_MANT_NUM_JOINTS * 4, (n) + GANON_MANT_NUM_JOINTS * 5, (n) + GANON_MANT_NUM_JOINTS * 6,                 \
	    (n) + GANON_MANT_NUM_JOINTS * 7, (n) + GANON_MANT_NUM_JOINTS * 8, (n) + GANON_MANT_NUM_JOINTS * 9, (n) + GANON_MANT_NUM_JOINTS * 10, (n) + GANON_MANT_NUM_JOINTS * 11

static u16 sVerticesMap[GANON_MANT_NUM_STRANDS * GANON_MANT_NUM_JOINTS] = {
    MAP_STRAND_TO_VTX(11), MAP_STRAND_TO_VTX(10), MAP_STRAND_TO_VTX(9), MAP_STRAND_TO_VTX(8), MAP_STRAND_TO_VTX(7), MAP_STRAND_TO_VTX(6),
    MAP_STRAND_TO_VTX(5),  MAP_STRAND_TO_VTX(4),  MAP_STRAND_TO_VTX(3), MAP_STRAND_TO_VTX(2), MAP_STRAND_TO_VTX(1), MAP_STRAND_TO_VTX(0),
};
...
void EnGanonMant_Tear(EnGanonMant* pthis)
{
	...
	for(i = 0; i < 4; i++)
	{
		pthis->strands[(s16)Rand_ZeroFloat(GANON_MANT_NUM_STRANDS - 0.1f)].torn[(s16)Rand_ZeroFloat(GANON_MANT_NUM_JOINTS - 0.1f)] = true;
	}
}
 ...
 */
void EnGanonMant_UpdateStrand(GlobalContext* globalCtx, EnGanonMant* pthis, Vec3f* root, Vec3f* pos, Vec3f* nextPos, Vec3f* rot, Vec3f* vel, s16 strandNum)
{
	f32 xDiff;
	f32 zDiff;
	f32 gravity;
	...
	for(i = 0; i < GANON_MANT_NUM_JOINTS; i++, pos++, vel++, rot++, nextPos++)
	{
		...
		else
		{
			...
			delta.x = 0;
			delta.z = (pthis->backPush + (sinf((strandNum * (2 * M_PI)) / 2.1f) * pthis->backSwayMagnitude)) * sBackSwayCoefficients[i];
			...
			delta.x = cosf((strandNum * M_PI) / (GANON_MANT_NUM_STRANDS - 1.0f)) * pthis->sideSwayMagnitude * sSideSwayCoefficients[i];
			delta.z = 0;
...
void EnGanonMant_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->updateHasRun)
	{
		...
		for(strandIdx = 0; strandIdx < GANON_MANT_NUM_STRANDS; strandIdx++)
		{
			...
			EnGanonMant_UpdateStrand(
			    globalCtx, pthis, &pthis->strands[strandIdx].root, pthis->strands[strandIdx].joints, pthis->strands[nextStrandIdx].joints, pthis->strands[strandIdx].rotations, pthis->strands[strandIdx].velocities, strandIdx);
			Matrix_Pop();
		}
