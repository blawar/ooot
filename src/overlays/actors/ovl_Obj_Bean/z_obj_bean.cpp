...
void ObjBean_InitDynaPoly(ObjBean* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 moveFlag)
{
	...
	{
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_obj_bean.c", 374, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void ObjBean_FindFloor(ObjBean* pthis, GlobalContext* globalCtx)
{
	...
	pthis->dyna.actor.floorHeight = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->dyna.actor.floorPoly, &sp20, &pthis->dyna.actor, &vec);
}
...
void ObjBean_UpdatePosition(ObjBean* pthis)
{
	...
	pthis->dyna.actor.scale.x = pthis->dyna.actor.scale.z = ((Math_SinS((pthis->unk_1B6.y * 8)) * 0.01f) + (temp_f20 * 0.06f) + 1.07f) * 0.1f;

	pthis->dyna.actor.scale.y = ((Math_CosS(((pthis->unk_1B6.z * 10))) * 0.2f) + 1.0f) * 0.1f;
	temp_f20 = Math_SinS(pthis->unk_1B6.x * 3);
	pthis->dyna.actor.shape.rot.y = (Math_SinS((s16)(pthis->unk_1B6.z * 2)) * 2100.0f) + ((f32)pthis->dyna.actor.home.rot.y + (temp_f20 * 1000.0f));
}
...
{
	pthis->stateFlags &= ~(BEAN_STATE_DRAW_LEAVES | BEAN_STATE_DRAW_PLANT | BEAN_STATE_DRAW_STALK | BEAN_STATE_DRAW_SOIL);
...
s32 ObjBean_CheckForHorseTrample(ObjBean* pthis, GlobalContext* globalCtx)
{
	...
	{
		if((currentActor->id == ACTOR_EN_HORSE) && (Math3D_Vec3fDistSq(&currentActor->world.pos, &pthis->dyna.actor.world.pos) < 10000.0f))
		{
			return true;
		}
...
void ObjBean_Break(ObjBean* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 36; i++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &pos, gravity, arg5, 40, 3, 0, scale, 0, 0, (s16)((scale >> 3) + 40), -1, 1, sBreakDlists[i & 1]);
	}
}
	...
	if(LINK_AGE_IN_YEARS == YEARS_ADULT)
	{
		if(Flags_GetSwitch(globalCtx, pthis->dyna.actor.params & 0x3F) || (mREG(1) == 1))
		{
			...
			if(path == 0x1F)
			{
				...
				// "No path data?"
				osSyncPrintf("パスデータが無い？(%s %d)(arg_data %xH)\n", "../z_obj_bean.c", 909, pthis->dyna.actor.params);
				osSyncPrintf(VT_RST);
			...
			if(globalCtx->setupPathList[path].count < 3)
			{
				...
				osSyncPrintf("パスデータ数が不正(%s %d)(arg_data %xH)\n", "../z_obj_bean.c", 921, pthis->dyna.actor.params);
				osSyncPrintf(VT_RST);
				Actor_Kill(&pthis->dyna.actor);
				return;
			}
...
void ObjBean_WaitForWater(ObjBean* pthis, GlobalContext* globalCtx)
{
	...
	if(!(pthis->stateFlags & BEAN_STATE_BEEN_WATERED) && Flags_GetEnv(globalCtx, 5) && (D_80B90E30 == NULL) && (pthis->dyna.actor.xzDistToPlayer < 50.0f))
	{
...
void ObjBean_GrowWaterPhase2(ObjBean* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->stalkSizeMultiplier >= 0.1f)
	{ // 100 Frames
...
void ObjBean_Fly(ObjBean* pthis, GlobalContext* globalCtx)
{
	...
	else if(func_8004356C(&pthis->dyna) != 0)
	{ // Player is on top
...
{
	Matrix_Translate(pthis->dyna.actor.home.pos.x, pthis->dyna.actor.home.pos.y, pthis->dyna.actor.home.pos.z, MTXMODE_NEW);
