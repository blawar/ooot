...
void EnTk_RestAnim(EnTk* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, anim, 1.0f, 0.0f, Animation_GetLastFrame(&gDampeRestAnim), ANIMMODE_LOOP, -10.0f);
...
void EnTk_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, &gDampeRestAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDampeRestAnim), ANIMMODE_LOOP, 0.0f);
	...
	if(gSaveContext.dayTime <= 0xC000 || gSaveContext.dayTime >= 0xE000 || !!LINK_IS_ADULT || globalCtx->sceneNum != SCENE_SPOT02)
	{
