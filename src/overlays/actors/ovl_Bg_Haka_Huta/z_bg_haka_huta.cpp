...
void BgHakaHuta_PlaySound(BgHakaHuta* pthis, GlobalContext* globalCtx, u16 sfx)
{
	...
	pos.z = (pthis->dyna.actor.shape.rot.y == 0) ? pthis->dyna.actor.world.pos.z + 120.0f : pthis->dyna.actor.world.pos.z - 120.0f;
...
void BgHakaHuta_SpawnEnemies(BgHakaHuta* pthis, GlobalContext* globalCtx)
{
	if(Flags_GetSwitch(globalCtx, pthis->dyna.actor.params) && !Player_InCsMode(globalCtx))
	{
		...
		{
			Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_EN_FIREFLY, (pthis->dyna.actor.world.pos.x + (-25.0f) * Math_CosS(pthis->dyna.actor.shape.rot.y) + 40.0f * Math_SinS(pthis->dyna.actor.shape.rot.y)), pthis->dyna.actor.world.pos.y - 10.0f,
			    (pthis->dyna.actor.world.pos.z - (-25.0f) * Math_SinS(pthis->dyna.actor.shape.rot.y) + Math_CosS(pthis->dyna.actor.shape.rot.y) * 40.0f), 0, pthis->dyna.actor.shape.rot.y + 0x8000, 0, 2);

			Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_EN_FIREFLY, (pthis->dyna.actor.world.pos.x + (-25.0f) * (Math_CosS(pthis->dyna.actor.shape.rot.y)) + Math_SinS(pthis->dyna.actor.shape.rot.y) * 80.0f),
			    pthis->dyna.actor.world.pos.y - 10.0f, (pthis->dyna.actor.world.pos.z - (-25.0f) * (Math_SinS(pthis->dyna.actor.shape.rot.y)) + Math_CosS(pthis->dyna.actor.shape.rot.y) * 80.0f), 0, pthis->dyna.actor.shape.rot.y, 0, 2);
		}
		else if(pthis->unk_16A == 1)
		{
			Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_EN_RD, (pthis->dyna.actor.home.pos.x + (-25.0f) * (Math_CosS(pthis->dyna.actor.shape.rot.y)) + Math_SinS(pthis->dyna.actor.shape.rot.y) * 100.0f), pthis->dyna.actor.home.pos.y - 40.0f,
			    (pthis->dyna.actor.home.pos.z - (-25.0f) * (Math_SinS(pthis->dyna.actor.shape.rot.y)) + Math_CosS(pthis->dyna.actor.shape.rot.y) * 100.0f), 0, pthis->dyna.actor.shape.rot.y, 0, 0xFD);
		}
	}
}
