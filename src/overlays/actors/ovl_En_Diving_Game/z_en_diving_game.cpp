...
void EnDivingGame_SpawnRuppy(EnDivingGame* pthis, GlobalContext* globalCtx)
{
	...
	rupee = (EnExRuppy*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_RUPPY, rupeePos.x, rupeePos.y, rupeePos.z, 0, (s16)Rand_CenteredFloat(3500.0f) - 1000, pthis->rupeesLeftToThrow, 0);
	if(rupee != NULL)
	{
...
void EnDivingGame_RupeeThrow(EnDivingGame* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->csCameraTimer == 0 || ((fabsf(pthis->camEye.x - pthis->unk_2D0.x) < 2.0f) && (fabsf(pthis->camEye.y - pthis->unk_2D0.y) < 2.0f) && (fabsf(pthis->camEye.z - pthis->unk_2D0.z) < 2.0f) &&
					 (fabsf(pthis->camLookAt.x - pthis->unk_2F4.x) < 2.0f) && (fabsf(pthis->camLookAt.y - pthis->unk_2F4.y) < 2.0f) && (fabsf(pthis->camLookAt.z - pthis->unk_2F4.z) < 2.0f)))
	{
		if(pthis->unk_2A2 != 0)
		{
...
s32 EnDivingGame_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnDivingGame* pthis = (EnDivingGame*)thisx;
	s32 pad;
