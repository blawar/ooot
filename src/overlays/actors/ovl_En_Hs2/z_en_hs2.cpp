...
s32 func_80A6F0B4(EnHs2* pthis, GlobalContext* globalCtx, u16 textId, EnHs2ActionFunc actionFunc)
{
	...
	if(ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)) < 0x2151 && pthis->actor.xzDistToPlayer < 100.0f)
	{
		pthis->unk_2A8 |= 0x1;
...
void EnHs2_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnHs2_OverrideLimbDraw, EnHs2_PostLimbDraw, pthis);
}
