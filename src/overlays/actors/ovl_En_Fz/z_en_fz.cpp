...
void EnFz_SpawnIceSmokeFreeze(EnFz* pthis, Vec3f* pos, Vec3f* velocity, Vec3f* accel, f32 xyScale, f32 xyScaleTarget, s16 primAlpha, u8 isTimerMod8);
...
void EnFz_UpdateTargetPos(EnFz* pthis, GlobalContext* globalCtx)
{
	...
	if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &pos, &pthis->wallHitPos, &hitPos, &hitPoly, true, false, false, true, &bgId))
	{
...
void EnFz_DrawIceSmoke(EnFz* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(pthis->iceSmoke); i++)
	{
		if(iceSmoke->type > 0)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_fz.c", 1424), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
