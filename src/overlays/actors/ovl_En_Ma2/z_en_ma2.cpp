...
u16 func_80AA1B58(EnMa2* pthis, GlobalContext* globalCtx)
{
	...
	if(!(gSaveContext.eventChkInf[1] & 0x100) && (globalCtx->sceneNum == SCENE_MALON_STABLE) && IS_DAY && (pthis->actor.shape.rot.z == 5))
	{
		return 1;
	}
	if(!(gSaveContext.eventChkInf[1] & 0x100) && (globalCtx->sceneNum == SCENE_SPOT20) && IS_NIGHT && (pthis->actor.shape.rot.z == 6))
	{
		return 2;
	}
