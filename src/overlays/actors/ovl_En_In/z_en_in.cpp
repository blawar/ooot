...
static struct_D_80AA1678 sAnimationInfo[] = {
    {&object_in_Anim_001CC0, 1.0f, ANIMMODE_LOOP, 0.0f}, {&object_in_Anim_001CC0, 1.0f, ANIMMODE_LOOP, -10.0f}, {&object_in_Anim_013C6C, 1.0f, ANIMMODE_LOOP, 0.0f}, {&object_in_Anim_013C6C, 1.0f, ANIMMODE_LOOP, -10.0f},
    {&object_in_Anim_000CB0, 1.0f, ANIMMODE_LOOP, 0.0f}, {&object_in_Anim_0003B4, 1.0f, ANIMMODE_LOOP, -10.0f}, {&object_in_Anim_001BE0, 1.0f, ANIMMODE_LOOP, 0.0f}, {&object_in_Anim_013D60, 1.0f, ANIMMODE_LOOP, 0.0f},
...
void func_80A795C8(EnIn* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->skelAnime.animation == &object_in_Anim_0003B4 || pthis->skelAnime.animation == &object_in_Anim_001BE0 || pthis->skelAnime.animation == &object_in_Anim_013D60)
	{
		phi_a3 = 1;
	}
	else
...
{
	Animation_Change(&pthis->skelAnime, sAnimationInfo[arg1].animation, 1.0f, 0.0f, Animation_GetLastFrame(sAnimationInfo[arg1].animation), sAnimationInfo[arg1].mode, sAnimationInfo[arg1].transitionRate);
}
...
{
	if(globalCtx->sceneNum == SCENE_SPOT20 && LINK_IS_CHILD && IS_DAY && pthis->actor.shape.rot.z == 1 && !(gSaveContext.eventChkInf[1] & 0x10))
	{
		return 1;
	}
	if(globalCtx->sceneNum == SCENE_MALON_STABLE && LINK_IS_CHILD && IS_DAY && pthis->actor.shape.rot.z == 3 && (gSaveContext.eventChkInf[1] & 0x10))
	{
		return 1;
	}
...
void func_80A7A304(EnIn* pthis, GlobalContext* globalCtx)
{
	if(pthis->skelAnime.animation == &object_in_Anim_015814 || pthis->skelAnime.animation == &object_in_Anim_01646C)
	{
		...
		{
			Audio_PlaySoundRandom(&pthis->actor.projectedPos, NA_SE_VO_IN_LASH_0, NA_SE_VO_IN_LASH_1 - NA_SE_VO_IN_LASH_0 + 1);
		}
	}
	...
	if(SkelAnime_Update(&pthis->skelAnime) != 0)
	{
		...
		if(pthis->animationIdx == 3 || pthis->animationIdx == 4)
		{
			...
			{
				Audio_PlaySoundGeneral(NA_SE_IT_INGO_HORSE_NEIGH, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
		}
		Animation_Change(&pthis->skelAnime, D_80A7B918[pthis->animationIdx], 1.0f, 0.0f, Animation_GetLastFrame(D_80A7B918[pthis->animationIdx]), 2, -10.0f);
	}
}
