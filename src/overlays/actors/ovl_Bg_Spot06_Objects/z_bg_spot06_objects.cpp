...
ActorInit Bg_Spot06_Objects_InitVars = {
    ACTOR_BG_SPOT06_OBJECTS,	      ACTORCAT_PROP, FLAGS, OBJECT_SPOT06_OBJECTS, ACTOR_FACTORY(BgSpot06Objects), (ActorFunc)BgSpot06Objects_Init, (ActorFunc)BgSpot06Objects_Destroy, (ActorFunc)BgSpot06Objects_Update, (ActorFunc)BgSpot06Objects_Draw,
...
void BgSpot06Objects_LockSpawnBubbles(BgSpot06Objects* pthis, GlobalContext* globalCtx, s32 flag)
{
	...
	{
		EffectSsBubble_Spawn(globalCtx, &pthis->dyna.actor.world.pos, 0.0f, 40.0f, 30.0f, (Rand_ZeroOne() * 0.05f) + 0.175f);
	}
}
...
void BgSpot06Objects_LockSwimToSurface(BgSpot06Objects* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.velocity.y <= 0.0f)
	{
		...
		{
			pthis->dyna.actor.home.pos.x = pthis->dyna.actor.world.pos.x - (Math_SinS(pthis->dyna.actor.shape.rot.y) * 16.0f);
			pthis->dyna.actor.home.pos.z = pthis->dyna.actor.world.pos.z - (Math_CosS(pthis->dyna.actor.shape.rot.y) * 16.0f);
