...
void EnFhgFire_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params == FHGFIRE_LIGHTNING_SHOCK) || (pthis->actor.params == FHGFIRE_LIGHTNING_BURST) || (pthis->actor.params == FHGFIRE_ENERGY_BALL))
	{
...
void EnFhgFire_LightningStrike(EnFhgFire* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->work[FHGFIRE_FIRE_MODE])
	{
		case STRIKE_INIT:
			...
			break;
		case STRIKE_BURST:
			pthis->actor.shape.rot.y = Camera_GetInputDirYaw(camera) + 0x8000 * (pthis->work[FHGFIRE_VARIANCE_TIMER] & 0xFF);
			...
			if(pthis->work[FHGFIRE_TIMER] == 0)
			{
				...
				Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 500, 0, 0, FHGFIRE_LIGHTNING_BURST);
				{
					Vec3f sp7C;
					...
					for(i = 0; i < 35; i++)
					{
						...
						sp70.y = -0.2f;
						EffectSsFhgFlash_SpawnLightBall(globalCtx, &pthis->actor.world.pos, &sp7C, &sp70, (s16)(Rand_ZeroOne() * 100.0f) + 240, FHGFLASH_LIGHTBALL_GREEN);
					}
				}
			...
			break;
		case STRIKE_TRAILS:
			pthis->actor.shape.rot.y = Camera_GetInputDirYaw(camera) + (pthis->work[FHGFIRE_VARIANCE_TIMER] & 0xFF) * 0x8000;
			...
			if(pthis->work[FHGFIRE_TIMER] == 30)
			{
				...
				{
					Actor_SpawnAsChild(
					    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, (i * 0x2000) + randY, 0x4000, FHGFIRE_LIGHTNING_TRAIL + i);
				}

				for(i = 0; i < 8; i++)
				{
					Actor_SpawnAsChild(
					    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, (i * 0x2000) + randY, 0, FHGFIRE_LIGHTNING_SHOCK);
				}
			}
...
void EnFhgFire_SpearLight(EnFhgFire* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->work[FHGFIRE_FIRE_MODE] == FHGFIRE_LIGHT_GREEN)
	{
		...
		for(i = 0; i < 2; i++)
		{
			...
			EffectSsFhgFlash_SpawnLightBall(globalCtx, &ballPos, &ballVel, &ballAccel, (s16)(Rand_ZeroOne() * 80.0f) + 150, FHGFLASH_LIGHTBALL_GREEN);
		}
	}
	...
	else
	{
		...
		pthis->actor.shape.rot.z += (s16)(Rand_ZeroOne() * 0x4E20) + 0x4000;
		{
			...
			for(i1 = 0; i1 < 3; i1++)
			{
				...
				spBC.y = -0.08f;
				EffectSsFhgFlash_SpawnLightBall(globalCtx, &spD4, &spC8, &spBC, (s16)(Rand_ZeroOne() * 80.0f) + 150, lightBallColor1);
			}
		}
		switch(pthis->work[FHGFIRE_FIRE_MODE])
		{
			case FHGFIRE_LIGHT_GREEN:
				...
				if((pthis->collider.base.acFlags & AC_HIT) || canBottleReflect1)
				{
					...
					for(i2 = 0; i2 < 30; i2++)
					{
						...
						EffectSsFhgFlash_SpawnLightBall(globalCtx, &pthis->actor.world.pos, &spA8, &sp9C, (s16)(Rand_ZeroOne() * 25.0f) + 50, FHGFLASH_LIGHTBALL_GREEN);
					}
					...
					else
					{
						...
						pthis->actor.world.rot.x = (s16)(Math_FAtan2F(dyPG, sqrtf((dxPG * dxPG) + (dzPG * dzPG))) * (0x8000 / M_PI)) + angleModX;
			...
			case FHGFIRE_LIGHT_BLUE:
				...
				else
				{
					...
					if((fabsf(dxPG) < 30.0f) && (fabsf(dzPG) < 30.0f) && (fabsf(dyPG) < 45.0f))
					{
						...
						Audio_PlaySoundGeneral(NA_SE_EN_FANTOM_HIT_THUNDER, &bossGnd->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
						Audio_PlaySoundGeneral(NA_SE_EN_FANTOM_DAMAGE, &bossGnd->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
					}
				}
				break;
			case FHGFIRE_LIGHT_REFLECT:
				if(pthis->work[FHGFIRE_TIMER] == 0)
				{
					...
					for(i3 = 0; i3 < 30; i3++)
					{
						...
						EffectSsFhgFlash_SpawnLightBall(globalCtx, &pthis->actor.world.pos, &sp88, &sp7C, (s16)(Rand_ZeroOne() * 40.0f) + 80, FHGFLASH_LIGHTBALL_GREEN);
					}
					pthis->actor.world.rot.y = Math_FAtan2F(dxL, dzL) * (0x8000 / M_PI);
					dxzL = sqrtf(SQ(dxL) + SQ(dzL));
					pthis->actor.world.rot.x = Math_FAtan2F(dyL, dxzL) * (0x8000 / M_PI);
		...
		if(pthis->work[FHGFIRE_FX_TIMER] == 0)
		{
			...
			if((pthis->actor.bgCheckFlags & 0x19) || killMode)
			{
				...
				for(i4 = 0; i4 < 30; i4++)
				{
					...
					sp60.y = -0.1f;
					EffectSsFhgFlash_SpawnLightBall(globalCtx, &pthis->actor.world.pos, &sp6C, &sp60, (s16)(Rand_ZeroOne() * 50.0f) + 100, lightBallColor2);
				}
				if(killMode == BALL_BURST)
				{
					Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FHG_FIRE, pthis->actor.world.pos.x, player->actor.world.pos.y + 20.0f, pthis->actor.world.pos.z, 0xC8, 0, 0, FHGFIRE_LIGHTNING_BURST);
				}
		...
		Lights_PointNoGlowSetInfo(&pthis->lightInfo, (s16)pthis->actor.world.pos.x, (s16)pthis->actor.world.pos.y, (s16)pthis->actor.world.pos.z, 255, 255, 255, 200);
