...
static Vec3f sKakarikoPosList[] = {
    {-1697.0f, 80.0f, 870.0f}, {57.0f, 320.0f, -673.0f}, {796.0f, 80.0f, 1639.0f}, {1417.0f, 465.0f, 169.0f}, {-60.0f, 0.0f, -46.0f}, {-247.0f, 80.0f, 854.0f}, {1079.0f, 80.0f, -47.0f},
};
...
void EnNiw_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(globalCtx->sceneNum == SCENE_SPOT01)
	{
		...
		{
			if(fabsf(pthis->actor.world.pos.x - sKakarikoPosList[i].x) < 40.0f && fabsf(pthis->actor.world.pos.z - sKakarikoPosList[i].z) < 40.0f)
			{
				pthis->unk_2AA = i;
...
void EnNiw_SpawnAttackCucco(EnNiw* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer5 == 0) && (pthis->unk_296 < 7))
	{
		...
		attackCucco = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_ATTACK_NIW, attackCuccoPos.x, attackCuccoPos.y, attackCuccoPos.z, 0, 0, 0, 0);
...
void EnNiw_FeatherDraw(EnNiw* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(pthis->feathers); i++, feather++)
	{
		if(feather->type == 1)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_niw.c", 1913), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
