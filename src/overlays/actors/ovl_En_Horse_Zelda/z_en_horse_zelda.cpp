...
static unknownStruct D_80A6E240[] = {
    {-1682, -500, 12578, 0x07}, {-3288, -500, 13013, 0x07}, {-5142, -417, 11630, 0x07}, {-5794, -473, 9573, 0x07}, {-6765, -500, 8364, 0x07}, {-6619, -393, 6919, 0x07}, {-5193, 124, 5433, 0x07},
    {-2970, 2, 4537, 0x07},	{-2949, -35, 4527, 0x07},   {-1907, -47, 2978, 0x07},	{2488, 294, 3628, 0x07},   {3089, 378, 4713, 0x07},   {1614, -261, 7596, 0x07},	 {754, -187, 9295, 0x07},
};
...
void func_80A6DD14(EnHorseZelda* pthis)
{
	...
	Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->animationIndex], splaySpeeds[pthis->animationIndex] * sp34 * 1.5f, 0.0f, Animation_GetLastFrame(sAnimationHeaders[pthis->animationIndex]), ANIMMODE_ONCE, 0.0f);
}
...
void func_80A6DFD4(Actor* thisx, GlobalContext* globalCtx, PSkinAwb* skin)
{
	...
	for(i = 0; i < pthis->colliderSphere.count; i++)
	{
		...
		pthis->colliderSphere.elements[i].dim.worldSphere.radius = pthis->colliderSphere.elements[i].dim.modelSphere.radius * pthis->colliderSphere.elements[i].dim.scale;
	}
