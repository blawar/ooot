...
void EnReeba_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelanime, &object_reeba_Skel_001EE8, &object_reeba_Anim_0001E4, pthis->jointTable, pthis->morphTable, 18);
...
void func_80AE5054(EnReeba* pthis, GlobalContext* globalCtx)
{
	...
	{
		Actor_SpawnFloorDustRing(globalCtx, &pthis->actor, &pthis->actor.world.pos, pthis->actor.shape.shadowScale, 1, 8.0f, 500, 10, 1);
	}
	...
	if(pthis->unk_278 == 0)
	{
		...
		else
		{
			...
			switch(pthis->unk_280)
			{
				...
				case 2:
				case 3:
					pthis->actor.world.rot.y = pthis->actor.yawTowardsPlayer + (player->actor.shape.rot.y - pthis->actor.yawTowardsPlayer) * (playerLinearVel * 0.15f);
					break;
				case 4:
					pthis->actor.world.rot.y = pthis->actor.yawTowardsPlayer - (800.0f * playerLinearVel);
					break;
...
void EnReeba_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if((pthis->actor.shape.yOffset >= -700.0f) && (pthis->actor.colChkInfo.health > 0) && (pthis->actionfunc != func_80AE56E0))
	{
