...
void EnIshi_SpawnFragmentsSmall(EnIshi* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(scales_55); i++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &pos, -420, phi_v0, 30, 5, 0, scales_55[i], 3, 10, 40, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_FIELD_KEEP, gFieldKakeraDL);
	}
}
...
void EnIshi_SpawnFragmentsLarge(EnIshi* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(scales_56); i++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &pthis->actor.world.pos, phi_v1, phi_v0, 30, 5, 0, scales_56[i], 5, 2, 70, KAKERA_COLOR_WHITE, OBJECT_GAMEPLAY_FIELD_KEEP, gSilverRockFragmentsDL);
	}
}
...
void EnIshi_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((type == ROCK_LARGE) && Flags_GetSwitch(globalCtx, ((pthis->actor.params >> 0xA) & 0x3C) | ((pthis->actor.params >> 6) & 3)))
	{
...
void EnIshi_Wait(EnIshi* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->collider.base.acFlags & AC_HIT) && (type == ROCK_SMALL) && pthis->collider.info.acHitInfo->toucher.dmgFlags & 0x40000048)
	{
