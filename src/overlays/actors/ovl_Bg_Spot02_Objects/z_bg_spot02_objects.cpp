...
static void* D_808AD850[] = {
    object_spot02_objects_Tex_0096B0, object_spot02_objects_Tex_00A2B0, object_spot02_objects_Tex_00AEB0, object_spot02_objects_Tex_00BAB0, object_spot02_objects_Tex_00C6B0, object_spot02_objects_Tex_00D2B0,
    object_spot02_objects_Tex_00DEB0, object_spot02_objects_Tex_00EAB0, object_spot02_objects_Tex_00F6B0, object_spot02_objects_Tex_0102B0, object_spot02_objects_Tex_010EB0, object_spot02_objects_Tex_011AB0,
};
...
void BgSpot02Objects_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	switch(thisx->params)
	{
		...
		case 2:
			...
			if(((gSaveContext.eventChkInf[1] & 0x2000) && (globalCtx->sceneNum == SCENE_SPOT02) && (thisx->params == 2)) || (LINK_IS_ADULT && (thisx->params == 1)))
			{
				Actor_Kill(thisx);
			}
			break;
...
void func_808ACA08(BgSpot02Objects* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer == 20)
	{
		...
		EffectSsHahen_SpawnBurst(globalCtx, &pthis->dyna.actor.world.pos, 30.0f, 0, 25, 5, 40, OBJECT_SPOT02_OBJECTS, 20, object_spot02_objects_DL_012D30);
	}
...
void func_808ACC34(BgSpot02Objects* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != 0 && globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 2)
	{
		pthis->unk_16A++;
...
void func_808ACCB8(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_spot02_objects.c", 600);
	...
	if(globalCtx->csCtx.state != 0 && globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 2)
	{
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_spot02_objects.c", 679), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_spot02_objects.c", 692);
}

void func_808AD3D4(BgSpot02Objects* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != 0 && globalCtx->csCtx.npcActions[2] != NULL && globalCtx->csCtx.npcActions[2]->action == 2)
	{
		if(pthis->timer == 2)
		{
...
void func_808AD450(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_spot02_objects.c", 736);
	...
	if(globalCtx->csCtx.state != 0 && globalCtx->csCtx.npcActions[2] != NULL)
	{
		...
		if((globalCtx->csCtx.npcActions[2]->action & 0xFFFF) == 2)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_spot02_objects.c", 795), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 2 * pthis->timer, -3 * pthis->timer, 32, 64, 1, 4 * pthis->timer, -6 * pthis->timer, 32, 64));
			gDPPipeSync(POLY_XLU_DISP++);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_spot02_objects.c", 818);
}
