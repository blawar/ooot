...
ActorInit En_Bw_InitVars = {
    ACTOR_EN_BW, ACTORCAT_ENEMY, FLAGS, OBJECT_BW, ACTOR_FACTORY(EnBw), (ActorFunc)EnBw_Init, (ActorFunc)EnBw_Destroy, (ActorFunc)EnBw_Update, (ActorFunc)EnBw_Draw, (ActorFunc)EnBw_Reset,
};
...
void EnBw_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &object_bw_Skel_0020F0, &object_bw_Anim_000228, pthis->jointTable, pthis->morphTable, 12);
...
void func_809CEA24(EnBw* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->actor.speedXZ != 0.0f) && (pthis->actor.bgCheckFlags & BG_STATE_3))
	{
		...
		{
			sp64 = BgCheck_AnyLineTest2(&globalCtx->colCtx, &pthis->actor.world.pos, &player->actor.world.pos, &sp68, &sp74, 1, 0, 0, 1);
			if(sp64 != 0)
			{
				sp74 = SEGMENTED_TO_VIRTUAL(sp74);
	...
	if((pthis->unk_234 == 0) && !Actor_TestFloorInDirection(&pthis->actor, globalCtx, 50.0f, pthis->unk_236 + pthis->unk_238))
	{
	...
	switch(pthis->unk_221)
	{
		...
		case 0:
			...
			if(sp64 == 0)
			{
				Math_SmoothStepToS(&pthis->actor.world.rot.y, pthis->actor.yawTowardsPlayer, 1, pthis->actor.speedXZ * 1000.0f, 0);
				if((pthis->actor.xzDistToPlayer < 90.0f) && (pthis->actor.yDistToPlayer < 50.0f) && Actor_IsFacingPlayer(&pthis->actor, 0x1554) &&
		...
		case 1:
			if(((sp64 == 0) && !(pthis->actor.bgCheckFlags & BG_STATE_3)) || Actor_IsFacingPlayer(&pthis->actor, 0x1C70))
			{
				...
				Math_SmoothStepToS(&pthis->actor.world.rot.y, pthis->actor.yawTowardsPlayer - 0x8000, 1, pthis->actor.speedXZ * 1000.0f, 0);
			}
			else
			{
				Math_SmoothStepToS(&pthis->actor.world.rot.y, pthis->unk_236 + pthis->unk_238, 1, pthis->actor.speedXZ * 1000.0f, 0);
	...
	if(thisx->colChkInfo.damageEffect != 6)
	{
		...
		if(pthis->unk_248 <= 0.4f)
		{
			...
			if(((globalCtx->gameplayFrames & 1) == 0) && (pthis->unk_220 < 5) && (pthis->unk_23C == 0))
			{
				...
				func_8002836C(globalCtx, &thisx->world.pos, &velocity, &accel, &sp48, &sp44, 0xB4, 0x28, 20.0f - (pthis->unk_248 * 40.0f));
			}
		}
		else
		{
		...
		if((pthis->unk_220 == 4) || (pthis->unk_220 == 6) || (pthis->unk_220 == 5) || (pthis->unk_220 == 1) || (pthis->unk_234 != 0))
		{
...
s32 EnBw_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnBw* pthis = (EnBw*)thisx;
...
static Vec3f sIceOffsets[] = {
    {20.0f, 10.0f, 0.0f}, {-20.0f, 10.0f, 0.0f}, {0.0f, 10.0f, -25.0f}, {10.0f, 0.0f, 15.0f}, {10.0f, 0.0f, -15.0f}, {0.0f, 10.0f, 25.0f}, {-10.0f, 0.0f, 15.0f}, {-10.0f, 0.0f, -15.0f},
};
...
void EnBw_Draw(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	else
	{
		...
		POLY_XLU_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnBw_OverrideLimbDraw, NULL, pthis, POLY_XLU_DISP);
	}
