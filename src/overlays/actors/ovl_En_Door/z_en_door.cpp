...
static Gfx* D_809FCEE4[5][2] = {
    {gDoorLeftDL, gDoorRightDL}, {gFireTempleDoorWithHandleFrontDL, gFireTempleDoorWithHandleBackDL}, {gWaterTempleDoorLeftDL, gWaterTempleDoorRightDL}, {object_haka_door_DL_0013B8, object_haka_door_DL_001420}, {gFieldDoor1DL, gFieldDoor2DL},
};
...
void EnDoor_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->actor.params & 0x40)
	{
		...
		other = (EnDoor*)Actor_SpawnAsChild(
		    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_DOOR, pthis->actor.world.pos.x + xOffset, pthis->actor.world.pos.y, pthis->actor.world.pos.z - zOffset, 0, pthis->actor.shape.rot.y + 0x8000, 0, pthis->actor.params & ~0x40);
		if(other != NULL)
		{
			other->unk_192 = 1;
		}
...
void EnDoor_SetupType(EnDoor* pthis, GlobalContext* globalCtx)
{
	...
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->requiredObjBankIndex))
	{
		...
		{
			doorType = (gSaveContext.dayTime > 0xC000 && gSaveContext.dayTime < 0xE000) ? DOOR_SCENEEXIT : DOOR_CHECKABLE;
		}
...
void EnDoor_Idle(EnDoor* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->playerIsOpening != 0)
	{
		...
		Animation_PlayOnceSetSpeed(&pthis->skelAnime, D_809FCECC[pthis->animStyle],
					   (player->stateFlags1 & PLAYER_STATE_SWIMMING) ? 0.75f : 1.5f); // Swimming?
...
void EnDoor_Open(EnDoor* pthis, GlobalContext* globalCtx)
{
	...
	if(DECR(pthis->lockTimer) == 0)
	{
		...
		{
			Audio_PlayActorSound2(&pthis->actor, (globalCtx->sceneNum == SCENE_HAKADAN || globalCtx->sceneNum == SCENE_HAKADANCH || globalCtx->sceneNum == SCENE_HIDAN) ? NA_SE_EV_IRON_DOOR_OPEN : NA_SE_OC_DOOR_OPEN);
		...
		{
			Audio_PlayActorSound2(&pthis->actor, (globalCtx->sceneNum == SCENE_HAKADAN || globalCtx->sceneNum == SCENE_HAKADANCH || globalCtx->sceneNum == SCENE_HIDAN) ? NA_SE_EV_IRON_DOOR_CLOSE : NA_SE_EV_DOOR_CLOSE);
		}
	}
}
...
s32 EnDoor_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	...
	if(limbIndex == 4)
	{
		...
		{
			phi_v0_2 = ((pthis->actor.shape.rot.y + pthis->skelAnime.jointTable[3].z) + rot->z) - Math_Vec3f_Yaw(&globalCtx->view.eye, &pthis->actor.world.pos);
