...
static void* D_80B2EB88[] = {
    gEffEnemyDeathFlame1Tex, gEffEnemyDeathFlame2Tex, gEffEnemyDeathFlame3Tex, gEffEnemyDeathFlame4Tex, gEffEnemyDeathFlame5Tex, gEffEnemyDeathFlame6Tex, gEffEnemyDeathFlame7Tex, gEffEnemyDeathFlame8Tex, gEffEnemyDeathFlame9Tex, gEffEnemyDeathFlame10Tex,
};
...
void EnVm_Wait(EnVm* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->unk_25E)
	{
		case 0:
			...
			if(pthis->actor.xzDistToPlayer <= pthis->beamSightRange && ABS(headRot) <= 0x2710 && pitch >= 0xE38 && pthis->actor.yDistToPlayer <= 80.0f && pthis->actor.yDistToPlayer >= -160.0f)
			{
				Math_SmoothStepToS(&pthis->beamRot.x, pitch, 10, 0xFA0, 0);
				if(Math_SmoothStepToS(&pthis->headRotY, pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y, 1, (ABS((s16)(dist * 180.0f)) / 3) + 0xFA0, 0) <= 5460)
				{
					pthis->timer--;
					if(pthis->timer == 0)
					{
	...
	if(pthis->unk_260 == 2)
	{
		...
		else
		{
			...
			EffectSsDeadDd_Spawn(globalCtx, &pthis->beamPos2, &D_80B2EAEC, &D_80B2EAEC, 150, -25, 0, 0, 255, 0, 255, 255, 255, 16, 20);
...
{
	Animation_Change(&pthis->skelAnime, &gBeamosAnim, -1.0f, Animation_GetLastFrame(&gBeamosAnim), 0.0f, ANIMMODE_ONCE, 0.0f);
	pthis->unk_260 = 0;
	pthis->timer = 180;
...
void EnVm_Stun(EnVm* pthis, GlobalContext* globalCtx)
{
	if(pthis->timer == 0)
	{
		if(SkelAnime_Update(&pthis->skelAnime))
		{
			...
			{
				Animation_Change(&pthis->skelAnime, &gBeamosAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gBeamosAnim), ANIMMODE_ONCE, 0.0f);
			}
			else
			{
				pthis->timer = 10;
...
void EnVm_Die(EnVm* pthis, GlobalContext* globalCtx)
{
	...
	if(--pthis->timer == 0)
	{
		bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0x6FF, BOMB_BODY);
...
void EnVm_CheckHealth(EnVm* pthis, GlobalContext* globalCtx)
{
	...
	{
		bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->actor.world.pos.x, pthis->actor.world.pos.y + 20.0f, pthis->actor.world.pos.z, 0, 0, 0x601, BOMB_BODY);
...
void EnVm_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	if(limbIndex == 2)
	{
		...
		if(pthis->unk_260 >= 3)
		{
			...
			if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->beamPos1, &pthis->beamPos3, &posResult, &poly, true, true, false, true, &bgId) == true)
			{
			...
			if(pthis->beamScale.z != 0.0f)
			{
				...
				Collider_SetQuadVertices(&pthis->colliderQuad1, &pthis->colliderQuad1.dim.quad[0], &pthis->colliderQuad1.dim.quad[1], &pthis->colliderQuad1.dim.quad[2], &pthis->colliderQuad1.dim.quad[3]);
			}
		}
		...
		Collider_SetQuadVertices(&pthis->colliderQuad2, &pthis->colliderQuad2.dim.quad[0], &pthis->colliderQuad2.dim.quad[1], &pthis->colliderQuad2.dim.quad[2], &pthis->colliderQuad2.dim.quad[3]);
	}
}
...
void EnVm_Draw(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_vm.c", 1014);

	func_80093D18(globalCtx->gfxCtx);
	func_80093D84(globalCtx->gfxCtx);
	SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnVm_OverrideLimbDraw, EnVm_PostLimbDraw, pthis);
	...
	if(pthis->unk_260 >= 3)
	{
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_vm.c", 1033), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_vm.c", 1044), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPSegment(POLY_XLU_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(D_80B2EB88[(globalCtx->gameplayFrames.whole() + 4) % 8]));
		gSPDisplayList(POLY_XLU_DISP++, gEffEnemyDeathFlameDL);
	}
	gSPSegment(POLY_OPA_DISP++, 0x08, func_80094E78(globalCtx->gfxCtx, 0, pthis->beamTexScroll));
	...
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_vm.c", 1063), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, gBeamosLaserDL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_vm.c", 1068);
}
