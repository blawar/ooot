...
void EnEncount1_SpawnLeevers(EnEncount1* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer == 0) && (globalCtx->csCtx.state == CS_STATE_IDLE) && (pthis->curNumSpawn <= pthis->maxCurSpawns) && (pthis->curNumSpawn < 5))
	{
		...
		else if(!(pthis->reduceLeevers && (pthis->actor.xzDistToPlayer > 1300.0f)))
		{
			...
			while((pthis->curNumSpawn < pthis->maxCurSpawns) && (pthis->curNumSpawn < spawnLimit) && (pthis->timer == 0))
			{
				...
				leever = (EnReeba*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_REEBA, spawnPos.x, spawnPos.y, spawnPos.z, 0, 0, 0, spawnParams);
...
void EnEncount1_SpawnTektites(EnEncount1* pthis, GlobalContext* globalCtx)
{
	...
	{
		pthis->timer = 10;
		if((fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) > 100.0f) || (pthis->actor.xzDistToPlayer > pthis->spawnRange))
		{
		...
		else
		{
			...
			if((pthis->curNumSpawn < pthis->maxCurSpawns) && (pthis->totalNumSpawn < pthis->maxTotalSpawns))
			{
				...
				spawnPos.y = floorY;
				if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_TITE, spawnPos.x, spawnPos.y, spawnPos.z, 0, 0, 0, TEKTITE_RED) != NULL)
				{ // Red tektite
					pthis->curNumSpawn++;
					pthis->totalNumSpawn++;
				}
...
void EnEncount1_SpawnStalchildOrWolfos(EnEncount1* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->curNumSpawn < pthis->maxCurSpawns) && (pthis->totalNumSpawn < pthis->maxTotalSpawns))
	{
		while((pthis->curNumSpawn < pthis->maxCurSpawns) && (pthis->totalNumSpawn < pthis->maxTotalSpawns))
		{
			if(globalCtx->sceneNum == SCENE_SPOT00)
			{
				if((player->unk_89E == 0) || (player->actor.floorBgId != BGCHECK_SCENE) || !(player->actor.bgCheckFlags & 1) || (player->stateFlags1 & PLAYER_STATE_SWIMMING))
				{ // Swimming?
				...
				spawnPos.x = player->actor.world.pos.x + (Math_SinS(spawnAngle) * spawnDist) + Rand_CenteredFloat(40.0f);
				spawnPos.y = player->actor.floorHeight + 120.0f;
				spawnPos.z = player->actor.world.pos.z + (Math_CosS(spawnAngle) * spawnDist) + Rand_CenteredFloat(40.0f);
				floorY = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &floorPoly, &bgId, &pthis->actor, &spawnPos);
				if(floorY <= BGCHECK_Y_MIN)
				{
					break;
				}
				if((player->actor.yDistToWater != BGCHECK_Y_MIN) && (floorY < (player->actor.world.pos.y - player->actor.yDistToWater)))
				{
					break;
				}
				spawnPos.y = floorY;
			...
			if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, spawnId, spawnPos.x, spawnPos.y, spawnPos.z, 0, 0, 0, spawnParams) != NULL)
			{
				pthis->curNumSpawn++;
