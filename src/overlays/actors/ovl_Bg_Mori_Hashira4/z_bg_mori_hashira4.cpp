...
void BgMoriHashira4_InitDynaPoly(BgMoriHashira4* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 moveFlag)
{
	s32 pad;
	...
	if(pthis->dyna.bgId == BG_ACTOR_MAX)
	{
		...
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_bg_mori_hashira4.c", 155, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void BgMoriHashira4_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->moriTexObjIndex < 0)
	{
		...
		// "Bank danger!"
		osSyncPrintf("Error : バンク危険！(arg_data 0x%04x)(%s %d)\n", pthis->dyna.actor.params, "../z_bg_mori_hashira4.c", 196);
		return;
	}
