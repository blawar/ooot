...
void EnGb_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gPoeSellerSkel, &gPoeSellerIdleAnim, pthis->jointTable, pthis->morphTable, 12);
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->dyna.actor.home.pos.x, pthis->dyna.actor.home.pos.y, pthis->dyna.actor.home.pos.z, 255, 255, 255, 200);
	...
	for(i = 0; i < ARRAY_COUNT(sCagedSoulPositions); i++)
	{
		...
		pthis->cagedSouls[i].unk_14.x = pthis->cagedSouls[i].translation.x = sCagedSoulPositions[i].x + pthis->dyna.actor.world.pos.x;
		pthis->cagedSouls[i].unk_14.y = pthis->cagedSouls[i].translation.y = sCagedSoulPositions[i].y + pthis->dyna.actor.world.pos.y;
		pthis->cagedSouls[i].unk_14.z = pthis->cagedSouls[i].translation.z = sCagedSoulPositions[i].z + pthis->dyna.actor.world.pos.z;
...
{
	Animation_Change(&pthis->skelAnime, &gPoeSellerSwingStickAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gPoeSellerSwingStickAnim), ANIMMODE_ONCE, 0.0f);
...
void func_80A2FC70(EnGb* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gPoeSellerIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gPoeSellerIdleAnim), ANIMMODE_LOOP, 0.0f);
		pthis->actionFunc = func_80A2F83C;
	}
	else if(pthis->skelAnime.curFrame == 18.0f)
	{
		...
		if(pthis->actionFunc)
		{
		} // these ifs cannot just contain a constant
...
void EnGb_DrawCagedSouls(EnGb* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		Matrix_Push();
		Matrix_Translate(pthis->cagedSouls[i].translation.x, pthis->cagedSouls[i].translation.y, pthis->cagedSouls[i].translation.z, MTXMODE_NEW);
