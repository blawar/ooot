...
void EnVali_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gBariSkel, &gBariLurkingAnim, pthis->jointTable, pthis->morphTable, EN_VALI_LIMB_MAX);
...
void EnVali_SetupFloatIdle(EnVali* pthis)
{
	...
	pthis->leftArmCollider.dim.quad[2] = pthis->leftArmCollider.dim.quad[3] = pthis->rightArmCollider.dim.quad[2] = pthis->rightArmCollider.dim.quad[3] = pthis->leftArmCollider.dim.quad[0] = pthis->leftArmCollider.dim.quad[1] =
	    pthis->rightArmCollider.dim.quad[0] = pthis->rightArmCollider.dim.quad[1] = pthis->actor.world.pos;

	pthis->leftArmCollider.dim.quad[2].y = pthis->leftArmCollider.dim.quad[3].y = pthis->rightArmCollider.dim.quad[2].y = pthis->rightArmCollider.dim.quad[3].y = pthis->leftArmCollider.dim.quad[0].y = pthis->leftArmCollider.dim.quad[1].y =
	    pthis->rightArmCollider.dim.quad[0].y = pthis->rightArmCollider.dim.quad[1].y = pthis->actor.world.pos.y - 10.0f;
...
void EnVali_SetupDivideAndDie(EnVali* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 3; i++)
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BILI, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, pthis->actor.world.rot.y, 0, 0);
...
void EnVali_Frozen(EnVali* pthis, GlobalContext* globalCtx)
{
	...
	if(temp_v1 > 0)
	{
		...
		if((pthis->timer % 2) != 0)
		{
			...
			EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &pos, 150, 150, 150, 250, 235, 245, 255, (Rand_ZeroOne() * 0.2f) + 1.3f);
		}
	}
...
void EnVali_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->bodyCollider.base.atFlags & AT_HIT) || (pthis->leftArmCollider.base.atFlags & AT_HIT) || (pthis->rightArmCollider.base.atFlags & AT_HIT))
	{
...
s32 EnVali_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnVali* pthis = (EnVali*)thisx;
	f32 curFrame;
...
void EnVali_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx)
{
	...
	if(pthis->actionFunc == EnVali_FloatIdle)
	{
		if((limbIndex == EN_VALI_LIMB_LEFT_FOREARM_BASE) || (limbIndex == EN_VALI_LIMB_RIGHT_FOREARM_BASE))
		{
			...
			{
				Collider_SetQuadVertices(&pthis->leftArmCollider, &sp30, &sp3C, &pthis->leftArmCollider.dim.quad[0], &pthis->leftArmCollider.dim.quad[1]);
			}
			else
			{
				Collider_SetQuadVertices(&pthis->rightArmCollider, &sp30, &sp3C, &pthis->rightArmCollider.dim.quad[0], &pthis->rightArmCollider.dim.quad[1]);
			}
		}
	}
}
