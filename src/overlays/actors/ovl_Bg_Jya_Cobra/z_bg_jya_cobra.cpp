...
void BgJyaCobra_SpawnRay(BgJyaCobra* pthis, GlobalContext* globalCtx) {
    Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_MIR_RAY, pthis->dyna.actor.world.pos.x,
                       pthis->dyna.actor.world.pos.y + 57.0f, pthis->dyna.actor.world.pos.z, 0, 0, 0, 6);
...
void BgJyaCobra_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    // "(jya cobra)"
    osSyncPrintf("(jya コブラ)(arg_data 0x%04x)(act %x)(txt %x)(txt16 %x)\n", pthis->dyna.actor.params, pthis,
                 &pthis->shadowTexture, ALIGN16((s32)(&pthis->shadowTexture)));
}
...
void func_80896918(BgJyaCobra* pthis, GlobalContext* globalCtx) {
    ...
    pthis->dyna.actor.shape.rot.y = pthis->dyna.actor.world.rot.y =
        (pthis->unk_16C * 0x2000) + pthis->dyna.actor.home.rot.y;
}
...
void func_80896ABC(BgJyaCobra* pthis, GlobalContext* globalCtx) {
    ...
    if (Math_ScaledStepToS(&pthis->unk_170, pthis->unk_16A * 0x2000, pthis->unk_16E)) {
        ...
        pthis->dyna.actor.shape.rot.y = pthis->dyna.actor.world.rot.y =
            (pthis->unk_16C * 0x2000) + pthis->dyna.actor.home.rot.y + pthis->unk_170;
    }
...
void BgJyaCobra_DrawShadow(BgJyaCobra* pthis, GlobalContext* globalCtx) {
    ...
    gDPLoadTextureBlock(POLY_XLU_DISP++, ALIGN16((s32)(&pthis->shadowTexture)), G_IM_FMT_I, G_IM_SIZ_8b, 0x40, 0x40, 0,
