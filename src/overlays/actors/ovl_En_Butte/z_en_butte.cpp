...
void EnButte_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gButterflySkel, &gButterflyAnim, pthis->jointTable, pthis->morphTable, 8);
...
void EnButte_FlyAround(EnButte* pthis, GlobalContext* globalCtx)
{
	...
	distSqFromHome = Math3D_Dist2DSq(pthis->actor.world.pos.x, pthis->actor.world.pos.z, pthis->actor.home.pos.x, pthis->actor.home.pos.z);
	func_809CD56C(pthis);
	Math_SmoothStepToF(&pthis->actor.speedXZ, flightParams->speedXZTarget, flightParams->speedXZScale, flightParams->speedXZStep, 0.0f);
	...
	if(((pthis->actor.params & 1) == 1) && (player->heldItemActionParam == PLAYER_AP_STICK) && (pthis->swordDownTimer <= 0) &&
	   ((Math3D_Dist2DSq(player->actor.world.pos.x, player->actor.world.pos.z, pthis->actor.home.pos.x, pthis->actor.home.pos.z) < SQ(120.0f)) || (pthis->actor.xzDistToPlayer < 60.0f)))
	{
...
void EnButte_FollowLink(EnButte* pthis, GlobalContext* globalCtx)
{
	...
	distSqFromHome = Math3D_Dist2DSq(pthis->actor.world.pos.x, pthis->actor.world.pos.z, pthis->actor.home.pos.x, pthis->actor.home.pos.z);
	if(!((player->heldItemActionParam == PLAYER_AP_STICK) && (fabsf(player->actor.speedXZ) < 1.8f) && (pthis->swordDownTimer <= 0) && (distSqFromHome < SQ(320.0f))))
	{
		EnButte_SetupFlyAround(pthis);
	}
	else if(distSqFromHome > SQ(240.0f))
	{
		distSqFromSword = Math3D_Dist2DSq(player->swordInfo[0].tip.x, player->swordInfo[0].tip.z, pthis->actor.world.pos.x, pthis->actor.world.pos.z);
...
void EnButte_TransformIntoFairy(EnButte* pthis, GlobalContext* globalCtx)
{
	...
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ELF, pthis->actor.focus.pos.x, pthis->actor.focus.pos.y, pthis->actor.focus.pos.z, 0, pthis->actor.shape.rot.y, 0, FAIRY_HEAL_TIMED);
