...
void EnGSwitch_SilverRupeeCollected(EnGSwitch* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.world.pos = player->actor.world.pos;
	pthis->actor.world.pos.y = player->actor.world.pos.y + 40.0f + (pthis->killTimer * 0.3f) * Math_SinS(pthis->killTimer * 0x3A98);
	if(LINK_IS_ADULT)
	{
		pthis->actor.world.pos.y += 20.0f;
	}
	...
	if(pthis->delayTimer == 0)
	{
		switch(pthis->moveMode)
		{
			...
			default:
				...
				{
					case MOVE_TARGET:
						if((fabsf(pthis->actor.world.pos.x - pthis->targetPos.x) > 5.0f) || (fabsf(pthis->actor.world.pos.y - pthis->targetPos.y) > 5.0f))
						{
					...
					case MOVE_HOME:
						...
						{
							if((fabsf(pthis->actor.world.pos.x - pthis->actor.home.pos.x) > 5.0f) || (fabsf(pthis->actor.world.pos.y - pthis->actor.home.pos.y) > 5.0f))
							{
...
void EnGSwitch_ArcheryPot(EnGSwitch* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->collider.base.acFlags & AC_HIT)
	{
		...
		for(i = 0, angle = 0; i < 30; i++, angle += 0x4E20)
		{
			...
			EffectSsKakera_Spawn(globalCtx, &pos, &vel, thisPos, -240, phi_s0, 10, 10, 0, scale, 0, 0x20, 60, KAKERA_COLOR_NONE, OBJECT_TSUBO, object_tsubo_DL_001960);
		}
...
void EnGSwitch_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->type != ENGSWITCH_SILVER_TRACKER) && (pthis->type != ENGSWITCH_SILVER_RUPEE) && (pthis->type != ENGSWITCH_TARGET_RUPEE))
	{
		Actor_MoveForward(&pthis->actor);
