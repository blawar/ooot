...
void EnDodojr_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &object_dodojr_Skel_0020E0, &object_dodojr_Anim_0009D4, pthis->jointTable, pthis->morphTable, 15);
...
s32 func_809F6CA4(EnDodojr* pthis, GlobalContext* globalCtx)
{
	...
	while(bomb != NULL)
	{
		...
		if((fabsf(xDist) >= fabsf(unkVec.x)) || (fabsf(yDist) >= fabsf(unkVec.y)) || (fabsf(zDist) >= fabsf(unkVec.z)))
		{
			bomb = bomb->next;
			continue;
		}
...
void func_809F6E54(EnDodojr* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->bomb == NULL) || (pthis->bomb->update == NULL) || ((pthis->bomb != NULL) && (pthis->bomb->parent != NULL)))
	{
...
{
	if((pthis->actionFunc == func_809F773C) || (pthis->actionFunc == func_809F77AC) || (pthis->actionFunc == func_809F784C) || (pthis->actionFunc == func_809F7A00) || (pthis->actionFunc == func_809F7AB8) || (pthis->actionFunc == func_809F7B3C) ||
	...
	else
	{
		...
		if((pthis->actor.colChkInfo.damageEffect == 1) && (pthis->actionFunc != func_809F78EC) && (pthis->actionFunc != func_809F786C))
		{
...
void func_809F72A4(EnDodojr* pthis, GlobalContext* globalCtx)
{
	...
	{
		if((pthis->actionFunc == func_809F74C4) || (pthis->actionFunc == func_809F758C) || (pthis->actionFunc == func_809F799C))
		{
			CollisionCheck_SetAT(globalCtx, &globalCtx->colChkCtx, &pthis->collider.base);
		}

		if((pthis->actionFunc == func_809F74C4) || (pthis->actionFunc == func_809F758C) || (pthis->actionFunc == func_809F786C) || (pthis->actionFunc == func_809F78EC) || (pthis->actionFunc == func_809F799C))
		{
...
s32 func_809F7D50(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	...
	if(limbIndex == 1)
	{
		Matrix_Scale((pthis->rootScale * 0.5f) + 1.0f, pthis->rootScale + 1.0f, (pthis->rootScale * 0.5f) + 1.0f, MTXMODE_APPLY);
	}
