...
s32 func_80A3ED24(GlobalContext* globalCtx, EnGo* pthis, struct_80034A14_arg1* arg2, f32 arg3, u16 (*getTextId)(GlobalContext*, Actor*), s16 (*unkFunc2)(GlobalContext*, Actor*))
{
	if(arg2->unk_00)
	{
...
{
	Animation_Change(
	    &pthis->skelAnime, sAnimationEntries[animIndex].animation, sAnimationEntries[animIndex].playSpeed * ((pthis->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f), 0.0f, Animation_GetLastFrame(sAnimationEntries[animIndex].animation),
	    sAnimationEntries[animIndex].mode, sAnimationEntries[animIndex].morphRate);
}
...
s32 EnGo_IsActorSpawned(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	else if(globalCtx->sceneNum == SCENE_HIDAN && !Flags_GetSwitch(globalCtx, (pthis->actor.params) >> 8) && LINK_IS_ADULT && (pthis->actor.params & 0xF0) == 0x10)
	{
		return true;
	}
	...
	else if(globalCtx->sceneNum == SCENE_SPOT16 && LINK_IS_CHILD && ((pthis->actor.params & 0xF0) == 0x20 || (pthis->actor.params & 0xF0) == 0x30 || (pthis->actor.params & 0xF0) == 0x40))
	{
		return true;
	}
	else if(globalCtx->sceneNum == SCENE_SPOT18 && LINK_IS_CHILD && ((pthis->actor.params & 0xF0) == 0x50 || (pthis->actor.params & 0xF0) == 0x60 || (pthis->actor.params & 0xF0) == 0x70))
	{
		return true;
	}
	else
...
void func_80A3F060(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnGo_BiggoronActionFunc && pthis->actionFunc != EnGo_FireGenericActionFunc && pthis->actionFunc != func_80A40B1C)
	{
		unkVal = 1;
	}
...
void EnGo_UpdateShadow(EnGo* pthis)
{
	...
	s16 shadowAlphaTarget = (pthis->skelAnime.animation == &gGoronAnim_004930 && currentFrame > 32.0f) || pthis->skelAnime.animation != &gGoronAnim_004930 ? 255 : 0;
...
void func_80A3F908(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc == EnGo_BiggoronActionFunc || pthis->actionFunc == EnGo_GoronLinkRolling || pthis->actionFunc == EnGo_FireGenericActionFunc || pthis->actionFunc == EnGo_Eyedrops || pthis->actionFunc == func_80A40DCC ||
	   pthis->actionFunc == EnGo_GetItem || pthis->actionFunc == func_80A40C78 || pthis->actionFunc == func_80A40B1C)
	{
		...
		if((pthis->actor.params & 0xF0) == 0x90)
		{
			isUnkCondition = func_80A3ED24(globalCtx, pthis, &pthis->unk_1E0, float1, EnGo_GetTextID, EnGo_SetFlagsGetStates);
		}
		else
		{
...
void EnGo_GoronLinkRolling(EnGo* pthis, GlobalContext* globalCtx)
{
	if((EnGo_FollowPath(pthis, globalCtx) == true) && Flags_GetSwitch(globalCtx, pthis->actor.params >> 8) && (pthis->unk_218 == 0))
	{
		pthis->actor.speedXZ = 0.0f;
		EnGo_SetupAction(pthis, func_80A4008C);
...
void EnGo_CurledUp(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	{
		Audio_PlaySoundGeneral(NA_SE_EN_GOLON_WAKE_UP, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
void EnGo_WakeUp(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	{
		Math_SmoothStepToF(&pthis->skelAnime.playSpeed, ((pthis->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f) * 0.5f, 0.1f, 1000.0f, 0.1f);
	...
	{
		Audio_PlaySoundGeneral(NA_SE_EN_GOLON_SIT_DOWN, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
void func_80A40494(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	if(!(frame >= 0.0f))
	{
		Audio_PlaySoundGeneral(NA_SE_EN_DODO_M_GND, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
void func_80A405CC(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	Math_SmoothStepToF(&pthis->skelAnime.playSpeed, (pthis->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f, 0.1f, 1000.0f, 0.1f);
...
void func_80A408D8(EnGo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->skelAnime.playSpeed != 0.0f)
	{
		Math_SmoothStepToF(&pthis->skelAnime.playSpeed, ((pthis->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f) * -1.0f, 0.1f, 1000.0f, 0.1f);
		frame = pthis->skelAnime.curFrame;
		frame += pthis->skelAnime.playSpeed;
		if(frame >= 12.0f)
		{
...
void EnGo_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else if(pthis->actionFunc == EnGo_GoronLinkRolling || pthis->actionFunc == func_80A3FEB4 || pthis->actionFunc == EnGo_StopRolling || pthis->actionFunc == func_80A3FEB4)
	{
