...
void ObjMure2_SpawnActors(ObjMure2* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < D_80B9A818[actorNum]; i++)
	{
		if(pthis->actorSpawnPtrList[i] != NULL)
		{
			...
			osSyncPrintf("Warning : 既に子供がいる(%s %d)(arg_data 0x%04x)\n", "../z_obj_mure2.c", 269, pthis->actor.params);
			continue;
		}

		if(((pthis->currentActorNum >> i) & 1) == 0)
		{
			pthis->actorSpawnPtrList[i] = Actor_Spawn(&globalCtx->actorCtx, globalCtx, sActorSpawnIDs[actorNum], spawnPos[i].x, spawnPos[i].y, spawnPos[i].z, pthis->actor.world.rot.x, 0, pthis->actor.world.rot.z, params);
			if(pthis->actorSpawnPtrList[i] != NULL)
			{
...
void func_80B9A534(ObjMure2* pthis)
{
	...
	for(i = 0; i < D_80B9A818[pthis->actor.params & 3]; i++)
	{
		if(pthis->actorSpawnPtrList[i] != NULL && (((pthis->currentActorNum >> i) & 1) == 0) && (pthis->actorSpawnPtrList[i]->update == NULL))
		{
			pthis->currentActorNum |= (1 << i);
			pthis->actorSpawnPtrList[i] = NULL;
		}
...
void func_80B9A668(ObjMure2* pthis, GlobalContext* globalCtx)
{
	if(Math3D_Dist1DSq(pthis->actor.projectedPos.x, pthis->actor.projectedPos.z) < (sDistSquared1[pthis->actor.params & 3] * pthis->unk_184))
	{
...
{
	func_80B9A534(pthis);
	if((sDistSquared2[pthis->actor.params & 3] * pthis->unk_184) <= Math3D_Dist1DSq(pthis->actor.projectedPos.x, pthis->actor.projectedPos.z))
	{
		pthis->actor.flags &= ~ACTOR_FLAG_4;
