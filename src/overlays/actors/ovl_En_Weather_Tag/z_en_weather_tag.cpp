...
ActorInit En_Weather_Tag_InitVars = {
    ACTOR_EN_WEATHER_TAG, ACTORCAT_PROP, FLAGS, OBJECT_GAMEPLAY_KEEP, ACTOR_FACTORY(EnWeatherTag), (ActorFunc)EnWeatherTag_Init, (ActorFunc)EnWeatherTag_Destroy, (ActorFunc)EnWeatherTag_Update, NULL, (ActorFunc)EnWeatherTag_Reset,
};
...
void EnWeatherTag_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	switch(pthis->actor.params & 0xF)
	{
		...
		case EN_WEATHER_TAG_TYPE_THUNDERSTORM_KAKARIKO:
			...
			if(!(gSaveContext.eventChkInf[4] & 0x100) || !(gSaveContext.eventChkInf[4] & 0x200) || !(gSaveContext.eventChkInf[4] & 0x400) || CHECK_QUEST_ITEM(QUEST_MEDALLION_SHADOW))
			{
				Actor_Kill(&pthis->actor);
			}
...
u8 WeatherTag_CheckEnableWeatherEffect(EnWeatherTag* pthis, GlobalContext* globalCtx, u8 arg2, u8 arg3, u8 arg4, u8 arg5, u16 arg6, u8 weatherMode)
{
	s32 pad;
	u8 ret = false;
	...
	{
		if((globalCtx->envCtx.indoors != 0) || !gSkyboxBlendingEnabled || (globalCtx->skyboxId != SKYBOX_NORMAL_SKY && globalCtx->envCtx.unk_1F == globalCtx->envCtx.unk_20))
		{
			D_8011FB38 = 1;
			if(globalCtx->envCtx.gloomySkyMode == 0 && (globalCtx->envCtx.indoors != 0 || (globalCtx->envCtx.unk_1F != 1 && globalCtx->envCtx.unk_21 == 0)))
			{
				D_8011FB38 = 0;
...
u8 WeatherTag_CheckRestoreWeather(EnWeatherTag* pthis, GlobalContext* globalCtx, u8 arg2, u8 arg3, u8 arg4, u8 arg5, u16 arg6)
{
	s32 pad;
	u8 ret = false;
	Player* player = GET_PLAYER(globalCtx);
