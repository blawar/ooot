...
void ObjDekujr_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	const auto& frameCount = globalCtx->frames;

	OPEN_DISPS(globalCtx->gfxCtx, "../z_obj_dekujr.c", 370);

	func_80093D18(globalCtx->gfxCtx);
	func_80093D84(globalCtx->gfxCtx);

	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_obj_dekujr.c", 379), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, object_dekujr_DL_0030D0);

	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, frameCount % 128, 0, 32, 32, 1, frameCount % 128, 0, 32, 32));
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_obj_dekujr.c", 399), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, object_dekujr_DL_0032D8);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_obj_dekujr.c", 409);
}
