...
void BgMizuBwall_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	switch((u16)pthis->dyna.actor.params & 0xF)
	{
		...
		case MIZUBWALL_RUTO_ROOM:
			...
			{
				Collider_InitTris(globalCtx, &pthis->collider);
				if(!Collider_SetTris(globalCtx, &pthis->collider, &pthis->dyna.actor, &sTrisInitRutoWall, pthis->elements))
				{
					osSyncPrintf("Error : コリジョンデータセット失敗(%s %d)(arg_data 0x%04x)\n", "../z_bg_mizu_bwall.c", 558, pthis->dyna.actor.params);
					Actor_Kill(&pthis->dyna.actor);
				}
				else
				{
		...
		case MIZUBWALL_UNUSED:
			...
			{
				Collider_InitTris(globalCtx, &pthis->collider);
				if(!Collider_SetTris(globalCtx, &pthis->collider, &pthis->dyna.actor, &sTrisInitUnusedWall, pthis->elements))
				{
					osSyncPrintf("Error : コリジョンデータセット失敗(%s %d)(arg_data 0x%04x)\n", "../z_bg_mizu_bwall.c", 638, pthis->dyna.actor.params);
					Actor_Kill(&pthis->dyna.actor);
				}
				else
				{
		...
		case MIZUBWALL_STINGER_ROOM_1:
			...
			{
				Collider_InitTris(globalCtx, &pthis->collider);
				if(!Collider_SetTris(globalCtx, &pthis->collider, &pthis->dyna.actor, &sTrisInitStingerWall, pthis->elements))
				{
					osSyncPrintf("Error : コリジョンデータセット失敗(%s %d)(arg_data 0x%04x)\n", "../z_bg_mizu_bwall.c", 724, pthis->dyna.actor.params);
					Actor_Kill(&pthis->dyna.actor);
				}
				else
				{
		...
		case MIZUBWALL_STINGER_ROOM_2:
			...
			else
			{
				...
				if(!Collider_SetTris(globalCtx, &pthis->collider, &pthis->dyna.actor, &sTrisInitStingerWall, pthis->elements))
				{
					osSyncPrintf("Error : コリジョンデータセット失敗(%s %d)(arg_data 0x%04x)\n", "../z_bg_mizu_bwall.c", 798, pthis->dyna.actor.params);
					Actor_Kill(&pthis->dyna.actor);
				}
				else
				{
...
void BgMizuBwall_SpawnDebris(BgMizuBwall* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(debrisOffsets); i++)
	{
		...
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_A_OBJ, debrisPos.x, debrisPos.y, debrisPos.z, 0, 0, 0, 0xB);
	}
}
