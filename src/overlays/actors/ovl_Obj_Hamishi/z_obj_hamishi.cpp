...
void ObjHamishi_Shake(ObjHamishi* pthis)
{
	if(pthis->shakeFrames > 0)
	{
		...
		pthis->actor.shape.rot.x = pthis->actor.home.rot.x + (s16)(Math_SinS(pthis->shakeRotPhase * 4) * pthis->shakeRotSize);
		pthis->actor.shape.rot.z = pthis->actor.home.rot.z + (s16)(Math_CosS(pthis->shakeRotPhase * 7) * pthis->shakeRotSize);
	}
	else
	{
...
void ObjHamishi_Break(ObjHamishi* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(sEffectScales); i++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &pthis->actor.world.pos, gravity, phi_v0, 30, 5, 0, sEffectScales[i], 3, 0, 70, 1, OBJECT_GAMEPLAY_FIELD_KEEP, gSilverRockFragmentsDL);
	}
