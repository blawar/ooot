...
static Color_RGB8 sBeamPrimColors_67[] = {
    {255, 255, 170}, {170, 255, 255}, {255, 170, 255}, {255, 255, 170}, {255, 255, 170}, {255, 255, 170},
};
...
static void* D_80BA5900[] = {
    gEffSunGraveSpark1Tex, gEffSunGraveSpark2Tex, gEffSunGraveSpark3Tex, gEffSunGraveSpark4Tex, gEffSunGraveSpark5Tex, gEffSunGraveSpark6Tex, gEffSunGraveSpark7Tex, gEffSunGraveSpark8Tex,
};
...
void ObjectKankyo_Fairies(ObjectKankyo* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->envCtx.unk_EE[3] < 64 && (gSaveContext.entranceIndex != 0x00EE || gSaveContext.sceneSetupIndex != 4 || globalCtx->envCtx.unk_EE[3]))
	{
			...
			case 2: // fairy trails
				...
				else if(pthis->effects[i].state == 2)
				{
					...
					{ // leader fairy
						...
						Math_SmoothStepToF(&pthis->effects[i].base.y, player->actor.world.pos.y + 50.0f, 0.5f, 1.0f, 0.2f);
						Math_SmoothStepToF(&pthis->effects[i].base.z, player->actor.world.pos.z, 0.5f, 1.0f, 0.2f);

						// results unused
						Math_SmoothStepToF(&pthis->effects[i].pos.x, Math_SinS(pthis->effects[i].angle - 0x8000) * pthis->effects[i].flightRadius, 0.5f, 2.0f, 0.2f);
						Math_SmoothStepToF(&pthis->effects[i].pos.z, Math_CosS(pthis->effects[i].angle - 0x8000) * pthis->effects[i].flightRadius, 0.5f, 2.0f, 0.2f);
						...
						pthis->effects[i].pos.x = Math_SinS(pthis->effects[i].angle - 0x8000) * pthis->effects[i].flightRadius;
						pthis->effects[i].pos.z = Math_CosS(pthis->effects[i].angle - 0x8000) * pthis->effects[i].flightRadius;
					}
					else
					{ // trailing fairy
						Math_SmoothStepToF(&pthis->effects[i].size, 0.1f, 0.10f, 0.001f, 0.00001f);
				...
				{
					maxDist = 130.0f;
					if(pthis->effects[i].base.x + pthis->effects[i].pos.x - baseX > maxDist || pthis->effects[i].base.x + pthis->effects[i].pos.x - baseX < -maxDist ||
					   pthis->effects[i].base.y + pthis->effects[i].pos.y - baseY > maxDist || pthis->effects[i].base.y + pthis->effects[i].pos.y - baseY < -maxDist ||
					   pthis->effects[i].base.z + pthis->effects[i].pos.z - baseZ > maxDist || pthis->effects[i].base.z + pthis->effects[i].pos.z - baseZ < -maxDist)
					{
...
void ObjectKankyo_DrawFairies(ObjectKankyo* pthis2, GlobalContext* globalCtx2)
{
	...
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 807);
		...
		for(i = 0; i < globalCtx->envCtx.unk_EE[3]; i++)
		{
			Matrix_Translate(pthis->effects[i].base.x + pthis->effects[i].pos.x, pthis->effects[i].base.y + pthis->effects[i].pos.y, pthis->effects[i].base.z + pthis->effects[i].pos.z, MTXMODE_NEW);
			...
			Matrix_Scale(pthis->effects[i].size * alphaScale, pthis->effects[i].size * alphaScale, pthis->effects[i].size * alphaScale, MTXMODE_APPLY);
			if(i < 32)
			{
				if(pthis->effects[i].state != 2)
				{
			...
			Matrix_RotateZ(DEG_TO_RAD(globalCtx->frames * 20.0f), MTXMODE_APPLY);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_object_kankyo.c", 913), G_MTX_LOAD);
			gSPDisplayList(POLY_XLU_DISP++, gKokiriDustMoteDL);
		}
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 922);
	}
}
...
void ObjectKankyo_DrawSnow(ObjectKankyo* pthis2, GlobalContext* globalCtx2)
{
	...
	if(!(globalCtx->cameraPtrs[0]->unk_14C & 0x100))
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 958);
		if(globalCtx->envCtx.unk_EE[2] < globalCtx->envCtx.unk_EE[3])
		{
			if(globalCtx->frames % 16 == 0)
			{
		...
		for(i = 0; i < globalCtx->envCtx.unk_EE[2]; i++)
		{
			switch(pthis->effects[i].state)
			{
				...
				case 1:
					...
					if(pthis->effects[i].base.y + pthis->effects[i].pos.y < pthis->actor.world.pos.y || pthis->effects[i].base.y + pthis->effects[i].pos.y < globalCtx->view.eye.y - 150.0f)
					{
						pthis->effects[i].state++;
					}
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_object_kankyo.c", 1107), G_MTX_LOAD);
		...
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1127);
	}
}
...
void ObjectKankyo_DrawLightning(ObjectKankyo* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1182);

	if(pthis->effects[0].state == 1)
	{
		Matrix_Translate(globalCtx->csCtx.npcActions[0]->startPos.x, globalCtx->csCtx.npcActions[0]->startPos.y, globalCtx->csCtx.npcActions[0]->startPos.z, MTXMODE_NEW);
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_object_kankyo.c", 1213), G_MTX_LOAD);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1233);
}
...
void ObjectKankyo_DrawSunGraveSpark(ObjectKankyo* pthis2, GlobalContext* globalCtx2)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1324);
	if(globalCtx->csCtx.state != 0)
	{
		if(globalCtx->csCtx.npcActions[1] != NULL && globalCtx->csCtx.npcActions[1]->action == 2 && pthis->requiredObjectLoaded)
		{
			...
			Matrix_Translate((end.x - start.x) * weight + start.x, (end.y - start.y) * weight + start.y, (end.z - start.z) * weight + start.z, MTXMODE_NEW);
			Matrix_Scale(pthis->effects[0].size, pthis->effects[0].size, pthis->effects[0].size, MTXMODE_APPLY);
			func_80093D84(globalCtx->gfxCtx);
			gDPPipeSync(POLY_XLU_DISP++);
			gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, (u8)(105 * pthis->effects[0].amplitude) + 150, 255, (u8)(105 * pthis->effects[0].amplitude) + 150, pthis->effects[0].alpha);
			gDPSetEnvColor(POLY_XLU_DISP++, (u8)(155 * pthis->effects[0].amplitude) + 100, (u8)(255 * pthis->effects[0].amplitude), 255 - (u8)(255 * pthis->effects[0].amplitude), pthis->effects[0].alpha);

			Matrix_Mult(&globalCtx->billboardMtxF, MTXMODE_APPLY);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_object_kankyo.c", 1416), G_MTX_LOAD);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1432);
}
...
void ObjectKankyo_DrawBeams(ObjectKankyo* pthis2, GlobalContext* globalCtx2)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1539);
	...
	if(pthis->requiredObjectLoaded)
	{
		for(i = 0; i < 6; i++)
		{
			if(pthis->effects[i].size > 0.001f)
			{
				...
				func_80093D84(globalCtx->gfxCtx);
				gDPPipeSync(POLY_XLU_DISP++);
				gDPSetPrimColor(POLY_XLU_DISP++, 0, 128, sBeamPrimColors_67[i].r, sBeamPrimColors_67[i].g, sBeamPrimColors_67[i].b, 128);
				gDPSetEnvColor(POLY_XLU_DISP++, sBeamEnvColors_67[i].r, sBeamEnvColors_67[i].g, sBeamEnvColors_67[i].b, 128);
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_object_kankyo.c", 1586), G_MTX_LOAD);
				gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, globalCtx->frames * 5, globalCtx->frames * 10, 32, 64, 1, globalCtx->frames * 5, globalCtx->frames * 10, 32, 64));
				gSPDisplayList(POLY_XLU_DISP++, gDemoKekkaiDL_005FF0);
			}
		}
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_object_kankyo.c", 1607);
}
