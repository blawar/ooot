...
void EnPoSisters_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gPoeSistersSkel, &gPoeSistersSwayAnim, pthis->jointTable, pthis->morphTable, 12);
	pthis->unk_22E.r = 255;
	pthis->unk_22E.g = 255;
	pthis->unk_22E.b = 210;
	pthis->unk_22E.a = 255;
	pthis->lightNode = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &pthis->lightInfo);
	Lights_PointGlowSetInfo(&pthis->lightInfo, pthis->actor.home.pos.x, pthis->actor.home.pos.y, pthis->actor.home.pos.z, 0, 0, 0, 0);
...
void func_80AD95D8(EnPoSisters* pthis)
{
	...
	{
		pthis->actor.world.rot.y = (pthis->collider.info.acHitInfo->toucher.dmgFlags & 0x1F824) ? (float)pthis->collider.base.ac->world.rot.y : (float)Actor_WorldYawTowardActor(&pthis->actor, pthis->collider.base.ac) + 0x8000;
	}
	if(pthis->unk_194 != 0)
	{
...
{
	Animation_Change(&pthis->skelAnime, &gPoeSistersAppearDisappearAnim, 1.5f, 0.0f, Animation_GetLastFrame(&gPoeSistersAppearDisappearAnim), ANIMMODE_ONCE, -3.0f);
	pthis->actor.speedXZ = 0.0f;
	pthis->unk_19C = 100;
...
void func_80AD97C8(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	{
		if((player->swordState == 0 || player->swordAnimation >= 24) && player->actor.world.pos.y - player->actor.floorHeight < 1.0f)
		{
...
void func_80AD9AA8(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	Actor* actor1 = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_PO_SISTERS, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0x400);
	Actor* actor2 = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_PO_SISTERS, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0x800);
	Actor* actor3 = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_PO_SISTERS, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0xC00);
	s32 pad;
	s32 pad1;
...
void func_80AD9C24(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx != NULL)
	{
		...
		EffectSsDeadDb_Spawn(globalCtx, &vec, &sZeroVector, &sZeroVector, 150, 0, 255, 255, 255, 155, 150, 150, 150, 1, 9, 0);
	}
...
void func_80AD9D44(EnPoSisters* pthis)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gPoeSistersAppearDisappearAnim, 0.5f, 0.0f, Animation_GetLastFrame(&gPoeSistersAppearDisappearAnim), ANIMMODE_ONCE_INTERP, 0.0f);
	}
	pthis->unk_22E.a = 0;
	pthis->unk_199 = 32;
...
{
	Animation_Change(&pthis->skelAnime, &gPoeSistersAppearDisappearAnim, 0.833f, 0.0f, Animation_GetLastFrame(&gPoeSistersAppearDisappearAnim), ANIMMODE_ONCE_INTERP, 0.0f);
...
void func_80ADAAA4(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_195 != 0)
	{
		Math_ScaledStepToS(&pthis->actor.shape.rot.y, pthis->actor.parent->shape.rot.y, (pthis->unk_195 == 2) ? 0x800 : 0x400);
...
void func_80ADAFC0(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	pthis->unk_234[0].x = (Math_SinS((pthis->actor.shape.rot.y + pthis->unk_19A * 0x3000) - 0x4000) * (3000.0f * pthis->actor.scale.x)) + pthis->actor.world.pos.x;
	pthis->unk_234[0].z = (Math_CosS((pthis->actor.shape.rot.y + pthis->unk_19A * 0x3000) - 0x4000) * (3000.0f * pthis->actor.scale.x)) + pthis->actor.world.pos.z;
	if(pthis->unk_19A < 8)
	{
...
void func_80ADB51C(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(temp != 0)
		{
			...
			Math_ScaledStepToS(&pthis->actor.shape.rot.y, pthis->actor.parent->shape.rot.y + (pthis->unk_195 * 0x4000) * phi_v0, phi_a2);
		}
	...
	if(pthis->unk_19C > 0)
	{
		if(pthis->unk_19A >= 16)
		{
			...
			if(pthis->unk_195 == 0)
			{
				...
				{
					pthis->actor.shape.rot.y += (0x580 - (pthis->unk_19C * 0x180)) * fabsf(Math_SinS(pthis->unk_196 * 0x800));
				}
...
void func_80ADBB6C(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	{
		Math_ScaledStepToS(&pthis->actor.world.rot.y, Actor_WorldYawTowardPoint(&pthis->actor, &pthis->actor.home.pos), 1820);
	}
}
...
void func_80ADC10C(EnPoSisters* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->collider.base.acFlags & AC_HIT)
	{
		...
		else if(pthis->collider.base.colType == 9 || (pthis->actor.colChkInfo.damageEffect == 0 && pthis->actor.colChkInfo.damage == 0))
		{
			if(pthis->unk_194 == 0)
			{
		...
		else if(pthis->unk_194 == 0 && pthis->actor.colChkInfo.damageEffect == 0xE && pthis->actionFunc == func_80ADB770)
		{
			if(pthis->unk_19C == 0)
			{
				pthis->unk_19C = -45;
...
void EnPoSisters_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_199 & 0x1F)
	{
		...
		else
		{
			...
			vec.z = pthis->actor.world.pos.z;
			pthis->actor.floorHeight = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->actor.floorPoly, &sp34, &pthis->actor, &vec);
		}
...
s32 EnPoSisters_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfxP)
{
	EnPoSisters* pthis = (EnPoSisters*)thisx;
	Color_RGBA8* color;
...
void EnPoSisters_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP)
{
	EnPoSisters* pthis = (EnPoSisters*)thisx;
	s32 i;
	s32 pad;
	...
	if(limbIndex == 8 && pthis->actionFunc != func_80ADB2B8)
	{
		...
		if(pthis->unk_198 > 0)
		{
			...
			if(pthis->actionFunc == func_80ADB17C || pthis->actionFunc == func_80ADBD38 || pthis->actionFunc == func_80ADBEE8)
			{
				Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->unk_234[0].x, pthis->unk_234[0].y + 15.0f, pthis->unk_234[0].z, color->r * temp_f2, color->g * temp_f2, color->b * temp_f2, 200);
			}
			else
			{
				Lights_PointGlowSetInfo(&pthis->lightInfo, pthis->unk_234[0].x, pthis->unk_234[0].y + 15.0f, pthis->unk_234[0].z, color->r * temp_f2, color->g * temp_f2, color->b * temp_f2, 200);
			}
		}
		else
		{
...
void EnPoSisters_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_22E.a == 255 || pthis->unk_22E.a == 0)
	{
		...
		POLY_OPA_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnPoSisters_OverrideLimbDraw, EnPoSisters_PostLimbDraw, &pthis->actor, POLY_OPA_DISP);
	}
	else
	{
		gDPSetEnvColor(POLY_XLU_DISP++, 255, 255, 255, pthis->unk_22E.a);
		gSPSegment(POLY_XLU_DISP++, 0x09, D_80116280);
		POLY_XLU_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnPoSisters_OverrideLimbDraw, EnPoSisters_PostLimbDraw, &pthis->actor, POLY_XLU_DISP);
	}
	if(!(pthis->unk_199 & 0x80))
	{
