...
void EnOkuta_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(thisx->params == 0)
	{
		SkelAnime_Init(globalCtx, &pthis->skelAnime, &gOctorokSkel, &gOctorokAppearAnim, pthis->jointTable, pthis->morphTable, 38);
		...
		thisx->floorHeight = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &thisx->floorPoly, &sp30, thisx, &thisx->world.pos);
		//! @bug calls WaterBox_GetSurfaceImpl directly
		if(!WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, thisx->world.pos.x, thisx->world.pos.z, &ySurface, &outWaterBox) || (ySurface <= thisx->floorHeight))
		{
			Actor_Kill(thisx);
		}
		else
...
void EnOkuta_SpawnRipple(EnOkuta* pthis, GlobalContext* globalCtx)
{
	...
	if((globalCtx->gameplayFrames % 7) == 0 && ((pthis->actionFunc != EnOkuta_Shoot) || ((pthis->actor.world.pos.y - pthis->actor.home.pos.y) < 50.0f)))
	{
	...
	else
	{
		if(Math_StepToF(&pthis->actor.scale.x, 0.0f, 0.0005f))
		{
			...
			for(i = 0; i < 20; i++)
			{
				...
				EffectSsDtBubble_SpawnCustomColor(globalCtx, &pthis->actor.world.pos, &velocity, &accel_59, &primColor_59, &envColor_59, Rand_S16Offset(100, 50), 25, 0);
			}
...
void EnOkuta_Freeze(EnOkuta* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer >= 64) && (pthis->timer & 1))
	{
		...
		EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &pos, 150, 150, 150, 250, 235, 245, 255, (Rand_ZeroOne() * 0.2f) + 1.9f);
	}
	Math_ApproachF(&pthis->actor.world.pos.y, pthis->actor.home.pos.y, 0.5f, 5.0f);
}
...
void EnOkuta_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(!(player->stateFlags1 & (PLAYER_STATE1_6 | PLAYER_STATE1_DEAD | PLAYER_STATE1_28 | PLAYER_STATE1_29)))
	{
		if(pthis->actor.params == 0)
		{
			...
			if(!WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &ySurface, &outWaterBox) || (ySurface < pthis->actor.floorHeight))
			{
		...
		if(pthis->actor.params == 0)
		{
			...
			pthis->collider.dim.height = (((sOctorockColliderInit.dim.height * pthis->headScale.y) - pthis->collider.dim.yShift) * pthis->actor.scale.y * 100.0f);
		}
		else
		{
			sp34 = false;
		...
		{
			if((pthis->actionFunc != EnOkuta_Die) && (pthis->actionFunc != EnOkuta_WaitToDie) && (pthis->actionFunc != EnOkuta_Freeze))
			{
...
s32 EnOkuta_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnOkuta* pthis = (EnOkuta*)thisx;
	f32 curFrame = pthis->skelAnime.curFrame;
	Vec3f scale;
...
void EnOkuta_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params == 0)
	{
		SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnOkuta_OverrideLimbDraw, NULL, pthis);
	}
	else
	{
