...
void ObjSyokudai_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y + 70.0f, pthis->actor.world.pos.z, 255, 255, 180, -1);
...
void ObjSyokudai_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &waterSurface, &dummy) && ((waterSurface - pthis->actor.world.pos.y) > 52.0f))
	{
		pthis->litTimer = 0;
		if(torchType == 1)
		{
		...
		if(interactionType != 0)
		{
			if(pthis->litTimer != 0)
			{
				if(interactionType < 0)
				{
					...
					{
						player->unk_860 = 210;
						Audio_PlaySoundGeneral(NA_SE_EV_FLAME_IGNITION, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
					}
			...
			else if((torchType != 0) && (((interactionType > 0) && (dmgFlags & 0x20800)) || ((interactionType < 0) && (player->unk_860 != 0))))
			{
