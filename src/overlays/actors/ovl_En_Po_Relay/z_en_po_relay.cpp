...
void EnPoRelay_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, oot::asset::skel::header::load(symbol::gDampeSkel), oot::asset::anim::header::load(symbol::gDampeFloatAnim), pthis->jointTable, pthis->morphTable,
                       18);
    ...
    if (pthis->actionTimer == 0 && Rand_ZeroOne() < 0.03f) {
        ...
        if (pthis->pathIndex < 23) {
            ...
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HONOTRAP,
                        Math_CosS(pthis->unk_19A) * speed + pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                        Math_SinS(pthis->unk_19A) * speed + pthis->actor.world.pos.z, 0,
...
void EnPoRelay_DisappearAndReward(EnPoRelay* pthis, GlobalContext* globalCtx) {
    ...
    if (pthis->actionTimer < 8) {
        ...
        EffectSsDeadDb_Spawn(globalCtx, &vec, &D_80AD8D30, &D_80AD8D3C, pthis->actionTimer * 10 + 80, 0, 255, 255, 255,
...
void EnPoRelay_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 14) {
        ...
        gSPDisplayList(POLY_OPA_DISP++, oot::asset::gfx::load(symbol::gDampeLanternDL));
        if (1) {}
        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_po_relay.c", 901);
        Matrix_MultVec3f(&D_80AD8D48, &vec);
        Lights_PointNoGlowSetInfo(&pthis->lightInfo, vec.x, vec.y, vec.z, pthis->lightColor.r, pthis->lightColor.g,
        ...
        gSPDisplayList(POLY_OPA_DISP++, oot::asset::gfx::load(symbol::gDampeHaloDL));
