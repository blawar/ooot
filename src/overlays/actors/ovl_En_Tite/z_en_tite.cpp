...
static Vec3f sIceChunks[12] = {
    {20.0f, 20.0f, 0.0f},  {10.0f, 40.0f, 10.0f}, {-10.0f, 40.0f, 10.0f}, {-20.0f, 20.0f, 0.0f}, {10.0f, 40.0f, -10.0f}, {-10.0f, 40.0f, -10.0f},
    {0.0f, 20.0f, -20.0f}, {10.0f, 0.0f, 10.0f},  {10.0f, 0.0f, -10.0f},  {0.0f, 20.0f, 20.0f},	 {-10.0f, 0.0f, 10.0f},	 {-10.0f, 0.0f, -10.0f},
};
...
void EnTite_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &object_tite_Skel_003A20, &object_tite_Anim_0012E4, pthis->jointTable, pthis->morphTable, 25);
...
void EnTite_Idle(EnTite* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_1)) && (pthis->actor.velocity.y <= 0.0f))
	{
	...
	if(SkelAnime_Update(&pthis->skelAnime) != 0)
	{
		...
		switch(pthis->vAttackState)
		{
			...
			case TEKTITE_MID_LUNGE:
				...
				if((pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_1)) || ((pthis->actor.params == TEKTITE_BLUE) && (pthis->actor.bgCheckFlags & BG_STATE_5)))
				{
...
void EnTite_MoveTowardPlayer(EnTite* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.bgCheckFlags & BG_STATE_1) || ((pthis->actor.params == TEKTITE_BLUE) && (pthis->actor.bgCheckFlags & BG_STATE_6)))
	{
		if(pthis->vQueuedJumps != 0)
		{
			pthis->vQueuedJumps--;
	...
	if(((pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_1)) || (pthis->actor.params == TEKTITE_BLUE && (pthis->actor.bgCheckFlags & (BG_STATE_5 | BG_STATE_6)))) && (pthis->actor.velocity.y <= 0.0f))
	{
...
void EnTite_Recoil(EnTite* pthis, GlobalContext* globalCtx)
{
	...
	angleToPlayer = (pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y);
	if((pthis->actor.speedXZ == 0.0f) && ((pthis->actor.bgCheckFlags & BG_STATE_0) || ((pthis->actor.params == TEKTITE_BLUE) && (pthis->actor.bgCheckFlags & BG_STATE_5))))
	{
		pthis->actor.world.rot.y = pthis->actor.shape.rot.y;
		pthis->collider.base.atFlags &= ~AT_HIT;
		if((pthis->actor.xzDistToPlayer > 300.0f) && (pthis->actor.yDistToPlayer > 80.0f) && (ABS((float)pthis->actor.shape.rot.x) < 4000) && (ABS((float)pthis->actor.shape.rot.z) < 4000) &&
		   ((pthis->actor.bgCheckFlags & BG_STATE_0) || ((pthis->actor.params == TEKTITE_BLUE) && (pthis->actor.bgCheckFlags & BG_STATE_5))))
		{
			EnTite_SetupIdle(pthis);
		}
		else if((pthis->actor.xzDistToPlayer < 180.0f) && (pthis->actor.yDistToPlayer <= 80.0f) && (ABS(angleToPlayer) <= 6000))
		{
			EnTite_SetupAttack(pthis);
		}
		else
...
{
	Animation_Change(&pthis->skelAnime, &object_tite_Anim_0012E4, 0.0f, 0.0f, (f32)Animation_GetLastFrame(&object_tite_Anim_0012E4), ANIMMODE_LOOP, 4.0f);
...
void EnTite_Stunned(EnTite* pthis, GlobalContext* globalCtx)
{
	...
	angleToPlayer = pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y;
	if(((pthis->actor.colorFilterTimer == 0) && (pthis->actor.speedXZ == 0.0f)) && ((pthis->actor.bgCheckFlags & BG_STATE_0) || ((pthis->actor.params == TEKTITE_BLUE) && (pthis->actor.bgCheckFlags & BG_STATE_5))))
	{
		pthis->actor.world.rot.y = pthis->actor.shape.rot.y;
		...
		else if(pthis->flipState == TEKTITE_FLIPPED)
		{
		    ...
		    ((pthis->actor.xzDistToPlayer > 300.0f) && (pthis->actor.yDistToPlayer > 80.0f) && (ABS((float)pthis->actor.shape.rot.x) < 4000) && (ABS((float)pthis->actor.shape.rot.z) < 4000)) &&
		    ((pthis->actor.bgCheckFlags & BG_STATE_0) || ((pthis->actor.params == TEKTITE_BLUE) && (pthis->actor.bgCheckFlags & BG_STATE_5))))
		{
			EnTite_SetupIdle(pthis);
		}
...
void EnTite_DeathCry(EnTite* pthis, GlobalContext* globalCtx)
{
	EffectSsDeadSound_SpawnStationary(globalCtx, &pthis->actor.projectedPos, NA_SE_EN_TEKU_DEAD, true, DEADSOUND_REPEAT_MODE_OFF, 40);
...
void EnTite_CheckDamage(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else if((thisx->colChkInfo.health != 0) && (globalCtx->actorCtx.unk_02 != 0) && (thisx->xzDistToPlayer <= 400.0f) && (thisx->bgCheckFlags & BG_STATE_0))
	{
		if(pthis->flipState == TEKTITE_FLIPPED)
		{
			EnTite_SetupFlipUpright(pthis);
...
void EnTite_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(thisx->colChkInfo.damageEffect != 0xE)
	{
		...
		if((pthis->actor.params == TEKTITE_BLUE) && (thisx->bgCheckFlags & BG_STATE_5))
		{
			floorPoly = thisx->floorPoly;
			if((((globalCtx->gameplayFrames % 8) == 0) || (thisx->velocity.y < 0.0f)) && (WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->backRightFootPos.x, pthis->backRightFootPos.z, &waterSurfaceY, &waterBox)) &&
			   (pthis->backRightFootPos.y <= waterSurfaceY))
			{
				pthis->backRightFootPos.y = waterSurfaceY;
				EffectSsGRipple_Spawn(globalCtx, &pthis->backRightFootPos, 0, 220, 0);
			}
			if(((((globalCtx->gameplayFrames.whole() + 2) % 8) == 0) || (thisx->velocity.y < 0.0f)) && (WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->backLeftFootPos.x, pthis->backLeftFootPos.z, &waterSurfaceY, &waterBox)) &&
			   (pthis->backLeftFootPos.y <= waterSurfaceY))
			{
				pthis->backLeftFootPos.y = waterSurfaceY;
				EffectSsGRipple_Spawn(globalCtx, &pthis->backLeftFootPos, 0, 220, 0);
			}
			if(((((globalCtx->gameplayFrames.whole() + 4) % 8) == 0) || (thisx->velocity.y < 0.0f)) && (WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->frontLeftFootPos.x, pthis->frontLeftFootPos.z, &waterSurfaceY, &waterBox)) &&
			...
			if(((((globalCtx->gameplayFrames.whole() + 1) % 8) == 0) || (thisx->velocity.y < 0.0f)) &&
			   (WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->frontRightFootPos.x, pthis->frontRightFootPos.z, &waterSurfaceY, &waterBox)) && (pthis->frontRightFootPos.y <= waterSurfaceY))
			{
		...
		if(thisx->bgCheckFlags & (BG_STATE_0 | BG_STATE_1))
		{
