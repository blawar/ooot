...
static InitChainEntry sInitChainCapturableFlame[] = {
    ICHAIN_U8(targetMode, 0, ICHAIN_CONTINUE), ICHAIN_F32(targetArrowOffset, 60, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneForward, 1000, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneScale, 400, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneDownward, 1000, ICHAIN_STOP),
};
...
void EnIceHono_InitDroppedFlame(Actor* thisx, GlobalContext* globalCtx)
{
	...
	pthis->actor.shape.rot.x = pthis->actor.shape.rot.y = pthis->actor.shape.rot.z = pthis->actor.world.rot.x = pthis->actor.world.rot.y = pthis->actor.world.rot.z = 0;
...
void EnIceHono_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, (s16)pthis->actor.world.pos.y + 10, pthis->actor.world.pos.z, 155, 210, 255, 0);
...
void EnIceHono_DropFlame(EnIceHono* pthis, GlobalContext* globalCtx)
{
	u32 bgFlag = pthis->actor.bgCheckFlags & BG_STATE_0;
	...
	if(bgFlag != 0)
	{
		...
		for(i = 0; i < 8; i++)
		{
			Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ICE_HONO, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, ((s32)(Rand_ZeroOne() * 1000.0f) + i * 0x2000) - 0x1F4, 0, 1);
		}
...
void EnIceHono_SpreadFlames(EnIceHono* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer == 46)
	{
		...
		{
			s32 rot = i * 0x1999;
			Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ICE_HONO, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, ((s32)(Rand_ZeroOne() * 1000.0f) + rot) - 0x1F4, 0, 2);
		}
	}
...
void EnIceHono_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params == -1) || (pthis->actor.params == 0))
	{
		...
		Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, (s16)pthis->actor.world.pos.y + 10, pthis->actor.world.pos.z, (s32)(155.0f * intensity), (s32)(210.0f * intensity), (s32)(255.0f * intensity), 1400);
	}
