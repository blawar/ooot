...
static InitChainEntry sInitChain[] = {
    ICHAIN_S8(naviEnemyId, 0x1B, ICHAIN_CONTINUE), ICHAIN_F32(targetArrowOffset, 500, ICHAIN_CONTINUE), ICHAIN_VEC3F_DIV1000(scale, 15, ICHAIN_CONTINUE), ICHAIN_F32(scale.y, 0, ICHAIN_CONTINUE), ICHAIN_F32_DIV1000(gravity, -1500, ICHAIN_STOP),
};
...
void EnTest_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gStalfosSkel, &gStalfosMiddleGuardAnim, pthis->jointTable, pthis->morphTable, STALFOS_LIMB_MAX);
	SkelAnime_Init(globalCtx, &pthis->upperSkelanime, &gStalfosSkel, &gStalfosMiddleGuardAnim, pthis->upperJointTable, pthis->upperMorphTable, STALFOS_LIMB_MAX);
	...
	slashBlure.p1StartColor[0] = slashBlure.p1StartColor[1] = slashBlure.p1StartColor[2] = slashBlure.p1StartColor[3] = slashBlure.p2StartColor[0] = slashBlure.p2StartColor[1] = slashBlure.p2StartColor[2] = slashBlure.p1EndColor[0] =
	    slashBlure.p1EndColor[1] = slashBlure.p1EndColor[2] = slashBlure.p2EndColor[0] = slashBlure.p2EndColor[1] = slashBlure.p2EndColor[2] = 255;
...
void EnTest_Destroy(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params != STALFOS_TYPE_2) && !Actor_FindNearby(globalCtx, &pthis->actor, ACTOR_EN_TEST, ACTORCAT_ENEMY, 8000.0f))
	{
		func_800F5B58();
	}
...
void EnTest_ChooseRandomAction(EnTest* pthis, GlobalContext* globalCtx)
{
	switch((u32)(Rand_ZeroOne() * 10.0f))
	{
		...
		case 5:
		case 6:
			if((pthis->actor.xzDistToPlayer < 220.0f) && (pthis->actor.xzDistToPlayer > 170.0f) && Actor_IsFacingPlayer(&pthis->actor, 0x71C) && Actor_IsTargeted(globalCtx, &pthis->actor))
			{
...
void EnTest_Idle(EnTest* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnTest_ReactToProjectile(globalCtx, pthis))
	{
		...
		else
		{
			...
			{
				if((pthis->actor.xzDistToPlayer < 220.0f) && (pthis->actor.xzDistToPlayer > 160.0f) && (Rand_ZeroOne() < 0.3f))
				{
...
void EnTest_SetupWalkAndBlock(EnTest* pthis)
{
	Animation_Change(&pthis->upperSkelanime, &gStalfosBlockWithShieldAnim, 2.0f, 0.0f, Animation_GetLastFrame(&gStalfosBlockWithShieldAnim), 2, 2.0f);
...
void EnTest_WalkAndBlock(EnTest* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnTest_ReactToProjectile(globalCtx, pthis))
	{
		...
		if((pthis->actor.params == STALFOS_TYPE_CEILING) && !Actor_TestFloorInDirection(&pthis->actor, globalCtx, pthis->actor.speedXZ, pthis->actor.world.rot.y))
		{
		...
		{
			Animation_Change(&pthis->skelAnime, &gStalfosFastAdvanceAnim, 0.0f, pthis->skelAnime.curFrame, Animation_GetLastFrame(&gStalfosFastAdvanceAnim), 0, -4.0f);
		...
		if((pthis->actor.xzDistToPlayer < 220.0f) && (pthis->actor.xzDistToPlayer > 160.0f) && (Actor_IsFacingPlayer(&pthis->actor, 0x71C)))
		{
			if(Actor_IsTargeted(globalCtx, &pthis->actor))
			{
...
void EnTest_SetupStopAndBlock(EnTest* pthis)
{
	Animation_Change(&pthis->skelAnime, &gStalfosBlockWithShieldAnim, 2.0f, 0.0f, Animation_GetLastFrame(&gStalfosBlockWithShieldAnim), 2, 2.0f);
	pthis->unk_7C8 = 0x15;
	pthis->actor.speedXZ = 0.0f;
...
void EnTest_StopAndBlock(EnTest* pthis, GlobalContext* globalCtx)
{
	...
	if((ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)) > 0x3E80) && (pthis->actor.params != STALFOS_TYPE_CEILING) && ((globalCtx->gameplayFrames % 2) != 0))
	{
		pthis->actor.world.rot.y = pthis->actor.yawTowardsPlayer;
		EnTest_SetupJumpBack(pthis);
	}
...
void func_808621D4(EnTest* pthis, GlobalContext* globalCtx)
{
	...
	if(player->swordState != 0)
	{
		...
		else if((Rand_ZeroOne() > 0.7f) && (pthis->actor.params != STALFOS_TYPE_CEILING) && (player->swordAnimation != 0x11))
		{
			EnTest_SetupJumpBack(pthis);
		}
		else
...
void EnTest_UpdateDamage(EnTest* pthis, GlobalContext* globalCtx)
{
	...
	else if(pthis->bodyCollider.base.acFlags & AC_HIT)
	{
		...
		if((pthis->actor.colChkInfo.damageEffect != STALFOS_DMGEFF_SLING) && (pthis->actor.colChkInfo.damageEffect != STALFOS_DMGEFF_FIREMAGIC))
		{
			...
			if((pthis->actor.colChkInfo.damageEffect == STALFOS_DMGEFF_STUN) || (pthis->actor.colChkInfo.damageEffect == STALFOS_DMGEFF_FREEZE) || (pthis->actor.colChkInfo.damageEffect == STALFOS_DMGEFF_LIGHT))
			{
...
void EnTest_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.colChkInfo.damageEffect != STALFOS_DMGEFF_FIREMAGIC)
	{
		...
		else if(pthis->actor.bgCheckFlags & BG_STATE_1)
		{
			...
			if((floorProperty == 5) || (floorProperty == 0xC) || func_80041D4C(&globalCtx->colCtx, pthis->actor.floorPoly, pthis->actor.floorBgId) == 9)
			{
				Actor_Kill(&pthis->actor);
				return;
			}
		...
		switch(pthis->unk_7DE)
		{
			...
			case 1:
				Animation_Change(&pthis->upperSkelanime, &gStalfosBlockWithShieldAnim, 2.0f, 0.0f, Animation_GetLastFrame(&gStalfosBlockWithShieldAnim), 2, 2.0f);
				AnimationContext_SetCopyTrue(globalCtx, pthis->skelAnime.limbCount, pthis->skelAnime.jointTable, pthis->upperSkelanime.jointTable, sJointCopyFlags);
				pthis->unk_7DE++;
				break;

			case 2:
				SkelAnime_Update(&pthis->upperSkelanime);
				SkelAnime_CopyFrameTableTrue(&pthis->skelAnime, pthis->skelAnime.jointTable, pthis->upperSkelanime.jointTable, sJointCopyFlags);
				break;
			...
			case 4:
				...
				SkelAnime_InterpFrameTable(pthis->skelAnime.limbCount, pthis->upperSkelanime.jointTable, pthis->upperSkelanime.jointTable, pthis->skelAnime.jointTable, 1.0f - (pthis->upperSkelanime.morphWeight / oldWeight));
				SkelAnime_CopyFrameTableTrue(&pthis->skelAnime, pthis->skelAnime.jointTable, pthis->upperSkelanime.jointTable, sJointCopyFlags);
				break;
		}
	...
	if((pthis->actor.colChkInfo.health > 0) || (pthis->actor.colorFilterTimer != 0))
	{
		...
		if((pthis->unk_7C8 >= 0xA) && ((pthis->actor.colorFilterTimer == 0) || (!(pthis->actor.colorFilterParams & 0x4000))))
		{
...
void EnTest_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	if(limbIndex == STALFOS_LIMB_SWORD)
	{
		...
		if((pthis->swordState >= 1) && ((pthis->actor.params != STALFOS_TYPE_INVISIBLE) || (globalCtx->actorCtx.unk_03 != 0)))
		{
...
void EnTest_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnTest_OverrideLimbDraw, EnTest_PostLimbDraw, pthis);
	}
	...
	if(pthis->iceTimer != 0)
	{
		...
		if((pthis->iceTimer % 4) == 0)
		{
			...
			EffectSsEnIce_SpawnFlyingVec3s(globalCtx, thisx, &pthis->bodyPartsPos[iceIndex], 150, 150, 150, 250, 235, 245, 255, 1.5f);
		}
	}
}
