...
ActorInit Bg_Hidan_Kowarerukabe_InitVars = {
    ...
    ACTOR_FACTORY(BgHidanKowarerukabe),
...
void BgHidanKowarerukabe_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(((pthis->dyna.actor.params & 0xFF) < CRACKED_STONE_FLOOR) || ((pthis->dyna.actor.params & 0xFF) > LARGE_BOMBABLE_WALL))
	{
...
void BgHidanKowarerukabe_FloorBreak(BgHidanKowarerukabe* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 5; i++)
	{
		for(j = 0; j < 5; j++)
		{
			...
			EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &thisx->world.pos, -550, arg5, 15, 15, 0, arg9, 2, 16, 100, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gBrownFragmentDL);
		}
	}
}
...
void func_8088A67C(BgHidanKowarerukabe* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 5; i++)
	{
		...
		for(j = 0; j < 5; j++)
		{
			...
			EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &thisx->world.pos, -540, arg5, 20, 20, 0, arg9, 2, 32, 100, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gBrownFragmentDL);
		}
	}
}
...
void BgHidanKowarerukabe_LargeWallBreak(BgHidanKowarerukabe* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 5; i++)
	{
		...
		for(j = 0; j < 5; j++)
		{
			...
			EffectSsKakera_Spawn(globalCtx, &pos, &velocity, &thisx->world.pos, -650, arg5, 20, 20, 0, arg9, 2, 32, 100, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gBrownFragmentDL);
		}
	}
}
