...
ActorInit En_Bb_InitVars = {
    ACTOR_EN_BB, ACTORCAT_ENEMY, FLAGS, OBJECT_BB, ACTOR_FACTORY(EnBb), (ActorFunc)EnBb_Init, (ActorFunc)EnBb_Destroy, (ActorFunc)EnBb_Update, (ActorFunc)EnBb_Draw, (ActorFunc)EnBb_Reset,
};
...
void EnBb_SpawnFlameTrail(GlobalContext* globalCtx, EnBb* pthis, s16 startAtZero)
{
	...
	for(i = 0; i < 5; i++)
	{
		next = (EnBb*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BB, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
		if(next != NULL)
		{
...
void EnBb_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &object_Bb_Skel_001A30, &object_Bb_Anim_000444, pthis->jointTable, pthis->morphTable, 16);
	pthis->unk_254 = 0;
	...
	if(thisx->params & 0xFF00)
	{
		...
		switch(thisx->params)
		{
			...
			case ENBB_WHITE:
				...
				blureInit.p1StartColor[0] = blureInit.p1StartColor[1] = blureInit.p1StartColor[2] = blureInit.p1StartColor[3] = blureInit.p2StartColor[0] = blureInit.p2StartColor[1] = blureInit.p2StartColor[2] = blureInit.p2StartColor[3] =
				    blureInit.p1EndColor[0] = blureInit.p1EndColor[1] = blureInit.p1EndColor[2] = blureInit.p2EndColor[0] = blureInit.p2EndColor[1] = blureInit.p2EndColor[2] = 255;
	...
	pthis->collider.elements[0].dim.worldSphere.radius = pthis->collider.elements[0].dim.modelSphere.radius * pthis->collider.elements[0].dim.scale;
}
...
void EnBb_Blue(EnBb* pthis, GlobalContext* globalCtx)
{
	...
	{
		Math_SmoothStepToF(&pthis->actor.world.pos.y, pthis->actor.floorHeight + 50.0f + pthis->flyHeightMod, 1.0f, 0.5f, 0.0f);
	}
...
void EnBb_Red(EnBb* pthis, GlobalContext* globalCtx)
{
	...
	{
		case BBRED_WAIT:
			if((Actor_WorldDistXYZToActor(&pthis->actor, &player->actor) <= 250.0f) && (ABS(yawDiff) <= 0x4000) && (pthis->timer == 0))
			{
				pthis->actor.speedXZ = 5.0f;
				pthis->actor.gravity = -1.0f;
...
void EnBb_CollisionCheck(EnBb* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->collider.base.acFlags & AC_HIT)
	{
		...
		switch(pthis->dmgEffect)
		{
			...
			default:
			block_15:
				if((pthis->dmgEffect == 14) || (pthis->dmgEffect == 12) || (pthis->dmgEffect == 11) || (pthis->dmgEffect == 10) || (pthis->dmgEffect == 7) || (pthis->dmgEffect == 5))
				{
				...
				{
					if(((pthis->action == BB_DOWN) && (pthis->timer < 190)) || ((pthis->actor.params != ENBB_WHITE) && (pthis->flameScaleX < 20.0f)))
					{
						Actor_ApplyDamage(&pthis->actor);
					}
					else
				...
				else if((pthis->actor.params == ENBB_WHITE) && ((pthis->action == BB_WHITE) || (pthis->action == BB_STUNNED)))
				{
					Actor_SetColorFilter(&pthis->actor, 0x4000, 0xFF, 0, 0xC);
				...
				else if(((pthis->action == BB_DOWN) && (pthis->timer < 190)) || ((pthis->actor.params != ENBB_WHITE) && (pthis->flameScaleX < 20.0f)))
				{
...
void EnBb_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->actor.colChkInfo.damageEffect != 0xD)
	{
		...
		if((pthis->actor.params <= ENBB_BLUE) && (pthis->actor.speedXZ >= -6.0f) && ((pthis->actor.flags & ACTOR_FLAG_15) == 0))
		{
		...
		pthis->collider.elements->dim.worldSphere.center.y = pthis->actor.world.pos.y + (pthis->actor.shape.yOffset * pthis->actor.scale.y);
		...
		if((pthis->action > BB_FLAME_TRAIL) && ((pthis->actor.colorFilterTimer == 0) || !(pthis->actor.colorFilterParams & 0x4000)) && (pthis->moveMode != BBMOVE_HIDDEN))
		{
...
static Vec3f sFireIceOffsets[] = {
    {13.0f, 10.0f, 0.0f},  {5.0f, 25.0f, 5.0f}, {-5.0f, 25.0f, 5.0f}, {-13.0f, 10.0f, 0.0f}, {5.0f, 25.0f, -5.0f}, {-5.0f, 25.0f, -5.0f},
    {0.0f, 10.0f, -13.0f}, {5.0f, 0.0f, 5.0f},	{5.0f, 0.0f, -5.0f},  {0.0f, 10.0f, 13.0f},  {-5.0f, 0.0f, 5.0f},  {-5.0f, 0.0f, -5.0f},
};
...
void EnBb_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->moveMode != BBMOVE_HIDDEN)
	{
		if(pthis->actor.params <= ENBB_BLUE)
		{
			...
			if(pthis->fireIceTimer != 0)
			{
				...
				if((pthis->fireIceTimer % 4) == 0)
				{
					...
					if((pthis->dmgEffect != 7) && (pthis->dmgEffect != 5))
					{
						EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &sp70, 0x96, 0x96, 0x96, 0xFA, 0xEB, 0xF5, 0xFF, 0.8f);
					}
					else
					{
						sp70.y -= 17.0f;
		...
		else
		{
			...
			if((pthis->maxSpeed != 0.0f) && (pthis->action == BB_WHITE) && !(globalCtx->gameplayFrames & 1) && (pthis->actor.colChkInfo.health != 0))
			{
