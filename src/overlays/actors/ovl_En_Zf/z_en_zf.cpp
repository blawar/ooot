...
s32 EnZf_PrimaryFloorCheck(EnZf* pthis, GlobalContext* globalCtx, f32 dist)
{
	...
	ret = !(pthis->actor.bgCheckFlags & BG_STATE_0);
...
void EnZf_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	blureInit.p1StartColor[0] = blureInit.p1StartColor[1] = blureInit.p1StartColor[2] = blureInit.p1StartColor[3] = blureInit.p2StartColor[0] = blureInit.p2StartColor[1] = blureInit.p2StartColor[2] = blureInit.p1EndColor[0] = blureInit.p1EndColor[1] =
	    blureInit.p1EndColor[2] = blureInit.p2EndColor[0] = blureInit.p2EndColor[1] = blureInit.p2EndColor[2] = 255;
	...
	if(thisx->params == ENZF_TYPE_DINOLFOS)
	{
		...
		SkelAnime_Init(globalCtx, &pthis->skelAnime, &gZfDinolfosSkel, &gZfCryingAnim, pthis->jointTable, pthis->morphTable, ENZF_LIMB_MAX);
	}
	else
	{ // Lizalfos
		thisx->colChkInfo.health = 6;
		thisx->naviEnemyId = 0x0F;
		SkelAnime_Init(globalCtx, &pthis->skelAnime, &gZfLizalfosSkel, &gZfCryingAnim, pthis->jointTable, pthis->morphTable, ENZF_LIMB_MAX);
	}

	if(thisx->params < ENZF_TYPE_LIZALFOS_MINIBOSS_A)
	{ // Not minibosses
...
void EnZf_Destroy(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params >= ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* miniboss */ && (Actor_FindNearby(globalCtx, &pthis->actor, ACTOR_EN_ZF, ACTORCAT_ENEMY, 10000.0f) == NULL))
	{
		func_800F5B58();
	}
...
s16 EnZf_FindPlatform(Vec3f* pos, s16 preferredIndex)
{
	...
	{
		i = preferredIndex;
		if(((sPlatformPositions[i].y - 150.0f) <= pos->y) && (pos->y <= (sPlatformPositions[i].y + 150.0f)) && ((sPlatformPositions[i].x - rangeXZ) <= pos->x) && (pos->x <= (sPlatformPositions[i].x + rangeXZ)) &&
...
s16 EnZf_FindNextPlatformAwayFromPlayer(Vec3f* pos, s16 curPlatform, s16 arg2, GlobalContext* globalCtx)
{
	...
	for(; curLoopPlatform >= minIndex; curLoopPlatform--)
	{
		...
		if((playerPlatform == -1) && (Math_Vec3f_DistXYZ(&player->actor.world.pos, &sPlatformPositions[curLoopPlatform]) < playerMaxDist))
		{
			continue;
		}
...
s32 EnZf_ChooseAction(GlobalContext* globalCtx, EnZf* pthis)
{
	...
	if(func_800354B4(globalCtx, &pthis->actor, 100.0f, 0x5DC0, 0x2AA8, pthis->actor.shape.rot.y))
	{
		...
		if((pthis->actor.bgCheckFlags & BG_STATE_3) && (ABS(angleToWall) < 0x2EE0) && (pthis->actor.xzDistToPlayer < 80.0f))
		{
	...
	{
		pthis->actor.shape.rot.y = pthis->actor.world.rot.y = pthis->actor.yawTowardsPlayer;
		if(((pthis->actor.bgCheckFlags & BG_STATE_3) && (angleToWall < 0x2EE0)) || (explosive->id == ACTOR_EN_BOM_CHU))
		{
...
void EnZf_SetupDropIn(EnZf* pthis)
{
	Animation_Change(&pthis->skelAnime, &gZfJumpingAnim, 0.0f, 9.0f, Animation_GetLastFrame(&gZfJumpingAnim), ANIMMODE_LOOP, 0.0f);
...
void EnZf_DropIn(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_1)) && (pthis->hopAnimIndex != 0))
	{
...
void func_80B45384(EnZf* pthis)
{
	Animation_Change(&pthis->skelAnime, &gZfCryingAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gZfCryingAnim), ANIMMODE_LOOP_INTERP, -4.0f);
...
void func_80B4543C(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnZf_DodgeRangedEngaging(globalCtx, pthis))
	{
		...
		else
		{
			...
			{
				if((pthis->actor.xzDistToPlayer < 200.0f) && (pthis->actor.xzDistToPlayer > 100.0f) && (Rand_ZeroOne() < 0.3f))
				{
					if(pthis->actor.params == ENZF_TYPE_DINOLFOS)
					{
...
void EnZf_SetupApproachPlayer(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	{ // miniboss
		...
		pthis->nextPlatform = EnZf_FindNextPlatformTowardsPlayer(&pthis->actor.world.pos, pthis->curPlatform, pthis->homePlatform, globalCtx);
		pthis->hopAnimIndex = 0;
	}
	pthis->actor.speedXZ = 0.0f;
	...
	if(!EnZf_DodgeRangedEngaging(globalCtx, pthis))
	{
		if(sp48 != pthis->curPlatform)
		{
			...
			else
			{
				...
				{
					if((Actor_WorldDistXZToPoint(&pthis->actor, &sPlatformPositions[pthis->nextPlatform]) < sp44) && !EnZf_PrimaryFloorCheck(pthis, globalCtx, 191.9956f))
					{
						EnZf_SetupJumpForward(pthis);
					...
					{
						pthis->actor.world.rot.y = Actor_WorldYawTowardPoint(&pthis->actor, &sPlatformPositions[pthis->curPlatform]);
					}
				}
				else
				{
		...
		if((sp48 == pthis->curPlatform) && (pthis->actor.xzDistToPlayer < 150.0f) && (player->swordState != 0) && (temp_v1 >= 0x1F40))
		{
			pthis->actor.shape.rot.y = pthis->actor.world.rot.y = pthis->actor.yawTowardsPlayer;
		...
		if(pthis->actor.params == ENZF_TYPE_DINOLFOS)
		{
			...
			if((pthis->actor.xzDistToPlayer < 180.0f) && (pthis->actor.xzDistToPlayer > 160.0f) && Actor_IsFacingPlayer(&pthis->actor, 0x71C))
			{
...
void EnZf_JumpForward(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params == ENZF_TYPE_DINOLFOS) && (pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_1)))
	{
...
void func_80B462E4(EnZf* pthis, GlobalContext* globalCtx)
{
	if((pthis->actor.params < ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* miniboss */ || Actor_TestFloorInDirection(&pthis->actor, globalCtx, 40.0f, (s16)(pthis->actor.shape.rot.y + 0x3FFF)) ||
...
void func_80B463E4(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnZf_DodgeRangedEngaging(globalCtx, pthis) && ((pthis->actor.params != ENZF_TYPE_DINOLFOS) || !EnZf_ChooseAction(globalCtx, pthis)))
	{
		...
		else if(Math_SinS(angleBehindPlayer - pthis->actor.shape.rot.y) < 0.0f)
		{ // Superfluous check
			pthis->actor.speedXZ += 0.25f;
			if(pthis->actor.speedXZ > 8.0f)
			{
		...
		if((Math_CosS(angleBehindPlayer - pthis->actor.shape.rot.y) < -0.85f) || (pthis->unk_3F0 == 0))
		{
			...
			if((pthis->actor.xzDistToPlayer <= 100.0f) && ((globalCtx->gameplayFrames % 4) == 0) && EnZf_CanAttack(globalCtx, pthis))
			{
				EnZf_SetupSlash(pthis);
			}
			else
...
void EnZf_SetupSlash(EnZf* pthis)
{
	Animation_Change(&pthis->skelAnime, &gZfSlashAnim, 1.25f, 0.0f, Animation_GetLastFrame(&gZfSlashAnim), ANIMMODE_ONCE, -4.0f);
...
{
	if((pthis->actor.bgCheckFlags & BG_STATE_0) && ((pthis->actor.velocity.y == 0.0f) || (pthis->actor.velocity.y == -4.0f)))
	{
		pthis->actor.speedXZ = 0.0f;
		pthis->hopAnimIndex = 0;
	}
	...
	if((pthis->actor.colorFilterTimer == 0) && (pthis->actor.bgCheckFlags & BG_STATE_0))
	{
		...
		else if((pthis->actor.params != ENZF_TYPE_DINOLFOS) || !EnZf_ChooseAction(globalCtx, pthis))
		{
			...
			else
			{
				...
				if((pthis->actor.params == ENZF_TYPE_DINOLFOS) && (pthis->actor.bgCheckFlags & BG_STATE_3) && (ABS(angleToWall) < 0x2EE0) && (pthis->actor.xzDistToPlayer < 90.0f))
				{
					pthis->actor.world.rot.y = pthis->actor.shape.rot.y;
					EnZf_SetupJumpUp(pthis);
				}
				else if(!EnZf_DodgeRangedEngaging(globalCtx, pthis))
				{
					...
					else if((pthis->actor.xzDistToPlayer <= 100.0f) && ((globalCtx->gameplayFrames % 4) != 0) && EnZf_CanAttack(globalCtx, pthis))
					{
						EnZf_SetupSlash(pthis);
					}
					else
...
void EnZf_SetupSheatheSword(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.speedXZ = 0.0f;
	pthis->curPlatform = EnZf_FindPlatform(&pthis->actor.world.pos, pthis->curPlatform);
	pthis->nextPlatform = EnZf_FindNextPlatformAwayFromPlayer(&pthis->actor.world.pos, pthis->curPlatform, pthis->homePlatform, globalCtx);
	pthis->actor.world.rot.y = pthis->actor.shape.rot.y;
...
void EnZf_HopAndTaunt(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(SkelAnime_Update(&pthis->skelAnime))
		{
			...
			Animation_Change(&pthis->skelAnime, sHoppingAnims[pthis->hopAnimIndex], 1.5f, 0.0f, lastFrame, ANIMMODE_ONCE, 0.0f);
		}
...
void EnZf_HopAway(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->hopAnimIndex)
	{
		...
		case 1:
			if((pthis->actor.bgCheckFlags & BG_STATE_1) || (pthis->actor.bgCheckFlags & BG_STATE_0))
			{
				Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_RIZA_ONGND);
				pthis->actor.velocity.y = 0.0f;
				pthis->actor.world.pos.y = pthis->actor.floorHeight;
...
{
	Animation_Change(&pthis->skelAnime, &gZfKnockedBackAnim, 1.5f, 0.0f, Animation_GetLastFrame(&gZfKnockedBackAnim), ANIMMODE_ONCE, -4.0f);
...
void EnZf_Damaged(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if(((pthis->actor.params != ENZF_TYPE_DINOLFOS) || !EnZf_ChooseAction(globalCtx, pthis)) && SkelAnime_Update(&pthis->skelAnime) && (pthis->actor.bgCheckFlags & BG_STATE_0))
	{
		if(D_80B4A1B4 != -1)
		{
		...
		else
		{
			...
			if((pthis->actor.params == ENZF_TYPE_DINOLFOS) && (pthis->actor.bgCheckFlags & BG_STATE_3) && (ABS(wallYawDiff) < 12000) && (pthis->actor.xzDistToPlayer < 90.0f))
			{
				EnZf_SetupJumpUp(pthis);
			}
			else if(!EnZf_DodgeRangedEngaging(globalCtx, pthis))
...
void EnZf_JumpUp(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime))
	{
		...
		else if(pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_1))
		{
			pthis->actor.velocity.y = 0.0f;
			pthis->actor.world.rot.y = pthis->actor.shape.rot.y = pthis->actor.yawTowardsPlayer;
			pthis->actor.speedXZ = 0.0f;
...
void func_80B483E4(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params < ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* not miniboss */ || Actor_TestFloorInDirection(&pthis->actor, globalCtx, 40.0f, (s16)(pthis->actor.shape.rot.y + 0x3FFF)) ||
	   Actor_TestFloorInDirection(&pthis->actor, globalCtx, -40.0f, (s16)(pthis->actor.shape.rot.y + 0x3FFF)))
	{
		Animation_PlayLoop(&pthis->skelAnime, &gZfSidesteppingAnim);
		...
		else if(Math_SinS(playerRotY - pthis->actor.shape.rot.y) < 0.0f)
		{ // Superfluous check
			pthis->actor.speedXZ = 6.0f;
		}
...
void EnZf_CircleAroundPlayer(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if(EnZf_FindPlatform(&player->actor.world.pos, -1) != pthis->curPlatform)
	{
		...
		if((pthis->actor.params >= ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* miniboss */ && (D_80B4A1B4 == pthis->actor.params))
		{
			EnZf_SetupHopAndTaunt(pthis);
		}
		else
	...
	else if((pthis->actor.params != ENZF_TYPE_DINOLFOS) || !EnZf_ChooseAction(globalCtx, pthis))
	{
		if(pthis->unk_3F0 == 0)
		{
			...
			else
			{
				...
				else if((pthis->actor.xzDistToPlayer < 280.0f) && (pthis->actor.xzDistToPlayer > 240.0f) && !EnZf_PrimaryFloorCheck(pthis, globalCtx, 191.9956f) && ((globalCtx->gameplayFrames % 2) == 0))
				{
					EnZf_SetupJumpForward(pthis);
				}
				else
...
void EnZf_SetupDie(EnZf* pthis)
{
	Animation_Change(&pthis->skelAnime, &gZfDyingAnim, 1.5f, 0.0f, Animation_GetLastFrame(&gZfDyingAnim), ANIMMODE_ONCE, -4.0f);
...
void EnZf_UpdateDamage(EnZf* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->bodyCollider.base.acFlags & AC_HIT) && (pthis->action <= ENZF_ACTION_STUNNED))
	{
		...
		if(((pthis->actor.params < ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* not miniboss */ || (D_80B4A1B4 != pthis->actor.params)) && (pthis->actor.colChkInfo.damageEffect != ENZF_DMGEFF_IMMUNE))
		{
			pthis->damageEffect = pthis->actor.colChkInfo.damageEffect;
			Actor_SetDropFlag(&pthis->actor, &pthis->bodyCollider.info, 0);

			if((pthis->actor.colChkInfo.damageEffect == ENZF_DMGEFF_STUN) || (pthis->actor.colChkInfo.damageEffect == ENZF_DMGEFF_ICE))
			{
			...
			else
			{
				...
				{
					if((D_80B4A1B4 != -1) && ((pthis->actor.colChkInfo.health + pthis->actor.colChkInfo.damage) >= 4) && (pthis->actor.colChkInfo.health < 4))
					{
...
void EnZf_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.colChkInfo.health > 0) && (pthis->alpha == 255))
	{
		...
		if((pthis->actor.params < ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* not miniboss */ || (D_80B4A1B4 != pthis->actor.params))
		{
	...
	if((pthis->action == ENZF_ACTION_SLASH) && (pthis->skelAnime.curFrame >= 14.0f) && (pthis->skelAnime.curFrame <= 20.0f))
	{
...
s32 EnZf_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnZf* pthis = (EnZf*)thisx;
...
void EnZf_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx)
{
	...
	if(limbIndex == ENZF_LIMB_SWORD)
	{
		...
		Collider_SetQuadVertices(&pthis->swordCollider, &pthis->swordCollider.dim.quad[0], &pthis->swordCollider.dim.quad[1], &pthis->swordCollider.dim.quad[2], &pthis->swordCollider.dim.quad[3]);
...
void EnZf_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->alpha == 255)
	{
		...
		POLY_OPA_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnZf_OverrideLimbDraw, EnZf_PostLimbDraw, pthis, POLY_OPA_DISP);
		...
		if(pthis->iceTimer != 0)
		{
			...
			if((pthis->iceTimer % 4) == 0)
			{
				...
				EffectSsEnIce_SpawnFlyingVec3f(globalCtx, thisx, &pthis->bodyPartsPos[icePosIndex], 150, 150, 150, 250, 235, 245, 255, 1.4f);
			}
			if(1)
			{
			}
...
s32 EnZf_DodgeRangedEngaging(GlobalContext* globalCtx, EnZf* pthis)
{
	...
	if(projectileActor != NULL)
	{
		...
		if((((pthis->actor.xzDistToPlayer < 90.0f) || (phi_t0 == 3)) && !EnZf_PrimaryFloorCheck(pthis, globalCtx, 135.0f)) || (projectileActor->id == ACTOR_ARMS_HOOK))
		{
			EnZf_SetupJumpUp(pthis);
			return true;
		}
