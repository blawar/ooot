...
void func_80B11E78(EnSyatekiNiw* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->unk_29C != 0) && (pthis->unk_29E == 0) && (pthis->actor.bgCheckFlags & BG_STATE_0))
	{
		pthis->unk_29C = 0;
	...
	if(pthis->unk_25C != 0)
	{
		...
		Math_SmoothStepToS(&pthis->actor.world.rot.y, Math_FAtan2F(tmpf1, tmpf2) * (0x8000 / M_PI), 3, pthis->unk_2C8.z, 0);
...
void func_80B12460(EnSyatekiNiw* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->unk_29A)
	{
		...
		case 2:
			...
			if((pthis->actor.bgCheckFlags & BG_STATE_0) && (pthis->actor.world.pos.z > 110.0f))
			{
		...
		case 5:
			...
			{
				Audio_PlaySoundGeneral(NA_SE_VO_LI_DOWN, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
				pthis->unk_25E = 0x14;
				pthis->unk_29A = 6;
				pthis->actor.speedXZ = 0.0f;
			}
	...
	Math_SmoothStepToS(&pthis->actor.world.rot.y, (s16)(Math_FAtan2F(player->actor.world.pos.x - pthis->actor.world.pos.x, player->actor.world.pos.z - pthis->actor.world.pos.z) * (0x8000 / M_PI)) + phi_f16, 5, pthis->unk_2C8.y, 0);
...
void func_80B128F8(EnSyatekiNiw* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.projectedPos.z > 200.0f) && (pthis->actor.projectedPos.z < 800.0f) && (sp26 > 0) && (sp26 < SCREEN_WIDTH) && (sp24 > 0) && (sp24 < SCREEN_HEIGHT))
	{
...
void func_80B129EC(EnSyatekiNiw* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->unk_25E == 0) || (pthis->actor.projectedPos.z < -70.0f) || (sp2E < 0) || (sp2E > SCREEN_WIDTH) || (sp2C < 0) || (sp2C > SCREEN_HEIGHT))
	{
	...
	if(pthis->unk_25C == 0)
	{
		...
		if((Rand_ZeroFloat(5.0f) < 1.0f) && (pthis->actor.bgCheckFlags & BG_STATE_0))
		{
...
s32 SyatekiNiw_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnSyatekiNiw* pthis = (EnSyatekiNiw*)thisx;
...
void EnSyatekiNiw_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != func_80B128F8)
	{
		...
		SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, SyatekiNiw_OverrideLimbDraw, NULL, pthis);
...
void func_80B13464(EnSyatekiNiw* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 5; i++, ptr++)
	{
		if(ptr->unk_00 == 1)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_syateki_niw.c", 1251), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
