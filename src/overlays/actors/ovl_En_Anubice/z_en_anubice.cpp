...
ActorInit En_Anubice_InitVars = {
    ACTOR_EN_ANUBICE, ACTORCAT_ENEMY, FLAGS, OBJECT_ANUBICE, ACTOR_FACTORY(EnAnubice), (ActorFunc)EnAnubice_Init, (ActorFunc)EnAnubice_Destroy, (ActorFunc)EnAnubice_Update, (ActorFunc)EnAnubice_Draw, (ActorFunc)EnAnubice_Reset,
};
...
void EnAnubice_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gAnubiceSkel, &gAnubiceIdleAnim, pthis->jointTable, pthis->morphTable, 16);
		...
		if(!pthis->hasSearchedForFlameCircles)
		{
			...
			while(currentProp != NULL)
			{
				...
				else
				{
					...
					osSyncPrintf(VT_FGCOL(YELLOW) "☆☆☆☆☆ 火は幾つ？ ☆☆☆☆☆ %x\n" VT_RST, pthis->flameCircles[flameCirclesFound]);
...
void EnAnubis_ShootFireball(EnAnubice* pthis, GlobalContext* globalCtx)
{
	...
	if(curFrame == 12.0f)
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ANUBICE_FIRE, pthis->fireballPos.x, pthis->fireballPos.y + 15.0f, pthis->fireballPos.z, pthis->fireballRot.x, pthis->fireballRot.y, 0, 0);
	}
...
void EnAnubice_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnAnubice_SetupDie && pthis->actionFunc != EnAnubice_Die && pthis->actor.shape.yOffset == 0.0f)
	{
...
s32 EnAnubis_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnAnubice* pthis = (EnAnubice*)thisx;
