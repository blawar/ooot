...
static Gfx* carpenterHeadDLists_42[] = {object_daiku_DL_005BD0, object_daiku_DL_005AC0, object_daiku_DL_005990, object_daiku_DL_005880};
...
static struct_D_80AA1678 sAnimations[] = {
    {&object_daiku_Anim_001AB0, 1.0f, 2, -7.0f}, {&object_daiku_Anim_007DE0, 1.0f, 0, -7.0f}, {&object_daiku_Anim_00885C, 1.0f, 0, -7.0f},
    {&object_daiku_Anim_000C44, 1.0f, 0, -7.0f}, {&object_daiku_Anim_000600, 1.0f, 0, -7.0f}, {&object_daiku_Anim_008164, 1.0f, 0, -7.0f},
};
...
void EnDaikuKakariko_SetAnimFromIndex(EnDaikuKakariko* pthis, s32 animIndex, s32* currentAnimIndex)
{
	...
	Animation_Change(&pthis->skelAnime, sAnimations[animIndex].animation, 1.0f, 0.0f, Animation_GetLastFrame(sAnimations[animIndex].animation), sAnimations[animIndex].mode, morphFrames);
...
void EnDaikuKakariko_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_daiku_Skel_007958, NULL, pthis->jointTable, pthis->morphTable, 17);
	Collider_InitCylinder(globalCtx, &pthis->collider);
	Collider_SetCylinder(globalCtx, &pthis->collider, &pthis->actor, &sCylinderInit);

	CollisionCheck_SetInfo2(&pthis->actor.colChkInfo, &sDamageTable, &sColChkInit);

	Animation_Change(&pthis->skelAnime, sAnimations->animation, 1.0f, 0.0f, Animation_GetLastFrame(sAnimations->animation), sAnimations->mode, sAnimations->transitionRate);
...
void EnDaikuKakariko_HandleTalking(EnDaikuKakariko* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if((sp26 >= 0) && (sp26 <= 320) && (sp24 >= 0) && (sp24 <= 240) && (pthis->talkState == 0) && (func_8002F2CC(&pthis->actor, globalCtx, 100.0f) == 1))
		{
...
s32 EnDaikuKakariko_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnDaikuKakariko* pthis = (EnDaikuKakariko*)thisx;
	Vec3s angle;
...
void EnDaikuKakariko_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_daiku_kakariko.c", 1104);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_daiku_kakariko.c", 1113);
}
...
void EnDaikuKakariko_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_daiku_kakariko.c", 1124);
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnDaikuKakariko_OverrideLimbDraw, EnDaikuKakariko_PostLimbDraw, thisx);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_daiku_kakariko.c", 1151);
}
