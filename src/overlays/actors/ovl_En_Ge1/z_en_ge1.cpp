...
void EnGe1_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gGerudoWhiteSkel, &gGerudoWhiteIdleAnim, pthis->jointTable, pthis->morphTable, GE1_LIMB_MAX);
...
s32 EnGe1_SetTalkAction(EnGe1* pthis, GlobalContext* globalCtx, u16 textId, f32 arg3, EnGe1ActionFunc actionFunc)
{
	if(Actor_ProcessTalkRequest(&pthis->actor, globalCtx))
	{
		...
		pthis->animation = &gGerudoWhiteIdleAnim;
		Animation_Change(&pthis->skelAnime, &gGerudoWhiteIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gGerudoWhiteIdleAnim), ANIMMODE_ONCE, -8.0f);
		return true;
	}
...
{
	Animation_Change(&pthis->skelAnime, &gGerudoWhiteIdleAnim, -1.0f, Animation_GetLastFrame(&gGerudoWhiteIdleAnim), 0.0f, ANIMMODE_ONCE, 8.0f);
	pthis->animation = &gGerudoWhiteIdleAnim;
	pthis->animFunc = EnGe1_CueUpAnimation;
}
...
void EnGe1_SetupOpen_GTGGuard(EnGe1* pthis, GlobalContext* globalCtx)
{
	if((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(globalCtx))
	{
		...
		Animation_Change(&pthis->skelAnime, &gGerudoWhiteClapAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gGerudoWhiteClapAnim), ANIMMODE_ONCE, -3.0f);
...
void EnGe1_GetReaction_GateGuard(EnGe1* pthis, GlobalContext* globalCtx)
{
	...
	if(EnGe1_SetTalkAction(pthis, globalCtx, reactionText, 100.0f, EnGe1_Talk_GateGuard))
	{
		...
		Animation_Change(&pthis->skelAnime, &gGerudoWhiteDismissiveAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gGerudoWhiteDismissiveAnim), ANIMMODE_ONCE, -8.0f);
	}
}
