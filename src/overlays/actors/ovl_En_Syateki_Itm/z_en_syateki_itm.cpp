...
static Vec3f sRupeePos[] = {
    {-40.0f, 0.0f, -90.0f}, {-20.0f, 0.0f, -90.0f}, {0.0f, 0.0f, -90.0f}, {20.0f, 0.0f, -90.0f}, {40.0f, 0.0f, -90.0f}, {-40.0f, 0.0f, -60.0f}, {-20.0f, 0.0f, -60.0f}, {0.0f, 0.0f, -60.0f}, {20.0f, 0.0f, -60.0f}, {40.0f, 0.0f, -60.0f},
};
...
void EnSyatekiItm_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	pthis->man = (EnSyatekiMan*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_SYATEKI_MAN, 140.0f, 0.0f, 255.0f, 0, -0x4000, 0, 0);
	if(pthis->man == NULL)
	{
		// "Spawn error"
	...
	for(i = 0; i < 10; i++)
	{
		pthis->markers[i] = (EnExRuppy*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_RUPPY, sRupeePos[i].x, sRupeePos[i].y, sRupeePos[i].z, 0, 0, 0, 4);
...
void EnSyatekiItm_Idle(EnSyatekiItm* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->signal == ENSYATEKI_START)
	{
		...
		player->actor.world.rot.x = player->actor.shape.rot.x = player->actor.world.rot.z = player->actor.shape.rot.z = 0;
...
void EnSyatekiItm_SpawnTargets(EnSyatekiItm* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer == 0)
	{
		...
		{
			pthis->targets[i] =
			    (EnGSwitch*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_G_SWITCH, pthis->targetHome[i].x, pthis->targetHome[i].y, pthis->targetHome[i].z, 0, 0, 0, (ENGSWITCH_TARGET_RUPEE << 0xC) | 0x3F);
