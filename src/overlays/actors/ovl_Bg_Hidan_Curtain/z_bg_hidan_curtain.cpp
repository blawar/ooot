...
void BgHidanCurtain_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(((pthis->type == 1) && Flags_GetTreasure(globalCtx, pthis->treasureFlag)) || (((pthis->type == 0) || (pthis->type == 6)) && Flags_GetClear(globalCtx, pthis->actor.room)))
	{
		Actor_Kill(&pthis->actor);
	}
...
void BgHidanCurtain_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if((globalCtx->cameraPtrs[MAIN_CAM]->setting == CAM_SET_SLOW_CHEST_CS) || (globalCtx->cameraPtrs[MAIN_CAM]->setting == CAM_SET_TURN_AROUND))
	{
...
void BgHidanCurtain_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_hidan_curtain.c", 685);
	...
	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, pthis->texScroll & 0x7F, 0, 0x20, 0x40, 1, 0, (pthis->texScroll * -0xF) & 0xFF, 0x20, 0x40));

	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_hidan_curtain.c", 698), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

	gSPDisplayList(POLY_XLU_DISP++, gEffFireCircleDL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_hidan_curtain.c", 702);
}
