...
static Vec3f deadDbOffsets_40[] = {
    {-14.0f, 5.0f, 5.0f}, {-20.0f, 12.0f, 0.0f}, {-5.0f, 10.0f, -1.0f}, {-10.0f, 8.0f, 14.0f}, {-3.0f, 10.0f, 7.0f}, {-10.0f, 11.0f, 0.0f}, {9.0f, 10.0f, -8.0f}, {4.0f, 10.0f, 3.0f}, {-6.0f, 13.0f, -5.0f}, {1.0f, 9.0f, 3.0f}, {-10.0f, 9.0f, 1.0f},
};
...
void DemoIk_Type1PlaySound(DemoIk* pthis)
{
	switch(pthis->actor.params)
	{
		case 0:
			...
			{
				Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_ARMOR_LAND1_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
			break;
		case 1:
			if(Animation_OnFrame(&pthis->skelAnime, 10.0f))
			{
				Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_ARMOR_LAND3_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
			break;
		case 2:
			if(Animation_OnFrame(&pthis->skelAnime, 9.0f))
			{
				Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_ARMOR_LAND2_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
			break;
	}
}
...
void DemoIk_Type1PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	if(limbIndex == 1)
	{
		...
		{
			case 0:
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_demo_ik_inArmer.c", 390), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016BE0);
				break;
			case 2:
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_demo_ik_inArmer.c", 396), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void DemoIk_Type2PlaySoundOnFrame(DemoIk* pthis, f32 frame)
{
	...
	{
		Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_ARMOR_OFF_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
...
s32 DemoIk_Type2OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	DemoIk* pthis = (DemoIk*)thisx;
...
void DemoIk_Type2PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	if(limbIndex == 1 && (frame >= 30.0f))
	{
		...
		{
			case 3:
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_demo_ik_inFace.c", 274), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_017028);
				break;
			case 4:
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_demo_ik_inFace.c", 280), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_017170);
				break;
			case 5:
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_demo_ik_inFace.c", 286), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016BE0);
				break;
			default:
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_demo_ik_inFace.c", 292), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void DemoIk_Type2Draw(DemoIk* pthis, GlobalContext* globalCtx)
{
	...
	gSPSegment(POLY_OPA_DISP++, 0x09, DemoIk_SetColors(gfxCtx, 255, 40, 0, 40, 0, 0));
	gSPSegment(POLY_OPA_DISP++, 0x0A, DemoIk_SetColors(gfxCtx, 255, 255, 255, 20, 40, 30));
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, DemoIk_Type2OverrideLimbDraw, DemoIk_Type2PostLimbDraw, pthis);
	CLOSE_DISPS(gfxCtx, "../z_demo_ik_inFace.c", 341);
}

static DemoIkActionFunc sActionFuncs[] = {
    DemoIk_Type1Action0, DemoIk_Type1Action1, DemoIk_Type1Action2, DemoIk_Type2Action0, DemoIk_Type2Action1, DemoIk_Type2Action2,
};
...
void DemoIk_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionMode < 0 || pthis->actionMode >= ARRAY_COUNT(sActionFuncs) || sActionFuncs[pthis->actionMode] == NULL)
	{
