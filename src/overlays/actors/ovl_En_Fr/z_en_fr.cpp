...
void EnFr_OrientUnderwater(EnFr* pthis)
{
	...
	pthis->actor.world.rot.y = pthis->actor.shape.rot.y = (s16)(sLogSpotToFromWater[pthis->actor.params].yaw * ((f32)0x8000 / M_PI)) + 0x8000;
...
void EnFr_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->objBankIndex))
	{
		...
		// frog
		SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_fr_Skel_00B498, &object_fr_Anim_001534, pthis->jointTable, pthis->morphTable, 24);
		// butterfly
		SkelAnime_Init(globalCtx, &pthis->skelAnimeButterfly, &gButterflySkel, &gButterflyAnim, pthis->jointTableButterfly, pthis->morphTableButterfly, 8);
		...
		Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.home.pos.x, pthis->actor.home.pos.y, pthis->actor.home.pos.z, 255, 255, 255, -1);
...
void EnFr_IsDivingIntoWater(EnFr* pthis, GlobalContext* globalCtx)
{
	...
	if(WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &waterSurface, &waterBox))
	{
...
void EnFr_SetupJumpingOutOfWater(EnFr* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &object_fr_Anim_0007BC, 1.0f, 0.0f, Animation_GetLastFrame(&object_fr_Anim_0007BC), ANIMMODE_ONCE, 0.0f);
...
void EnFr_OrientOnLogSpot(EnFr* pthis, GlobalContext* globalCtx)
{
	...
	if((rotYRemaining == 0) && (pthis->skelAnime.curFrame == pthis->skelAnime.endFrame))
	{
		...
		Animation_Change(&pthis->skelAnime, &object_fr_Anim_001534, 1.0f, 0.0f, Animation_GetLastFrame(&object_fr_Anim_001534), ANIMMODE_LOOP, 0.0f);
	}
}
...
void EnFr_JumpingUp(EnFr* pthis, GlobalContext* globalCtx)
{
	...
	if(EnFr_IsBelowLogSpot(pthis, &yDistToLogSpot))
	{
		pthis->isJumpingUp = false;
		pthis->actor.gravity = 0.0f;
		Animation_Change(&pthis->skelAnime, &object_fr_Anim_0011C0, 1.0f, 0.0f, Animation_GetLastFrame(&object_fr_Anim_0011C0), ANIMMODE_LOOP, 0.0f);
		pthis->actionFunc = EnFr_ChooseJumpFromLogSpot;
	}
...
void EnFr_Activate(EnFr* pthis, GlobalContext* globalCtx)
{
	...
	else if(globalCtx->msgCtx.msgMode == MSGMODE_PAUSED)
...
void EnFr_ActivateCheckFrogSong(EnFr* pthis, GlobalContext* globalCtx)
{
	if(sEnFrPointers.flags == 11)
	{
		...
		if((gSaveContext.eventChkInf[13] & 0x2)	    // ZL
		   ...
		   && (gSaveContext.eventChkInf[13] & 0x40))
		{ // SoS
...
s32 EnFr_IsFrogSongComplete(EnFr* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->ocarinaNote == (*msgCtx).lastOcaNoteIdx)
...
void EnFr_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->posButterflyLight.x, pthis->posButterflyLight.y, pthis->posButterflyLight.z, 255, 255, 255, lightRadius);
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnFr_OverrideLimbDraw, EnFr_PostLimbDraw, pthis);
