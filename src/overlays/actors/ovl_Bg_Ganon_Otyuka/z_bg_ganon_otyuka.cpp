...
void BgGanonOtyuka_Fall(BgGanonOtyuka* pthis, GlobalContext* globalCtx)
{
		...
		if(pthis->dyna.actor.world.pos.y < -750.0f)
		{
			if(player->actor.world.pos.y < -400.0f)
			{
				...
				for(i = 0; i < 30; i++)
				{
					...
					func_8002836C(globalCtx, &pos, &velocity, &accel, &sDustPrimColor, &sDustEnvColor, (s16)Rand_ZeroFloat(100.0f) + 250, 5, (s16)Rand_ZeroFloat(5.0f) + 15);
				}
	...
	else
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EV_STONEDOOR_STOP, &pthis->dyna.actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
		else
		{
	...
	while(actor != NULL)
	{
		if(actor->id == ACTOR_BG_GANON_OTYUKA)
		{
			...
			if(platform->dyna.actor.projectedPos.z > spBC)
			{
				...
				Matrix_Translate(platform->dyna.actor.world.pos.x, platform->dyna.actor.world.pos.y, platform->dyna.actor.world.pos.z, MTXMODE_NEW);
				phi_s1 = NULL;
