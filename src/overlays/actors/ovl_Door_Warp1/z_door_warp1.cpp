...
void DoorWarp1_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params != WARP_SAGES && pthis->actor.params != WARP_BLUE_CRYSTAL && pthis->actor.params != WARP_YELLOW && pthis->actor.params != WARP_DESTINATION)
	{
		Lights_PointNoGlowSetInfo(&pthis->upperLightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
		pthis->upperLight = LightContext_InsertLight(globalCtx2, &globalCtx2->lightCtx, &pthis->upperLightInfo);

		Lights_PointNoGlowSetInfo(&pthis->lowerLightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
...
void DoorWarp1_Destroy(Actor* thisx, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 3; i++)
	{
		globalCtx->envCtx.adjAmbientColor[i] = globalCtx->envCtx.adjFogColor[i] = globalCtx->envCtx.adjLight1Color[i] = 0;
	}
...
void DoorWarp1_SetupWarp(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->actor.params)
	{
		...
		case WARP_UNK_7:
		default:
			Lights_PointNoGlowSetInfo(&pthis->upperLightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 200, 255, 255, 255);
			Lights_PointNoGlowSetInfo(&pthis->lowerLightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 200, 255, 255, 255);
			break;
...
void DoorWarp1_SetupBlueCrystal(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gWarpCrystalSkel, &gWarpCrystalAnim, NULL, NULL, 0);
	Animation_ChangeImpl(&pthis->skelAnime, &gWarpCrystalAnim, 0, Animation_GetLastFrame(&gWarpCrystalAnim), Animation_GetLastFrame(&gWarpCrystalAnim), ANIMMODE_ONCE, 0.0f, 1);
	...
	for(i = 0; i < 3; i++)
	{
		globalCtx->envCtx.adjAmbientColor[i] = globalCtx->envCtx.adjFogColor[i] = globalCtx->envCtx.adjLight1Color[i] = -255;
	}
...
void func_80999214(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 3; i++)
	{
		globalCtx->envCtx.adjAmbientColor[i] = globalCtx->envCtx.adjFogColor[i] = globalCtx->envCtx.adjLight1Color[i] = -255.0f * darkness;
	}
...
void DoorWarp1_WarpAppear(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params != WARP_YELLOW && pthis->actor.params != WARP_ORANGE && pthis->actor.params != WARP_GREEN && pthis->actor.params != WARP_RED)
	{
		if(pthis->scale < 100)
		{
			pthis->scale += 2;
...
void DoorWarp1_ChildWarpOut(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->upperLightInfo, (s16)player->actor.world.pos.x + 10.0f, (s16)player->actor.world.pos.y + 10.0f, (s16)player->actor.world.pos.z + 10.0f, 235, 255, 255, 255);
	Lights_PointNoGlowSetInfo(&pthis->lowerLightInfo, (s16)player->actor.world.pos.x - 10.0f, (s16)player->actor.world.pos.y - 10.0f, (s16)player->actor.world.pos.z - 10.0f, 235, 255, 255, 255);
...
void func_8099A508(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	Audio_PlaySoundGeneral(NA_SE_EV_LINK_WARP, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	Animation_ChangeImpl(&pthis->skelAnime, &gWarpCrystalAnim, 1.0f, Animation_GetLastFrame(&gWarpCrystalAnim), Animation_GetLastFrame(&gWarpCrystalAnim), ANIMMODE_ONCE, 40.0f, 1);
...
void DoorWarp1_AdultWarpOut(DoorWarp1* pthis, GlobalContext* globalCtx)
{
	...
	if(temp_f0_2 > 0.0f)
	{
		...
		for(i = 0; i < 3; i++)
		{
			globalCtx->envCtx.adjAmbientColor[i] = globalCtx->envCtx.adjFogColor[i] = globalCtx->envCtx.adjLight1Color[i] = -255.0f * temp_f0_2;
		}
