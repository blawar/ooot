...
static Vec3f sLeftChestPos[] = {
    {0.0f, 0.0f, 0.0f}, {-100.0f, 20.0f, -245.0f}, {-100.0f, 20.0f, -685.0f}, {-100.0f, 20.0f, -1125.0f}, {-100.0f, 20.0f, -1565.0f}, {-100.0f, 20.0f, -2005.0f},
};

static Vec3f sRightChestPos[] = {
    {0.0f, 0.0f, 0.0f}, {140.0f, 20.0f, -245.0f}, {140.0f, 20.0f, -685.0f}, {140.0f, 20.0f, -1125.0f}, {140.0f, 20.0f, -1565.0f}, {140.0f, 20.0f, -2005.0f},
};
...
static s32 sItemEtcTypes[] = {
    0, ITEM_ETC_RUPEE_GREEN_CHEST_GAME, ITEM_ETC_RUPEE_GREEN_CHEST_GAME, ITEM_ETC_RUPEE_BLUE_CHEST_GAME, ITEM_ETC_RUPEE_BLUE_CHEST_GAME, ITEM_ETC_RUPEE_RED_CHEST_GAME,
};
...
void EnChanger_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(globalCtx->roomCtx.curRoom.num >= 6)
	{
		...
		pthis->finalChest = (EnBox*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_BOX, 20.0f, 20.0f, -2500.0f, 0, 0x7FFF, 0, rewardChestParams);
		if(pthis->finalChest != NULL)
		{
			...
			{
				rewardParams = ((gSaveContext.itemGetInf[1] & 0x800) ? (ITEM_ETC_RUPEE_PURPLE_CHEST_GAME) : (ITEM_ETC_HEART_PIECE_CHEST_GAME)) & 0xFF;
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_ITEM_ETCETERA, 20.0f, 20.0f, -2500.0f, 0, 0, 0, ((sTreasureFlags[5] & 0x1F) << 8) + rewardParams);
	...
	pthis->leftChest = (EnBox*)Actor_SpawnAsChild(
	    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_BOX, sLeftChestPos[globalCtx->roomCtx.curRoom.num].x, sLeftChestPos[globalCtx->roomCtx.curRoom.num].y, sLeftChestPos[globalCtx->roomCtx.curRoom.num].z, 0, -0x3FFF, 0, leftChestParams);
	...
	if(pthis->leftChest != NULL)
	{
		...
		{
			Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_ITEM_ETCETERA, sLeftChestPos[globalCtx->roomCtx.curRoom.num].x, sLeftChestPos[globalCtx->roomCtx.curRoom.num].y, sLeftChestPos[globalCtx->roomCtx.curRoom.num].z, 0, 0, 0,
			    ((pthis->leftChestNum & 0x1F) << 8) + (leftChestItem & 0xFF));
		}
	}

	pthis->rightChest = (EnBox*)Actor_SpawnAsChild(
	    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_BOX, sRightChestPos[globalCtx->roomCtx.curRoom.num].x, sRightChestPos[globalCtx->roomCtx.curRoom.num].y, sRightChestPos[globalCtx->roomCtx.curRoom.num].z, 0, 0x3FFF, 0, rightChestParams);
	...
	if(pthis->rightChest != NULL)
	{
		...
		Actor_Spawn(
		    &globalCtx->actorCtx, globalCtx, ACTOR_ITEM_ETCETERA, sRightChestPos[globalCtx->roomCtx.curRoom.num].x, sRightChestPos[globalCtx->roomCtx.curRoom.num].y, sRightChestPos[globalCtx->roomCtx.curRoom.num].z, 0, 0, 0,
		...
		switch(temp_s0_2)
		{
			case CHEST_LEFT:
				...
				else
				{
					...
					Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_EX_ITEM, xPos, yPos, zPos, 0, 0, 0, temp_s0_2);
				}
				break;
			case CHEST_RIGHT:
