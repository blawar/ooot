...
void EnJj_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	{
		case JABUJABU_MAIN:
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gJabuJabuSkel, &gJabuJabuAnim, pthis->jointTable, pthis->morphTable, 22);
			...
			pthis->bodyCollisionActor = (DynaPolyActor*)Actor_SpawnAsChild(
			    &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EN_JJ, pthis->dyna.actor.world.pos.x - 10.0f, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, 0, pthis->dyna.actor.world.rot.y, 0, JABUJABU_COLLISION);
...
void EnJj_WaitForFish(EnJj* pthis, GlobalContext* globalCtx)
{
	...
	if((Math_Vec3f_DistXZ(&feedingSpot_45, &player->actor.world.pos) < 300.0f) && globalCtx->isPlayerDroppingFish(globalCtx))
	{
...
void EnJj_CutsceneUpdate(EnJj* pthis, GlobalContext* globalCtx)
{
	switch(globalCtx->csCtx.npcActions[2]->action)
	{
		...
		case 2:
			...
			if(!(pthis->unk_30A & 8))
			{
				pthis->dust = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EFF_DUST, -1100.0f, 105.0f, -27.0f, 0, 0, 0, EFF_DUST_TYPE_0);
				pthis->unk_30A |= 8;
			}
			break;
