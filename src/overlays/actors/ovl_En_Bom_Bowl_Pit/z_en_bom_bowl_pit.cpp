...
void EnBomBowlPit_DetectHit(EnBomBowlPit* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->cameraPtrs[MAIN_CAM]->setting == CAM_SET_CHU_BOWLING)
	{
		...
		while(chu != NULL)
		{
			...
			if(((fabsf(chuPosDiff.x) < 40.0f) || (BREG(2))) && ((fabsf(chuPosDiff.y) < 40.0f) || (BREG(2))) && ((fabsf(chuPosDiff.z) < 40.0f) || (BREG(2))))
			{
...
void EnBomBowlPit_CameraDollyIn(EnBomBowlPit* pthis, GlobalContext* globalCtx)
{
	...
	if((fabsf(pthis->unk_18C.x - pthis->unk_198.x) < 5.0f) && (fabsf(pthis->unk_18C.y - pthis->unk_198.y) < 5.0f) && (fabsf(pthis->unk_18C.z - pthis->unk_198.z) < 5.0f) && (fabsf(pthis->unk_180.x - pthis->unk_1BC.x) < 5.0f) &&
...
void EnBomBowlPit_SpawnPrize(EnBomBowlPit* pthis, GlobalContext* globalCtx)
{
	if(pthis->timer == 0)
	{
		pthis->exItem = (EnExItem*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_ITEM, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z - 70.0f, 0, 0, 0, pthis->prizeIndex);
