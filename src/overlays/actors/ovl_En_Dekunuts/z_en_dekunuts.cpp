...
void EnDekunuts_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		SkelAnime_Init(globalCtx, &pthis->skelAnime, &gDekuNutsSkel, &gDekuNutsStandAnim, pthis->jointTable, pthis->morphTable, 25);
		...
		Actor_SpawnAsChild(&globalCtx->actorCtx, thisx, globalCtx, ACTOR_EN_DEKUNUTS, thisx->world.pos.x, thisx->world.pos.y, thisx->world.pos.z, 0, thisx->world.rot.y, 0, DEKUNUTS_FLOWER);
	}
}
...
void EnDekunuts_SetupBeStunned(EnDekunuts* pthis)
{
	...
	Actor_SetColorFilter(&pthis->actor, 0, 0xFF, 0, Animation_GetLastFrame(&gDekuNutsDamageAnim) * pthis->animFlagAndTimer);
}
...
void EnDekunuts_Wait(EnDekunuts* pthis, GlobalContext* globalCtx)
{
	...
	pthis->collider.dim.height = ((pthis->skelAnime.curFrame.clamp(9.0f, 12.0f) - 9.0f) * 9.0f) + 5.0f;
	...
	if(hasSlowPlaybackSpeed && ((pthis->actor.xzDistToPlayer > 160.0f) && (fabsf(pthis->actor.yDistToPlayer) < 120.0f)) && ((pthis->animFlagAndTimer == 0) || (pthis->actor.xzDistToPlayer < 480.0f)))
	{
...
void EnDekunuts_ThrowNut(EnDekunuts* pthis, GlobalContext* globalCtx)
{
	...
	else if(Animation_OnFrame(&pthis->skelAnime, 6.0f))
	{
		...
		if(Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_NUTSBALL, spawnPos.x, spawnPos.y, spawnPos.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z, 0) != NULL)
		{
...
void EnDekunuts_Burrow(EnDekunuts* pthis, GlobalContext* globalCtx)
{
	...
	{
		pthis->collider.dim.height = ((3.0f - pthis->skelAnime.curFrame.clamp(1.0f, 3.0f)) * 12.0f) + 5.0f;
	}
...
void EnDekunuts_Run(EnDekunuts* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->runAwayCount == 0) && Actor_WorldDistXZToPoint(&pthis->actor, &pthis->actor.home.pos) < 20.0f && fabsf(pthis->actor.world.pos.y - pthis->actor.home.pos.y) < 2.0f)
	{
...
void EnDekunuts_Die(EnDekunuts* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime))
	{
		...
		EffectSsDeadDb_Spawn(globalCtx, &effectPos, &effectVelAndAccel_63, &effectVelAndAccel_63, 200, 0, 255, 255, 255, 255, 150, 150, 150, 1, 13, 1);
...
void EnDekunuts_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params != DEKUNUTS_FLOWER)
	{
		...
		{
			Actor_SetFocus(&pthis->actor, 20.0f - ((pthis->skelAnime.curFrame * 20.0f) / Animation_GetLastFrame(&gDekuNutsBurrowAnim)));
		}
		else
		{
...
s32 EnDekunuts_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnDekunuts* pthis = (EnDekunuts*)thisx;
	f32 x;
	f32 y;
...
void EnDekunuts_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnDekunuts_OverrideLimbDraw, NULL, pthis);
	}
}
