...
void BgYdanSp_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		for(i = 0; i < 3; i++)
		{
			tri[i].x = pthis->dyna.actor.world.pos.x + (cossY * ti1->dim.vtx[i].x) - (sinsY * ti1->dim.vtx[i].y * nSinsX);
			tri[i].y = pthis->dyna.actor.world.pos.y + (ti1->dim.vtx[i].y * cossX);
			tri[i].z = pthis->dyna.actor.world.pos.z - (sinsY * ti1->dim.vtx[i].x) + (ti1->dim.vtx[i].y * cossY * nSinsX);
		}
...
void BgYdanSp_BurnFloorWeb(BgYdanSp* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer % 3) == 0)
	{
		...
		for(i = 0; i < 6; i++)
		{
			...
			EffectSsDeadDb_Spawn(globalCtx, &pthis->dyna.actor.home.pos, &velocity, &accel_38, 60, 6, 255, 255, 150, 170, 255, 0, 0, 1, 0xE, 1);
			rot2 += 0x2AAA;
		}
	}
}
...
void BgYdanSp_BurnWallWeb(BgYdanSp* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer % 3) == 0)
	{
		...
		for(i = 0; i < 6; i++)
		{
			...
			EffectSsDeadDb_Spawn(globalCtx, &pthis->dyna.actor.home.pos, &velocity, &accel_42, 80, 6, 255, 255, 150, 170, 255, 0, 0, 1, 0xE, 1);
			rot2 += 0x2AAA;
		}
	}
}
...
void BgYdanSp_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_ydan_sp.c", 781);
	func_80093D84(globalCtx->gfxCtx);
	if(thisx->params == WEB_WALL)
	{
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_ydan_sp.c", 787), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	else if(pthis->actionFunc == BgYdanSp_FloorWebBroken)
	{
		...
		if(pthis->timer == 40)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_ydan_sp.c", 808), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		...
		for(i = 0; i < 8; i++)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_ydan_sp.c", 830), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	else
	{
		Matrix_Translate(0.0f, (thisx->home.pos.y - thisx->world.pos.y) * 10.0f, 0.0f, MTXMODE_APPLY);
		Matrix_Scale(1.0f, ((thisx->home.pos.y - thisx->world.pos.y) + 10.0f) * 0.1f, 1.0f, MTXMODE_APPLY);
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_ydan_sp.c", 849), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, gDTWebFloorDL);
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_ydan_sp.c", 856);
}
