...
void EnFu_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelanime, &gWindmillManSkel, &gWindmillManPlayStillAnim, pthis->jointTable, pthis->morphTable, FU_LIMB_MAX);
...
void func_80A1DA04(EnFu* pthis, GlobalContext* globalCtx)
{
	if(Actor_TextboxIsClosing(&pthis->actor, globalCtx))
	{
		...
		{
			Animation_Change(&pthis->skelanime, &gWindmillManPlayStillAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gWindmillManPlayStillAnim), ANIMMODE_ONCE, -4.0f);
		}
	}
}
...
void EnFu_WaitChild(EnFu* pthis, GlobalContext* globalCtx)
{
	...
	if(func_80A1D94C(pthis, globalCtx, textID, func_80A1DA04))
	{
		if(textID == 0x5033)
		{
			Animation_Change(&pthis->skelanime, &gWindmillManPlayAndMoveHeadAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gWindmillManPlayAndMoveHeadAnim), ANIMMODE_ONCE, -4.0f);
		}
	}
}
...
void EnFu_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelanime, (AnimationHeader*)pthis->skelanime.animation, 1.0f, 0.0f, Animation_GetLastFrame(pthis->skelanime.animation), ANIMMODE_ONCE, 0.0f);
	}
