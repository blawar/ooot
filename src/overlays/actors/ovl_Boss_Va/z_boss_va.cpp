...
void BossVa_SpawnZapperCharge(GlobalContext* globalCtx, BossVaEffect* effect, BossVa* pthis, Vec3f* pos, Vec3s* rot,
                              s16 scale, u8 mode);
void BossVa_SpawnTumor(GlobalContext* globalCtx, BossVaEffect* effect, BossVa* pthis, Vec3f* offset, s16 scale, u8 mode);
void BossVa_SpawnSparkBall(GlobalContext* globalCtx, BossVaEffect* effect, BossVa* pthis, Vec3f* offset, s16 scale,
                           u8 mode);
...
void BossVa_AttachToBody(BossVa* pthis) {
    ...
    switch (pthis->actor.params) {
        ...
        case BOSSVA_ZAPPER_3:
            pthis->actor.shape.rot.y = sInitRot[pthis->actor.params].y;
            pthis->actor.shape.rot.x = (sInitRot[pthis->actor.params].x +
                                       (s16)(Math_CosS(sInitRot[pthis->actor.params].y) * vaBody->actor.shape.rot.x)) -
                                      (s16)(Math_SinS(sInitRot[pthis->actor.params].y) * vaBody->actor.shape.rot.z);
            pthis->actor.shape.rot.z = (s16)(Math_CosS(sInitRot[pthis->actor.params].y) * vaBody->actor.shape.rot.z) +
                                      (sInitRot[pthis->actor.params].z +
                                       (s16)(Math_SinS(sInitRot[pthis->actor.params].y) * vaBody->actor.shape.rot.x));
            break;
    }
...
void BossVa_Spark(GlobalContext* globalCtx, BossVa* pthis, s32 count, s16 scale, f32 xzSpread, f32 ySpread, u8 mode,
                  f32 range, u8 fixed) {
    s32 i;
    s16 index;
    Vec3f offset;
...
void BossVa_KillBari(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    for (i = 7; i >= 0; i--) {
        ...
        pos.y =
            Rand_CenteredFloat(50.0f) + (pthis->actor.world.pos.y + (pthis->actor.shape.yOffset * pthis->actor.scale.y));
...
void BossVa_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    switch (pthis->actor.params) {
        case BOSSVA_BODY:
            Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_VA, 0.0f, 80.0f, 400.0f, 0, 0,
                               0, BOSSVA_DOOR);
            if (Flags_GetClear(globalCtx, globalCtx->roomCtx.curRoom.num)) {
                ...
                Actor_Spawn(&globalCtx->actorCtx, globalCtx, warpId, pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                            pthis->actor.world.pos.z, 0, 0, 0,
                            0); //! params could be WARP_DUNGEON_CHILD however pthis can also spawn Ru1
                Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_ITEM_B_HEART, pthis->actor.world.pos.x + 160.0f,
                ...
                if (gSaveContext.eventChkInf[7] & 0x40) {
                    ...
                    for (i = BOSSVA_BARI_LOWER_5; i >= BOSSVA_BARI_UPPER_1; i--) {
                        Actor_SpawnAsChild(
                            &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_VA,
                            sInitPosOffsets[i].x + pthis->actor.world.pos.x,
                            sInitPosOffsets[i].y + pthis->actor.world.pos.y,
                            sInitPosOffsets[i].z + pthis->actor.world.pos.z, sInitRot[i].x + pthis->actor.world.rot.x,
                            sInitRot[i].y + pthis->actor.world.rot.y, sInitRot[i].z + pthis->actor.world.rot.z, i);
                    }
                ...
                for (i = BOSSVA_ZAPPER_3; i >= BOSSVA_SUPPORT_1; i--) {
                    Actor_SpawnAsChild(
                        &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_VA,
                        sInitPosOffsets[i].x + pthis->actor.world.pos.x, sInitPosOffsets[i].y + pthis->actor.world.pos.y,
                        sInitPosOffsets[i].z + pthis->actor.world.pos.z, sInitRot[i].x + pthis->actor.world.rot.x,
                        sInitRot[i].y + pthis->actor.world.rot.y, sInitRot[i].z + pthis->actor.world.rot.z, i);
                }
...
void BossVa_SetupBodyPhase2(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    for (i = BOSSVA_BARI_UPPER_5; i >= BOSSVA_BARI_UPPER_1; i--) {
        Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_VA,
                           sInitPosOffsets[i].x + pthis->actor.world.pos.x,
                           sInitPosOffsets[i].y + pthis->actor.world.pos.y,
                           sInitPosOffsets[i].z + pthis->actor.world.pos.z, sInitRot[i].x + pthis->actor.world.rot.x,
                           sInitRot[i].y + pthis->actor.world.rot.y, sInitRot[i].z + pthis->actor.world.rot.z, i);
...
void BossVa_BodyPhase2(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    func_800F436C(&pthis->actor.projectedPos, NA_SE_EN_BALINADE_LEVEL - SFX_FLAG,
                  (pthis->vaBodySpinRate * 0.00025f) + 1.0f);
}
...
void BossVa_BodyPhase4(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    if (pthis->timer == 0) {
        ...
        if (Math_SmoothStepToS(&pthis->vaBodySpinRate, (s16)((sFightPhase - PHASE_4 + 1) * 500.0f) + 0xFA0, 1, 0x12C,
                               0) == 0) {
            if (pthis->actor.speedXZ == 0.0f) {
    ...
    if (sFightPhase <= PHASE_4) {
        if (Math_Vec3f_DistXZ(&pthis->actor.world.pos, &pthis->actor.home.pos) >= 400.0f) {
            Math_SmoothStepToS(&pthis->actor.world.rot.y, Math_Vec3f_Yaw(&pthis->actor.world.pos, &pthis->actor.home.pos),
                               1, 0x5DC, 0);
        } else if (player->invincibilityTimer != 0) {
            Math_SmoothStepToS(&pthis->actor.world.rot.y, pthis->actor.yawTowardsPlayer + 0x8000, 1, 0x12C, 0);
        } else if ((globalCtx->gameplayFrames & 0x80) == 0) {
    ...
    switch (sCsState) {
        ...
        case DEATH_CORE_DEAD:
            ...
            if (!pthis->isDead) {
                ...
                pthis->timer = 30;
                sCsState++;
                EffectSsDeadSound_SpawnStationary(globalCtx, &pthis->actor.projectedPos, NA_SE_EN_BALINADE_DEAD, 1, 1,
                                                  0x28);
                pthis->onCeiling = 2; // Not used by body
...
void BossVa_SetupSupportCut(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    sBodyState++;
    sFightPhase++;
    Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BOSS_VA, pthis->armTip.x, pthis->armTip.y + 20.0f, pthis->armTip.z,
                0, pthis->actor.shape.rot.y, 0, stumpParams);
        ...
        case DEATH_CORE_BURST:
            if (!pthis->burst) {
                if ((globalCtx->gameplayFrames % 2) != 0) {
                    BossVa_Tumor(globalCtx, pthis, 1, (s16)Rand_CenteredFloat(5.0f) + 6, 7.0f, 5.0f, TUMOR_ARM,
                                 (pthis->timer2 >> 3) + 1, true);
                }
...
void BossVa_ZapperAttack(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    if ((sp8E >= ABS(tmp17) || pthis->burst) && !(sBodyState & 0x80) && !(player->stateFlags1 & 0x04000000)) {
        ...
        if (!pthis->burst) {
            ...
            sp96 = pthis->actor.shape.rot.x + pthis->skelAnime.jointTable[1].z + pthis->skelAnime.jointTable[2].z +
                   pthis->skelAnime.jointTable[3].z + pthis->skelAnime.jointTable[4].z + pthis->skelAnime.jointTable[5].z;
    ...
    if (pthis->burst && (pthis->burst != 2)) { // burst can never be 2
        if (pthis->timer2 >= 32) {
            if (pthis->timer2 == 32) {
...
void BossVa_ZapperDeath(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    switch (sCsState) {
        ...
        case DEATH_ZAPPER_3:
            if (!pthis->burst) {
                if (((pthis->actor.params == BOSSVA_ZAPPER_1) && (pthis->timer2 < 16)) ||
                    ((pthis->actor.params == BOSSVA_ZAPPER_2) && (pthis->timer2 < 24)) ||
                    (pthis->actor.params == BOSSVA_ZAPPER_3)) {

                    if ((pthis->timer2 % 2) == 0 && (pthis->timer2 >= 0)) {
                        if (pthis->timer2 < 8) {
                            BossVa_Tumor(globalCtx, pthis, 1, (s16)Rand_CenteredFloat(5.0f) + 0xD, 0.0f, 0.0f, TUMOR_ARM,
                                         0.6f, true);
                        } else {
                            BossVa_Tumor(globalCtx, pthis, 1, (s16)Rand_CenteredFloat(5.0f) + 6, 0.0f, 7.0f, TUMOR_ARM,
                                         (pthis->timer2 >> 3) + 1, true);
                        }
...
void BossVa_ZapperEnraged(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    if ((ABS(tmp16) <= 0x4650 || pthis->burst) && !(sBodyState & 0x80) && !(player->stateFlags1 & 0x04000000)) {
        if (!pthis->burst) {
            ...
            sp6A = pthis->actor.shape.rot.x + pthis->skelAnime.jointTable[1].x + pthis->skelAnime.jointTable[2].x +
                   pthis->skelAnime.jointTable[3].x + pthis->skelAnime.jointTable[4].x + pthis->skelAnime.jointTable[5].x;
...
void BossVa_BariIntro(BossVa* pthis, GlobalContext* globalCtx) {
    ...
    switch (sCsState) {
        ...
        case INTRO_LOOK_SUPPORT:
            if (pthis->actor.params != BOSSVA_BARI_UPPER_1) {
                Math_SmoothStepToF(&pthis->actor.world.pos.y,
                                   sInitPosOffsets[pthis->actor.params + 10].y + pthis->actor.home.pos.y, 0.3f, 1.0f,
                                   0.15f);
...
void BossVa_ZapperPostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    switch (limbIndex) {
        ...
        case 7:
            ...
            Collider_SetQuadVertices(&pthis->colliderLightning, &pthis->colliderLightning.dim.quad[0],
                                     &pthis->colliderLightning.dim.quad[1], &pthis->colliderLightning.dim.quad[2],
                                     &pthis->colliderLightning.dim.quad[3]);
            Matrix_Pop();
            break;
    }
}
...
void BossVa_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (pthis->actor.params) {
        ...
        case BOSSVA_SUPPORT_3:
            if (!pthis->isDead) {
                SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                                      pthis->skelAnime.dListCount, BossVa_SupportOverrideLimbDraw,
                                      BossVa_SupportPostLimbDraw, pthis);
            }
            break;
        case BOSSVA_ZAPPER_1:
        case BOSSVA_ZAPPER_2:
        case BOSSVA_ZAPPER_3:
            if (!pthis->isDead) {
                SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                                      pthis->skelAnime.dListCount, BossVa_ZapperOverrideLimbDraw,
                                      BossVa_ZapperPostLimbDraw, pthis);
            }
            break;
        case BOSSVA_STUMP_1:
        case BOSSVA_STUMP_2:
        case BOSSVA_STUMP_3:
            SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                                  pthis->skelAnime.dListCount, NULL, NULL, NULL);
            break;
        default:
            if (!pthis->isDead) {
                SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                                  BossVa_BariOverrideLimbDraw, BossVa_BariPostLimbDraw, pthis);
                Collider_UpdateSpheres(0, &pthis->colliderSph);
                if (sCsState < BOSSVA_BATTLE) {
...
void BossVa_SpawnSpark(GlobalContext* globalCtx, BossVaEffect* effect, BossVa* pthis, Vec3f* offset, s16 scale,
                       u8 mode) {
    Player* player = GET_PLAYER(globalCtx);
    s16 index;
...
void BossVa_SpawnTumor(GlobalContext* globalCtx, BossVaEffect* effect, BossVa* pthis, Vec3f* offset, s16 scale,
                       u8 mode) {
    Vec3f pos = { 0.0f, -1000.0f, 0.0f };
    s16 i;
