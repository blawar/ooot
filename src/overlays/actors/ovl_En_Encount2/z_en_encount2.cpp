...
void EnEncount2_Wait(EnEncount2* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->actor.xzDistToPlayer < 700.0f) && (Flags_GetSwitch(globalCtx, 0x37)))
	{
		...
		if(((scene == SCENE_GANON_DEMO) || (scene == SCENE_GANON_FINAL) || (scene == SCENE_GANON_SONOGO) || (scene == SCENE_GANONTIKA_SONOGO)) && (!pthis->collapseSpawnerInactive))
		{
...
void EnEncount2_SpawnRocks(EnEncount2* pthis, GlobalContext* globalCtx)
{
	...
	if(spawnerState != ENCOUNT2_INACTIVE)
	{
		...
		if((pthis->numSpawnedRocks < maxRocks) && (pthis->timerBetweenRockSpawns == 0))
		{
			...
			spawnedRock = (EnFireRock*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FIRE_ROCK, tempVec2X, tempVec1Y, tempVec2Z, 0, 0, 0, spawnedRockType);
			if(spawnedRock != NULL)
			{
	...
	if(objBankIndex >= 0)
	{
		...
		for(i = 0; i < ARRAY_COUNT(pthis->particles); particle++, i++)
		{
			if(particle->isAlive)
			{
				...
				gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_encount2.c", 669), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
