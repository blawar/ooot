...
ActorInit Bg_Spot01_Idohashira_InitVars = {
    ...
    ACTOR_FACTORY(BgSpot01Idohashira),
...
s32 func_808AB29C(BgSpot01Idohashira* pthis, GlobalContext* globalCtx)
{
	...
	if(npcAction != NULL)
	{
		...
		thisPos->y = func_808AB1DC(initPos.y, tempY, npcAction->endFrame, npcAction->startFrame, globalCtx->csCtx.frames) + initPos.y;
...
void func_808AB700(BgSpot01Idohashira* pthis, GlobalContext* globalCtx)
{
	...
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(localGfxCtx, "../z_bg_spot01_idohashira.c", 699), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
