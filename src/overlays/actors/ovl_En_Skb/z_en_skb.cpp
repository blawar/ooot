...
void EnSkb_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gStalchildSkel, &gStalchildUncurlingAnim, pthis->jointTable, pthis->morphTable, 20);
...
void func_80AFCFF0(EnSkb* pthis, GlobalContext* globalCtx)
{
	if((Math_SmoothStepToF(&pthis->actor.shape.yOffset, -8000.0f, 1.0f, 500.0f, 0.0f) != 0.0f) && (globalCtx->gameplayFrames & 1))
	{
		EnSkb_SpawnDebris(globalCtx, pthis, &pthis->actor.world.pos);
	}
...
void EnSkb_Advance(EnSkb* pthis, GlobalContext* globalCtx)
{
	...
	{
		if(((prevKeyFrame < 9) && (((s32)playSpeed + thisKeyFrame) >= 8)) || !((prevKeyFrame >= 16) || (((s32)playSpeed + thisKeyFrame) < 15)))
		{
...
void func_80AFD47C(EnSkb* pthis)
{
	Animation_Change(&pthis->skelAnime, &gStalchildAttackingAnim, -0.4f, pthis->skelAnime.curFrame - 1.0f, 0.0f, ANIMMODE_ONCE_INTERP, 0.0f);
	pthis->collider.base.atFlags &= ~4;
	pthis->unk_280 = 5;
	pthis->unk_281 = 0;
				...
				else
				{
					...
					if(pthis->unk_283 == 0)
					{
						if((pthis->actor.colChkInfo.damageEffect == 0xD) ||
						   ((pthis->actor.colChkInfo.damageEffect == 0xE) && ((player->swordAnimation >= 4 && player->swordAnimation <= 11) || (player->swordAnimation == 20 || player->swordAnimation == 21))))
						{
