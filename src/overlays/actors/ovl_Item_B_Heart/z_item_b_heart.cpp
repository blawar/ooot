...
void ItemBHeart_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_item_b_heart.c", 506);
	...
	if(flag)
	{
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_item_b_heart.c", 551), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	else
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_item_b_heart.c", 557), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_OPA_DISP++, gGiHeartBorderDL);
		gSPDisplayList(POLY_OPA_DISP++, gGiHeartContainerDL);
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_item_b_heart.c", 561);
}
