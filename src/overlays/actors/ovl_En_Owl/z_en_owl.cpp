...
void EnOwl_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gOwlFlyingSkel, &gOwlFlyAnim, pthis->jointTable, pthis->morphTable, 21);
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime2, &gOwlPerchingSkel, &gOwlPerchAnim, pthis->jointTable2, pthis->morphTable2, 16);
...
void EnOwl_LookAtLink(EnOwl* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.shape.rot.y = pthis->actor.world.rot.y = Math_Vec3f_Yaw(&pthis->actor.world.pos, &player->actor.world.pos);
}
...
void func_80ACB748(EnOwl* pthis, GlobalContext* globalCtx)
{
	...
	switch(owlType)
	{
		case 7:
			...
			if((globalCtx->csCtx.frames > 324) || ((globalCtx->csCtx.frames >= 142 && (globalCtx->csCtx.frames <= 266))))
			{
...
void EnOwl_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(!(pthis->actionFlags & 0x80) && func_80ACC624(pthis, globalCtx))
	{
		if((pthis->skelAnime.animation == &gOwlTakeoffAnim &&
		    (pthis->skelAnime.curFrame == 2.0f || pthis->skelAnime.curFrame == 9.0f || pthis->skelAnime.curFrame == 23.0f || pthis->skelAnime.curFrame == 40.0f || pthis->skelAnime.curFrame == 58.0f)) ||
...
void EnOwl_ChangeMode(EnOwl* pthis, EnOwlActionFunc actionFunc, OwlFunc arg2, SkelAnime* skelAnime, AnimationHeader* animation, f32 morphFrames)
{
	...
	Animation_Change(pthis->curSkelAnime, animation, 1.0f, 0.0f, Animation_GetLastFrame(animation), ANIMMODE_ONCE, morphFrames);
