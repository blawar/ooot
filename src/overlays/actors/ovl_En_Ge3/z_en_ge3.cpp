...
void EnGe3_ChangeAction(EnGe3* pthis, s32 i)
{
	...
	Animation_Change(&pthis->skelAnime, sAnimations[i], 1.0f, 0.0f, (f32)Animation_GetLastFrame(sAnimations[i]), sAnimationModes[i], -8.0f);
...
void EnGe3_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gGerudoRedSkel, NULL, pthis->jointTable, pthis->morphTable, GELDB_LIMB_MAX);
...
{
	if((ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)) <= 0x2300) && (pthis->actor.xzDistToPlayer < 100.0f))
	{
		func_80038290(globalCtx, &pthis->actor, &pthis->headRot, &pthis->unk_306, pthis->actor.focus.pos);
	}
	else
