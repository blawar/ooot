...
static EnViewerInitData sInitData[] = {
    ...
    {OBJECT_HORSE_ZELDA, OBJECT_HORSE_ZELDA, 1, 0, ENVIEWER_SHADOW_HORSE, 20, ENVIEWER_DRAW_HORSE, &gHorseZeldaSkel, &gHorseZeldaGallopingAnim},
    /* ENVIEWER_TYPE_1_IMPA */
    {OBJECT_IM, OBJECT_OPENING_DEMO1, 1, 0, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_IMPA, &gImpaSkel, &object_opening_demo1_Anim_0029CC},
    /* ENVIEWER_TYPE_2_ZELDA */
    {OBJECT_ZL4, OBJECT_OPENING_DEMO1, 1, 0, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_ZELDA, &gChildZeldaSkel, &object_opening_demo1_Anim_000450},
    /* ENVIEWER_TYPE_3_GANONDORF */
    {OBJECT_GNDD, OBJECT_GNDD, 1, -6, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_GANONDORF, &object_gndd_Skel_0119E8, &object_gndd_Anim_002928},
    /* ENVIEWER_TYPE_4_HORSE_GANONDORF */
    {OBJECT_HORSE_GANON, OBJECT_HORSE_GANON, 1, 0, ENVIEWER_SHADOW_HORSE, 20, ENVIEWER_DRAW_HORSE, &gHorseGanonSkel, &gHorseGanonRearingAnim},
    /* ENVIEWER_TYPE_5_GANONDORF */
    {OBJECT_GNDD, OBJECT_GNDD, 1, -6, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_GANONDORF, &object_gndd_Skel_0119E8, &object_gndd_Anim_0005B4},
    /* ENVIEWER_TYPE_6_HORSE_GANONDORF */
    {OBJECT_HORSE_GANON, OBJECT_HORSE_GANON, 1, 0, ENVIEWER_SHADOW_HORSE, 20, ENVIEWER_DRAW_HORSE, &gHorseGanonSkel, &gHorseGanonGallopingAnim},
    /* ENVIEWER_TYPE_7_GANONDORF */
    {OBJECT_GNDD, OBJECT_GNDD, 1, -6, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_GANONDORF, &object_gndd_Skel_0119E8, &object_gndd_Anim_004260},
    /* ENVIEWER_TYPE_8_GANONDORF */
    {OBJECT_GNDD, OBJECT_GNDD, 1, -6, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_GANONDORF, &object_gndd_Skel_0119E8, &object_gndd_Anim_0050A8},
    /* ENVIEWER_TYPE_9_GANONDORF */
    {OBJECT_GANON, OBJECT_GANON, 1, -6, ENVIEWER_SHADOW_NONE, 10, ENVIEWER_DRAW_GANONDORF, &gDorfSkel, &object_ganon_Anim_011348},
};
...
void EnViewer_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_5_GANONDORF || type == ENVIEWER_TYPE_7_GANONDORF || type == ENVIEWER_TYPE_8_GANONDORF || type == ENVIEWER_TYPE_9_GANONDORF)
	{
		sGanondorfCape = (EnGanonMant*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_GANON_MANT, 0.0f, 0.0f, 0.0f, 0, 0, 0, 35);
	}
}
...
void EnViewer_InitAnimGanondorfOrZelda(EnViewer* pthis, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* anim)
{
	s16 type = pthis->actor.params >> 8;

	if(type == ENVIEWER_TYPE_2_ZELDA || type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_5_GANONDORF || type == ENVIEWER_TYPE_7_GANONDORF || type == ENVIEWER_TYPE_8_GANONDORF || type == ENVIEWER_TYPE_9_GANONDORF)
	{
	...
	if(type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_7_GANONDORF || type == ENVIEWER_TYPE_8_GANONDORF || type == ENVIEWER_TYPE_9_GANONDORF)
	{
...
void EnViewer_InitAnimHorse(EnViewer* pthis, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* anim)
{
	...
	type = pthis->actor.params >> 8;
	if(!(type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_4_HORSE_GANONDORF || type == ENVIEWER_TYPE_7_GANONDORF || type == ENVIEWER_TYPE_8_GANONDORF || type == ENVIEWER_TYPE_9_GANONDORF))
	{
		Animation_PlayLoopSetSpeed(&pthis->skin.skelAnime, anim, 3.0f);
	}
	else
...
void EnViewer_InitImpl(EnViewer* pthis, GlobalContext* globalCtx)
{
	...
	if(!Object_IsLoaded(&globalCtx->objectCtx, skelObjBankIndex) || !Object_IsLoaded(&globalCtx->objectCtx, pthis->animObjBankIndex))
	{
	...
	ActorShape_Init(&pthis->actor.shape, initData->yOffset * 100, sShadowDrawFuncs[initData->shadowType], initData->shadowScale);
...
void EnViewer_UpdateImpl(EnViewer* pthis, GlobalContext* globalCtx)
{
	...
	else if(type == ENVIEWER_TYPE_3_GANONDORF)
	{
		...
		if(gSaveContext.sceneSetupIndex == 5)
		{
			...
			{
				Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_6K, 32.0f, 101.0f, 1226.0f, 0, 0, 0, 0xC);
			}
		}
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EV_HORSE_GROAN, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
	}
	...
	if(type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_4_HORSE_GANONDORF)
	{
		if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL)
		{
			...
			else if(pthis->skin.skelAnime.animation != &gHorseGanonGallopingAnim && globalCtx->csCtx.npcActions[1]->action == 12)
			{
	...
	else if(type == ENVIEWER_TYPE_1_IMPA)
	{
		if(gSaveContext.sceneSetupIndex == 5)
		{
			...
			{
				Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_ITEM_OCARINA, 4.0f, 81.0f, 2600.0f, 0, 0, 0, 0);
			}
		}
		else
		{
			...
			{
				Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_ITEM_OCARINA, 4.0f, 81.0f, 2035.0f, 0, 0, 0, 1);
			}
		}
		switch(pthis->state)
		{
			case 0:
				if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 6 && pthis->skin.skelAnime.animation != &object_opening_demo1_Anim_002574)
				{
				...
				break;
			case 1:
				if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 2 && pthis->skin.skelAnime.animation != &object_opening_demo1_Anim_0029CC)
				{
		...
		{ // Hyrule Field
			switch(pthis->state)
			{
				case 0:
					...
					{
						if(globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 6 && pthis->skin.skelAnime.animation != &object_opening_demo1_Anim_001410)
						{
				...
				case 1:
					if(globalCtx->csCtx.state != CS_STATE_IDLE)
					{
						if(globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 2 && pthis->skin.skelAnime.animation != &object_opening_demo1_Anim_000450)
						{
	...
	else if(type == ENVIEWER_TYPE_7_GANONDORF)
	{
		...
		{
			case 0:
				if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL && globalCtx->csCtx.npcActions[1]->action == 7)
				{
...
s32 EnViewer_Ganondorf3OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	if(gSaveContext.sceneSetupIndex == 4)
	{
...
void EnViewer_Ganondorf9PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	if(limbIndex == 11)
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1365);
		func_80093D84(globalCtx->gfxCtx);
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_viewer.c", 1370), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, SEGMENTED_TO_VIRTUAL(object_ganon_DL_00BE90));
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1372);
	}
}

void EnViewer_GanondorfPostLimbDrawUpdateCapeVec(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	if(limbIndex == 15)
	{
...
void EnViewer_DrawGanondorf(EnViewer* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1405);
	type = pthis->actor.params >> 8;
	if(type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_5_GANONDORF || type == ENVIEWER_TYPE_7_GANONDORF || type == ENVIEWER_TYPE_8_GANONDORF)
	{
		if(gSaveContext.sceneSetupIndex != 4)
		{
			frames = 149;
	...
	{
		SkelAnime_DrawFlexOpa(globalCtx, pthis->skin.skelAnime.skeleton, pthis->skin.skelAnime.jointTable, pthis->skin.skelAnime.dListCount, NULL, EnViewer_Ganondorf9PostLimbDraw, pthis);
	}
	else if(type == ENVIEWER_TYPE_3_GANONDORF)
	{
		SkelAnime_DrawFlexOpa(globalCtx, pthis->skin.skelAnime.skeleton, pthis->skin.skelAnime.jointTable, pthis->skin.skelAnime.dListCount, EnViewer_Ganondorf3OverrideLimbDraw, EnViewer_GanondorfPostLimbDrawUpdateCapeVec, pthis);
		EnViewer_UpdateGanondorfCape(globalCtx, pthis);
	}
	else if(type == ENVIEWER_TYPE_3_GANONDORF || type == ENVIEWER_TYPE_5_GANONDORF || type == ENVIEWER_TYPE_7_GANONDORF || type == ENVIEWER_TYPE_8_GANONDORF)
	{
		if((globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[1] != NULL))
		{
			SkelAnime_DrawFlexOpa(globalCtx, pthis->skin.skelAnime.skeleton, pthis->skin.skelAnime.jointTable, pthis->skin.skelAnime.dListCount, NULL, EnViewer_GanondorfPostLimbDrawUpdateCapeVec, pthis);
			EnViewer_UpdateGanondorfCape(globalCtx, pthis);
		}
	}
	else
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1511);
}
...
s32 EnViewer_ZeldaOverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	if(globalCtx->sceneNum == SCENE_SPOT00)
	{ // Hyrule Field
		if(limbIndex == 2)
...
void EnViewer_ZeldaPostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	if(globalCtx->sceneNum == SCENE_TOKINOMA)
	{
		if(limbIndex == 16)
		{
			OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1568);
			gSPDisplayList(POLY_OPA_DISP++, gChildZeldaOcarinaOfTimeDL);
			CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1570);
		}
	}
}
...
{
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1583);
	if(globalCtx->sceneNum == SCENE_SPOT00)
	{ // Hyrule Field
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skin.skelAnime.skeleton, pthis->skin.skelAnime.jointTable, pthis->skin.skelAnime.dListCount, EnViewer_ZeldaOverrideLimbDraw, EnViewer_ZeldaPostLimbDraw, pthis);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1690);
}

s32 EnViewer_ImpaOverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	if(limbIndex == 16)
	{
...
{
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1717);
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skin.skelAnime.skeleton, pthis->skin.skelAnime.jointTable, pthis->skin.skelAnime.dListCount, EnViewer_ImpaOverrideLimbDraw, NULL, pthis);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1740);
}
...
void EnViewer_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1760);
	if(pthis->isVisible)
	{
		type = pthis->actor.params >> 8;
		if(type <= ENVIEWER_TYPE_2_ZELDA)
		...
		else if((globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL) || type == ENVIEWER_TYPE_9_GANONDORF)
		{
			func_80093D18(globalCtx->gfxCtx);
			sDrawFuncs[pthis->drawFuncIndex](pthis, globalCtx);
		}
	}
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1784);
}
...
void EnViewer_UpdatePosition(EnViewer* pthis, GlobalContext* globalCtx)
{
	...
	if(type <= ENVIEWER_TYPE_2_ZELDA)
	{ // zelda's horse, impa and zelda
		if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.frames < globalCtx->csCtx.npcActions[0]->endFrame)
		{
	...
	{ // ganondorf and ganondorf's horse
		if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL && globalCtx->csCtx.frames < globalCtx->csCtx.npcActions[1]->endFrame)
		{
...
void EnViewer_DrawFireEffects(EnViewer* pthis2, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 1941);
	for(i = 0; i < ARRAY_COUNT(pthis->fireEffects); i++)
	{
		switch(pthis->fireEffects[i].state)
		{
			case 0:
				...
				break;
			case 1:
				Math_SmoothStepToF(&pthis->fireEffects[i].lerpFactor, 1.0f, 1.0f, pthis->fireEffects[i].lerpFactorSpeed, pthis->fireEffects[i].lerpFactorSpeed);
				pthis->fireEffects[i].pos.x = pthis->fireEffects[i].startPos.x + (pthis->fireEffects[i].endPos.x - pthis->fireEffects[i].startPos.x) * pthis->fireEffects[i].lerpFactor;
				pthis->fireEffects[i].pos.y = pthis->fireEffects[i].startPos.y + (pthis->fireEffects[i].endPos.y - pthis->fireEffects[i].startPos.y) * pthis->fireEffects[i].lerpFactor;
				pthis->fireEffects[i].pos.z = pthis->fireEffects[i].startPos.z + (pthis->fireEffects[i].endPos.z - pthis->fireEffects[i].startPos.z) * pthis->fireEffects[i].lerpFactor;
		...
		func_80093D84(globalCtx->gfxCtx);
		Matrix_Translate(pthis->fireEffects[i].pos.x, pthis->fireEffects[i].pos.y, pthis->fireEffects[i].pos.z, MTXMODE_NEW);
		Matrix_Scale(pthis->fireEffects[i].scale, pthis->fireEffects[i].scale, pthis->fireEffects[i].scale, MTXMODE_APPLY);
		gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 0, 0, 32, 64, 1, 0, (uintptr_t)(globalCtx->frames * (10 * i - 20)) % 512, 32, 128));
		gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, 255, 255, 170, 255);
		gDPSetEnvColor(POLY_XLU_DISP++, 255, 50, 00, 255);
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_viewer.c", 2027), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPMatrix(POLY_XLU_DISP++, SEGMENT_ADDRESS(0x01000000), G_MTX_NOPUSH | G_MTX_MUL | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, gEffFire1DL);
	}
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_viewer.c", 2034);
}
