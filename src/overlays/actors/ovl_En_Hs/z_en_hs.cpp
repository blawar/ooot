...
ActorInit En_Hs_InitVars = {
    ACTOR_EN_HS, ACTORCAT_NPC, FLAGS, OBJECT_HS, ACTOR_FACTORY(EnHs), (ActorFunc)EnHs_Init, (ActorFunc)EnHs_Destroy, (ActorFunc)EnHs_Update, (ActorFunc)EnHs_Draw, (ActorFunc)EnHs_Reset,
};
...
void EnHs_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_hs_Skel_006260, &object_hs_Anim_0005C0, pthis->jointTable, pthis->morphTable, 16);
...
void func_80A6E7BC(EnHs* pthis, GlobalContext* globalCtx)
{
	if((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CHOICE) && Message_ShouldAdvance(globalCtx))
	{
		...
		Animation_Change(&pthis->skelAnime, &object_hs_Anim_0005C0, 1.0f, 0.0f, Animation_GetLastFrame(&object_hs_Anim_0005C0), ANIMMODE_LOOP, 8.0f);
	}
...
void func_80A6E8CC(EnHs* pthis, GlobalContext* globalCtx)
{
	...
	if((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(globalCtx))
	{
		...
		Animation_Change(&pthis->skelAnime, &object_hs_Anim_000528, 1.0f, 0.0f, Animation_GetLastFrame(&object_hs_Anim_000528), ANIMMODE_LOOP, 8.0f);
	}
...
void func_80A6E9AC(EnHs* pthis, GlobalContext* globalCtx)
{
	...
	if(Actor_ProcessTalkRequest(&pthis->actor, globalCtx))
	{
		if(func_8002F368(globalCtx) == 7)
		{
			...
			Animation_Change(&pthis->skelAnime, &object_hs_Anim_000304, 1.0f, 0.0f, Animation_GetLastFrame(&object_hs_Anim_000304), ANIMMODE_LOOP, 8.0f);
			pthis->unk_2AA = 40;
