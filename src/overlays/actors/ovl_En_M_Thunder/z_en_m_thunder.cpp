...
void EnMThunder_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 255, 255, 255, 0);
	...
	{
		if(!gSaveContext.magicAcquired || gSaveContext.unk_13F0 || (((pthis->actor.params & 0xFF00) >> 8) && !(func_80087708(globalCtx, (pthis->actor.params & 0xFF00) >> 8, 0))))
		{
			Audio_PlaySoundGeneral(NA_SE_IT_ROLLING_CUT, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			Audio_PlaySoundGeneral(NA_SE_IT_SWORD_SWING_HARD, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		...
		pthis->unk_1C4 = 8;
		Audio_PlaySoundGeneral(NA_SE_IT_ROLLING_CUT_LV1, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		pthis->unk_1AC = 1.0f;
	}
	else
	{
...
void func_80A9F350(EnMThunder* pthis, GlobalContext* globalCtx)
{
	...
	if(!(player->stateFlags1 & PLAYER_STATE_SWORD_CHARGE))
	{
		Actor_Kill(&pthis->actor);
	}
}
...
void func_80A9F408(EnMThunder* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_1CA == 0)
	{
		...
		{
			if((gSaveContext.unk_13F0) || (((pthis->actor.params & 0xFF00) >> 8) && !(func_80087708(globalCtx, (pthis->actor.params & 0xFF00) >> 8, 4))))
			{
	...
	if(player->stateFlags2 & PLAYER_STATE2_17)
	{
		...
		else
		{
			...
			func_80A9EFE0(pthis, func_80A9F9B4);
			pthis->unk_1C4 = 8;
			Audio_PlaySoundGeneral(sSfxIds[pthis->unk_1C6], &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			pthis->unk_1AC = 1.0f;
			return;
		}
	}
	...
	if(player->unk_858 > 0.15f)
	{
		...
		if(pthis->actor.child == NULL)
		{
			Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EFF_DUST, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, pthis->actor.shape.rot.y, 0, pthis->unk_1C7 + 2);
		}
...
void EnMThunder_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, redGreen, redGreen, (u32)(blueRadius * 100.0f), (s32)(blueRadius * 800.0f));
}
