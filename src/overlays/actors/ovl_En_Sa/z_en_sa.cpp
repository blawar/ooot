...
static void* mouthTextures_67[] = {
    gSariaMouthClosed2Tex, gSariaMouthSmilingOpenTex, gSariaMouthFrowningTex, gSariaMouthSuprisedTex, gSariaMouthClosedTex,
};
...
ActorInit En_Sa_InitVars = {
    ACTOR_EN_SA, ACTORCAT_NPC, FLAGS, OBJECT_SA, ACTOR_FACTORY(EnSa), (ActorFunc)EnSa_Init, (ActorFunc)EnSa_Destroy, (ActorFunc)EnSa_Update, (ActorFunc)EnSa_Draw, (ActorFunc)EnSa_Reset,
};
...
s16 func_80AF5560(EnSa* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_209 == TEXT_STATE_AWAITING_NEXT || pthis->unk_209 == TEXT_STATE_EVENT || pthis->unk_209 == TEXT_STATE_CLOSING || pthis->unk_209 == TEXT_STATE_DONE_HAS_NEXT)
	{
...
{
	Animation_Change(&pthis->skelAnime, sAnimationInfo[action].animation, 1.0f, 0.0f, Animation_GetLastFrame(sAnimationInfo[action].animation), sAnimationInfo[action].mode, sAnimationInfo[action].transitionRate);
}
...
s32 func_80AF5DFC(EnSa* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->sceneNum == SCENE_KOKIRI_HOME5 && !LINK_IS_ADULT && INV_CONTENT(ITEM_OCARINA_FAIRY) == ITEM_OCARINA_FAIRY && !(gSaveContext.eventChkInf[4] & 1))
	{
		return 1;
	}
...
void func_80AF5F34(EnSa* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->sceneNum == SCENE_SPOT05 && pthis->actionFunc == func_80AF6448 && pthis->skelAnime.animation == &gSariaStopPlayingOcarinaAnim)
	{
		phi_a3 = 1;
	}
	if(globalCtx->sceneNum == SCENE_SPOT05 && pthis->actionFunc == func_80AF68E4 && pthis->skelAnime.animation == &gSariaOcarinaToMouthAnim)
	{
		phi_a3 = 1;
	}
	pthis->unk_1E0.position = player->actor.world.pos;
	pthis->unk_1E0.height = 4.0f;
	func_80034A14(&pthis->actor, &pthis->unk_1E0, 2, phi_a3);
}

s32 func_80AF603C(EnSa* pthis)
{
	if(pthis->skelAnime.animation != &gSariaPlayingOcarinaAnim && pthis->skelAnime.animation != &gSariaOcarinaToMouthAnim)
	{
		return 0;
	}
...
void func_80AF6448(EnSa* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->sceneNum == SCENE_SPOT04)
	{
		if(pthis->unk_1E0.unk_00 != 0)
		{
			switch(pthis->actor.textId)
			{
				...
				case 0x1003:
					...
					break;
				case 0x1031:
					if(pthis->unk_208 == 0 && pthis->unk_20B != 4 && pthis->skelAnime.animation == &gSariaHandsBehindBackWaitAnim)
					{
						func_80AF5CD4(pthis, 4);
						pthis->mouthIndex = 3;
					}
		...
		else if(!CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) && ((gSaveContext.infTable[0] & 2) || (gSaveContext.infTable[0] & 8)))
		{
			if(pthis->unk_20B != 3)
			{
...
void func_80AF68E4(EnSa* pthis, GlobalContext* globalCtx)
{
	...
	if(csAction != NULL)
	{
		...
		if(csAction->action == 3)
		{
			...
			if(phi_v0 == 0)
			{
				Audio_PlaySoundGeneral(NA_SE_PL_WALK_GROUND, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
				pthis->unk_20C = 8;
			}
		}
...
void EnSa_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->skelAnime.animation == &gSariaOcarinaToMouthAnim && pthis->skelAnime.curFrame >= Animation_GetLastFrame(&gSariaOcarinaToMouthAnim))
	{
		EnSa_ChangeAnim(pthis, 6);
	}
...
s32 EnSa_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnSa* pthis = (EnSa*)thisx;
	s32 pad;
	Vec3s sp18;
...
void EnSa_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_sa.c", 1444);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_sa.c", 1497);
}
