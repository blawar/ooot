...
static InitChainEntry sInitChain[] = {
    ICHAIN_VEC3F_DIV1000(scale, 5, ICHAIN_CONTINUE), ICHAIN_F32_DIV1000(gravity, -500, ICHAIN_CONTINUE), ICHAIN_F32(minVelocityY, -4, ICHAIN_CONTINUE), ICHAIN_U8(targetMode, 2, ICHAIN_CONTINUE), ICHAIN_F32(targetArrowOffset, 4000, ICHAIN_STOP),
};
...
void EnFirefly_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gKeeseSkeleton, &gKeeseFlyAnim, pthis->jointTable, pthis->morphTable, 28);
...
void EnFirefly_SetupFrozenFall(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i <= 7; i++)
	{
		...
		EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &iceParticlePos, 150, 150, 150, 250, 235, 245, 255, (Rand_ZeroOne() * 0.15f) + 0.85f);
	}
...
s32 EnFirefly_ReturnToPerch(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	if(Actor_WorldDistXZToPoint(&player->actor, &pthis->actor.home.pos) > 300.0f)
	{
		...
		Math_ScaledStepToS(&pthis->actor.shape.rot.y, Actor_WorldYawTowardPoint(&pthis->actor, &pthis->actor.home.pos), 0x300);
		Math_ScaledStepToS(&pthis->actor.shape.rot.x, Actor_WorldPitchTowardPoint(&pthis->actor, &pthis->actor.home.pos) + 0x1554, 0x100);
		return 1;
	}
...
s32 EnFirefly_SeekTorch(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	if(closestTorch != NULL)
	{
		...
		{
			Math_ScaledStepToS(&pthis->actor.shape.rot.y, Actor_WorldYawTowardActor(&pthis->actor, &closestTorch->actor), 0x300);
			Math_ScaledStepToS(&pthis->actor.shape.rot.x, Actor_WorldPitchTowardPoint(&pthis->actor, &flamePos) + 0x1554, 0x100);
			return 1;
		}
	}
	return 0;
...
void EnFirefly_FlyIdle(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.speedXZ = (Rand_ZeroOne() * 1.5f) + 1.5f;
	if(pthis->onFire || (pthis->actor.params == KEESE_ICE_FLY) || ((EnFirefly_ReturnToPerch(pthis, globalCtx) == 0) && (EnFirefly_SeekTorch(pthis, globalCtx) == 0)))
	{
		if(skelanimeUpdated)
		{
			rand = Rand_ZeroOne();
	...
	if((pthis->timer == 0) && (pthis->actor.xzDistToPlayer < 200.0f) && (Player_GetMask(globalCtx) != PLAYER_MASK_SKULL))
	{
...
void EnFirefly_DiveAttack(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	else if(Actor_IsFacingPlayer(&pthis->actor, 0x2800))
	{
		...
		Math_SmoothStepToS(&pthis->actor.shape.rot.x, Actor_WorldPitchTowardPoint(&pthis->actor, &preyPos) + 0x1554, 2, 0x400, 0x100);
	}
	else
	{
...
void EnFirefly_FlyAway(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	if(((fabsf(pthis->actor.world.pos.y - pthis->maxAltitude) < 10.0f) && (Math_Vec3f_DistXZ(&pthis->actor.world.pos, &pthis->actor.home.pos) < 20.0f)) || (pthis->timer == 0))
	{
...
void EnFirefly_DisturbDiveAttack(EnFirefly* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		preyPos.y = player->actor.world.pos.y + 20.0f;
		preyPos.z = player->actor.world.pos.z;
		Math_ScaledStepToS(&pthis->actor.shape.rot.x, Actor_WorldPitchTowardPoint(&pthis->actor, &preyPos) + 0x1554, 0x100);
...
void EnFirefly_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	pthis->actor.focus.pos.x = (10.0f * Math_SinS(pthis->actor.shape.rot.x) * Math_SinS(pthis->actor.shape.rot.y)) + pthis->actor.world.pos.x;
	pthis->actor.focus.pos.y = (10.0f * Math_CosS(pthis->actor.shape.rot.x)) + pthis->actor.world.pos.y;
	pthis->actor.focus.pos.z = (10.0f * Math_SinS(pthis->actor.shape.rot.x) * Math_CosS(pthis->actor.shape.rot.y)) + pthis->actor.world.pos.z;
}

s32 EnFirefly_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnFirefly* pthis = (EnFirefly*)thisx;
	...
	else
	{
		if((pthis->auraType == KEESE_AURA_FIRE) || (pthis->auraType == KEESE_AURA_ICE))
		{
			if((limbIndex == 15) || (limbIndex == 21))
			{
				...
				func_8002843C(globalCtx, &effPos, &effVelocity_75, &effAccel_75, effPrimColor, effEnvColor, 250, effScaleStep, effLife);
			}
		}
	}
