...
void BgYdanHasi_UpdateFloatingBlock(BgYdanHasi* pthis, GlobalContext* globalCtx)
{
	...
	pthis->dyna.actor.world.pos.x = ((Math_SinS(pthis->dyna.actor.world.rot.y) * framesAfterMath) + pthis->dyna.actor.home.pos.x);
	pthis->dyna.actor.world.pos.z = ((Math_CosS(pthis->dyna.actor.world.rot.y) * framesAfterMath) + pthis->dyna.actor.home.pos.z);
	waterBox = &globalCtx->colCtx.colHeader->waterBoxes[1];
