...
static DemoImActionFunc sActionFuncs[] = {
    func_809856F8, func_80985718, func_80985738, func_80985770, func_809857B0, func_809857F0, func_80985830, func_80985C10, func_80985C40, func_80985C94, func_809863BC, func_809863DC, func_80986430, func_80986494, func_809864D4, func_809868E8,
    func_80986908, func_80986948, func_80986D40, func_80986DC8, func_80986E20, func_80986E40, func_80986EAC, func_80986F08, func_80986F28, func_80986F88, func_80986FA8, func_80987288, func_809872A8, func_809872F0, func_80987330,
};
...
void DemoIm_ChangeAnim(DemoIm* pthis, AnimationHeader* animHeaderSeg, u8 animMode, f32 transitionRate, s32 playBackwards)
{
...
void func_809853B4(DemoIm* pthis, GlobalContext* globalCtx)
{
	...
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_EFFECT, playerX, playerY, playerZ, 0, 0, 0, 0xD);
...
{
	if((globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[5] != NULL) && (globalCtx->csCtx.npcActions[5]->action == 2))
	{
		Animation_Change(&pthis->skelAnime, &gImpaIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaIdleAnim), ANIMMODE_LOOP, 0.0f);
		pthis->action = 2;
		pthis->drawConfig = 1;
...
{
	if((globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[5] != NULL) && (globalCtx->csCtx.npcActions[5]->action == 3))
	{
		Animation_Change(&pthis->skelAnime, &gImpaRaiseArmsAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaRaiseArmsAnim), ANIMMODE_ONCE, 4.0f);
		pthis->action = 4;
	}
}
...
void func_80985640(DemoIm* pthis, s32 arg1)
{
	if(arg1 != 0)
	{
		Animation_Change(&pthis->skelAnime, &gImpaPresentShadowMedallionAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaPresentShadowMedallionAnim), ANIMMODE_LOOP, 0.0f);
		pthis->action = 5;
	}
}
...
{
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, (kREG(17) + 24.0f) + pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 6);
}
...
void func_80985948(DemoIm* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gImpaPrepareSealGanonAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaPrepareSealGanonAnim), ANIMMODE_ONCE, 0.0f);
		pthis->action = 8;
		pthis->drawConfig = 2;
		pthis->alpha = 0;
		pthis->actor.shape.shadowAlpha = 0;
...
void func_80985B34(DemoIm* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gImpaSealGanonAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaSealGanonAnim), ANIMMODE_ONCE, -8.0f);
		pthis->action = 8;
		pthis->drawConfig = 2;
...
void func_80985FE8(DemoIm* pthis, s32 arg1)
{
	if(arg1 != 0)
	{
		Animation_Change(&pthis->skelAnime, &gImpaWhistlingAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaWhistlingAnim), ANIMMODE_LOOP, -8.0f);
	}
}
...
void func_809860DC(DemoIm* pthis, s32 arg1)
{
	if(arg1 != 0)
	{
		Animation_Change(&pthis->skelAnime, &gImpaIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaIdleAnim), ANIMMODE_LOOP, -8.0f);
		pthis->unk_2D0 = 0;
	}
}
...
{
	Animation_Change(&pthis->skelAnime, &gImpaStartWhistlingAnim, -1.0f, Animation_GetLastFrame(&gImpaStartWhistlingAnim), 0.0f, ANIMMODE_ONCE, -8.0f);
	pthis->action = 14;
	pthis->drawConfig = 1;
}
	...
	if(arg2 != 0)
	{
		...
		if(*unk_278 >= 0.0f)
		{
			if(pthis->unk_27C == 0)
			{
				...
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ARROW, spawnPosX, spawnPosY, spawnPosZ, 0xFA0, pthis->actor.shape.rot.y, 0, ARROW_CS_NUT);
				pthis->unk_27C = 1;
			}
		}
		else
...
{
	Animation_Change(&pthis->skelAnime, &gImpaThrowDekuNutAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gImpaThrowDekuNutAnim), ANIMMODE_ONCE, -8.0f);
	pthis->action = 17;
	pthis->drawConfig = 1;
}
