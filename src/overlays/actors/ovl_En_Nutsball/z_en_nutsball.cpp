...
void func_80ABBBA8(EnNutsball* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.bgCheckFlags & BG_STATE_3) || (pthis->actor.bgCheckFlags & BG_STATE_0) || (pthis->collider.base.atFlags & AT_HIT) || (pthis->collider.base.acFlags & AC_HIT) || (pthis->collider.base.ocFlags1 & OC1_HIT))
	{
		...
		{
			if((pthis->collider.base.atFlags & AT_HIT) && (pthis->collider.base.atFlags & AT_TYPE_ENEMY) && (pthis->collider.base.atFlags & AT_BOUNCED))
			{
...
void EnNutsball_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_nutsball.c", 327);

	func_80093D18(globalCtx->gfxCtx);
	Matrix_Mult(&globalCtx->billboardMtxF, MTXMODE_APPLY);
	Matrix_RotateZ(thisx->home.rot.z * 9.58738e-05f, MTXMODE_APPLY);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_nutsball.c", 333), G_MTX_MODELVIEW | G_MTX_LOAD);
	gSPDisplayList(POLY_OPA_DISP++, sDLists[thisx->params]);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_nutsball.c", 337);
}
