...
s32 func_8097CDB0(DemoGo* pthis, GlobalContext* globalCtx, u16 npcAction)
{
	...
	if((csCtx->state != CS_STATE_IDLE) && (csCtx->npcActions[actionIdx] != NULL) && (csCtx->npcActions[actionIdx]->action == npcAction))
	{
		return 1;
	}
	return 0;
...
void func_8097D29C(DemoGo* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_demo_go.c", 732);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_demo_go.c", 746);
}
