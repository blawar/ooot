...
void EnMk_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_mk_Skel_005DF0, &object_mk_Anim_000D88, pthis->jointTable, pthis->morphTable, 13);
...
void func_80AACC04(EnMk* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		Animation_Change(&pthis->skelAnime, &object_mk_Anim_000D88, 1.0f, 0.0f, Animation_GetLastFrame(&object_mk_Anim_000D88), ANIMMODE_LOOP, -4.0f);
		pthis->flags &= ~2;
	}
}
...
void func_80AACCA0(EnMk* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		Animation_Change(&pthis->skelAnime, &object_mk_Anim_000724, 1.0f, 0.0f, Animation_GetLastFrame(&object_mk_Anim_000724), ANIMMODE_LOOP, -4.0f);
		pthis->flags &= ~2;
	}
}
...
void func_80AACE2C(EnMk* pthis, GlobalContext* globalCtx)
{
	if((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(globalCtx))
	{
		...
		Animation_Change(&pthis->skelAnime, &object_mk_Anim_000AC0, 1.0f, 0.0f, Animation_GetLastFrame(&object_mk_Anim_000AC0), ANIMMODE_ONCE, -4.0f);
		pthis->flags &= ~2;
...
void func_80AACEE8(EnMk* pthis, GlobalContext* globalCtx)
{
	if((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(globalCtx))
	{
		...
		Animation_Change(&pthis->skelAnime, &object_mk_Anim_000AC0, 1.0f, 0.0f, Animation_GetLastFrame(&object_mk_Anim_000AC0), ANIMMODE_LOOP, -4.0f);
		pthis->flags &= ~2;
			...
			else
			{
				switch(playerExchangeItem)
				{
					...
					case EXCH_ITEM_FROG:
						...
						Animation_Change(&pthis->skelAnime, &object_mk_Anim_000368, 1.0f, 0.0f, Animation_GetLastFrame(&object_mk_Anim_000368), ANIMMODE_ONCE, -4.0f);
						pthis->flags &= ~2;
