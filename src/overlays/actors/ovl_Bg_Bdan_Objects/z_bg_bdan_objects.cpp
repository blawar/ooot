...
void BgBdanObjects_Init(Actor* pthisx, GlobalContext* globalCtx)
{
	...
	if(pthisx->params == 0)
	{
		...
		else
		{
			...
			{
				if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EN_BIGOKUTA, pthisx->home.pos.x, pthisx->home.pos.y, pthisx->home.pos.z, 0, pthisx->shape.rot.y + 0x8000, 0, 3) != NULL)
				{
...
void func_8086C1A0(BgBdanObjects* pthis, GlobalContext* globalCtx)
{
	if(Math_SmoothStepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y + 500.0f, 0.5f, 7.5f, 1.0f) < 0.1f)
	{
...
void func_8086C29C(BgBdanObjects* pthis, GlobalContext* globalCtx)
{
	...
	{
		Actor_SpawnAsChild(
		    &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EN_BIGOKUTA, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y + 140.0f, pthis->dyna.actor.world.pos.z, 0, pthis->dyna.actor.shape.rot.y + 0x8000, 0, 0);
...
void func_8086C3D8(BgBdanObjects* pthis, GlobalContext* globalCtx)
{
	...
	if(Math_StepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y + -70.0f, pthis->dyna.actor.velocity.y))
	{
...
void func_8086C7D0(BgBdanObjects* pthis, GlobalContext* globalCtx)
{
	if(Math_SmoothStepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y + 965.0f, 0.5f, 15.0f, 0.2f) < 0.01f)
	{
		Audio_PlayActorSound2(&pthis->dyna.actor, NA_SE_EV_BUYOSTAND_STOP_A);
		pthis->actionFunc = BgBdanObjects_DoNothing;
	}
...
void func_8086C874(BgBdanObjects* pthis, GlobalContext* globalCtx)
{
	...
	pthis->dyna.actor.world.pos.y = pthis->dyna.actor.home.pos.y - (sinf(pthis->timer * (M_PI / 256.0f)) * 471.24f); // pi * 150
	if(pthis->timer == 0)
	{
		pthis->timer = 512;
	}
