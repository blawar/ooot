...
ActorInit En_Dh_InitVars = {
    ACTOR_EN_DH, ACTORCAT_ENEMY, FLAGS, OBJECT_DH, ACTOR_FACTORY(EnDh), (ActorFunc)EnDh_Init, (ActorFunc)EnDh_Destroy, (ActorFunc)EnDh_Update, (ActorFunc)EnDh_Draw, (ActorFunc)EnDh_Reset,
};
...
void EnDh_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_dh_Skel_007E88, &object_dh_Anim_005880, pthis->jointTable, pthis->limbRotTable, 16);
...
void EnDh_SpawnDebris(GlobalContext* globalCtx, EnDh* pthis, Vec3f* spawnPos, f32 spread, s32 arg4, f32 accelXZ, f32 scale)
{
	Vec3f pos;
...
void EnDh_SetupWalk(EnDh* pthis)
{
	Animation_Change(&pthis->skelAnime, &object_dh_Anim_003A8C, 1.0f, 0.0f, Animation_GetLastFrame(&object_dh_Anim_003A8C) - 3.0f, ANIMMODE_LOOP, -6.0f);
...
void EnDh_Attack(EnDh* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &object_dh_Anim_004658, -1.0f, pthis->skelAnime.curFrame, 0.0f, ANIMMODE_ONCE, -4.0f);
		pthis->actionState = 4;
	...
	switch(pthis->actionState)
	{
		...
		case 2:
			...
			{
				pthis->collider2.base.atFlags = pthis->collider2.elements[0].info.toucherFlags = AT_ON | AT_TYPE_ENEMY; // also TOUCH_ON | TOUCH_SFX_WOOD
		...
		case 3:
			...
			{
				Animation_Change(&pthis->skelAnime, &object_dh_Anim_004658, 1.0f, 20.0f, Animation_GetLastFrame(&object_dh_Anim_004658), ANIMMODE_ONCE, -6.0f);
				pthis->actionState = 0;
			}
			else
			{
				Animation_Change(&pthis->skelAnime, &object_dh_Anim_004658, -1.0f, Animation_GetLastFrame(&object_dh_Anim_004658), 0.0f, ANIMMODE_ONCE, -4.0f);
				pthis->actionState++;
				pthis->collider2.base.atFlags = pthis->collider2.elements[0].info.toucherFlags = AT_NONE; // also TOUCH_NONE
				pthis->collider2.elements[0].info.toucher.dmgFlags = pthis->collider2.elements[0].info.toucher.damage = 0;
			}
...
void EnDh_Burrow(EnDh* pthis, GlobalContext* globalCtx)
{
	switch(pthis->actionState)
	{
		case 0:
			pthis->actionState++;
			pthis->drawDirtWave++;
			pthis->collider1.base.atFlags = pthis->collider1.info.toucherFlags = AT_ON | AT_TYPE_ENEMY; // also TOUCH_ON | TOUCH_SFX_WOOD
...
void EnDh_Death(EnDh* pthis, GlobalContext* globalCtx)
{
	...
	{
		if(((s32)pthis->skelAnime.curFrame == 53) || ((s32)pthis->skelAnime.curFrame == 56) || ((s32)pthis->skelAnime.curFrame == 61))
		{
...
void EnDh_CollisionCheck(EnDh* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->collider2.base.acFlags & AC_HIT) && !pthis->retreat)
	{
		...
		if((pthis->actor.colChkInfo.damageEffect != 0) && (pthis->actor.colChkInfo.damageEffect != 6))
		{
			...
			{
				if(((lastHealth >= 15) && (pthis->actor.colChkInfo.health < 15)) || ((lastHealth >= 9) && (pthis->actor.colChkInfo.health < 9)) || ((lastHealth >= 3) && (pthis->actor.colChkInfo.health < 3)))
				{
					pthis->retreat++;
				}
...
void EnDh_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.colChkInfo.health > 0)
	{
		...
		if(((pthis->curAction != DH_DAMAGE) && (pthis->actor.shape.yOffset == 0.0f)) || ((player->unk_844 != 0) && (player->unk_845 != pthis->unk_258)))
		{
