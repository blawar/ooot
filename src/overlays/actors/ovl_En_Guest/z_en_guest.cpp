...
void EnGuest_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->osAnimeBankIndex) != 0)
	{
		...
		SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_boj_Skel_0000F0, NULL, pthis->jointTable, pthis->morphTable, 16);
		gSegments[6] = VIRTUAL_TO_PHYSICAL(gObjectTable[pthis->osAnimeBankIndex].vromStart.get());
		Animation_Change(&pthis->skelAnime, &gObjOsAnim_42AC, 1.0f, 0.0f, Animation_GetLastFrame(&gObjOsAnim_42AC), ANIMMODE_LOOP, 0.0f);
...
s32 EnGuest_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnGuest* pthis = (EnGuest*)thisx;
	Vec3s sp3C;
