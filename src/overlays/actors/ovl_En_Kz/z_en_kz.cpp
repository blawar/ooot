...
s32 func_80A9C95C(GlobalContext* globalCtx, EnKz* pthis, s16* arg2, f32 unkf, callback1_800343CC callback1, callback2_800343CC callback2)
{
	Player* player = GET_PLAYER(globalCtx);
	s16 sp32;
	s16 sp30;
...
void func_80A9CB18(EnKz* pthis, GlobalContext* globalCtx)
{
	...
	if(func_80A9C95C(globalCtx, pthis, &pthis->unk_1E0.unk_00, 340.0f, EnKz_GetText, func_80A9C6C0))
	{
		...
		if(LINK_IS_ADULT)
		{
			if((INV_CONTENT(ITEM_TRADE_ADULT) == ITEM_PRESCRIPTION) && (func_8002F368(globalCtx) == EXCH_ITEM_PRESCRIPTION))
			{
...
void EnKz_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelanime, &gKzSkel, NULL, pthis->jointTable, pthis->morphTable, 12);
	...
	if(LINK_IS_ADULT)
	{
		...
		{
			Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BG_ICE_SHELTER, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0x04FF);
		}
