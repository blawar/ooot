...
void ObjIcePoly_Melt(ObjIcePoly* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 2; i++)
	{
		pos.x = pthis->actor.world.pos.x + pthis->actor.scale.x * (Rand_S16Offset(20, 20) * (Rand_ZeroOne() < 0.5f ? -1 : 1));
		pos.y = pthis->actor.world.pos.y + pthis->actor.scale.y * Rand_ZeroOne() * 50.0f;
		pos.z = pthis->actor.world.pos.z + pthis->actor.scale.x * (Rand_S16Offset(20, 20) * (Rand_ZeroOne() < 0.5f ? -1 : 1));
		func_8002829C(globalCtx, &pos, &vel, &accel, &sColorWhite, &sColorGray, Rand_S16Offset(0x15E, 0x64) * pthis->actor.scale.x, pthis->actor.scale.x * 20.0f);
	}
