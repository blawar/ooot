...
static InitChainEntry sInitChain[] = {
    ICHAIN_F32_DIV1000(gravity, -1200, ICHAIN_CONTINUE), ICHAIN_F32_DIV1000(minVelocityY, -20000, ICHAIN_CONTINUE), ICHAIN_VEC3F_DIV1000(scale, 150, ICHAIN_CONTINUE),
    ICHAIN_F32(uncullZoneForward, 900, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneScale, 100, ICHAIN_CONTINUE),	    ICHAIN_F32(uncullZoneDownward, 800, ICHAIN_STOP),
};
...
void ObjTsubo_SpawnCollectible(ObjTsubo* pthis, GlobalContext* globalCtx)
{
	...
	{
		Item_DropCollectible(globalCtx, &pthis->actor.world.pos, (dropParams | (((pthis->actor.params >> 9) & 0x3F) << 8)));
	}
}
...
void ObjTsubo_AirBreak(ObjTsubo* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, angle = 0; i < 15; i++, angle += 0x4E20)
	{
		...
		EffectSsKakera_Spawn(
		    globalCtx, &pos, &velocity, &pthis->actor.world.pos, -240, arg5, 10, 10, 0, (Rand_ZeroOne() * 95.0f) + 15.0f, 0, 32, 60, KAKERA_COLOR_NONE, sObjectIds[(pthis->actor.params >> 8) & 1], D_80BA1B8C[(pthis->actor.params >> 8) & 1]);
	}
...
void ObjTsubo_WaterBreak(ObjTsubo* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, angle = 0; i < 15; i++, angle += 0x4E20)
	{
		...
		EffectSsKakera_Spawn(
		    globalCtx, &pos, &velocity, &pthis->actor.world.pos, -180, phi_s0, 30, 30, 0, (Rand_ZeroOne() * 95.0f) + 15.0f, 0, 32, 70, KAKERA_COLOR_NONE, sObjectIds[(pthis->actor.params >> 8) & 1], D_80BA1B8C[(pthis->actor.params >> 8) & 1]);
	}
}
...
void ObjTsubo_Idle(ObjTsubo* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->collider.base.acFlags & AC_HIT) && (pthis->collider.info.acHitInfo->toucher.dmgFlags & 0x4FC1FFFC))
	{
