...
static s16 sScenes[] = {
    SCENE_YDAN, SCENE_DDAN, SCENE_BMORI1, SCENE_HIDAN, SCENE_MIZUSIN, SCENE_JYASINZOU, SCENE_HAKADAN, SCENE_GANON, SCENE_MEN,
};
...
void ObjOshihiki_InitDynapoly(ObjOshihiki* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 moveFlag)
{
	...
	if(pthis->dyna.bgId == BG_ACTOR_MAX)
	{
		...
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_obj_oshihiki.c", 280, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
ObjOshihiki* ObjOshihiki_GetBlockUnder(ObjOshihiki* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->floorBgIds[pthis->highestFloor] != BGCHECK_SCENE) && (fabsf(pthis->dyna.actor.floorHeight - pthis->dyna.actor.world.pos.y) < 0.001f))
	{
	...
	else if(dyna->actor.id == ACTOR_OBJ_SWITCH)
	{
		...
		switch(dyna->actor.params & 0x33)
		{
			case 0x20: // Normal blue switch
				if((dynaSwitchFlag == ((pthis->dyna.actor.params >> 8) & 0x3F)) && Flags_GetSwitch(globalCtx, dynaSwitchFlag))
				{
					return 0;
				}
				break;
...
void ObjOshihiki_SetFloors(ObjOshihiki* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 5; i++)
	{
		...
		pthis->floorHeights[i] = BgCheck_EntityRaycastFloor6(&globalCtx->colCtx, floorPoly, floorBgId, &pthis->dyna.actor, &colCheckPoint, 0.0f);
	}
}
...
s16 ObjOshihiki_GetHighestFloor(ObjOshihiki* pthis)
{
	...
	else if((pthis->floorBgIds[temp + 1] == BGCHECK_SCENE) && ((pthis->floorHeights[temp + 1] - pthis->floorHeights[highestFloor]) > -0.001f))
	{
		highestFloor = temp + 1;
	}
	...
	else if((pthis->floorBgIds[temp + 2] == BGCHECK_SCENE) && ((pthis->floorHeights[temp + 2] - pthis->floorHeights[highestFloor]) > -0.001f))
	{
		highestFloor = temp + 2;
	}
	...
	else if((pthis->floorBgIds[temp + 3] == BGCHECK_SCENE) && ((pthis->floorHeights[temp + 3] - pthis->floorHeights[highestFloor]) > -0.001f))
	{
		highestFloor = temp + 3;
	}
	return highestFloor;
...
s32 ObjOshihiki_CheckWall(GlobalContext* globalCtx, s16 angle, f32 direction, ObjOshihiki* pthis)
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		if(BgCheck_EntityLineTest3(&globalCtx->colCtx, &faceVtx, &faceVtxNext, &posResult, &outPoly, true, false, false, true, &bgId, &pthis->dyna.actor, 0.0f))
		{
			return true;
		}
	}
...
s32 ObjOshihiki_MoveWithBlockUnder(ObjOshihiki* pthis, GlobalContext* globalCtx)
{
	...
	if((blockUnder != NULL) && (blockUnder->stateFlags & PUSHBLOCK_SETUP_PUSH) && !ObjOshihiki_CheckWall(globalCtx, blockUnder->dyna.unk_158, blockUnder->direction, pthis))
	{
...
void ObjOshihiki_OnScene(ObjOshihiki* pthis, GlobalContext* globalCtx)
{
	...
	{
		if(ObjOshihiki_StrongEnough(pthis) && !ObjOshihiki_CheckWall(globalCtx, pthis->dyna.unk_158, pthis->dyna.unk_150, pthis))
		{
	...
	if(ObjOshihiki_CheckFloor(pthis, globalCtx))
	{
		...
		else
		{
			...
			if(dynaPolyActor != NULL)
			{
				...
				if((pthis->timer <= 0) && (fabsf(pthis->dyna.unk_150) > 0.001f))
				{
					if(ObjOshihiki_StrongEnough(pthis) && ObjOshihiki_NoSwitchPress(pthis, dynaPolyActor, globalCtx) && !ObjOshihiki_CheckWall(globalCtx, pthis->dyna.unk_158, pthis->dyna.unk_150, pthis))
					{
						pthis->direction = pthis->dyna.unk_150;
						ObjOshihiki_SetupPush(pthis, globalCtx);
					}
...
void ObjOshihiki_Push(ObjOshihiki* pthis, GlobalContext* globalCtx)
{
	...
	pthis->pushSpeed = pthis->pushSpeed.clampMax(2.0f);
...
void ObjOshihiki_Fall(ObjOshihiki* pthis, GlobalContext* globalCtx)
{
	...
	if(ObjOshihiki_CheckGround(pthis, globalCtx))
	{
		...
		Audio_PlayActorSound2(&pthis->dyna.actor, SurfaceType_GetSfx(&globalCtx->colCtx, pthis->floorPolys[pthis->highestFloor], pthis->floorBgIds[pthis->highestFloor]) + SFX_FLAG);
	}
}
...
void ObjOshihiki_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	pthis->stateFlags &= ~(PUSHBLOCK_SETUP_FALL | PUSHBLOCK_FALL | PUSHBLOCK_SETUP_PUSH | PUSHBLOCK_PUSH | PUSHBLOCK_SETUP_ON_ACTOR | PUSHBLOCK_ON_ACTOR | PUSHBLOCK_SETUP_ON_SCENE | PUSHBLOCK_ON_SCENE);
