...
ActorInit Obj_Kibako_InitVars = {
    ACTOR_OBJ_KIBAKO, ACTORCAT_PROP, FLAGS, OBJECT_GAMEPLAY_DANGEON_KEEP, ACTOR_FACTORY(ObjKibako), (ActorFunc)ObjKibako_Init, (ActorFunc)ObjKibako_Destroy, (ActorFunc)ObjKibako_Update, (ActorFunc)ObjKibako_Draw, (ActorFunc)ObjKibako_Reset,
};
...
void ObjKibako_SpawnCollectible(ObjKibako* pthis, GlobalContext* globalCtx)
{
	...
	{
		Item_DropCollectible(globalCtx, &pthis->actor.world.pos, collectible | (((pthis->actor.params >> 8) & 0x3F) << 8));
	}
}
...
void ObjKibako_AirBreak(ObjKibako* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, angle = 0; i < 12; i++, angle += 0x4E20)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, breakPos, -200, phi_s0, 10, 10, 0, (Rand_ZeroOne() * 30.0f) + 10.0f, 0, 32, 60, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gSmallWoodenBoxFragmentDL);
	}
...
void ObjKibako_WaterBreak(ObjKibako* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, angle = 0; i < 12; i++, angle += 0x4E20)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, breakPos, -180, phi_s0, 30, 30, 0, (Rand_ZeroOne() * 30.0f) + 10.0f, 0, 32, 70, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gSmallWoodenBoxFragmentDL);
	}
}
