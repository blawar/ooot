...
void BgBdanSwitch_InitDynaPoly(BgBdanSwitch* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 flag)
{
	...
	{
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_bg_bdan_switch.c", 325, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void BgBdanSwitch_Init(Actor* pthisx, GlobalContext* globalCtx)
{
	...
	switch(type)
	{
		...
		case YELLOW_TALL_2:
			...
			break;
		default:
			osSyncPrintf("不正な ARG_DATA(arg_data 0x%04x)(%s %d)\n", pthis->dyna.actor.params, "../z_bg_bdan_switch.c", 454);
...
void func_8086DDC0(BgBdanSwitch* pthis, GlobalContext* globalCtx)
{
	if((((pthis->dyna.actor.params & 0xFF) != YELLOW_TALL_2) || (func_8005B198() == pthis->dyna.actor.category)) || (pthis->unk_1DA <= 0))
	{
		pthis->unk_1C8 += 0.3f;
...
{
	func_800D1694(pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y + (pthis->dyna.actor.shape.yOffset * pthis->unk_1D0), pthis->dyna.actor.world.pos.z, &pthis->dyna.actor.shape.rot);
