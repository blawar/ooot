...
void EnGoma_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{ // Egg
		...
		SkelAnime_Init(globalCtx, &pthis->skelanime, &gObjectGolSkel, &gObjectGolStandAnim, pthis->jointTable, pthis->morphTable, GOMA_LIMB_MAX);
...
void EnGoma_SetupFlee(EnGoma* pthis)
{
	Animation_Change(&pthis->skelanime, &gObjectGolRunningAnim, 2.0f, 0.0f, Animation_GetLastFrame(&gObjectGolRunningAnim), ANIMMODE_LOOP, -2.0f);
...
void EnGoma_Flee(EnGoma* pthis, GlobalContext* globalCtx)
{
	...
	Math_ApproachS(&pthis->actor.world.rot.y, Actor_WorldYawTowardActor(&pthis->actor, &GET_PLAYER(globalCtx)->actor) + 0x8000, 3, 2000);
...
void EnGoma_Egg(EnGoma* pthis, GlobalContext* globalCtx)
{
	...
	if(fabsf(pthis->actor.world.pos.x - player->actor.world.pos.x) < 100.0f && fabsf(pthis->actor.world.pos.z - player->actor.world.pos.z) < 100.0f)
	{
...
void EnGoma_SetupHatch(EnGoma* pthis, GlobalContext* globalCtx)
{
	Animation_Change(&pthis->skelanime, &gObjectGolJumpHeadbuttAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolJumpHeadbuttAnim), ANIMMODE_ONCE, 0.0f);
...
void EnGoma_SetupHurt(EnGoma* pthis, GlobalContext* globalCtx)
{
	Animation_Change(&pthis->skelanime, &gObjectGolDamagedAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolDamagedAnim), ANIMMODE_ONCE, -2.0f);
...
void EnGoma_SetupDie(EnGoma* pthis)
{
	Animation_Change(&pthis->skelanime, &gObjectGolDeathAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolDeathAnim), ANIMMODE_ONCE, -2.0f);
...
void EnGoma_SetupDead(EnGoma* pthis)
{
	Animation_Change(&pthis->skelanime, &gObjectGolDeadTwitchingAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolDeadTwitchingAnim), ANIMMODE_LOOP, -2.0f);
...
void EnGoma_SetupChasePlayer(EnGoma* pthis)
{
	Animation_Change(&pthis->skelanime, &gObjectGolRunningAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolRunningAnim), ANIMMODE_LOOP, -5.0f);
...
void EnGoma_SetupPrepareJump(EnGoma* pthis)
{
	Animation_Change(&pthis->skelanime, &gObjectGolPrepareJumpAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolPrepareJumpAnim), ANIMMODE_ONCE, -5.0f);
...
void EnGoma_SetupLand(EnGoma* pthis)
{
	Animation_Change(&pthis->skelanime, &gObjectGolLandFromJumpAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gObjectGolLandFromJumpAnim), ANIMMODE_ONCE, 0.0f);
...
void EnGoma_Stand(EnGoma* pthis, GlobalContext* globalCtx)
{
	SkelAnime_Update(&pthis->skelanime);
	Math_ApproachZeroF(&pthis->actor.speedXZ, 0.5f, 2.0f);
	Math_ApproachS(&pthis->actor.shape.rot.y, Actor_WorldYawTowardActor(&pthis->actor, &GET_PLAYER(globalCtx)->actor), 2, 3000);
...
void EnGoma_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	switch(pthis->gomaType)
	{
		case ENGOMA_NORMAL:
			...
			Matrix_Translate(pthis->actor.world.pos.x, pthis->actor.world.pos.y + ((pthis->actor.shape.yOffset * pthis->actor.scale.y) + globalCtx->mainCamera.skyboxOffset.y), pthis->actor.world.pos.z, MTXMODE_NEW);
			...
			SkelAnime_DrawOpa(globalCtx, pthis->skelanime.skeleton, pthis->skelanime.jointTable, EnGoma_OverrideLimbDraw, NULL, pthis);
			break;
		...
		case ENGOMA_EGG:
			...
			Matrix_Scale(0.95f - pthis->eggSquishAmount, pthis->eggSquishAmount + 1.05f, 0.95f - pthis->eggSquishAmount, MTXMODE_APPLY);
			Matrix_RotateZ(-(pthis->eggSquishAngle * 0.1f), MTXMODE_APPLY);
			Matrix_RotateY(-(pthis->eggSquishAngle * 0.15f), MTXMODE_APPLY);
...
void EnGoma_SpawnHatchDebris(EnGoma* pthis, GlobalContext* globalCtx2)
{
	...
	{
		Actor_SpawnAsChild(
		    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_GOMA, Rand_CenteredFloat(10.0f) + pthis->actor.world.pos.x, Rand_CenteredFloat(10.0f) + pthis->actor.world.pos.y + 15.0f, Rand_CenteredFloat(10.0f) + pthis->actor.world.pos.z, 0,
		    Rand_CenteredFloat(0x10000 - 0.01f), 0, i + 10);
	}
}
