...
ActorInit En_Floormas_InitVars = {
    ACTOR_EN_FLOORMAS, ACTORCAT_ENEMY, FLAGS, OBJECT_WALLMASTER, ACTOR_FACTORY(EnFloormas), (ActorFunc)EnFloormas_Init, (ActorFunc)EnFloormas_Destroy, (ActorFunc)EnFloormas_Update, (ActorFunc)EnFloormas_Draw, (ActorFunc)EnFloormas_Reset,
};
...
void EnFloormas_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gWallmasterSkel, &gWallmasterWaitAnim, pthis->jointTable, pthis->morphTable, 25);
	...
	else
	{
		...
		pthis->actor.parent = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_FLOORMAS, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, invisble + SPAWN_SMALL);
		...
		pthis->actor.child = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_FLOORMAS, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, invisble + SPAWN_SMALL);
...
void EnFloormas_SetupTurn(EnFloormas* pthis)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gFloormasterTurnAnim, -1.0f, Animation_GetLastFrame(&gFloormasterTurnAnim), 0.0f, ANIMMODE_ONCE, -3.0f);
	}
...
{
	Animation_Change(&pthis->skelAnime, &gWallmasterHoverAnim, 3.0f, 0, Animation_GetLastFrame(&gWallmasterHoverAnim), ANIMMODE_ONCE, -3.0f);
	pthis->actor.speedXZ = 0.0f;
...
void EnFloormas_SetupSplit(EnFloormas* pthis)
{
	...
	Animation_Change(&pthis->skelAnime, &gWallmasterJumpAnim, 1.0f, 41.0f, Animation_GetLastFrame(&gWallmasterJumpAnim), ANIMMODE_ONCE, 0.0f);
...
void EnFloormas_BigWalk(EnFloormas* pthis, GlobalContext* globalCtx)
{
	...
	if(((animPastFrame || (Animation_OnFrame(&pthis->skelAnime, 12.0f))) || (Animation_OnFrame(&pthis->skelAnime, 24.0f) != 0)) || (Animation_OnFrame(&pthis->skelAnime, 36.0f) != 0))
	{
...
{
	SkelAnime_Update(&pthis->skelAnime);
	if(Animation_OnFrame(&pthis->skelAnime, 0.0f) || Animation_OnFrame(&pthis->skelAnime, 12.0f) || Animation_OnFrame(&pthis->skelAnime, 24.0f) || Animation_OnFrame(&pthis->skelAnime, 36.0f))
	{
		Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_FALL_WALK);
	}
...
void EnFloormas_Turn(EnFloormas* pthis, GlobalContext* globalCtx)
{
	...
	if(((pthis->skelAnime.playSpeed > 0.0f) && Animation_OnFrame(&pthis->skelAnime, 21.0f)) || ((pthis->skelAnime.playSpeed < 0.0f) && Animation_OnFrame(&pthis->skelAnime, 6.0f)))
	{
		if(pthis->actor.scale.x > 0.004f)
		{
			Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_FALL_WALK);
	...
	if(pthis->skelAnime.curFrame >= 7.0f && pthis->skelAnime.curFrame < 22.0f)
	{
		...
		pthis->actor.world.pos.x -= (pthis->actor.scale.x * 2700.0f) * (Math_SinS(pthis->actor.shape.rot.y + 0x4268) - sp30);
		pthis->actor.world.pos.z -= (pthis->actor.scale.x * 2700.0f) * (Math_CosS(pthis->actor.shape.rot.y + 0x4268) - sp2C);
	}
}
...
void EnFloormas_JumpAtLink(EnFloormas* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->actor.yDistToPlayer < -10.0f) && (pthis->collider.base.ocFlags1 & OC1_HIT) && (&player->actor == pthis->collider.base.oc))
	{
...
void EnFloormas_GrabLink(EnFloormas* pthis, GlobalContext* globalCtx)
{
	...
	if(!(player->stateFlags2 & PLAYER_STATE2_7) || (player->invincibilityTimer < 0))
	{
		...
		if(((parent->actionFunc == EnFloormas_GrabLink) || parent->actionFunc == EnFloormas_SmWait) && (child->actionFunc == EnFloormas_GrabLink || child->actionFunc == EnFloormas_SmWait))
		{
...
void EnFloormas_SmSlaveJumpAtMaster(EnFloormas* pthis, GlobalContext* globalCtx)
{
	...
	else if((((primFloormas->world.pos.y - pthis->actor.world.pos.y) < -10.0f) && (fabsf(pthis->actor.world.pos.x - primFloormas->world.pos.x) < 10.0f)) && (fabsf(pthis->actor.world.pos.z - primFloormas->world.pos.z) < 10.0f))
	{
		EnFloormas_SetupSmWait(pthis);
	...
	if(fabsf(pthis->actor.world.pos.x - primFloormas->world.pos.x) < 5.0f && fabsf(pthis->actor.world.pos.z - primFloormas->world.pos.z) < 5.0f)
	{
		...
		if((pthis->actor.colChkInfo.damageEffect != 0) || (pthis->actor.colChkInfo.damage != 0))
		{
			if(pthis->collider.base.colType != COLTYPE_HARD)
			{
				...
				else
				{
					...
					{
						EffectSsFCircle_Spawn(globalCtx, &pthis->actor, &pthis->actor.world.pos, pthis->actor.scale.x * 4000.0f, pthis->actor.scale.x * 4000.0f);
					}
...
void EnFloormas_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnFloormas_SmWait)
	{
		...
		if(pthis->actionFunc != EnFloormas_GrabLink)
		{
			if(pthis->actionFunc != EnFloormas_Split && pthis->actionFunc != EnFloormas_TakeDamage && pthis->actor.freezeTimer == 0)
			{
...
s32 EnFloormas_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnFloormas* pthis = (EnFloormas*)thisx;
...
void EnFloormas_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	POLY_OPA_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnFloormas_OverrideLimbDraw, EnFloormas_PostLimbDraw, pthis, POLY_OPA_DISP);
	if(pthis->collider.base.colType == COLTYPE_HARD)
	{
...
void EnFloormas_DrawHighlighted(Actor* thisx, GlobalContext* globalCtx)
{
	...
	POLY_XLU_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnFloormas_OverrideLimbDraw, EnFloormas_PostLimbDraw, pthis, POLY_XLU_DISP);
	if(pthis->collider.base.colType == COLTYPE_HARD)
	{
