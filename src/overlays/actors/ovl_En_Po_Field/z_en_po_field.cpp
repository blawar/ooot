...
void EnPoField_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gPoeFieldSkel, &gPoeFieldFloatAnim, pthis->jointTable, pthis->morphTable, 10);
...
void EnPoField_WaitForSpawn(EnPoField* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actionTimer == 0)
	{
		...
		{
			if(fabsf(sSpawnPositions[i].x - player->actor.world.pos.x) < 150.0f && fabsf(sSpawnPositions[i].z - player->actor.world.pos.z) < 150.0f)
			{
				...
				else if(player->stateFlags1 & PLAYER_STATE_HORSE_MOUNTED || Rand_ZeroOne() < 0.4f)
				{
				...
				pthis->actor.world.pos.y = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->actor.floorPoly, &bgId, &pthis->actor, &pthis->actor.world.pos);
...
void EnPoField_Flee(EnPoField* pthis, GlobalContext* globalCtx)
{
	...
	{
		phi_t0 = (s16)(pthis->actor.yawTowardsPlayer - Actor_WorldYawTowardPoint(&pthis->actor, &sFieldMiddle) - 0x8000) * 0.2f;
	}
	else
	{
		phi_t0 = 0;
...
void EnPoField_Death(EnPoField* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actionTimer < 8)
	{
		if(pthis->actionTimer < 5)
		{
			...
			sp6C.x = Math_SinS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) + 0x4800) * sp68 + pthis->actor.world.pos.x;
			sp6C.z = Math_CosS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) + 0x4800) * sp68 + pthis->actor.world.pos.z;
		}
		else
		{
			sp6C.y = pthis->actor.world.pos.y + 40.0f + 15.0f * (pthis->actionTimer - 5);
			sp6C.x = Math_SinS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) + 0x4800) * 23.0f + pthis->actor.world.pos.x;
			sp6C.z = Math_CosS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) + 0x4800) * 23.0f + pthis->actor.world.pos.z;
		}
		EffectSsDeadDb_Spawn(globalCtx, &sp6C, &D_80AD7114, &D_80AD7120, pthis->actionTimer * 10 + 80, 0, 255, 255, 255, 255, 0, 0, 255, 1, 9, 1);
...
void EnPoField_SoulUpdateProperties(EnPoField* pthis, s32 arg1)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, info->lightColor.r, info->lightColor.g, info->lightColor.b, pthis->lightColor.a * (200.0f / 255));
}
...
void func_80AD58D4(EnPoField* pthis, GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(
	    &pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->lightInfo.params.point.color[0], pthis->lightInfo.params.point.color[1], pthis->lightInfo.params.point.color[2],
...
void EnPoField_DrawFlame(EnPoField* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->flameTimer != 0)
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_en_po_field.c", 1669);
		func_80093D84(globalCtx->gfxCtx);
		gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 0, 0, 32, 64, 1, 0, (globalCtx->gameplayFrames.whole() * -20) % 512, 32, 128));
		...
		else
		{
			...
			Matrix_Scale((pthis->flameScale * 0.7f) + 0.00090000004f, (0.003f - pthis->flameScale) + 0.003f, 0.003f, MTXMODE_APPLY);
		}
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_po_field.c", 1709), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, gEffFire1DL);
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_po_field.c", 1712);
	}
}
...
void EnPoField_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc == EnPoField_Flee || pthis->actionFunc == EnPoField_Damage || pthis->actionFunc == EnPoField_Appear)
	{
		Actor_MoveForward(&pthis->actor);
	}
...
s32 EnPoField_OverrideLimbDraw2(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfxP)
{
	EnPoField* pthis = (EnPoField*)thisx;
...
void EnPoField_PostLimDraw2(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP)
{
	...
	{
		gSPMatrix((*gfxP)++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_po_field.c", 1916), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	if(limbIndex == 7)
	{
		...
		Lights_PointGlowSetInfo(&pthis->lightInfo, vec.x, vec.y, vec.z, pthis->soulColor.r, pthis->soulColor.g, pthis->soulColor.b, pthis->soulColor.a * (200.0f / 255));
	}
}
...
void EnPoField_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnPoField_WaitForSpawn)
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_en_po_field.c", 1976);
		func_80093D18(globalCtx->gfxCtx);
		func_80093D84(globalCtx->gfxCtx);
		gSPSegment(POLY_OPA_DISP++, 0x0A, Gfx_EnvColor(globalCtx->gfxCtx, info->envColor.r, info->envColor.g, info->envColor.b, 255));
		if(pthis->lightColor.a == 255 || pthis->lightColor.a == 0)
		{
			...
			POLY_OPA_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnPoField_OverrideLimbDraw2, EnPoField_PostLimDraw2, &pthis->actor, POLY_OPA_DISP);
		}
		else
		{
			...
			POLY_XLU_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnPoField_OverrideLimbDraw2, EnPoField_PostLimDraw2, &pthis->actor, POLY_XLU_DISP);
		}
		gDPPipeSync(POLY_OPA_DISP++);
		gDPSetEnvColor(POLY_OPA_DISP++, pthis->soulColor.r, pthis->soulColor.g, pthis->soulColor.b, 255);
		Matrix_Put(&sLimb7Mtx);
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_po_field.c", 2033), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_OPA_DISP++, gPoeFieldLanternDL);
		gSPDisplayList(POLY_OPA_DISP++, gPoeFieldLanternTopDL);
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_po_field.c", 2039);
	}
...
void EnPoField_DrawSoul(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_po_field.c", 2077);
	if(pthis->actionFunc == EnPoField_SoulIdle)
	{
		func_80093D18(globalCtx->gfxCtx);
		gSPSegment(POLY_OPA_DISP++, 0x0A, Gfx_EnvColor(globalCtx->gfxCtx, info->envColor.r, info->envColor.g, info->envColor.b, 255));
		Lights_PointGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->soulColor.r, pthis->soulColor.g, pthis->soulColor.b, 200);
		gDPSetEnvColor(POLY_OPA_DISP++, pthis->soulColor.r, pthis->soulColor.g, pthis->soulColor.b, 255);
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_po_field.c", 2104), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_OPA_DISP++, gPoeFieldLanternDL);
		gSPDisplayList(POLY_OPA_DISP++, gPoeFieldLanternTopDL);
	}
	else
	{
		func_80093D84(globalCtx->gfxCtx);
		gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 0, 0, 0x20, 0x40, 1, 0, (globalCtx->gameplayFrames.whole() * info->unk_9) & 0x1FF, 0x20, 0x80));
		gSPSegment(POLY_XLU_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(info->soulTexture));
		gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, info->primColor.r, info->primColor.g, info->primColor.b, pthis->lightColor.a);
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_po_field.c", 2143), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, gPoeFieldSoulDL);
	}
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_po_field.c", 2149);
	EnPoField_DrawFlame(pthis, globalCtx);
}
