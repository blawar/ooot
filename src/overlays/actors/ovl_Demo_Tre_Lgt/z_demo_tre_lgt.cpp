...
void func_809937B4(DemoTreLgt* pthis, GlobalContext* globalCtx, f32 currentFrame)
{
	...
	SkelCurve_SetAnim(skelCurve, sTransformUpdIdx[gSaveContext.linkAge], 1.0f, sDemoTreLgtInfo[gSaveContext.linkAge].endFrame + sDemoTreLgtInfo[gSaveContext.linkAge].unk_08, currentFrame, 1.0f);
...
void func_80993848(DemoTreLgt* pthis, GlobalContext* globalCtx)
{
	...
	{
		if(currentFrame <= (sDemoTreLgtInfo[((void)0, gSaveContext.linkAge)].endFrame + sDemoTreLgtInfo[((void)0, gSaveContext.linkAge)].unk_08))
		{
			pthis->unk_170 = ((((sDemoTreLgtInfo[((void)0, gSaveContext.linkAge)].endFrame - currentFrame) / sDemoTreLgtInfo[((void)0, gSaveContext.linkAge)].unk_08) * 255.0f) + 255.0f);
		}
		else
		{
			pthis->unk_170 = 0;
	...
	{
		pthis->unk_174 = ((((sDemoTreLgtInfo[((void)0, gSaveContext.linkAge)].unk_0C - currentFrame) / 10.0f) * 255.0f) + 255.0f);
	}
	else
	{
		pthis->unk_174 = 0;
	...
	{
		pthis->status |= 1;
		Audio_PlaySoundGeneral(NA_SE_EV_TRE_BOX_FLASH, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
