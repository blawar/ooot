...
void EnTrap_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->collider.base.ocFlags1 & OC1_HIT)
	{
		...
		angleToCollidedActor = thisx->world.rot.y + Math_Vec3f_Yaw(&pthis->collider.base.oc->world.pos, &thisx->world.pos);
		touchingActor = true;
	}
	...
	if(thisx->colorFilterTimer == 0)
	{
		...
		{
			if(!(thisx->params & (SPIKETRAP_MODE_LINEAR | SPIKETRAP_MODE_CIRCULAR)))
			{ // if in 4-way mode:
		...
		if(thisx->params & SPIKETRAP_MODE_LINEAR)
		{
			...
			if(pthis->vContinue != 0.0f)
			{
				...
				if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &thisx->world.pos, &posAhead, &colPoint, &colPoly, true, true, false, true, &bgId) == true)
				{
					pthis->vContinue = 0.0f;
				}
			}
			...
			{
				angleToCollidedActor = Math_Vec3f_Yaw(&thisx->world.pos, &pthis->collider.base.oc->world.pos) - thisx->world.rot.y;
				...
				{ // movement direction relative to spike trap
					case DIR_FWD:
						...
						if(pthis->vMovementMetric != 0.0f)
						{
							...
							pthis->vMovementMetric = Math_SmoothStepToF(&thisx->world.pos.z, pthis->targetPosFwd.z, 1.0f, pthis->moveSpeedForwardBack.z, 0.0f);
							pthis->vMovementMetric += Math_SmoothStepToF(&thisx->world.pos.x, pthis->targetPosFwd.x, 1.0f, pthis->moveSpeedForwardBack.x, 0.0f);
						}
						break;
					case DIR_LEFT:
						...
						if(touchingActor && (pthis->vMovementMetric != 0.0f) && (angleToCollidedActor <= -0x2000) && (angleToCollidedActor > -0x6000))
						{
							pthis->vMovementMetric = 0.0f;
							break;
						}
						if(pthis->vMovementMetric != 0.0f)
						{
							...
							pthis->vMovementMetric = Math_SmoothStepToF(&thisx->world.pos.x, pthis->targetPosLeft.x, 1.0f, pthis->moveSpeedLeftRight.x, 0.0f);
							pthis->vMovementMetric += Math_SmoothStepToF(&thisx->world.pos.z, pthis->targetPosLeft.z, 1.0f, pthis->moveSpeedLeftRight.z, 0.0f);
						}
						break;
					case DIR_BACK:
						...
						if(pthis->vMovementMetric != 0.0f)
						{
							...
							pthis->vMovementMetric = Math_SmoothStepToF(&thisx->world.pos.z, pthis->targetPosBack.z, 1.0f, pthis->moveSpeedForwardBack.z, 0.0f);
							pthis->vMovementMetric += Math_SmoothStepToF(&thisx->world.pos.x, pthis->targetPosBack.x, 1.0f, pthis->moveSpeedForwardBack.x, 0.0f);
						}
						break;
					case DIR_RIGHT:
						...
						if(pthis->vMovementMetric != 0.0f)
						{
							...
							pthis->vMovementMetric = Math_SmoothStepToF(&thisx->world.pos.x, pthis->targetPosRight.x, 1.0f, pthis->moveSpeedLeftRight.x, 0.0f);
							pthis->vMovementMetric += Math_SmoothStepToF(&thisx->world.pos.z, pthis->targetPosRight.z, 1.0f, pthis->moveSpeedLeftRight.z, 0.0f);
						}
						break;
				}
