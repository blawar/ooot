...
static AnimationHeader* sAnimations[] = {
    &gHorseGanonIdleAnim, &gHorseGanonWhinnyAnim, &gHorseGanonWalkingAnim, &gHorseGanonTrottingAnim, &gHorseGanonGallopingAnim, &gHorseGanonRearingAnim,
};
...
static unk_D_80A69248 D_80A69248[] = {
    {0x09B8, 0x0126, 0x0E2C, 0x07}, {0x0C11, 0x017A, 0x1269, 0x07}, {0x064E, 0xFEFB, 0x1DAC, 0x07}, {0x02F2, 0xFF45, 0x244F, 0x07}, {0xF96E, 0xFE0C, 0x3122, 0x07}, {0xF328, 0xFE0C, 0x32D5, 0x07}, {0xEBEA, 0xFE5F, 0x2D6E, 0x07},
    {0xE95E, 0xFE27, 0x2565, 0x07}, {0xE593, 0xFE0C, 0x20AC, 0x07}, {0xE625, 0xFE77, 0x1B07, 0x07}, {0xEBB7, 0x007C, 0x1539, 0x07}, {0xF466, 0x0002, 0x11B9, 0x07}, {0xF47B, 0xFFDD, 0x11AF, 0x07}, {0xF88D, 0xFFD1, 0x0BA2, 0x07},
};
...
{
	if((pthis->skin.skelAnime.curFrame > D_80A692B8[pthis->soundCount]) && (pthis->soundCount != 0 || !(pthis->skin.skelAnime.curFrame > D_80A692B8[1])))
	{
...
void func_80A68B20(EnHorseGanon* pthis)
{
	...
	{
		Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->currentAnimation], splaySpeeds[pthis->currentAnimation] * sp30 * 1.5f, 0.0f, Animation_GetLastFrame(sAnimations[pthis->currentAnimation]), ANIMMODE_ONCE, -3.0f);
	}
	else
	{
		Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->currentAnimation], splaySpeeds[pthis->currentAnimation] * sp30 * 1.5f, 0.0f, Animation_GetLastFrame(sAnimations[pthis->currentAnimation]), ANIMMODE_ONCE, 0.0f);
	}
}
...
void func_80A68FA8(Actor* thisx, GlobalContext* globalCtx, PSkinAwb* skin)
{
	...
	for(index = 0; index < pthis->colliderHead.count; index++)
	{
		...
		pthis->colliderHead.elements[index].dim.worldSphere.radius = pthis->colliderHead.elements[index].dim.modelSphere.radius * pthis->colliderHead.elements[index].dim.scale;
	}
