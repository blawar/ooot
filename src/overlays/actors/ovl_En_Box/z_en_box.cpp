...
void EnBox_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	else if((pthis->type == ENBOX_TYPE_SWITCH_FLAG_FALL_BIG || pthis->type == ENBOX_TYPE_SWITCH_FLAG_FALL_SMALL) && !Flags_GetSwitch(globalCtx, pthis->switchFlag))
	{
	...
	else if((pthis->type == ENBOX_TYPE_ROOM_CLEAR_BIG || pthis->type == ENBOX_TYPE_ROOM_CLEAR_SMALL) && !Flags_GetClear(globalCtx, pthis->dyna.actor.room))
	{
...
void EnBox_Fall(EnBox* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.bgCheckFlags & BG_STATE_0)
	{
		...
		Audio_PlaySoundGeneral(NA_SE_EV_COFFIN_CAP_BOUND, &pthis->dyna.actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
void func_809C9700(EnBox* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		{
			if((globalCtx->msgCtx.lastPlayedSong == OCARINA_SONG_LULLABY && pthis->type == ENBOX_TYPE_9) || (globalCtx->msgCtx.lastPlayedSong == OCARINA_SONG_SUNS && pthis->type == ENBOX_TYPE_10))
			{
				pthis->dyna.actor.flags &= ~ACTOR_FLAG_25;
...
void EnBox_AppearInit(EnBox* pthis, GlobalContext* globalCtx)
{
	if(func_8005B198() == pthis->dyna.actor.category || pthis->unk_1A8 != 0)
	{
		...
		pthis->unk_1A8 = 0;
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_KANKYO, pthis->dyna.actor.home.pos.x, pthis->dyna.actor.home.pos.y, pthis->dyna.actor.home.pos.z, 0, 0, 0, 0x0011);
		Audio_PlaySoundGeneral(NA_SE_EV_TRE_BOX_APPEAR, &pthis->dyna.actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
...
void EnBox_WaitOpen(EnBox* pthis, GlobalContext* globalCtx)
{
	...
	pthis->movementFlags |= ENBOX_MOVE_IMMOBILE;
	if(pthis->unk_1F4 != 0)
	{ // itemPageAngle is modified by player code
		...
		if(pthis->unk_1F4 > 0)
		{
			switch(pthis->type)
			{
				...
				default:
					Actor_SpawnAsChild(
					    &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_DEMO_TRE_LGT, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, pthis->dyna.actor.shape.rot.x,
	...
	else
	{
		...
		if(sp4C.z > -50.0f && sp4C.z < 0.0f && fabsf(sp4C.y) < 10.0f && fabsf(sp4C.x) < 20.0f && Player_IsFacingActor(&pthis->dyna.actor, 0x3000, globalCtx))
		{
...
void EnBox_SpawnIceSmoke(EnBox* pthis, GlobalContext* globalCtx)
{
	...
	if(Rand_ZeroOne() < 0.3f)
	{
		...
		pos = pthis->dyna.actor.world.pos;
		if(pthis->type == ENBOX_TYPE_SMALL || pthis->type == ENBOX_TYPE_6 || pthis->type == ENBOX_TYPE_ROOM_CLEAR_SMALL || pthis->type == ENBOX_TYPE_SWITCH_FLAG_FALL_SMALL)
		{
...
void EnBox_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->dyna.actor.params >> 5 & 0x7F) == 0x7C && pthis->actionFunc == EnBox_Open && pthis->skelanime.curFrame > 45 && pthis->iceSmokeTimer < 100)
	{
...
Gfx* func_809CA4A0(GraphicsContext* gfxCtx)
{
	...
	dList = dListHead;
	gDPSetRenderMode(
	    dListHead++, AA_EN | Z_CMP | Z_UPD | IM_RD | CLR_ON_CVG | CVG_DST_WRAP | ZMODE_XLU | FORCE_BL | GBL_c1(G_BL_CLR_FOG, G_BL_A_SHADE, G_BL_CLR_IN, G_BL_1MA),
	    AA_EN | Z_CMP | Z_UPD | IM_RD | CLR_ON_CVG | CVG_DST_WRAP | ZMODE_XLU | FORCE_BL | GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA));
	gSPEndDisplayList(dListHead++);
...
Gfx* func_809CA518(GraphicsContext* gfxCtx)
{
	...
	dList = dListHead;
	gDPSetRenderMode(dListHead++, AA_EN | Z_CMP | Z_UPD | IM_RD | CVG_DST_CLAMP | ZMODE_OPA | ALPHA_CVG_SEL | GBL_c1(G_BL_CLR_FOG, G_BL_A_SHADE, G_BL_CLR_IN, G_BL_1MA), G_RM_AA_ZB_OPA_SURF2);
	gSPEndDisplayList(dListHead++);
...
void EnBox_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	and cleared by Open
	*/
	if((pthis->alpha == 255 && !(pthis->type == ENBOX_TYPE_4 || pthis->type == ENBOX_TYPE_6)) || (!CHECK_FLAG_ALL(pthis->dyna.actor.flags, ACTOR_FLAG_7) && (pthis->type == ENBOX_TYPE_4 || pthis->type == ENBOX_TYPE_6)))
	{
