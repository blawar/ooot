...
void EnShopnuts_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gBusinessScrubSkel, &gBusinessScrubAnim_4574, pthis->jointTable, pthis->morphTable, 18);
	Collider_InitCylinder(globalCtx, &pthis->collider);
	Collider_SetCylinder(globalCtx, &pthis->collider, &pthis->actor, &sCylinderInit);
	CollisionCheck_SetInfo(&pthis->actor.colChkInfo, NULL, &sColChkInfoInit);
	Collider_UpdateCylinder(&pthis->actor, &pthis->collider);

	if(((pthis->actor.params == 0x0002) && (gSaveContext.itemGetInf[0] & 0x800)) || ((pthis->actor.params == 0x0009) && (gSaveContext.infTable[25] & 4)) || ((pthis->actor.params == 0x000A) && (gSaveContext.infTable[25] & 8)))
	{
...
void EnShopnuts_Wait(EnShopnuts* pthis, GlobalContext* globalCtx)
{
	...
	pthis->collider.dim.height = ((CLAMP((float)pthis->skelAnime.curFrame, 9.0f, 13.0f) - 9.0f) * 9.0f) + 5.0f;
...
void EnShopnuts_Burrow(EnShopnuts* pthis, GlobalContext* globalCtx)
{
	...
	{
		pthis->collider.dim.height = ((4.0f - CLAMP_MAX((float)pthis->skelAnime.curFrame, 4.0f)) * 10.0f) + 5.0f;
	}
...
void EnShopnuts_SpawnSalesman(EnShopnuts* pthis, GlobalContext* globalCtx)
{
	if(SkelAnime_Update(&pthis->skelAnime))
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_DNS, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z, pthis->actor.params);
		Actor_Kill(&pthis->actor);
	}
	else
	{
...
void EnShopnuts_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		Actor_SetFocus(&pthis->actor, 20.0f - ((pthis->skelAnime.curFrame * 20.0f) / Animation_GetLastFrame(&gBusinessScrubAnim_39C)));
	}
	else
	{
...
s32 EnShopnuts_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnShopnuts* pthis = (EnShopnuts*)thisx;
...
void EnShopnuts_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnShopnuts_OverrideLimbDraw, EnShopnuts_PostLimbDraw, pthis);
}
