...
void func_80A5475C(EnHeishi2* pthis, GlobalContext* globalCtx) {
    ...
    if (((pthis->type != 2) && (pthis->type != 5)) ||
        ((yawDiff = ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)),
          !(pthis->actor.xzDistToPlayer > 120.0f)) &&
         (yawDiff < 0x4300))) {
...
void EnHeishi2_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnHeishi2_OverrideLimbDraw,
                      EnHeishi2_PostLimbDraw, pthis);
