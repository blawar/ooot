...
s32 EnSsh_CheckCeilingPos(EnSsh* pthis, GlobalContext* globalCtx)
{
	...
	if(!BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.world.pos, &posB, &pthis->ceilingPos, &poly, false, false, true, true, &bgId))
	{
		return false;
	}
	else
...
f32 EnSsh_SetAnimation(EnSsh* pthis, s32 animIndex)
{
	...
	Animation_Change(&pthis->skelAnime, animation[animIndex], playbackSpeed[animIndex], 0.0f, frameCount, mode[animIndex], -6.0f);
...
s32 EnSsh_CheckHitFront(EnSsh* pthis)
{
	...
	if(pthis->colCylinder[2].base.acFlags)
	{
	} // Needed for matching
