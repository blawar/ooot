...
void EnBoom_Fly(EnBoom* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		{
			collided = BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.prevPos, &pthis->actor.world.pos, &hitPoint, &pthis->actor.wallPoly, true, true, true, true, &hitDynaID);
			...
			if(collided)
			{
				...
				if(func_8002F9EC(globalCtx, &pthis->actor, pthis->actor.wallPoly, hitDynaID, &hitPoint) != 0 ||
				   (hitDynaID != BGCHECK_SCENE && ((hitActor = DynaPoly_GetActor(&globalCtx->colCtx, hitDynaID)) != NULL) && hitActor->actor.id == ACTOR_BG_BDAN_OBJECTS && hitActor->actor.params == 0))
				{
					collided = false;
				}
				else
