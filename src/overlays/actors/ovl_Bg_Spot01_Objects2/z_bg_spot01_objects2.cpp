...
static Gfx* D_808AC510[] = {
    gKakarikoPotionShopSignDL, gKakarikoShootingGallerySignDL, gKakarikoBazaarSignDL, gKakarikoConstructionSiteDL, gKakarikoShootingGalleryDL,
};
		...
		switch(pthis->dyna.actor.params & 7)
		{
			...
			case 3: // Shooting Gallery, spawns Carpenter Sabooro during the day
				...
				if(IS_DAY)
				{
					...
					Actor_SpawnAsChild(
					    &globalCtx->actorCtx, thisx, globalCtx, ACTOR_EN_DAIKU_KAKARIKO, position.x, position.y, position.z, thisx->world.rot.x, thisx->world.rot.y, thisx->world.rot.z, ((((s32)thisx->params >> 8) & 0xFF) << 8) + 1);
				}
				break;
