...
s32 func_809B55EC(EnAttackNiw* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.projectedPos.z < -20.0f) || (sp1E < 0) || (sp1E > SCREEN_WIDTH) || (sp1C < 0) || (sp1C > SCREEN_HEIGHT))
	{
		return 0;
	}
	else
...
void EnAttackNiw_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.xyzDistToPlayerSq < SQ(tmpf1))
	{
		...
		if((pthis->actor.parent->update != NULL) && (pthis->actor.parent != NULL) && (cucco != NULL) && (cucco->timer9 == 0) && (player->invincibilityTimer == 0))
		{
