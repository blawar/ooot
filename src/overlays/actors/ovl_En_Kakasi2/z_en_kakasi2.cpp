...
void func_80A90264(EnKakasi2* pthis, GlobalContext* globalCtx)
{
	...
	if((BREG(1) != 0) && (pthis->actor.xzDistToPlayer < pthis->maxSpawnDistance.x) && (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < pthis->maxSpawnDistance.y))
	{
		pthis->actor.draw = func_80A90948;
		Collider_InitCylinder(globalCtx, &pthis->collider);
		Collider_SetCylinder(globalCtx, &pthis->collider, &pthis->actor, &sCylinderInit);
	...
	else if((pthis->actor.xzDistToPlayer < pthis->maxSpawnDistance.x) && (fabsf(player->actor.world.pos.y - pthis->actor.world.pos.y) < pthis->maxSpawnDistance.y) && (gSaveContext.eventChkInf[9] & 0x1000))
	{
		pthis->unk_194 = 0;
