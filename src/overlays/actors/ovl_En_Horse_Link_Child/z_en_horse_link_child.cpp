...
static AnimationHeader* sAnimations[] = {
    &gChildEponaIdleAnim, &gChildEponaWhinnyAnim, &gChildEponaWalkingAnim, &gChildEponaTrottingAnim, &gChildEponaGallopingAnim,
};
...
{
	if((pthis->skin.skelAnime.curFrame > D_80A6AF5C_34[pthis->unk_1F0]) && !((pthis->unk_1F0 == 0) && (pthis->skin.skelAnime.curFrame > D_80A6AF5C_34[1])))
	{
		Audio_PlaySoundGeneral(NA_SE_EV_KID_HORSE_WALK, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		pthis->unk_1F0++;
...
void func_80A6948C(EnHorseLinkChild* pthis)
{
	...
	else if(pthis->skin.skelAnime.curFrame == 0.0f)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EV_KID_HORSE_RUN, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
		else if(pthis->animationIdx == 1)
		{
			if(Rand_ZeroOne() > 0.5f)
			{
				Audio_PlaySoundGeneral(NA_SE_EV_KID_HORSE_GROAN, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
			else
			{
				Audio_PlaySoundGeneral(NA_SE_EV_KID_HORSE_NEIGH, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			}
		}
	}
}
...
void func_80A6993C(EnHorseLinkChild* pthis, s32 newAnimationIdx)
{
	...
	if(pthis->animationIdx != newAnimationIdx)
	{
		...
		Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], func_80A695A4(pthis), 0.0f, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), ANIMMODE_ONCE, -5.0f);
	}
}
...
void func_80A699FC(EnHorseLinkChild* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime))
	{
		...
		else
		{
			...
			{
				Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], func_80A695A4(pthis), 0.0f, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), ANIMMODE_ONCE, 0.0);
			}
		}
	}
}
...
void func_80A69C18(EnHorseLinkChild* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime))
	{
		...
		{
			Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], func_80A695A4(pthis), 0.0f, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), ANIMMODE_ONCE, 0.0f);
		}
	}
}
...
void func_80A6A068(EnHorseLinkChild* pthis, GlobalContext* globalCtx)
{
	...
	if(((gSaveContext.eventChkInf[1] & 0x40) && (DREG(53) != 0)) || ((globalCtx->sceneNum == SCENE_SPOT20) && (gSaveContext.cutsceneIndex == 0xFFF1)))
	{
		func_80A6A4DC(pthis);
	}
	else
	...
	{
		Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], func_80A695A4(pthis), pthis->skin.skelAnime.curFrame, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), 2, 0.0f);
	}
}
	...
	else
	{
		...
		if(SkelAnime_Update(&pthis->skin.skelAnime))
		{
			if(Math_CosS(yawDiff) < 0.0f)
			{
				...
				Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], D_80A6AF64[pthis->animationIdx], 0.0f, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), ANIMMODE_ONCE, -5.0f);
			}
			else
			{
				func_80A6A4DC(pthis);
