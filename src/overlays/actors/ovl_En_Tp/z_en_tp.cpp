...
void EnTp_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->actor.params <= TAILPASARAN_HEAD)
	{
		...
		for(i = 0; i <= 6; i++)
		{
			next = (EnTp*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_TP, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0 * i);

			if(0 * i)
			{
			} // Very fake, but needed to get the s registers right
			if(next != NULL)
			{
...
void EnTp_Tail_FollowHead(EnTp* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		else
		{
			...
			pthis->actor.world.pos.x = pthis->actor.home.pos.x + Math_SinS(phase) * (Math_SinS(angle) * pthis->horizontalVariation);
			pthis->actor.world.pos.z = pthis->actor.home.pos.z + Math_SinS(phase) * (Math_CosS(angle) * pthis->horizontalVariation);
		}
	}
}
...
void EnTp_Head_ApproachPlayer(EnTp* pthis, GlobalContext* globalCtx)
{
	...
	Audio_PlaySoundGeneral(NA_SE_EN_TAIL_FLY - SFX_FLAG, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
void EnTp_Die(EnTp* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer <= 0)
	{
		if(pthis->actor.params == TAILPASARAN_HEAD_DYING)
		{
			...
			EffectSsDeadDb_Spawn(globalCtx, &effectPos, &effectVelAccel, &effectVelAccel, 100, 0, 255, 255, 255, 255, 0, 0, 255, 1, 9, 1);
		...
		else
		{
			for(i = 0; i < 1; i++)
			{
				now = (EnTp*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_TP, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, TAILPASARAN_FRAGMENT);
...
void EnTp_Head_TakeOff(EnTp* pthis, GlobalContext* globalCtx)
{
	...
	Math_SmoothStepToF(&pthis->actor.speedXZ, 2.5f, 0.1f, 0.2f, 0.0f);
	Math_SmoothStepToF(&pthis->actor.world.pos.y, player->actor.world.pos.y + 85.0f + pthis->horizontalVariation, 1.0f, pthis->actor.speedXZ * 0.25f, 0.0f);
	Audio_PlaySoundGeneral(NA_SE_EN_TAIL_FLY - SFX_FLAG, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	...
	pthis->actor.world.pos.y += Math_CosF(pthis->heightPhase) * ((pthis->actor.speedXZ * 0.25f) + pthis->extraHeightVariation);
	...
	Math_SmoothStepToS(&pthis->actor.world.rot.y, Math_Vec3f_Yaw(&pthis->actor.world.pos, &pthis->actor.home.pos), 1, 750, 0);
...
void EnTp_Head_Wait(EnTp* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.xzDistToPlayer < 200.0f)
	{
		...
		if(pthis->timer != 0)
		{
			...
			pthis->actor.world.pos.x = pthis->actor.home.pos.x + (Math_SinS(2000 * pthis->unk_15C) * (Math_SinS(yaw) * pthis->horizontalVariation));
			pthis->actor.world.pos.z = pthis->actor.home.pos.z + (Math_SinS(2000 * pthis->unk_15C) * (Math_CosS(yaw) * pthis->horizontalVariation));
		}
		else
		{
	...
	else
	{
		...
		else
		{
			...
			pthis->actor.world.pos.x = pthis->actor.home.pos.x + (Math_SinS(2000 * pthis->unk_15C) * (Math_SinS(yaw) * 6.0f));
			pthis->actor.world.pos.z = pthis->actor.home.pos.z + (Math_SinS(2000 * pthis->unk_15C) * (Math_CosS(yaw) * 6.0f));
		}
	}
...
void EnTp_Head_BurrowReturnHome(EnTp* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(closeToFloor && ((globalCtx->gameplayFrames & 1) != 0))
		{
			...
			EffectSsDtBubble_SpawnCustomColor(globalCtx, &bubblePos, &bubbleVelocity, &bubbleAccel_65, &bubblePrimColor_65, &bubbleEnvColor_65, Rand_S16Offset(100, 50), 20, 0);
		}
	}
}
...
void EnTp_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params <= TAILPASARAN_HEAD)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EN_TAIL_CRY, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
