...
void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* pthis, GlobalContext* globalCtx, EnIk* enIk)
{
	...
	for(i = 0; i < 8; i++)
	{
		Actor* actor = Actor_Spawn(
		    &globalCtx->actorCtx, globalCtx, ACTOR_BG_JYA_HAHENIRON, pthis->dyna.actor.world.pos.x, Rand_ZeroOne() * 80.0f + pthis->dyna.actor.world.pos.y + 20.0f, pthis->dyna.actor.world.pos.z, 0, (s16)(Rand_ZeroOne() * 0x4000) + rotY - 0x2000, 0,
		    0);
		if(actor != NULL)
		{
	...
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_JYA_HAHENIRON, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y + 150.0f, pthis->dyna.actor.world.pos.z, 0, 0, 0, 1);
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_JYA_HAHENIRON, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, 0, 0, 0, 2);
	sins = Math_SinS(rotY);
	coss = Math_CosS(rotY);
	for(j = 0; j < 32; j++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &vel, &pos, -350, unkArg5, D_80899500[j & 7], 4, 0, D_808994E0[j & 7], 0, 5, D_808994F0[j & 7], -1, OBJECT_JYA_IRON, gObjectJyaIronDL_000880);
...
void BgJyaIronobj_SpawnThoneParticles(BgJyaIronobj* pthis, GlobalContext* arg1, EnIk* enIk)
{
	...
	{
		Actor* actor = Actor_Spawn(
		    &arg1->actorCtx, arg1, ACTOR_BG_JYA_HAHENIRON, pthis->dyna.actor.world.pos.x, (Rand_ZeroOne() * 80.0f) + pthis->dyna.actor.world.pos.y + 10.0f, pthis->dyna.actor.world.pos.z, 0, ((s16)(s32)(Rand_ZeroOne() * 0x4000) + rotY) - 0x2000, 0,
		    0);
		if(actor != NULL)
		{
			actor->speedXZ = Rand_ZeroOne() * 8.0f + 9.0f;
	...
	for(j = 0; j < 32; j++)
	{
		...
		vel.y = Rand_ZeroOne() * 8.0f - 3.0f;
		vel.z = 2.0f * (Rand_ZeroOne() * 6.0f - 3.0f) + (Rand_ZeroOne() * coss * 8.0f * temp_f22);
		EffectSsKakera_Spawn(arg1, &pos, &vel, &pos, -350, unkArg5, D_80899530[j & 7], 4, 0, D_80899510[j & 7], 0, 5, D_80899520[j & 7], -1, OBJECT_JYA_IRON, gObjectJyaIronDL_000880);
		if(Rand_ZeroOne() < 0.26f)
		{
