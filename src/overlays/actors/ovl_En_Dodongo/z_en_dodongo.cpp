...
void EnDodongo_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gDodongoSkel, &gDodongoWaitAnim, pthis->jointTable, pthis->morphTable, 31);
		...
		if(pthis->timer != 0)
		{
			...
			if(pthis->timer == 10)
			{
				for(i = 10; i >= 0; i--)
				{
					...
					func_8002836C(globalCtx, &pos, &deathFireVel, &deathFireAccel, &pthis->bombSmokePrimColor, &pthis->bombSmokeEnvColor, 400, 10, 10);
				}
...
void EnDodongo_SweepTail(EnDodongo* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime))
	{
		...
		else
		{
			...
			pthis->timer = 18;
			pthis->colliderBody.base.atFlags = pthis->sphElements[1].info.toucherFlags = pthis->sphElements[2].info.toucherFlags = AT_ON | AT_TYPE_ENEMY; // also TOUCH_ON | TOUCH_SFX_WOOD
...
void EnDodongo_Death(EnDodongo* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime) != 0)
	{
		if(pthis->timer == 0)
		{
			bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 6, BOMB_BODY);
			if(bomb != NULL)
			{
				bomb->timer = 0;
				pthis->timer = 8;
...
void EnDodongo_UpdateQuad(EnDodongo* pthis, GlobalContext* globalCtx)
{
	...
	Collider_SetQuadVertices(&pthis->colliderAT, &pthis->colliderAT.dim.quad[a], &pthis->colliderAT.dim.quad[b], &pthis->colliderAT.dim.quad[c], &pthis->colliderAT.dim.quad[d]);
}
...
s32 EnDodongo_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnDodongo* pthis = (EnDodongo*)thisx;
...
void EnDodongo_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	{
		case 2:
			if((pthis->actionState == DODONGO_BREATHE_FIRE) && (29.0f < pthis->skelAnime.curFrame) && (pthis->skelAnime.curFrame < 43.0f))
			{
...
void EnDodongo_Draw(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->iceTimer != 0)
	{
		...
		if((pthis->iceTimer % 4) == 0)
		{
			...
			EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &pthis->icePos[index], 150, 150, 150, 250, 235, 245, 255, 1.8f);
		}
	}
}
