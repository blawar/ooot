...
void EnAm_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gArmosSkel, &gArmosRicochetAnim, pthis->jointTable, pthis->morphTable, 14);
...
{
	Animation_Change(&pthis->skelAnime, &gArmosDamagedAnim, 1.0f, 4.0f, Animation_GetLastFrame(&gArmosDamagedAnim) - 6.0f, ANIMMODE_ONCE, 0.0f);
...
void EnAm_Sleep(EnAm* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->unk_258 != 0) || ((pthis->hurtCollider.base.ocFlags1 & OC1_HIT) && (pthis->hurtCollider.base.oc == &player->actor)) || (pthis->hurtCollider.base.acFlags & AC_HIT))
	{
...
void EnAm_RotateToInit(EnAm* pthis, GlobalContext* globalCtx)
{
	if(pthis->skelAnime.curFrame == 8.0f)
	{
		if((pthis->dyna.actor.world.pos.x == pthis->dyna.actor.home.pos.x) && (pthis->dyna.actor.world.pos.z == pthis->dyna.actor.home.pos.z))
		{
	...
	if(pthis->deathTimer < 52)
	{
		...
		else if(pthis->skelAnime.curFrame > 11.0f)
		{
			...
			else
			{
				...
				if(((Actor_WorldDistXZToPoint(&pthis->dyna.actor, &pthis->dyna.actor.home.pos) > 240.0f) || (pthis->attackTimer == 0)) && (pthis->deathTimer == 0))
				{
...
void EnAm_SetupStunned(EnAm* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, &gArmosDamagedAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gArmosDamagedAnim), ANIMMODE_ONCE, 0.0f);
...
void EnAm_Ricochet(EnAm* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->dyna.actor.velocity.y <= 0.0f) && !EnAm_CanMove(pthis, globalCtx, pthis->dyna.actor.speedXZ * 1.5f, pthis->dyna.actor.world.rot.y))
	{
		pthis->dyna.actor.speedXZ = 0.0f;
	}
...
void EnAm_TransformSwordHitbox(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Collider_SetQuadVertices(&pthis->hitCollider, &pthis->hitCollider.dim.quad[0], &pthis->hitCollider.dim.quad[1], &pthis->hitCollider.dim.quad[2], &pthis->hitCollider.dim.quad[3]);
}
	...
	if(pthis->deathTimer == 0)
	{
		...
		else if((pthis->hurtCollider.base.acFlags & AC_HIT) && (pthis->behavior >= AM_BEHAVIOR_5))
		{
			...
			if(pthis->dyna.actor.colChkInfo.damageEffect != AM_DMGEFF_MAGIC_FIRE_LIGHT)
			{
				...
				if((pthis->dyna.actor.colChkInfo.damageEffect == AM_DMGEFF_NUT) || (pthis->dyna.actor.colChkInfo.damageEffect == AM_DMGEFF_STUN) || (pthis->dyna.actor.colChkInfo.damageEffect == AM_DMGEFF_ICE))
				{
				...
				else if((pthis->dyna.actor.colChkInfo.damageEffect == AM_DMGEFF_KILL) || (pthis->behavior == AM_BEHAVIOR_STUNNED))
				{
					pthis->dyna.actor.colChkInfo.health = 0;
	...
	if(pthis->dyna.actor.colChkInfo.damageEffect != AM_DMGEFF_MAGIC_FIRE_LIGHT)
	{
		...
		if(pthis->deathTimer != 0)
		{
			...
			if(pthis->deathTimer == 0)
			{
				...
				EnAm_SpawnEffects(pthis, globalCtx);
				bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, 0, 0, 2, BOMB_BODY);
				if(bomb != NULL)
				{
					bomb->timer = 0;
				}
...
static Vec3f sIcePosOffsets[] = {
    {20.0f, 40.0f, 0.0f},  {10.0f, 60.0f, 10.0f}, {-10.0f, 60.0f, 10.0f}, {-20.0f, 40.0f, 0.0f}, {10.0f, 60.0f, -10.0f}, {-10.0f, 60.0f, -10.0f},
    {0.0f, 40.0f, -20.0f}, {10.0f, 20.0f, 10.0f}, {10.0f, 20.0f, -10.0f}, {0.0f, 40.0f, 20.0f},	 {-10.0f, 20.0f, 10.0f}, {-10.0f, 20.0f, -10.0f},
};
...
void EnAm_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->iceTimer != 0)
	{
		...
		if((pthis->iceTimer % 4) == 0)
		{
			...
			EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->dyna.actor, &sp68, 150, 150, 150, 250, 235, 245, 255, 1.4f);
		}
	}
