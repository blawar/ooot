...
void BgDodoago_WaitExplosives(BgDodoago* pthis, GlobalContext* globalCtx)
{
	...
	if(explosive != NULL)
	{
		pthis->state = (Math_Vec3f_Yaw(&pthis->dyna.actor.world.pos, &explosive->world.pos) >= pthis->dyna.actor.shape.rot.y) ? BGDODOAGO_EYE_RIGHT : BGDODOAGO_EYE_LEFT;

		if(((globalCtx->roomCtx.unk_74[BGDODOAGO_EYE_LEFT] == 255) && (pthis->state == BGDODOAGO_EYE_RIGHT)) || ((globalCtx->roomCtx.unk_74[BGDODOAGO_EYE_RIGHT] == 255) && (pthis->state == BGDODOAGO_EYE_LEFT)))
		{
...
void BgDodoago_OpenJaw(BgDodoago* pthis, GlobalContext* globalCtx)
{
	Vec3f pos;
	Vec3f dustOffsets[] = {
	    {0.0f, -200.0f, 430.0f},   {20.0f, -200.0f, 420.0f}, {-20.0f, -200.0f, 420.0f}, {40.0, -200.0f, 380.0f},  {-40.0, -200.0f, 380.0f}, {50.0, -200.0f, 350.0f},
	    {-50.0f, -200.0f, 350.0f}, {60.0f, -200.0f, 320.0f}, {-60.0f, -200.0f, 320.0f}, {70.0f, -200.0f, 290.0f}, {-70.0, -200.0f, 290.0f},
	};
	s32 i;
	...
	{
		BgDodoago_SetupAction(pthis, BgDodoago_DoNothing);
		Audio_PlaySoundGeneral(NA_SE_EV_STONE_BOUND, &pthis->dyna.actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	else
	{
		Audio_PlaySoundGeneral(NA_SE_EV_STONE_STATUE_OPEN - SFX_FLAG, &pthis->dyna.actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
...
void BgDodoago_Draw(Actor* pthisx, GlobalContext* globalCtx)
{
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_dodoago.c", 672);

	if(Flags_GetEventChkInf(0xB0))
	{
		func_80093D18(globalCtx->gfxCtx);
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_dodoago.c", 677), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_OPA_DISP++, gDodongoLowerJawDL);
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_dodoago.c", 681);
}
