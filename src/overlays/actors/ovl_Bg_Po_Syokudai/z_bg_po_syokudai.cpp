...
void BgPoSyokudai_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Lights_PointGlowSetInfo(&pthis->lightInfo, thisx->world.pos.x, (s16)thisx->world.pos.y + 65, thisx->world.pos.z, 0,
                            0, 0, 0);
    ...
    if (pthis->flameColor == POE_FLAME_PURPLE && Flags_GetSwitch(globalCtx, POE_TORCH_FLAG + POE_FLAME_GREEN) &&
        ...
        Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_PO_SISTERS, thisx->world.pos.x,
                    ...
                    (pthis->flameColor << 8) + thisx->params + 0x1000);
...
void BgPoSyokudai_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPDisplayList(POLY_OPA_DISP++, oot::asset::gfx::load(symbol::gGoldenTorchDL));
    ...
    if (Flags_GetSwitch(globalCtx, pthis->actor.params)) {
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0,
                                    (pthis->flameTextureScroll * -20) & 0x1FF, 32, 128));
