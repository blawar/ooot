...
void BgMizuShutter_Move(BgMizuShutter* pthis, GlobalContext* globalCtx)
{
	if(Flags_GetSwitch(globalCtx, (u16)pthis->dyna.actor.params & 0x3F))
	{
		...
		if((pthis->dyna.actor.world.pos.x == pthis->openPos.x) && (pthis->dyna.actor.world.pos.y == pthis->openPos.y) && (pthis->dyna.actor.world.pos.z == pthis->openPos.z))
		{
	...
	else
	{
		...
		if((pthis->dyna.actor.world.pos.x == pthis->closedPos.x) && (pthis->dyna.actor.world.pos.y == pthis->closedPos.y) && (pthis->dyna.actor.world.pos.z == pthis->closedPos.z))
		{
...
void BgMizuShutter_Draw(BgMizuShutter* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_mizu_shutter.c", 410);
	func_80093D18(globalCtx->gfxCtx);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_mizu_shutter.c", 415), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

	if(pthis->displayList != NULL)
	{
		gSPDisplayList(POLY_OPA_DISP++, pthis->displayList);
	}
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_mizu_shutter.c", 422);
}
