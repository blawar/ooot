...
void EnFireRock_Fall(EnFireRock* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.bgCheckFlags & BG_STATE_0) && (pthis->timer == 0))
	{
		switch(pthis->type)
		{
			...
			case FIRE_ROCK_BROKEN_PIECE1:
				Actor_SpawnFloorDustRing(globalCtx, &pthis->actor, &pthis->actor.world.pos, pthis->actor.shape.shadowScale, 1, 8.0f, 500, 10, 0);
				for(i = 0; i < 5; i++)
				{
					flamePos.x = Rand_CenteredFloat(20.0f) + pthis->actor.world.pos.x;
				...
				break;
			default:
				Actor_SpawnFloorDustRing(globalCtx, &pthis->actor, &pthis->actor.world.pos, pthis->actor.shape.shadowScale, 3, 8.0f, 200, 10, 0);
	...
	if(nextRockType != FIRE_ROCK_SPAWNED_FALLING1)
	{
		for(i = 0; i < 2; i++)
		{
			spawnedFireRock = (EnFireRock*)Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_EN_FIRE_ROCK, Rand_CenteredFloat(3.0f) + pthis->actor.world.pos.x, Rand_CenteredFloat(3.0f) + (pthis->actor.world.pos.y + 10.0f), Rand_CenteredFloat(3.0f) + pthis->actor.world.pos.z, 0, 0,
			    0, nextRockType);
...
void FireRock_WaitSpawnRocksFromCeiling(EnFireRock* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.xzDistToPlayer < 200.0f)
	{
		if((pthis->playerNearby == 0) && (pthis->timer2 == 0))
		{
			pthis->timer2 = 30;
			spawnedFireRock = (EnFireRock*)Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_EN_FIRE_ROCK, Rand_CenteredFloat(3.0f) + pthis->actor.world.pos.x, pthis->actor.world.pos.y + 10.0f, Rand_CenteredFloat(3.0f) + pthis->actor.world.pos.z, 0, 0, 0,
...
void EnFireRock_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->type != FIRE_ROCK_CEILING_SPOT_SPAWNER)
	{
		...
		{
			if((pthis->type == FIRE_ROCK_SPAWNED_FALLING1) || (pthis->type == FIRE_ROCK_SPAWNED_FALLING2) || (pthis->type == FIRE_ROCK_BROKEN_PIECE1))
			{
...
void EnFireRock_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_fire_rock.c", 747);
	Matrix_Translate(thisx->world.pos.x + pthis->relativePos.x, thisx->world.pos.y + pthis->relativePos.y, thisx->world.pos.z + pthis->relativePos.z, MTXMODE_NEW);
	...
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_fire_rock.c", 768), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, object_efc_star_field_DL_000DE0);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_fire_rock.c", 773);
}
