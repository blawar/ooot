...
static BgJyaMegamiPieceInit sPiecesInit[] = {
    {{-50.0f, -21.28f, -38.92f}, -1.6f, 0xFED4, 0xFE70, 40},	      {{-4.32f, -13.44f, -13.76f}, 0.0f, 0x04B0, 0x0190, 42},  {{30.92f, -20.480001f, -28.84f}, 1.6f, 0xFCE0, 0x0320, 44},
    {{-24.08f, -40.960003f, -21.359999f}, -1.0f, 0xFD44, 0x01F4, 36}, {{-44.8f, -73.92f, -49.76f}, -3.2f, 0x03E8, 0xFF38, 30}, {{-9.0f, -76.479996f, -13.24f}, -1.0f, 0xFC18, 0x0258, 26},
    {{-10.240001f, -52.56f, -13.400001f}, 1.6f, 0x0258, 0xFE70, 34},  {{34.04f, -61.72f, -37.04f}, 1.5f, 0x0258, 0x0258, 28},  {{59.48f, -38.399998f, -49.4f}, 1.8f, 0x01F4, 0x0000, 38},
    {{-19.04f, -112.24f, -35.120003f}, -1.6f, 0x012C, 0x0320, 22},    {{12.24f, -99.04f, -31.64f}, 1.8f, 0xFC18, 0xFE70, 24},  {{49.16f, -81.24f, -55.52f}, 2.4f, 0x02BC, 0x012C, 32},
...
void BgJyaMegami_SpawnEffect(GlobalContext* globalCtx, Vec3f* pos, Vec3f* velocity, s32 num, s32 arg4)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, pos, velocity, pos, -90, arg5, D_8089B16C[idx], 4, 0, D_8089B14C[idx], 0, 5, D_8089B15C[idx], KAKERA_COLOR_NONE, OBJECT_JYA_OBJ, gMegami2DL);
...
void BgJyaMegami_Explode(BgJyaMegami* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->explosionTimer % 4 == 0) && (pthis->explosionTimer > 30) && (pthis->explosionTimer < 80) && (pthis->explosionTimer > 40))
	{
...
static void* sRightSideCrumbles[] = {
    gMegamiRightCrumble1Tex, gMegamiRightCrumble2Tex, gMegamiRightCrumble3Tex, gMegamiRightCrumble4Tex, gMegamiRightCrumble5Tex,
};

static void* sLeftSideCrumbles[] = {
    gMegamiLeftCrumble1Tex, gMegamiLeftCrumble2Tex, gMegamiLeftCrumble3Tex, gMegamiLeftCrumble4Tex, gMegamiLeftCrumble5Tex,
};

void BgJyaMegami_DrawFace(BgJyaMegami* pthis, GlobalContext* globalCtx)
{
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_jya_megami.c", 706);
	...
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_jya_megami.c", 716), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, gMegami1DL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_jya_megami.c", 720);
}

static Gfx* sDLists[] = {
    gMegamiPiece1DL, gMegamiPiece2DL, gMegamiPiece3DL, gMegamiPiece4DL, gMegamiPiece5DL, gMegamiPiece6DL, gMegamiPiece7DL, gMegamiPiece8DL, gMegamiPiece9DL, gMegamiPiece10DL, gMegamiPiece11DL, gMegamiPiece12DL, gMegamiPiece13DL,
};
...
void BgJyaMegami_DrawExplode(BgJyaMegami* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_jya_megami.c", 746);
	...
	for(i = 0; i < ARRAY_COUNT(pthis->pieces); i++)
	{
		...
		Matrix_Translate(piece->pos.x + sPiecesInit[i].unk_00.x, piece->pos.y + sPiecesInit[i].unk_00.y, piece->pos.z + sPiecesInit[i].unk_00.z, MTXMODE_NEW);
		Matrix_RotateY(piece->rotVelY * (M_PI / 0x8000), MTXMODE_APPLY);
		Matrix_RotateX(piece->rotVelX * (M_PI / 0x8000), MTXMODE_APPLY);
		Matrix_Scale(0.1f, 0.1f, 0.1f, MTXMODE_APPLY);
		Matrix_Translate(sPiecesInit[i].unk_00.x * -10.0f, sPiecesInit[i].unk_00.y * -10.0f, sPiecesInit[i].unk_00.z * -10.0f, MTXMODE_APPLY);

		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_jya_megami.c", 778), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_OPA_DISP++, sDLists[i]);
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_jya_megami.c", 783);
}
