...
#define AT_FINISH_LINE(actor) (Math3D_PointInSquare2D(sFinishLine[0], sFinishLine[1], sFinishLine[2], sFinishLine[3], (actor)->world.pos.x, (actor)->world.pos.z))
#define AT_RANCH_EXIT(actor) (Math3D_PointInSquare2D(sRanchExit[0], sRanchExit[1], sRanchExit[2], sRanchExit[3], (actor)->world.pos.x, (actor)->world.pos.z))
...
static Vec3f sFencePos[] = {
    {820.0f, -44.0f, -1655.0f}, {1497.0f, -21.0f, -1198.0f}, {1655.0f, -44.0f, -396.0f}, {1291.0f, -44.0f, 205.0f}, {379.0f, -21.0f, 455.0f}, {-95.0f, -21.0f, 455.0f}, {-939.0f, 1.0f, 455.0f}, {-1644.0f, -21.0f, -1035.0f},
};
...
s32 EnHorseGameCheck_InitIngoRace(EnHorseGameCheckBase* base, GlobalContext* globalCtx)
{
	...
	pthis->ingoHorse = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, -250.0f, 1.0f, -1650.0f, 0, 0x4000, 0, 0x8003);
...
s32 EnHorseGameCheck_UpdateIngoRace(EnHorseGameCheckBase* base, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 3; i++)
	{
		if((player->rideActor != NULL) && (Math3D_Vec3f_DistXYZ(&sIngoRaceCheckpoints[i], &player->rideActor->world.pos) < 400.0f))
		{
...
s32 EnHorseGameCheck_UpdateMalonRace(EnHorseGameCheckBase* base, GlobalContext* globalCtx)
{
	...
	else if((pthis->raceFlags & MALONRACE_PLAYER_ON_MARK) && !(pthis->raceFlags & MALONRACE_PLAYER_START) && !AT_FINISH_LINE(player->rideActor))
	{
	...
	if(pthis->result == MALONRACE_NO_RESULT)
	{
		...
		if((player2->rideActor != NULL) && (pthis->raceFlags & MALONRACE_PLAYER_START) && AT_FINISH_LINE(player2->rideActor))
		{
...
static EnHorseGameCheckFunc sInitFuncs[] = {
    NULL, EnHorseGameCheck_InitIngoRace, EnHorseGameCheck_InitGerudoArchery, EnHorseGameCheck_InitType3, EnHorseGameCheck_InitMalonRace,
};

static EnHorseGameCheckFunc sDestroyFuncs[] = {
    NULL, EnHorseGameCheck_DestroyIngoRace, EnHorseGameCheck_DestroyGerudoArchery, EnHorseGameCheck_DestroyType3, EnHorseGameCheck_DestroyMalonRace,
};

static EnHorseGameCheckFunc sUpdateFuncs[] = {
    NULL, EnHorseGameCheck_UpdateIngoRace, EnHorseGameCheck_UpdateGerudoArchery, EnHorseGameCheck_UpdateType3, EnHorseGameCheck_UpdateMalonRace,
};
