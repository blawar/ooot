...
void BgHakaSgami_Spin(BgHakaSgami* pthis, GlobalContext* globalCtx)
{
	...
	for(i = iterateCount - 2; i < iterateCount; i++)
	{
		...
		for(j = 0; j < 3; j++)
		{
			scytheVertices[j].x = pthis->actor.world.pos.x + elementInit->dim.vtx[j].z * actorRotYSin + elementInit->dim.vtx[j].x * actorRotYCos;
			scytheVertices[j].y = pthis->actor.world.pos.y + elementInit->dim.vtx[j].y;
			scytheVertices[j].z = pthis->actor.world.pos.z + elementInit->dim.vtx[j].z * actorRotYCos - elementInit->dim.vtx[j].x * actorRotYSin;
		}
		...
		Collider_SetTrisVertices(&pthis->colliderScythe, (i + 2) % 4, &scytheVertices[0], &scytheVertices[1], &scytheVertices[2]);
	}

	if((pthis->unk_151 == 0) || (globalCtx->actorCtx.unk_03 != 0))
	{
		scytheVertices[0].x = pthis->actor.world.pos.x + blureEffectVertices1_34[pthis->actor.params].z * actorRotYSin + blureEffectVertices1_34[pthis->actor.params].x * actorRotYCos;
		scytheVertices[0].y = pthis->actor.world.pos.y + blureEffectVertices1_34[pthis->actor.params].y;
		scytheVertices[0].z = pthis->actor.world.pos.z + blureEffectVertices1_34[pthis->actor.params].z * actorRotYCos - blureEffectVertices1_34[pthis->actor.params].x * actorRotYSin;
		scytheVertices[1].x = pthis->actor.world.pos.x + blureEffectVertices2_34[pthis->actor.params].z * actorRotYSin + blureEffectVertices2_34[pthis->actor.params].x * actorRotYCos;
		scytheVertices[1].y = pthis->actor.world.pos.y + blureEffectVertices2_34[pthis->actor.params].y;
		scytheVertices[1].z = pthis->actor.world.pos.z + blureEffectVertices2_34[pthis->actor.params].z * actorRotYCos - blureEffectVertices2_34[pthis->actor.params].x * actorRotYSin;
