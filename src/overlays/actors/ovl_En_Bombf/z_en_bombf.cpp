...
void EnBombf_Move(EnBombf* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if((pthis->actor.bgCheckFlags & BG_STATE_1) && (pthis->actor.velocity.y < -6.0f))
		{
...
void EnBombf_Explode(EnBombf* pthis, GlobalContext* globalCtx)
{
	...
	pthis->explosionCollider.elements[0].dim.worldSphere.radius = pthis->explosionCollider.elements[0].dim.modelSphere.radius;
...
void EnBombf_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(thisx->params == BOMBFLOWER_BODY)
	{
		if((thisx->velocity.y > 0.0f) && (thisx->bgCheckFlags & BG_STATE_4))
		{
			thisx->velocity.y = -thisx->velocity.y;
		}

		// rebound bomb off the wall it hits
		if((thisx->speedXZ != 0.0f) && (thisx->bgCheckFlags & BG_STATE_3))
		{
...
Gfx* EnBombf_NewMtxDList(GraphicsContext* gfxCtx, GlobalContext* globalCtx)
{
	...
	gSPMatrix(displayListHead++, Matrix_NewMtx(gfxCtx, "../z_en_bombf.c", 1021), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
