...
void DemoEffect_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	objectIndex = sEffectTypeObjects[effectType] == OBJECT_GAMEPLAY_KEEP ? 0 : Object_GetIndex(&globalCtx->objectCtx, sEffectTypeObjects[effectType]);
	...
	switch(effectType)
	{
		...
		case DEMO_EFFECT_TRIFORCE_SPOT:
			...
			crystalLight = (DemoEffect*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_CRYSTAL_LIGHT);
			...
			lightRing =
			    (DemoEffect*)Actor_SpawnAsChild(&globalCtx->actorCtx, &crystalLight->actor, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_LIGHTRING_TRIFORCE);
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL)
	{
		...
		switch(globalCtx->csCtx.npcActions[pthis->csActionId]->action)
		{
			case 2:
				...
				{
					Common_PlaySfx2(NA_SE_EV_MEDAL_APPEAR_S - SFX_FLAG);
				}
				if(pthis->getItem.drawId != GID_ARROW_LIGHT)
				{
...
void DemoEffect_InitTimeWarp(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	else if(gSaveContext.sceneSetupIndex == 5 || gSaveContext.sceneSetupIndex == 4 || (gSaveContext.entranceIndex == 0x0324 && !((gSaveContext.eventChkInf[12] & 0x200))))
	{
...
void DemoEffect_UpdateTriforceSpot(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL)
	{
		...
		if(gSaveContext.entranceIndex == 0x00A0 && gSaveContext.sceneSetupIndex == 6 && globalCtx->csCtx.frames == 143)
		{
...
void DemoEffect_UpdateLightRingTriforce(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	{
		if(globalCtx->csCtx.npcActions[pthis->csActionId] != NULL && globalCtx->csCtx.npcActions[pthis->csActionId]->action == 2)
		{
			blueOrb = (DemoEffect*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_BLUE_ORB);
...
void DemoEffect_UpdateCreationFireball(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	effect = (DemoEffect*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_BLUE_ORB);
	if(effect != NULL)
	{
		Actor_SetScale(&effect->actor, 0.0f);
	}

	effect = (DemoEffect*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_LIGHTRING_EXPANDING);
	if(effect != NULL)
	{
		Actor_SetScale(&effect->actor, 0.1f);
	}

	effect = (DemoEffect*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_LIGHTRING_SHRINKING);
	if(effect != NULL)
	{
		Actor_SetScale(&effect->actor, 0.2f);
	}

	Common_PlaySfx2(NA_SE_IT_DM_RING_EXPLOSION);
	Actor_Kill(&pthis->actor);
}
...
void DemoEffect_UpdateBlueOrbGrow(DemoEffect* pthis, GlobalContext* globalCtx)
{
	if(pthis->actor.parent != NULL)
	{
		...
		// u8 to float
		Actor_SetScale(&pthis->actor, (((5.0f - (s32)pthis->blueOrb.scale) * 0.01f) * 10.0f) * pthis->actor.parent->scale.x);
	}
	else
	{
...
void DemoEffect_UpdateLightEffect(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL)
	{
		...
		if(globalCtx->sceneNum == SCENE_SPOT04 && gSaveContext.sceneSetupIndex == 6 && globalCtx->csCtx.frames == 197)
		{
...
void DemoEffect_UpdateGodLgtDin(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL)
	{
		...
		{
			fireBall = (DemoEffect*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_FIRE_BALL);
		...
		if(globalCtx->csCtx.npcActions[pthis->csActionId]->action == 3)
		{
			...
			else
			{
				...
				lightRing = (DemoEffect*)Actor_Spawn(
				    &globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.world.rot.x + 0x4000, pthis->actor.world.rot.y, pthis->actor.world.rot.z,
...
void DemoEffect_UpdateGodLgtFarore(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL)
	{
		...
		if(globalCtx->csCtx.npcActions[pthis->csActionId]->action == 3)
		{
			lgtShower = (DemoEffect*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y - 150.0f, pthis->actor.world.pos.z, 0, 0, 0, DEMO_EFFECT_LGT_SHOWER);
 ...
 */
void DemoEffect_MoveJewelSpherical(f32 degrees, f32 frameDivisor, Vec3f startPos, Vec3f endPos, f32 radius, Vec3s rotation, DemoEffect* pthis)
{
	s32 pad;
	s32 pad2;
	f32 distance;
	...
	ySpherical = (pthis->actor.world.pos.y * cosf(rotation.x * (M_PI / 0x8000))) - (sinf(rotation.x * (M_PI / 0x8000)) * pthis->actor.world.pos.z);
	xzSpherical = (pthis->actor.world.pos.z * cosf(rotation.x * (M_PI / 0x8000))) + (sinf(rotation.x * (M_PI / 0x8000)) * pthis->actor.world.pos.y);

	pthis->actor.world.pos.x = (xPos * cosf(rotation.y * (M_PI / 0x8000))) - (sinf(rotation.y * (M_PI / 0x8000)) * xzSpherical);
	pthis->actor.world.pos.y = ySpherical;
	pthis->actor.world.pos.z = (xzSpherical * cosf(rotation.y * (M_PI / 0x8000))) + (sinf(rotation.y * (M_PI / 0x8000)) * xPos);
...
void DemoEffect_JewelSparkle(DemoEffect* pthis, GlobalContext* globalCtx, s32 spawnerCount)
{
	...
	for(i = 0; i < spawnerCount; i++)
	{
		...
		EffectSsKiraKira_SpawnDispersed(globalCtx, &pthis->actor.world.pos, &velocity, &accel, &primColor, &envColor, 3000, 16);
	}
}
...
void DemoEffect_UpdateDust(DemoEffect* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL && globalCtx->csCtx.npcActions[pthis->csActionId]->action == 2)
	{
...
s32 DemoEffect_CheckCsAction(DemoEffect* pthis, GlobalContext* globalCtx, s32 csActionCompareId)
{
	if(globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[pthis->csActionId] != NULL && globalCtx->csCtx.npcActions[pthis->csActionId]->action == csActionCompareId)
	{
		return 1;
	}
...
void DemoEffect_DrawJewel(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(!DemoEffect_CheckCsAction(pthis, globalCtx, 1))
	{
		...
		if(!(pthis->effectFlags & 0x1))
		{
			...
			{
				case DEMO_EFFECT_JEWEL_KOKIRI:
					gSPSegment(POLY_XLU_DISP++, 9, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 4) % 256, (256 - ((frames * 2) % 256)) - 1, 64, 64, 1, (frames * 2) % 256, (256 - (frames % 256)) - 1, 16, 16));
					break;

				case DEMO_EFFECT_JEWEL_GORON:
					gSPSegment(POLY_XLU_DISP++, 9, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 4) % 128, (256 - ((frames * 2) % 256)) - 1, 32, 64, 1, (frames * 2) % 256, (256 - (frames % 256)) - 1, 16, 8));
					break;

				case DEMO_EFFECT_JEWEL_ZORA:
					gSPSegment(POLY_XLU_DISP++, 9, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 4) % 256, (256 - ((frames * 2) % 256)) - 1, 32, 32, 1, (frames * 2) % 256, (256 - (frames % 256)) - 1, 16, 16));
					break;
			}
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2597), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2599), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			func_80093D84(globalCtx->state.gfxCtx);
			func_8002ED80(&pthis->actor, globalCtx, 0);
			gDPSetPrimColor(POLY_XLU_DISP++, 0, 128, pthis->primXluColor[0], pthis->primXluColor[1], pthis->primXluColor[2], 255);
			...
			gDPSetPrimColor(POLY_OPA_DISP++, 0, 128, pthis->primOpaColor[0], pthis->primOpaColor[1], pthis->primOpaColor[2], 255);
...
void DemoEffect_DrawCrystalLight(Actor* thisx, GlobalContext* globalCtx)
{
	...
	gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 2) % 512, 512 - (frames % 512) - 1, 128, 128, 1, 512 - ((frames * 2) % 512) - 1, 0, 64, 64));
	Matrix_Push();
	Matrix_RotateY(0.0f, MTXMODE_APPLY);
	Matrix_RotateX((11.0 * M_PI) / 180.0, MTXMODE_APPLY);
	Matrix_Translate(0.0f, 150.0f, 0.0f, MTXMODE_APPLY);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2661), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	Matrix_RotateX((11.0 * M_PI) / 180.0, MTXMODE_APPLY);
	Matrix_Translate(0.0f, 150.0f, 0.0f, MTXMODE_APPLY);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2672), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, gCrystalLightDL);
	Matrix_Pop();
	Matrix_Push();
	...
	Matrix_RotateX((11.0 * M_PI) / 180.0, MTXMODE_APPLY);
	Matrix_Translate(0.0f, 150.0f, 0.0f, MTXMODE_APPLY);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2683), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, gCrystalLightDL);
	Matrix_Pop();
...
void DemoEffect_DrawFireBall(Actor* thisx, GlobalContext* globalCtx)
{
	...
	func_80093D84(globalCtx->state.gfxCtx);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2709), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPMatrix(POLY_XLU_DISP++, globalCtx->billboardMtx, G_MTX_NOPUSH | G_MTX_MUL | G_MTX_MODELVIEW);
	gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 32, 1, 0, 128 - ((frames * 20) % 128) - 1, 32, 32));
	...
	if(!DemoEffect_CheckCsAction(pthis, globalCtx, 2))
	{
		if(gSaveContext.entranceIndex == 0x00A0)
		{
			if(gSaveContext.sceneSetupIndex == 4)
			{
				...
				{
					Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_EV_GOD_FLYING - SFX_FLAG);
				}
			}
			else
			{
		...
		gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 4) % 512, 0, 128, 64, 1, (frames * 2) % 256, 512 - ((frames * 70) % 512) - 1, 64, 32));
		gSPSegment(POLY_XLU_DISP++, 9, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 16, 96, 1, (frames * 10) % 256, 256 - ((frames * 30) % 512) - 1, 8, 32));
		gDPSetPrimColor(POLY_XLU_DISP++, 128, 128, pthis->primXluColor[0], pthis->primXluColor[1], pthis->primXluColor[2], 255);
		gDPSetEnvColor(POLY_XLU_DISP++, pthis->envXluColor[0], pthis->envXluColor[1], pthis->envXluColor[2], 255);
		func_80093D84(globalCtx->state.gfxCtx);
		Matrix_Push();
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2801), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2824), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void DemoEffect_DrawLightEffect(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(!DemoEffect_CheckCsAction(pthis, globalCtx, 1))
	{
		...
		else
		{
			...
			func_80093D84(globalCtx->state.gfxCtx);
			gDPSetPrimColor(POLY_XLU_DISP++, 0, 128, pthis->primXluColor[0], pthis->primXluColor[1], pthis->primXluColor[2], *alpha);
			gDPSetEnvColor(POLY_XLU_DISP++, pthis->envXluColor[0], pthis->envXluColor[1], pthis->envXluColor[2], 255);
			Matrix_Scale(((pthis->light.scaleFlag & 1) * 0.05f) + 1.0f, ((pthis->light.scaleFlag & 1) * 0.05f) + 1.0f, ((pthis->light.scaleFlag & 1) * 0.05f) + 1.0f, MTXMODE_APPLY);
			Matrix_Push();
			Matrix_Mult(&globalCtx->billboardMtxF, MTXMODE_APPLY);
			Matrix_RotateZ(pthis->light.rotation * (M_PI / 180.0f), MTXMODE_APPLY);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2866), G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
			if(disp)
			{
			};
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2874), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, disp);
		}
	}
...
void DemoEffect_DrawBlueOrb(Actor* thisx, GlobalContext* globalCtx)
{
	...
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2901), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void DemoEffect_DrawLgtShower(Actor* thisx, GlobalContext* globalCtx)
{
	...
	func_80093D84(globalCtx->state.gfxCtx);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2927), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 5) % 1024, 0, 256, 64, 1, (frames * 10) % 128, 512 - ((frames * 50) % 512), 32, 16));
...
void DemoEffect_DrawLightRing(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	gDPSetEnvColor(POLY_XLU_DISP++, 0, 100, 255, 255);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 2963), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 5) % 64, 512 - ((frames * 2) % 512) - 1, 16, 128, 1, 0, 0, 8, 1024));
...
void DemoEffect_DrawTriforceSpot(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(gSaveContext.entranceIndex != 0x0400 || globalCtx->csCtx.frames < 885)
	{
		...
		if(pthis->triforceSpot.lightColumnOpacity > 0)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 3011), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPSegment(POLY_XLU_DISP++, 9, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 256 - ((frames * 4) % 256) - 1, 64, 64, 1, 0, 256 - ((frames * 2) % 256) - 1, 64, 32));
			vertices[86].n.a = vertices[87].n.a = vertices[88].n.a = vertices[89].n.a = vertices[92].n.a = vertices[93].n.a = vertices[94].n.a = vertices[95].n.a = (s8)pthis->triforceSpot.lightColumnOpacity;
		...
		if(pthis->triforceSpot.triforceSpotOpacity != 0)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 3042), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			...
			if(pthis->triforceSpot.triforceSpotOpacity < 250)
			{
				...
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 3053), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 16, 1, 0, 0, 16, 8));
			...
			else
			{
				...
				Matrix_RotateY(pthis->triforceSpot.rotation * (M_PI / 0x8000), MTXMODE_APPLY);
				gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", 3085), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPSegment(POLY_OPA_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 16, 1, 0, 0, 16, 8));
...
s32 DemoEffect_DrawTimewarpLimbs(GlobalContext* globalCtx, SkelAnimeCurve* skelCuve, s32 limbIndex, void* thisx)
{
	...
	gDPSetEnvColor(POLY_XLU_DISP++, pthis->envXluColor[0], pthis->envXluColor[1], pthis->envXluColor[2], 255);
	gSPSegment(POLY_XLU_DISP++, 8, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (frames * 6) % 1024, 256 - ((frames * 16) % 256) - 1, 256, 64, 1, (frames * 4) % 512, 128 - ((frames * 12) % 128) - 1, 128, 32));
...
void DemoEffect_DrawTimeWarp(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(effectType == DEMO_EFFECT_TIMEWARP_TIMEBLOCK_LARGE || effectType == DEMO_EFFECT_TIMEWARP_TIMEBLOCK_SMALL || Flags_GetEnv(globalCtx, 1) || gSaveContext.sceneSetupIndex >= 4 || gSaveContext.entranceIndex == 0x0324)
	{
