...
static InitChainEntry sInitChain[] = {
    ICHAIN_F32(uncullZoneForward, 3000, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneScale, 3000, ICHAIN_CONTINUE),	     ICHAIN_F32(uncullZoneDownward, 3000, ICHAIN_CONTINUE),
    ICHAIN_F32_DIV1000(gravity, -500, ICHAIN_CONTINUE),	  ICHAIN_F32_DIV1000(minVelocityY, -12000, ICHAIN_CONTINUE), ICHAIN_VEC3F_DIV1000(scale, 1000, ICHAIN_STOP),
};
...
void BgMoriBigst_InitDynapoly(BgMoriBigst* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 moveFlag)
{
	...
	if(pthis->dyna.bgId == BG_ACTOR_MAX)
	{
		...
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_bg_mori_bigst.c", 190, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void BgMoriBigst_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	osSyncPrintf(
	    "mori (bigST.鍵型天井)(arg : %04x)(sw %d)(noE %d)(roomC %d)(playerPosY %f)\n", pthis->dyna.actor.params, Flags_GetSwitch(globalCtx, (pthis->dyna.actor.params >> 8) & 0x3F), Flags_GetTempClear(globalCtx, pthis->dyna.actor.room),
	    Flags_GetClear(globalCtx, pthis->dyna.actor.room), GET_PLAYER(globalCtx)->actor.world.pos.y);
...
void BgMoriBigst_SetupStalfosFight(BgMoriBigst* pthis, GlobalContext* globalCtx)
{
	...
	stalfos = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EN_TEST, 209.0f, 827.0f, -3320.0f, 0, 0, 0, 1);
	if(stalfos != NULL)
	{
...
void BgMoriBigst_StalfosFight(BgMoriBigst* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->dyna.actor.home.rot.z == 0) && ((pthis->dyna.actor.home.pos.y - 5.0f) <= GET_PLAYER(globalCtx)->actor.world.pos.y))
	{
...
void BgMoriBigst_SetupStalfosPairFight(BgMoriBigst* pthis, GlobalContext* globalCtx)
{
	...
	Flags_UnsetClear(globalCtx, pthis->dyna.actor.room);
	stalfos1 = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EN_TEST, 70.0f, 827.0f, -3383.0f, 0, 0, 0, 5);
	if(stalfos1 != NULL)
	{
		pthis->dyna.actor.child = NULL;
		pthis->dyna.actor.home.rot.z++;
	...
	stalfos2 = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_EN_TEST, 170.0f, 827.0f, -3260.0f, 0, 0, 0, 5);
	if(stalfos2 != NULL)
	{
		pthis->dyna.actor.child = NULL;
		pthis->dyna.actor.home.rot.z++;
...
void BgMoriBigst_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_mori_bigst.c", 541);
	func_80093D18(globalCtx->gfxCtx);

	gSPSegment(POLY_OPA_DISP++, 0x08, gObjectTable[pthis->moriTexObjIndex].vromStart.get());

	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_mori_bigst.c", 548), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

	gSPDisplayList(POLY_OPA_DISP++, gMoriBigstDL);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_mori_bigst.c", 553);
}
