...
static void* sFaceTextures[] = {object_lightswitch_Tex_000C20, object_lightswitch_Tex_000420, object_lightswitch_Tex_001420};
...
void ObjLightswitch_InitCollider(ObjLightswitch* pthis, GlobalContext* globalCtx)
{
	...
	func_800D1694(pthis->actor.world.pos.x, pthis->actor.world.pos.y + (pthis->actor.shape.yOffset * pthis->actor.scale.y), pthis->actor.world.pos.z, &pthis->actor.shape.rot);
...
void ObjLightswitch_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.params & 1) == 1)
	{
		...
		if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_OBJ_OSHIHIKI, pthis->actor.home.pos.x, pthis->actor.home.pos.y, pthis->actor.home.pos.z, 0, pthis->actor.home.rot.y, 0, (0xFF << 8) | PUSHBLOCK_SMALL_START_ON) ==
		   NULL)
		{
			osSyncPrintf(VT_COL(RED, WHITE));
			// "Push-pull block occurrence failure"
			osSyncPrintf("押引ブロック発生失敗(%s %d)(arg_data 0x%04x)\n", "../z_obj_lightswitch.c", 452, pthis->actor.params);
			osSyncPrintf(VT_RST);
			removeSelf = true;
		}
	}
...
void ObjLightswitch_TurnOff(ObjLightswitch* pthis, GlobalContext* globalCtx)
{
	if((pthis->actor.params >> 4 & 3) != OBJLIGHTSWITCH_TYPE_1 || func_8005B198() == pthis->actor.category || pthis->toggleDelay <= 0)
	{
		pthis->timer--;
