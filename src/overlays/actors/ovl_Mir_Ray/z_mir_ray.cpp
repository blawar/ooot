...
void MirRay_MakeShieldLight(MirRay* pthis, GlobalContext* globalCtx) {
    ...
    if (MirRay_CheckInFrustum(&pthis->sourcePt, &pthis->poolPt, player->actor.world.pos.x,
                              player->actor.world.pos.y + 30.0f, player->actor.world.pos.z, pthis->sourceEndRad,
                              pthis->poolEndRad)) {
        ...
        Lights_PointNoGlowSetInfo(&pthis->lightInfo, lightPt.x, lightPt.y, lightPt.z, dataEntry->color.r,
                                  dataEntry->color.g, dataEntry->color.b, pthis->lightPointRad);
    } else {
        // Fade down
        Math_StepToS(&pthis->lightPointRad, 0, 6);
        Lights_PointSetColorAndRadius(&pthis->lightInfo, dataEntry->color.r, dataEntry->color.g, dataEntry->color.b,
                                      pthis->lightPointRad);
    }
}
...
void MirRay_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->sourcePt.x, pthis->sourcePt.y, pthis->sourcePt.z, 255, 255, 255,
                              100);
...
void MirRay_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (!pthis->unLit) {
        if (sMirRayData[pthis->actor.params].params & 2) {
            if (sMirRayData[pthis->actor.params].params & 4) { // Beams from mirrors
                MirRay_SetupCollider(pthis);
            }
...
void MirRay_SetIntensity(MirRay* pthis, GlobalContext* globalCtx) {
    ...
    if (MirRay_CheckInFrustum(&pthis->sourcePt, &pthis->poolPt, shieldMtx->xw, shieldMtx->yw, shieldMtx->zw,
                              pthis->sourceEndRad, pthis->poolEndRad)) {
...
void MirRay_SetupReflectionPolys(MirRay* pthis, GlobalContext* globalCtx, MirRayShieldReflection* reflection) {
    ...
    for (i = 0; i < 6; i++) {
        posA.x = ((*shieldMtx).xw + (pthis->shieldCorners[i].x * (*shieldMtx).xx)) +
                 (pthis->shieldCorners[i].y * (*shieldMtx).xy);
        posA.y = ((*shieldMtx).yw + (pthis->shieldCorners[i].x * (*shieldMtx).yx)) +
                 (pthis->shieldCorners[i].y * (*shieldMtx).yy);
        posA.z = ((*shieldMtx).zw + (pthis->shieldCorners[i].x * (*shieldMtx).zx)) +
                 (pthis->shieldCorners[i].y * (*shieldMtx).zy);
