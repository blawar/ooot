...
static MirRayDataEntry sMirRayData[] = {{{-1160, 686, -880}, {-920, 480, -889}, 30, 50, 1.0f, 50, 150, 0.8f, 255, 255, 255, 0x02}, {{-1856, 1092, -190}, {-1703, 841, -186}, 30, 70, 0.88f, 54, 150, 0.8f, 255, 255, 255, 0x02},
					{{1367, 738, -860}, {1091, 476, -860}, 30, 85, 0.0f, 0, 150, 0.8f, 255, 255, 255, 0x00},   {{2200, 1103, -220}, {2040, 843, -220}, 30, 60, 0.0f, 0, 150, 0.8f, 255, 255, 255, 0x01},
					{{-560, 2169, -310}, {-560, 1743, -310}, 30, 70, 0.0f, 0, 150, 0.8f, 255, 255, 255, 0x00}, {{60, 1802, -1090}, {60, 973, -1090}, 30, 70, 0.0f, 0, 150, 0.9f, 255, 255, 255, 0x0D},
					{{1140, 480, -860}, {1140, 480, -860}, 30, 30, 1.0f, 10, 100, 0.9f, 255, 255, 255, 0x0E},  {{-560, 1743, -310}, {-560, 1743, -310}, 30, 30, 0.0f, 0, 100, 0.94f, 255, 255, 255, 0x0C},
					{{60, 1743, -310}, {60, 1743, -310}, 30, 30, 0.0f, 0, 100, 0.94f, 255, 255, 255, 0x0C},	   {{-1174, 448, 1194}, {-1174, 148, 1194}, 50, 100, 1.0f, 50, 150, 0.8f, 255, 255, 255, 0x03}};
...
void MirRay_SetupCollider(MirRay* pthis)
{
	Vec3f colliderOffset;
	MirRayDataEntry* dataEntry = &sMirRayData[pthis->actor.params];

	colliderOffset.x = (pthis->poolPt.x - pthis->sourcePt.x) * dataEntry->unk_10;
	...
	pthis->colliderSph.elements[0].dim.worldSphere.center.x = colliderOffset.x + pthis->sourcePt.x;
	pthis->colliderSph.elements[0].dim.worldSphere.center.y = colliderOffset.y + pthis->sourcePt.y;
	pthis->colliderSph.elements[0].dim.worldSphere.center.z = colliderOffset.z + pthis->sourcePt.z;
	pthis->colliderSph.elements[0].dim.worldSphere.radius = dataEntry->unk_14 * pthis->colliderSph.elements->dim.scale;
}
...
void MirRay_MakeShieldLight(MirRay* pthis, GlobalContext* globalCtx)
{
	...
	if(MirRay_CheckInFrustum(&pthis->sourcePt, &pthis->poolPt, player->actor.world.pos.x, player->actor.world.pos.y + 30.0f, player->actor.world.pos.z, pthis->sourceEndRad, pthis->poolEndRad))
	{
		if(dataEntry->params & 8)
		{ // Light beams from mirrors
			Math_Vec3f_Diff(&player->actor.world.pos, &pthis->sourcePt, &reflectionPt);
		}
		else
		{ // Light beams from windows
			Math_Vec3f_Diff(&pthis->poolPt, &pthis->sourcePt, &reflectionPt);
		}

		lightPt.x = (dataEntry->unk_18 * reflectionPt.x) + pthis->sourcePt.x;
		lightPt.y = (dataEntry->unk_18 * reflectionPt.y) + pthis->sourcePt.y;
		lightPt.z = (dataEntry->unk_18 * reflectionPt.z) + pthis->sourcePt.z;

		// Fade up
		Math_StepToS(&pthis->lightPointRad, dataEntry->lgtPtMaxRad, 6);
		Lights_PointNoGlowSetInfo(&pthis->lightInfo, lightPt.x, lightPt.y, lightPt.z, dataEntry->color.r, dataEntry->color.g, dataEntry->color.b, pthis->lightPointRad);
	}
	else
	{
		// Fade down
		Math_StepToS(&pthis->lightPointRad, 0, 6);
		Lights_PointSetColorAndRadius(&pthis->lightInfo, dataEntry->color.r, dataEntry->color.g, dataEntry->color.b, pthis->lightPointRad);
	}
}
...
void MirRay_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	pthis->sourcePt.x = dataEntry->sourcePoint.x;
	pthis->sourcePt.y = dataEntry->sourcePoint.y;
	pthis->sourcePt.z = dataEntry->sourcePoint.z;
	pthis->sourceEndRad = dataEntry->sourceEndRadius;

	pthis->poolPt.x = dataEntry->poolPoint.x;
	pthis->poolPt.y = dataEntry->poolPoint.y;
	pthis->poolPt.z = dataEntry->poolPoint.z;
	pthis->poolEndRad = dataEntry->poolEndRadius;

	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->sourcePt.x, pthis->sourcePt.y, pthis->sourcePt.z, 255, 255, 255, 100);
	pthis->lightNode = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &pthis->lightInfo);

	pthis->shieldCorners[0].x = -536.0f;
	pthis->shieldCorners[0].y = -939.0f;

	pthis->shieldCorners[1].x = -1690.0f;
	pthis->shieldCorners[1].y = 0.0f;

	pthis->shieldCorners[2].x = -536.0f;
	pthis->shieldCorners[2].y = 938.0f;

	pthis->shieldCorners[3].x = 921.0f;
	pthis->shieldCorners[3].y = 0.0f;

	pthis->shieldCorners[4].x = 758.0f;
	pthis->shieldCorners[4].y = 800.0f;

	pthis->shieldCorners[5].x = 758.0f;
	pthis->shieldCorners[5].y = -800.0f;
	...
	if(dataEntry->params & 2)
	{
		...
		if(!(dataEntry->params & 4))
		{ // Beams not from mirrors
			MirRay_SetupCollider(pthis);
		}
	}

	Collider_InitQuad(globalCtx, &pthis->shieldRay);
	Collider_SetQuad(globalCtx, &pthis->shieldRay, &pthis->actor, &sQuadInit);

	// Spirit Temple top room mirrors
	if((pthis->actor.params == 5) || (pthis->actor.params == 7) || (pthis->actor.params == 8))
	{
		pthis->actor.room = -1;
	}
}
...
void MirRay_Destroy(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(sMirRayData[pthis->actor.params].params & 2)
	{
		Collider_DestroyJntSph(globalCtx, &pthis->colliderSph);
	}

	Collider_DestroyQuad(globalCtx, &pthis->shieldRay);
}
...
void MirRay_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(!pthis->unLit)
	{
		if(sMirRayData[pthis->actor.params].params & 2)
		{
			if(sMirRayData[pthis->actor.params].params & 4)
			{ // Beams from mirrors
				MirRay_SetupCollider(pthis);
			}
		...
		{
			CollisionCheck_SetAT(globalCtx, &globalCtx->colChkCtx, &pthis->shieldRay.base);
		}
		MirRay_MakeShieldLight(pthis, globalCtx);
...
void MirRay_SetIntensity(MirRay* pthis, GlobalContext* globalCtx)
{
	f32 sp4C[3];
	f32 temp_f0;
	f32 temp_f0_2;
	...
	pthis->reflectIntensity = 0.0f;

	if(MirRay_CheckInFrustum(&pthis->sourcePt, &pthis->poolPt, shieldMtx->xw, shieldMtx->yw, shieldMtx->zw, pthis->sourceEndRad, pthis->poolEndRad))
	{
		temp_f0 = sqrtf(SQ(shieldMtx->zz) + (SQ(shieldMtx->xz) + SQ(shieldMtx->yz)));

		if(temp_f0 == 0.0f)
		{
			pthis->reflectRange = 1.0f;
		}
		else
		{
			pthis->reflectRange = 1.0f / temp_f0;
		}

		// If light beam is adirectional, always reflect, else only reflect if shield is pointing in correct direction
		if(sMirRayData[pthis->actor.params].params & 1)
		{
			pthis->reflectIntensity = 1.0f;
		}
		else
		{
			sp4C[0] = pthis->poolPt.x - pthis->sourcePt.x;
			sp4C[1] = pthis->poolPt.y - pthis->sourcePt.y;
			sp4C[2] = pthis->poolPt.z - pthis->sourcePt.z;
			...
			if(temp_f2_2 < 0.0f)
			{
				...
				{
					pthis->reflectIntensity = -temp_f2_2 / (temp_f0 * temp_f0_2);
				}
			}
		}
	}
...
void MirRay_SetupReflectionPolys(MirRay* pthis, GlobalContext* globalCtx, MirRayShieldReflection* reflection)
{
	Player* player = GET_PLAYER(globalCtx);
	MtxF* shieldMtx;
	s32 i;
	...
	sp60.x = -((*shieldMtx).xz * pthis->reflectRange) * pthis->reflectIntensity * 400.0f;
	sp60.y = -((*shieldMtx).yz * pthis->reflectRange) * pthis->reflectIntensity * 400.0f;
	sp60.z = -((*shieldMtx).zz * pthis->reflectRange) * pthis->reflectIntensity * 400.0f;

	sp60 = sp60; // Need something involving sp60 or the whole function changes

	for(i = 0; i < 6; i++)
	{
		posA.x = ((*shieldMtx).xw + (pthis->shieldCorners[i].x * (*shieldMtx).xx)) + (pthis->shieldCorners[i].y * (*shieldMtx).xy);
		posA.y = ((*shieldMtx).yw + (pthis->shieldCorners[i].x * (*shieldMtx).yx)) + (pthis->shieldCorners[i].y * (*shieldMtx).yy);
		posA.z = ((*shieldMtx).zw + (pthis->shieldCorners[i].x * (*shieldMtx).zx)) + (pthis->shieldCorners[i].y * (*shieldMtx).zy);
...
void MirRay_RemoveSimilarReflections(MirRayShieldReflection* reflection)
{
	...
	for(i = 0; i < 6; i++)
	{
		for(j = i + 1; j < 6; j++)
		{
			...
			{
				if((reflection[j].reflectionPoly != NULL) && (ABS(reflection[i].reflectionPoly->normal.x - reflection[j].reflectionPoly->normal.x) < 100) &&
				   (ABS(reflection[i].reflectionPoly->normal.y - reflection[j].reflectionPoly->normal.y) < 100) && (ABS(reflection[i].reflectionPoly->normal.z - reflection[j].reflectionPoly->normal.z) < 100) &&
...
void MirRay_ReflectedBeam(MirRay* pthis, GlobalContext* globalCtx, MirRayShieldReflection* reflection)
{
	Player* player = GET_PLAYER(globalCtx);
	s32 i;
	f32 temp_f0;
	...
	spE8.x = -(shieldMtx->xz * pthis->reflectRange) * pthis->reflectIntensity * 400.0f;
	spE8.y = -(shieldMtx->yz * pthis->reflectRange) * pthis->reflectIntensity * 400.0f;
	spE8.z = -(shieldMtx->zz * pthis->reflectRange) * pthis->reflectIntensity * 400.0f;
	...
	Collider_SetQuadVertices(&pthis->shieldRay, &vecA, &vecB, &vecC, &vecD);
	...
	for(i = 0; i < 6; i++)
	{
		...
		if(currentReflection->reflectionPoly != NULL)
		{
			...
			if(Math3D_LineSegVsPlane(normalVec.x, normalVec.y, normalVec.z, currentReflection->reflectionPoly->dist, &vecB, &vecD, &sp118, 1))
			{
				...
				if(temp_f0 < (pthis->reflectIntensity * 600.0f))
				{
				...
				currentReflection->mtx.xx = currentReflection->mtx.yy = currentReflection->mtx.zz = currentReflection->mtx.ww = 1.0f;
				currentReflection->mtx.yx = currentReflection->mtx.zx = currentReflection->mtx.wx = currentReflection->mtx.xy = currentReflection->mtx.zy = currentReflection->mtx.wy = currentReflection->mtx.xz = currentReflection->mtx.yz =
				    currentReflection->mtx.wz = currentReflection->mtx.xw = currentReflection->mtx.yw = currentReflection->mtx.zw;

				if(Math3D_LineSegVsPlane(normalVec.x, normalVec.y, normalVec.z, currentReflection->reflectionPoly->dist, &sp10C, &sp100, &intersection, 1))
				{
...
void MirRay_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	pthis->reflectIntensity = 0.0f;
	if((D_80B8E670 == 0) && !pthis->unLit && Player_HasMirrorShieldSetToDraw(globalCtx))
	{
...
s32 MirRay_CheckInFrustum(Vec3f* vecA, Vec3f* vecB, f32 pointx, f32 pointy, f32 pointz, s16 radiusA, s16 radiusB)
{
	...
	dist = (((pointx - vecA->x) * vecdiff.x) + ((pointy - vecA->y) * vecdiff.y) + ((pointz - vecA->z) * vecdiff.z)) / dist;
