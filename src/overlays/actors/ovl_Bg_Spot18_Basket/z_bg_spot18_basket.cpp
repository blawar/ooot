...
void func_808B7770(BgSpot18Basket* pthis, GlobalContext* globalCtx, f32 arg2)
{
	...
	for(i = 0, count = 2; i != count; i++)
	{
		...
		if(!(arg2 < Rand_ZeroOne()))
		{
			...
			func_800286CC(globalCtx, &position, &velocity, &acceleration, ((Rand_ZeroOne() * 16) + 80), ((Rand_ZeroOne() * 30) + 80));
		}
	}
}
...
void BgSpot18Basket_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	func_808B7AEC(pthis);
	Actor_SpawnAsChild(
	    &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_BG_SPOT18_FUTA, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y, pthis->dyna.actor.world.pos.z, pthis->dyna.actor.shape.rot.x, pthis->dyna.actor.shape.rot.y + 0x1555,
...
void func_808B7BCC(BgSpot18Basket* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->colliderJntSph.base.acFlags & AC_HIT)
	{
		...
		if(colliderBaseAc != NULL)
		{
			...
			{
				if(Math3D_Dist2DSq(colliderBaseAc->world.pos.z, pthis->colliderJntSph.base.ac->world.pos.x, pthis->dyna.actor.world.pos.z, pthis->dyna.actor.world.pos.x) < SQ(32.0f))
				{
...
void func_808B81A0(BgSpot18Basket* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_216 == 1)
	{
		...
		else if(pthis->unk_218 == 2)
		{
			...
			{
				collectible = Item_DropCollectible(globalCtx, &tempVector, ((actor->params & 0x3F) << 8) | ITEM00_HEART_PIECE);
				if(collectible != NULL)
				{
					collectible->actor.velocity.y = 11.0f;
					collectible->actor.world.rot.y = D_808B85E4[1];
