...
static Vec3f dustPosOffsets_91[] = {
    {11.0f, -11.0f, -6.0f}, {0.0f, 14.0f, -13.0f}, {14.0f, -2.0f, -10.0f}, {10.0f, -6.0f, -8.0f}, {8.0f, 6.0f, 8.0f}, {13.0f, 8.0f, -10.0f}, {-14.0f, 1.0f, -14.0f}, {5.0f, 12.0f, -9.0f}, {11.0f, 6.0f, -7.0f}, {14.0f, 14.0f, -14.0f},
};
...
static void* sMouthTextures[] = {gDaruniaMouthSeriousTex, gDaruniaMouthGrinningTex, gDaruniaMouthOpenTex, gDaruniaMouthHappyTex};
...
void DemoDu_CsFireMedallion_SpawnDoorWarp(DemoDu* pthis, GlobalContext* globalCtx)
{
	...
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DOOR_WARP1, posX, posY, posZ, 0, 0, 0, WARP_SAGES);
}
...
void func_80969F38(DemoDu* pthis, GlobalContext* globalCtx)
{
	...
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_EFFECT, posX, posY, posZ, 0, 0, 0, DEMO_EFFECT_MEDAL_FIRE);
...
void DemoDu_CsFireMedallion_AdvanceTo04(DemoDu* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		...
		{
			Animation_Change(&pthis->skelAnime, &gDaruniaItemGiveAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaItemGiveAnim), 2, 0.0f);
...
void DemoDu_CsFireMedallion_AdvanceTo05(DemoDu* pthis, s32 animFinished)
{
	if(animFinished)
	{
		Animation_Change(&pthis->skelAnime, &gDaruniaItemGiveIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaItemGiveIdleAnim), 0, 0.0f);
...
{
	Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_EN_GOLON_LAND_BIG);
}
...
void DemoDu_CsPlaySfx_DaruniaFalling(GlobalContext* globalCtx)
{
	...
	{
		Common_PlaySfx2(NA_SE_EV_OBJECT_FALL);
	}
}
...
void DemoDu_CsPlaySfx_DaruniaHitsLink(GlobalContext* globalCtx)
{
	...
	Common_PlaySfxAtPos(&player->actor.projectedPos, NA_SE_EN_DARUNIA_HIT_LINK);
	Audio_PlaySoundGeneral(NA_SE_VO_LI_DAMAGE_S_KID, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
}

// Cutscene: Darunia gives Link the Goron's Ruby.
void DemoDu_CsPlaySfx_HitBreast(DemoDu* pthis)
{
	Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_EN_DARUNIA_HIT_BREAST - SFX_FLAG);
}
...
void DemoDu_CsPlaySfx_LinkEscapeFromGorons(GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.frames == 1400)
	{
		...
		Audio_PlaySoundGeneral(NA_SE_VO_LI_FALL_L_KID, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
...
void DemoDu_CsPlaySfx_LinkSurprised(GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.frames == 174)
	{
		...
		Audio_PlaySoundGeneral(NA_SE_VO_LI_SURPRISE_KID, &player->actor.projectedPos, 4U, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
...
void DemoDu_CsGoronsRuby_SpawnDustWhenHittingLink(DemoDu* pthis, GlobalContext* globalCtx)
{
	if(Animation_OnFrame(&pthis->skelAnime, 31.0f) || Animation_OnFrame(&pthis->skelAnime, 41.0f))
	{
		...
		for(i = 4; i >= 0; --i)
		{
			...
			func_8002829C(globalCtx, &position, &velocity, &accel, &primColor, &envColor, Rand_ZeroOne() * 40.0f + 200.0f, 0);
		}
...
void DemoDu_CsGoronsRuby_AdvanceTo02(DemoDu* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		...
		if((npcAction != NULL) && (npcAction->action != 1))
		{
			Animation_Change(&pthis->skelAnime, &gDaruniaStandUpAfterFallingAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaStandUpAfterFallingAnim), 2, 0.0f);
...
void DemoDu_CsGoronsRuby_AdvanceTo05(DemoDu* pthis, s32 animFinished)
{
	if(animFinished)
	{
		Animation_Change(&pthis->skelAnime, &gDaruniaIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaIdleAnim), ANIMMODE_LOOP, 0.0f);
...
void DemoDu_CsGoronsRuby_AdvanceTo06(DemoDu* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		...
		{
			Animation_Change(&pthis->skelAnime, &gDaruniaHitBreastAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaHitBreastAnim), 2, -4.0f);
...
void DemoDu_CsGoronsRuby_AdvanceTo08(DemoDu* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		...
		{
			Animation_Change(&pthis->skelAnime, &gDaruniaHitLinkAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaHitLinkAnim), 2, 0.0f);
...
void DemoDu_CsGoronsRuby_AdvanceTo09(DemoDu* pthis, s32 animFinished)
{
	if(animFinished)
	{
		Animation_Change(&pthis->skelAnime, &gDaruniaHitBreastAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gDaruniaHitBreastAnim), 2, 0.0f);
...
{
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, kREG(16) + 22.0f + pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 3);
}
...
void DemoDu_Draw_02(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_demo_du_inKenjyanomaDemo02.c", 275);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_demo_du_inKenjyanomaDemo02.c", 304);
}
...
static DemoDuActionFunc sUpdateFuncs[] = {
    DemoDu_UpdateCs_FM_00, DemoDu_UpdateCs_FM_01, DemoDu_UpdateCs_FM_02, DemoDu_UpdateCs_FM_03, DemoDu_UpdateCs_FM_04, DemoDu_UpdateCs_FM_05, DemoDu_UpdateCs_FM_06, DemoDu_UpdateCs_GR_00, DemoDu_UpdateCs_GR_01, DemoDu_UpdateCs_GR_02,
    DemoDu_UpdateCs_GR_03, DemoDu_UpdateCs_GR_04, DemoDu_UpdateCs_GR_05, DemoDu_UpdateCs_GR_06, DemoDu_UpdateCs_GR_07, DemoDu_UpdateCs_GR_08, DemoDu_UpdateCs_GR_09, DemoDu_UpdateCs_GR_10, DemoDu_UpdateCs_GR_11, DemoDu_UpdateCs_GR_12,
    DemoDu_UpdateCs_GR_13, DemoDu_UpdateCs_AG_00, DemoDu_UpdateCs_AG_01, DemoDu_UpdateCs_AG_02, DemoDu_UpdateCs_CR_00, DemoDu_UpdateCs_CR_01, DemoDu_UpdateCs_CR_02, DemoDu_UpdateCs_CR_03, DemoDu_UpdateCs_CR_04,
};
...
void DemoDu_Draw_01(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_demo_du.c", 615);
	...
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, NULL, NULL, pthis);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_demo_du.c", 638);
}
