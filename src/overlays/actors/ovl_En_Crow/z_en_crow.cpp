...
void EnCrow_SetupDamaged(EnCrow* pthis, GlobalContext* globalCtx)
{
	...
	{ // Ice arrows
		...
		for(i = 0; i < 8; i++)
		{
			...
			EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &iceParticlePos, 150, 150, 150, 250, 235, 245, 255, ((Rand_ZeroOne() * 0.15f) + 0.85f) * scale);
		}
	}
	else if(pthis->actor.colChkInfo.damageEffect == 2)
...
void EnCrow_SetupRespawn(EnCrow* pthis)
{
	if(sDeathCount == 10)
	{
		...
		sDeathCount = 0;
		pthis->collider.elements[0].dim.worldSphere.radius = sJntSphInit.elements[0].dim.modelSphere.radius * 0.03f * 100.0f;
	}
	else
	{
...
void EnCrow_FlyIdle(EnCrow* pthis, GlobalContext* globalCtx)
{
	...
	if((Math_SmoothStepToS(&pthis->actor.shape.rot.y, pthis->aimRotY, 5, 0x300, 0x10) == 0) && skelanimeUpdated && (Rand_ZeroOne() < 0.1f))
	{
	...
	if((Math_SmoothStepToS(&pthis->actor.shape.rot.x, pthis->aimRotX, 10, 0x100, 8) == 0) && (skelanimeUpdated) && (Rand_ZeroOne() < 0.1f))
	{
	...
	if((pthis->timer == 0) && (pthis->actor.xzDistToPlayer < 300.0f) && !(player->stateFlags1 & PLAYER_STATE_HORSE_MOUNTED) && (pthis->actor.yDistToWater < -40.0f) && (Player_GetMask(globalCtx) != PLAYER_MASK_SKULL))
	{
...
void EnCrow_DiveAttack(EnCrow* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer == 0) || (Player_GetMask(globalCtx) == PLAYER_MASK_SKULL) || (pthis->collider.base.atFlags & AT_HIT) || (pthis->actor.bgCheckFlags & (BG_STATE_0 | BG_STATE_3)) || (player->stateFlags1 & PLAYER_STATE_HORSE_MOUNTED) || (pthis->actor.yDistToWater > -40.0f))
	{
...
void EnCrow_Damaged(EnCrow* pthis, GlobalContext* globalCtx)
{
	...
	if(!(pthis->actor.flags & ACTOR_FLAG_15))
	{
		...
		{
			EffectSsDeadDb_Spawn(globalCtx, &pthis->actor.world.pos, &sZeroVecAccel, &sZeroVecAccel, pthis->actor.scale.x * 10000.0f, 0, 255, 255, 255, 255, 255, 0, 0, 1, 9, 1);
