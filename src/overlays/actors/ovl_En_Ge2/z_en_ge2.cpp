...
static EnGe2ActionFunc sActionFuncs[] = {
    EnGe2_Walk, EnGe2_AboutTurn, EnGe2_TurnPlayerSpotted, EnGe2_KnockedOut, EnGe2_CaptureTurn, EnGe2_CaptureCharge, EnGe2_CaptureClose, EnGe2_Stand, EnGe2_WaitLookAtPlayer,
};

static AnimationHeader* sAnimations[] = {
    &gGerudoPurpleWalkingAnim,	&gGerudoPurpleLookingAboutAnim, &gGerudoPurpleLookingAboutAnim, &gGerudoPurpleFallingToGroundAnim, &gGerudoPurpleLookingAboutAnim,
    &gGerudoPurpleChargingAnim, &gGerudoPurpleLookingAboutAnim, &gGerudoPurpleLookingAboutAnim, &gGerudoPurpleLookingAboutAnim,
};

static u8 sAnimModes[] = {
    ANIMMODE_LOOP, ANIMMODE_ONCE, ANIMMODE_LOOP, ANIMMODE_ONCE, ANIMMODE_LOOP, ANIMMODE_LOOP, ANIMMODE_LOOP, ANIMMODE_LOOP, ANIMMODE_ONCE,
};

void EnGe2_ChangeAction(EnGe2* pthis, s32 i)
{
	pthis->actionFunc = sActionFuncs[i];
	Animation_Change(&pthis->skelAnime, sAnimations[i], 1.0f, 0.0f, Animation_GetLastFrame(sAnimations[i]), sAnimModes[i], -8.0f);
...
void EnGe2_KnockedOut(EnGe2* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->stateFlags & GE2_STATE_ANIMCOMPLETE)
	{
		...
		EffectSsKiraKira_SpawnDispersed(globalCtx, &effectPos, &effectVelocity_62, &effectAccel_62, &effectPrimColor_62, &effectEnvColor_62, 1000, 16);
	}
}
...
{
	if((ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)) <= 0x4300) && (pthis->actor.xzDistToPlayer < 200.0f))
	{
		func_80038290(globalCtx, &pthis->actor, &pthis->headRot, &pthis->unk_2EE, pthis->actor.focus.pos);
	}
	else
...
void EnGe2_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(Ge2_DetectPlayerInUpdate(globalCtx, pthis, &pthis->actor.focus.pos, pthis->actor.shape.rot.y, pthis->yDetectRange))
		{
			// "Discovered!"
...
void EnGe2_UpdateStunned(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if((pthis->collider.base.acFlags & 2) && ((pthis->collider.info.acHitInfo == NULL) || !(pthis->collider.info.acHitInfo->toucher.dmgFlags & 0x80)))
	{
