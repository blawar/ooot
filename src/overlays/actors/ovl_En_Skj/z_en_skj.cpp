...
static SkullkidAnimationEntry sSkullKidAnimations[] = {
    {&gSkullKidBackflipAnim, ANIMMODE_ONCE, 0.0f}, {&gSkullKidShootNeedleAnim, ANIMMODE_ONCE, 0.0f},	  {&gSkullKidPlayFluteAnim, ANIMMODE_LOOP, 0.0f},      {&gSkullKidDieAnim, ANIMMODE_ONCE, 0.0f},	  {&gSkullKidHitAnim, ANIMMODE_ONCE, 0.0f},
    {&gSkullKidLandAnim, ANIMMODE_ONCE, 0.0f},	   {&gSkullKidLookLeftAndRightAnim, ANIMMODE_LOOP, 0.0f}, {&gSkullKidFightingStanceAnim, ANIMMODE_LOOP, 0.0f}, {&gSkullKidWalkToPlayerAnim, ANIMMODE_LOOP, 0.0f}, {&gSkullKidWaitAnim, ANIMMODE_LOOP, 0.0f},
};
...
void EnSkj_ChangeAnim(EnSkj* pthis, u8 animIndex)
{
	...
	Animation_Change(&pthis->skelAnime, sSkullKidAnimations[animIndex].animation, 1.0f, 0.0f, endFrame, sSkullKidAnimations[animIndex].mode, sSkullKidAnimations[animIndex].morphFrames);
}
...
s32 EnSkj_ShootNeedle(EnSkj* pthis, GlobalContext* globalCtx)
{
	...
	needle = (EnSkjneedle*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_SKJNEEDLE, pos2.x, pos2.y, pos2.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z, 0);
	if(needle != NULL)
	{
...
void EnSkj_Fight(EnSkj* pthis, GlobalContext* globalCtx)
{
	...
	{
		yawDistToPlayer = Math_SmoothStepToS(&pthis->actor.shape.rot.y, pthis->actor.yawTowardsPlayer, 5, pthis->unk_2F0, 0);
...
void EnSkj_SpawnDeathEffect(EnSkj* pthis, GlobalContext* globalCtx)
{
	...
	EffectSsDeadDb_Spawn(globalCtx, &effectPos, &effectVel, &effectAccel, 100, 10, 255, 255, 255, 255, 0, 0, 255, 1, 9, 1);
}
...
void EnSkj_WaitForSong(EnSkj* pthis, GlobalContext* globalCtx)
{
	...
	if(!(gSaveContext.itemGetInf[1] & 0x40) && ((globalCtx->msgCtx.msgMode == MSGMODE_OCARINA_FAIL) || (globalCtx->msgCtx.msgMode == MSGMODE_OCARINA_FAIL_NO_TEXT)))
	{
...
void EnSkj_WaitForPlayback(EnSkj* pthis, GlobalContext* globalCtx)
{
	...
	else if(globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_0F)
	{ // completed the game
		Common_PlaySfx(NA_SE_SY_CORRECT_CHIME);
		Message_CloseTextbox(globalCtx);
		globalCtx->msgCtx.ocarinaMode = OCARINA_MODE_04;
		...
		switch(globalCtx->msgCtx.msgMode)
		{
			case MSGMODE_MEMORY_GAME_LEFT_SKULLKID_WAIT:
				...
				if(!Audio_IsSfxPlaying(NA_SE_SY_METRONOME))
				{
					...
					{
						sOcarinaMinigameSkullKids[SKULL_KID_RIGHT].skullkid->minigameState = SKULL_KID_OCARINA_PLAY_NOTES;
					}
...
void EnSkj_CleanupOcarinaGame(EnSkj* pthis, GlobalContext* globalCtx)
{
	...
	if((sOcarinaMinigameSkullKids[SKULL_KID_LEFT].unk0 == 2) && (sOcarinaMinigameSkullKids[SKULL_KID_RIGHT].unk0 == 2))
	{
		func_800F5C2C();
		Actor_Kill(&pthis->actor);
	}
