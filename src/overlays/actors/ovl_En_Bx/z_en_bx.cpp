...
void EnBx_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisx->xzDistToPlayer <= 70.0f) || (pthis->collider.base.atFlags & AT_HIT) ||
        (pthis->collider.base.acFlags & AC_HIT) || (pthis->colliderQuad.base.atFlags & AT_HIT)) {
        if ((thisx->xzDistToPlayer <= 70.0f) || (&player->actor == pthis->collider.base.at) ||
            (&player->actor == pthis->collider.base.ac) || (&player->actor == pthis->colliderQuad.base.at)) {
            tmp33 = player->invincibilityTimer & 0xFF;
            tmp32 = thisx->world.rot.y;
            if (!(thisx->params & 0x80)) {
                tmp32 = thisx->yawTowardsPlayer;
            }
            if ((&player->actor != pthis->collider.base.at) && (&player->actor != pthis->collider.base.ac) &&
                (&player->actor != pthis->colliderQuad.base.at) && (player->invincibilityTimer <= 0)) {
