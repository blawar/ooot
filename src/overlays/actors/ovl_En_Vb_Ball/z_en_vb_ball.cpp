...
void EnVbBall_SpawnDebris(GlobalContext* globalCtx, BossFdEffect* effect, Vec3f* position, Vec3f* velocity, Vec3f* acceleration, f32 scale)
{
	s16 i;
...
void EnVbBall_SpawnDust(GlobalContext* globalCtx, BossFdEffect* effect, Vec3f* position, Vec3f* velocity, Vec3f* acceleration, f32 scale)
{
	s16 i;
...
void EnVbBall_UpdateBones(EnVbBall* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.bgCheckFlags & BG_STATE_0) && (pthis->actor.velocity.y <= 0.0f))
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EN_VALVAISA_LAND, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
		for(i = 0; i < 10; i++)
		{
			...
			EnVbBall_SpawnDust(globalCtx, bossFd->effects, &dustPos, &dustVel, &dustAcc, Rand_ZeroFloat(80.0f) + 200.0f);
		}
	}
			...
			if((pthis->actor.params == 100) || (pthis->actor.params == 101))
			{
				...
				for(i = 0; i < spawnNum; i++)
				{
					...
					newActor = (EnVbBall*)Actor_SpawnAsChild(
					    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_VB_BALL, pthis->actor.world.pos.x + spawnOffset.x, pthis->actor.world.pos.y + spawnOffset.y, pthis->actor.world.pos.z + spawnOffset.z, 0, 0,
					    pthis->actor.world.rot.z * 0.5f, pthis->actor.params + 1);
					if(newActor != NULL)
					{
						if((i == 0) && (pthis->actor.params == 100))
						{
							Audio_PlaySoundGeneral(NA_SE_EN_VALVAISA_ROCK, &newActor->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
						}
				...
				for(i = 0; i < 15; i++)
				{
					...
					EnVbBall_SpawnDebris(globalCtx, bossFd->effects, &debrisPos1, &debrisVel1, &debrisAcc1, (s16)Rand_ZeroFloat(12.0f) + 15);
				}
				for(i = 0; i < 10; i++)
				{
					Vec3f dustVel = {0.0f, 0.0f, 0.0f};
					...
					EnVbBall_SpawnDust(globalCtx, bossFd->effects, &dustPos, &dustVel, &dustAcc, Rand_ZeroFloat(100.0f) + 350.0f);
				}
			}
			else
			{
				for(i = 0; i < 5; i++)
				{
					...
					EnVbBall_SpawnDebris(globalCtx, bossFd->effects, &debrisPos2, &debrisVel2, &debrisAcc2, (s16)Rand_ZeroFloat(12.0f) + 15);
				}
				Actor_Kill(&pthis->actor);
			}
		}
