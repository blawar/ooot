...
static s16 entrances[] = {
    0x036D, 0x003F, 0x0598, 0x059C, 0x05A0, 0x05A4, 0x05A8, 0x05AC, 0x05B0, 0x05B4, 0x05B8, 0x05BC, 0x05C0, 0x05C4, 0x05FC,
};
...
void DoorAna_WaitOpen(DoorAna* pthis, GlobalContext* globalCtx)
{
	...
	{
		if((pthis->actor.targetMode != 0) && (globalCtx->sceneLoadFlag == 0) && (player->stateFlags1 & PLAYER_STATE1_31) && (player->unk_84F == 0))
		{
		...
		{
			if(!Player_InCsMode(globalCtx) && !(player->stateFlags1 & (PLAYER_STATE_HORSE_MOUNTED | PLAYER_STATE_SWIMMING)) && pthis->actor.xzDistToPlayer <= 15.0f && -50.0f <= pthis->actor.yDistToPlayer && pthis->actor.yDistToPlayer <= 15.0f)
			{
...
void DoorAna_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	OPEN_DISPS(globalCtx->gfxCtx, "../z_door_ana.c", 440);

	func_80093D84(globalCtx->gfxCtx);
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_door_ana.c", 446), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, gGrottoDL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_door_ana.c", 449);
}
