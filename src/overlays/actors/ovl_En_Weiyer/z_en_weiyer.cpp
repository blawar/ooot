...
ActorInit En_Weiyer_InitVars = {
    ACTOR_EN_WEIYER, ACTORCAT_ENEMY, FLAGS, OBJECT_EI, ACTOR_FACTORY(EnWeiyer), (ActorFunc)EnWeiyer_Init, (ActorFunc)EnWeiyer_Destroy, (ActorFunc)EnWeiyer_Update, (ActorFunc)EnWeiyer_Draw, (ActorFunc)EnWeiyer_Reset,
};
...
void EnWeiyer_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gStingerSkel, &gStingerIdleAnim, pthis->jointTable, pthis->morphTable, 19);
...
void func_80B32804(EnWeiyer* pthis, GlobalContext* globalCtx)
{
	...
	if(!WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &pthis->actor.home.pos.y, &waterBox) || ((pthis->actor.home.pos.y - 5.0f) <= pthis->actor.floorHeight))
	{
...
void func_80B328E8(EnWeiyer* pthis, GlobalContext* globalCtx)
{
	...
	if(Math_ScaledStepToS(&pthis->actor.shape.rot.y, pthis->unk_196, 182))
	{
		...
		if(pthis->unk_194 == 0)
		{
			pthis->unk_196 = Rand_S16Offset(0x2000, 0x2000) * ((Rand_ZeroOne() < 0.5f) ? -1 : 1) + pthis->actor.shape.rot.y;
			pthis->unk_194 = 30;
	...
	if(pthis->actor.home.pos.y < pthis->actor.world.pos.y)
	{
		...
		else
		{
			...
			pthis->unk_280 = Rand_ZeroOne() * ((pthis->actor.home.pos.y - pthis->actor.floorHeight) / 2.0f) + pthis->actor.floorHeight;
		}
	}
	else
	{
		...
		{
			pthis->unk_280 = Rand_ZeroOne() * (pthis->actor.home.pos.y - pthis->actor.floorHeight) + pthis->actor.floorHeight;
		}
...
void func_80B32E34(EnWeiyer* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->unk_194 == 0) || ((pthis->actor.home.pos.y + 20.0f) <= player->actor.world.pos.y) || (pthis->collider.base.atFlags & AT_HIT))
	{
		func_80B32538(pthis);
	}
	else
	{
		...
		if((player->actor.yDistToWater < 50.0f) && (pthis->actor.yDistToWater < 20.0f) && Actor_IsFacingPlayer(&pthis->actor, 0x2000))
		{
			func_80B327D8(pthis);
		}
	}
...
void func_80B33018(EnWeiyer* pthis, GlobalContext* globalCtx)
{
	...
	Math_ScaledStepToS(&pthis->actor.shape.rot.x, 0, 0x800);
	curFrame = pthis->skelAnime.curFrame;
	Math_StepToF(&pthis->unk_27C, (pthis->actor.home.pos.y - pthis->actor.floorHeight) / 4.0f + pthis->actor.floorHeight, 1.0f);
	pthis->actor.world.pos.y = pthis->unk_27C - cosf((curFrame - 5.0f) * (M_PI / 40)) * 3.0f;
...
s32 EnWeiyer_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	if(limbIndex == 1)
	{
		pos->z += 2000.0f;
...
void EnWeiyer_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_weiyer.c", 1193);
	...
	if(pthis->actionFunc != func_80B33338)
	{
		...
		POLY_OPA_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnWeiyer_OverrideLimbDraw, NULL, &pthis->actor, POLY_OPA_DISP);
	}
	else
	{
		...
		POLY_XLU_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnWeiyer_OverrideLimbDraw, NULL, &pthis->actor, POLY_XLU_DISP);
	}

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_weiyer.c", 1240);
}
