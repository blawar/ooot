...
void EnTr_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		case TR_KOUME:
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_tr_Skel_011688, &object_tr_Anim_003FC8, pthis->jointTable, pthis->morphTable, 27);
		...
		case TR_KOTAKE:
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_tr_Skel_00C530, &object_tr_Anim_001CDC, pthis->jointTable, pthis->morphTable, 27);
...
void EnTr_CrySpellcast(EnTr* pthis, GlobalContext* globalCtx)
{
	if(pthis->timer == 11)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EN_TWINROBA_SHOOT_VOICE, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
	}
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		if(globalCtx->csCtx.npcActions[pthis->actionIndex] != NULL)
		{
			switch(globalCtx->csCtx.npcActions[pthis->actionIndex]->action)
			{
				...
				case 6:
					Animation_Change(&pthis->skelAnime, D_80B24380[pthis->actor.params], 1.0f, 0.0f, Animation_GetLastFrame(D_80B24380[pthis->actor.params]), ANIMMODE_ONCE, -5.0f);
					EnTr_SetupAction(pthis, EnTr_CrySpellcast);
					pthis->animation = D_80B24378[pthis->actor.params];
					pthis->timer = 39;
					Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, pthis->actor.params + 9);
...
void EnTr_WaitToReappear(EnTr* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		if((globalCtx->csCtx.npcActions[pthis->actionIndex] != NULL) && ((globalCtx->csCtx.npcActions[pthis->actionIndex]->action == 3) || (globalCtx->csCtx.npcActions[pthis->actionIndex]->action == 5)))
		{
...
void EnTr_TakeOff(EnTr* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		if((globalCtx->csCtx.npcActions[pthis->actionIndex] != NULL) && (globalCtx->csCtx.npcActions[pthis->actionIndex]->action == 3))
		{
			Animation_Change(&pthis->skelAnime, D_80B24378[pthis->actor.params], 1.0f, 0.0f, lastFrame, ANIMMODE_LOOP, -10.0f);
			pthis->animation = NULL;
			EnTr_SetupAction(pthis, EnTr_ChooseAction2);
		}
	}
...
void EnTr_TurnLookOverShoulder(EnTr* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		if((globalCtx->csCtx.npcActions[pthis->actionIndex] != NULL) && (globalCtx->csCtx.npcActions[pthis->actionIndex]->action == 2))
		{
			Animation_Change(&pthis->skelAnime, D_80B24368[pthis->actor.params], 1.0f, 0.0f, lastFrame, ANIMMODE_ONCE, -4.0f);
		...
		if(globalCtx->csCtx.npcActions[pthis->actionIndex] != NULL)
		{
			switch(globalCtx->csCtx.npcActions[pthis->actionIndex]->action)
			{
				...
				case 7:
					...
					Animation_PlayLoop(&pthis->skelAnime, &object_tr_Anim_0049C8);
					pthis->animation = NULL;
					pthis->timer = ((pthis->actor.params != TR_KOUME) ? ((u8)frames * 0x400) + 0x8000 : (u8)frames * 0x400);
					break;
			}
		}
	}
...
void EnTr_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime) != 0)
	{
		if(pthis->animation != NULL)
		{
			...
			{
				EnTr_SetupAction(pthis, EnTr_ChooseAction2);
				Animation_Change(&pthis->skelAnime, &object_tr_Anim_0049C8, 1.0f, 0.0f, Animation_GetLastFrame(&object_tr_Anim_0049C8), ANIMMODE_LOOP, -5.0f);
			}
			else
			{
				Animation_PlayLoop(&pthis->skelAnime, pthis->animation);
...
void EnTr_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		OPEN_DISPS(globalCtx->gfxCtx, "../z_en_tr.c", 840);
		...
		SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnTr_OverrideLimbDraw, NULL, pthis);
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_tr.c", 854);
	}
}
