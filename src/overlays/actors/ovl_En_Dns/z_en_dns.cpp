...
static const char* D_809F0424[] = {
    "デクの実売り            ", "デクの棒売り            ", "ハートの欠片売り        ", "デクの種売り            ", "デクの盾売り            ", "バクダン売り            ",
    "矢売り                  ", "赤のくすり売り          ", "緑のくすり売り          ", "デクの棒持てる数を増やす", "デクの実持てる数を増やす",
};
...
static DnsItemEntry* sItemEntries[] = {
    &D_809F0450, &D_809F0460, &D_809F0470, &D_809F0480, &D_809F0490, &D_809F04A0, &D_809F04B0, &D_809F04C0, &D_809F04D0, &D_809F04E0, &D_809F04F0,
};
...
void EnDns_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	// "Deku Salesman"
	osSyncPrintf(VT_FGCOL(GREEN) "◆◆◆ 売りナッツ『%s』 ◆◆◆" VT_RST "\n", D_809F0424[pthis->actor.params], pthis->actor.params);
	Actor_ProcessInitChain(&pthis->actor, sInitChain);
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gBusinessScrubSkel, &gBusinessScrubNervousTransitionAnim, pthis->jointTable, pthis->morphTable, 18);
...
void EnDns_Change(EnDns* pthis, u8 arg1)
{
	...
	Animation_Change(&pthis->skelAnime, sAnimInfo[arg1].anim, 1.0f, 0.0f, (f32)frameCount, sAnimInfo[arg1].mode, sAnimInfo[arg1].transitionRate);
}
