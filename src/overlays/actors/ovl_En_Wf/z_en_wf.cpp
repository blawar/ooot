...
s32 EnWf_ChangeAction(GlobalContext* globalCtx, EnWf* pthis, s16 mustChoose) {
    ...
    if (func_800354B4(globalCtx, &pthis->actor, 100.0f, 0x5DC0, 0x2AA8, pthis->actor.shape.rot.y)) {
        ...
        if ((pthis->actor.bgCheckFlags & 8) && (ABS(wallYawDiff) < 0x2EE0) && (pthis->actor.xzDistToPlayer < 120.0f)) {
    ...
    if (explosive != NULL) {
        ...
        if (((pthis->actor.bgCheckFlags & 8) && (wallYawDiff < 0x2EE0)) || (explosive->id == ACTOR_EN_BOM_CHU)) {
    ...
    if (mustChoose) {
        ...
        if ((pthis->actor.xzDistToPlayer <= 80.0f) && !Actor_OtherIsTargeted(globalCtx, &pthis->actor) &&
...
void EnWf_RunAtPlayer(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    if (!EnWf_DodgeRanged(globalCtx, pthis)) {
        ...
        if ((pthis->actor.xzDistToPlayer < (150.0f + baseRange)) && (player->swordState != 0) &&
        ...
        if (!Actor_IsFacingPlayer(&pthis->actor, 0x11C7)) {
            ...
            if (!Actor_OtherIsTargeted(globalCtx, &pthis->actor) &&
                ((Rand_ZeroOne() > 0.03f) || ((pthis->actor.xzDistToPlayer <= 80.0f) && (ABS(temp_v1) < 0x38E0)))) {
                EnWf_SetupSlash(pthis);
...
void EnWf_RunAroundPlayer(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    if (!EnWf_DodgeRanged(globalCtx, pthis) && !EnWf_ChangeAction(globalCtx, pthis, false)) {
        ...
        if ((pthis->actor.bgCheckFlags & 8) ||
            !Actor_TestFloorInDirection(&pthis->actor, globalCtx, pthis->actor.speedXZ, pthis->actor.shape.rot.y)) {
            angle2 = (pthis->actor.bgCheckFlags & 8)
                         ? (pthis->actor.wallYaw - pthis->actor.yawTowardsPlayer) - pthis->runAngle
                         : 0;
        ...
        if ((animPrevFrame != (s32)pthis->skelAnime.curFrame) && (animFrameSpeedDiff <= 0) &&
        ...
        if ((Math_CosS(angle1 - pthis->actor.shape.rot.y) < -0.85f) && !Actor_OtherIsTargeted(globalCtx, &pthis->actor) &&
...
void EnWf_Slash(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    if (((curFrame == 15) && !Actor_IsTargeted(globalCtx, &pthis->actor) &&
         (!Actor_IsFacingPlayer(&pthis->actor, 0x2000) || (pthis->actor.xzDistToPlayer >= 100.0f))) ||
        SkelAnime_Update(&pthis->skelAnime)) {
...
void EnWf_Damaged(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    if (!EnWf_ChangeAction(globalCtx, pthis, false) && SkelAnime_Update(&pthis->skelAnime)) {
        if (pthis->actor.bgCheckFlags & 1) {
            ...
            if ((pthis->actor.bgCheckFlags & 8) && (ABS(angleToWall) < 12000) && (pthis->actor.xzDistToPlayer < 120.0f)) {
                EnWf_SetupSomersaultAndAttack(pthis);
...
void EnWf_Blocking(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&pthis->skelAnime)) {
        ...
        if ((ABS(yawDiff) <= 0x4000) && (pthis->actor.xzDistToPlayer < 60.0f) &&
...
void EnWf_Sidestep(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    if ((pthis->actor.bgCheckFlags & 8) ||
        !Actor_TestFloorInDirection(&pthis->actor, globalCtx, pthis->actor.speedXZ, pthis->actor.shape.rot.y)) {
        s16 angle =
            (pthis->actor.bgCheckFlags & 8) ? (pthis->actor.wallYaw - pthis->actor.yawTowardsPlayer) - pthis->runAngle : 0;
...
void EnWf_UpdateDamage(EnWf* pthis, GlobalContext* globalCtx) {
    ...
    } else if ((pthis->colliderCylinderBody.base.acFlags & AC_HIT) ||
               (pthis->colliderCylinderTail.base.acFlags & AC_HIT)) {
        if (pthis->action >= WOLFOS_ACTION_WAIT) {
            s16 yawDiff = pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y;

            if ((!(pthis->colliderCylinderBody.base.acFlags & AC_HIT) &&
                 (pthis->colliderCylinderTail.base.acFlags & AC_HIT)) ||
                (ABS(yawDiff) > 19000)) {
            ...
            if (pthis->actor.colChkInfo.damageEffect != ENWF_DMGEFF_ICE_MAGIC) {
                ...
                if ((pthis->actor.colChkInfo.damageEffect == ENWF_DMGEFF_STUN) ||
                    (pthis->actor.colChkInfo.damageEffect == ENWF_DMGEFF_UNDEF)) {
...
void EnWf_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((pthis->action != WOLFOS_ACTION_WAIT_TO_APPEAR) || !pthis->unk_300) {
        ...
        SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                              pthis->skelAnime.dListCount, EnWf_OverrideLimbDraw, EnWf_PostLimbDraw, &pthis->actor);
