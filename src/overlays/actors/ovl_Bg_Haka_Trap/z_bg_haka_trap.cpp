...
static Gfx* sDLists_59[5] = {
    object_haka_objects_DL_007610, object_haka_objects_DL_009860, object_haka_objects_DL_007EF0, object_haka_objects_DL_008A20, object_haka_objects_DL_0072C0,
};
...
void BgHakaTrap_Destroy(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.params != HAKA_TRAP_PROPELLER)
	{
		if(pthis->dyna.actor.params != HAKA_TRAP_GUILLOTINE_SLOW)
		{
			...
			if((pthis->dyna.actor.params == HAKA_TRAP_SPIKED_WALL) || (pthis->dyna.actor.params == HAKA_TRAP_SPIKED_WALL_2))
			{
...
void func_808802D8(BgHakaTrap* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 2; i++)
	{
		...
		EffectSsDeadDb_Spawn(globalCtx, &vector, &zeroVec_48, &zeroVec_48, 130, 20, 255, 255, 150, 170, 255, 0, 0, 1, 9, false);
	}
...
void func_80880484(BgHakaTrap* pthis, GlobalContext* globalCtx)
{
	...
	sp24 = Math_StepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y - 185.0f, pthis->dyna.actor.velocity.y);
	timer = pthis->timer;
...
void func_808806BC(BgHakaTrap* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 3; i++)
	{
		temp = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->dyna.actor.floorPoly, &sp64, &pthis->dyna.actor, &vector) - 25.0f;
		if(tempf20 < temp)
		{
			tempf20 = temp;
		}
...
void func_808809E4(BgHakaTrap* pthis, GlobalContext* globalCtx, s16 arg2)
{
	...
	if((fabsf(sp18.x) < 70.0f) && (fabsf(sp18.y) < 100.0f) && (sp18.z < 500.0f) && (GET_PLAYER(globalCtx)->currentBoots != PLAYER_BOOTS_IRON))
	{
