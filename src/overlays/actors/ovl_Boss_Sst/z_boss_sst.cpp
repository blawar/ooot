...
static Vec3f sCameraAtPoints[] = {
    {ROOM_CENTER_X - 50.0f, ROOM_CENTER_Y + 300.0f, ROOM_CENTER_Z + 0.0f},   {ROOM_CENTER_X + 150.0f, ROOM_CENTER_Y + 300.0f, ROOM_CENTER_Z + 100.0f}, {ROOM_CENTER_X + 0.0f, ROOM_CENTER_Y + 600.0f, ROOM_CENTER_Z + 100.0f},
    {ROOM_CENTER_X + 50.0f, ROOM_CENTER_Y + 400.0f, ROOM_CENTER_Z + 200.0f}, {ROOM_CENTER_X + 50.0f, ROOM_CENTER_Y + 200.0f, ROOM_CENTER_Z + 200.0f},  {ROOM_CENTER_X - 50.0f, ROOM_CENTER_Y + 0.0f, ROOM_CENTER_Z + 200.0f},
    {ROOM_CENTER_X - 150.0f, ROOM_CENTER_Y + 0.0f, ROOM_CENTER_Z + 100.0f},  {ROOM_CENTER_X - 60.0f, ROOM_CENTER_Y + 180.0f, ROOM_CENTER_Z + 730.0f},
};

static Vec3f sCameraEyePoints[] = {
    {ROOM_CENTER_X + 250.0f, ROOM_CENTER_Y + 800.0f, ROOM_CENTER_Z + 800.0f}, {ROOM_CENTER_X - 150.0f, ROOM_CENTER_Y + 700.0f, ROOM_CENTER_Z + 1400.0f}, {ROOM_CENTER_X + 250.0f, ROOM_CENTER_Y + 100.0f, ROOM_CENTER_Z + 750.0f},
    {ROOM_CENTER_X + 50.0f, ROOM_CENTER_Y + 200.0f, ROOM_CENTER_Z + 900.0f},  {ROOM_CENTER_X + 50.0f, ROOM_CENTER_Y + 200.0f, ROOM_CENTER_Z + 900.0f},	 {ROOM_CENTER_X + 350.0f, ROOM_CENTER_Y + 400.0f, ROOM_CENTER_Z + 1200.0f},
    {ROOM_CENTER_X - 50.0f, ROOM_CENTER_Y + 200.0f, ROOM_CENTER_Z + 800.0f},  {ROOM_CENTER_X - 50.0f, ROOM_CENTER_Y + 200.0f, ROOM_CENTER_Z + 800.0f},
};
...
void BossSst_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	{
		sFloor = (BgSstFloor*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_SST_FLOOR, sRoomCenter.x, sRoomCenter.y, sRoomCenter.z, 0, 0, 0, BONGOFLOOR_REST);
		SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gBongoHeadSkel, &gBongoHeadEyeOpenIdleAnim, pthis->jointTable, pthis->morphTable, 45);
		...
		{
			Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_WARP1, ROOM_CENTER_X, ROOM_CENTER_Y, ROOM_CENTER_Z + 400.0f, 0, 0, 0, WARP_DUNGEON_ADULT);
			Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_ITEM_B_HEART, ROOM_CENTER_X, ROOM_CENTER_Y, ROOM_CENTER_Z - 200.0f, 0, 0, 0, 0);
			Actor_Kill(&pthis->actor);
		}
		else
		{
			sHands[LEFT] = (BossSst*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BOSS_SST, pthis->actor.world.pos.x + 200.0f, pthis->actor.world.pos.y, pthis->actor.world.pos.z + 400.0f, 0, pthis->actor.shape.rot.y, 0, BONGO_LEFT_HAND);
			sHands[RIGHT] =
			    (BossSst*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BOSS_SST, pthis->actor.world.pos.x + (-200.0f), pthis->actor.world.pos.y, pthis->actor.world.pos.z + 400.0f, 0, pthis->actor.shape.rot.y, 0, BONGO_RIGHT_HAND);
	...
	else
	{
		...
		{
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gBongoLeftHandSkel, &gBongoLeftHandIdleAnim, pthis->jointTable, pthis->morphTable, 27);
			pthis->vParity = -1;
			pthis->colliderJntSph.elements[0].dim.modelSphere.center.z *= -1;
		}
		else
		{
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gBongoRightHandSkel, &gBongoRightHandIdleAnim, pthis->jointTable, pthis->morphTable, 27);
			pthis->vParity = 1;
		}
		...
		if(pthis->timer <= 198)
		{
			...
			else if(revealStateTimer >= 45)
			{
				...
				else if(revealStateTimer == 85)
				{
					if(!(gSaveContext.eventChkInf[7] & 0x80))
					{
						TitleCard_InitBossName(globalCtx, &globalCtx->actorCtx.titleCtx, SEGMENTED_TO_VIRTUAL(gBongoTitleCardTex), 160, 180, 128, 40);
					}
...
{
	SkelAnime_Update(&pthis->skelAnime);
	if(!pthis->ready && ((HAND_STATE(sHands[LEFT]) == HAND_BEAT) || (HAND_STATE(sHands[LEFT]) == HAND_WAIT)) && ((HAND_STATE(sHands[RIGHT]) == HAND_BEAT) || (HAND_STATE(sHands[RIGHT]) == HAND_WAIT)))
	{
		pthis->ready = true;
	}
	...
	{
		Math_ApproachS(&pthis->actor.shape.rot.y, Actor_WorldYawTowardPoint(&GET_PLAYER(globalCtx)->actor, &sRoomCenter) + 0x8000, 4, 0x400);
...
{
	SkelAnime_Update(&pthis->skelAnime);
	if(sHands[LEFT]->ready && (sHands[LEFT]->actionFunc == BossSst_HandReadyCharge) && sHands[RIGHT]->ready && (sHands[RIGHT]->actionFunc == BossSst_HandReadyCharge))
	{
...
void BossSst_HeadSetupCharge(BossSst* pthis)
{
	Animation_Change(&pthis->skelAnime, &gBongoHeadChargeAnim, 0.5f, 0.0f, Animation_GetLastFrame(&gBongoHeadChargeAnim), ANIMMODE_ONCE_INTERP, -5.0f);
...
void BossSst_HeadStunned(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	{
		pthis->actor.world.pos.y = (sinf((currentFrame - 11.0f) * (M_PI / 5)) * 50.0f) + (pthis->actor.home.pos.y - 250.0f);
	}
...
void BossSst_HeadDeath(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	else if(pthis->timer == 48)
	{
		...
		player->actor.world.pos.x = sRoomCenter.x + (400.0f * Math_SinS(pthis->actor.shape.rot.y)) + (Math_CosS(pthis->actor.shape.rot.y) * -120.0f);
		player->actor.world.pos.z = sRoomCenter.z + (400.0f * Math_CosS(pthis->actor.shape.rot.y)) - (Math_SinS(pthis->actor.shape.rot.y) * -120.0f);
...
void BossSst_HeadFinish(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	{
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_WARP1, ROOM_CENTER_X, ROOM_CENTER_Y, ROOM_CENTER_Z, 0, 0, 0, WARP_DUNGEON_ADULT);
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_ITEM_B_HEART, (Math_SinS(pthis->actor.shape.rot.y) * 200.0f) + ROOM_CENTER_X, ROOM_CENTER_Y, Math_CosS(pthis->actor.shape.rot.y) * 200.0f + ROOM_CENTER_Z, 0, 0, 0, 0);
...
void BossSst_HandOffbeatEnd(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		else if((sHead->actionFunc != BossSst_HeadIntro) && ((sHead->timer % 7) == 5) && ((sHead->timer % 28) != 5))
		{
			BossSst_HandSetupOffbeat(pthis);
		}
	}
...
void BossSst_HandRetreat(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if((Math_SmoothStepToF(&pthis->actor.world.pos.y, ROOM_CENTER_Y + 250.0f, 0.5f, 70.0f, 5.0f) < 1.0f) && inPosition && (diff < 10.0f))
		{
			pthis->timer = 8;
		}
	}
...
void BossSst_HandReadySlam(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(Math_StepToF(&pthis->actor.world.pos.y, ROOM_CENTER_Y + 300.0f, 30.0f) && (pthis->actor.xzDistToPlayer < 140.0f))
		{
			pthis->timer = 20;
		}
...
void BossSst_HandClap(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer != 0)
	{
		...
		if(pthis->timer == 0)
		{
			if(dropFlag_270)
			{
				Item_DropCollectible(globalCtx, &pthis->actor.world.pos, (Rand_ZeroOne() < 0.5f) ? ITEM00_ARROWS_SMALL : ITEM00_MAGIC_SMALL);
				dropFlag_270 = false;
			}
...
void BossSst_HandGrab(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.world.rot.y = ((1.0f - sinf(pthis->timer * (M_PI / 60.0f))) * (pthis->vParity * 0x2000)) + pthis->targetYaw;
...
void BossSst_HandSwing(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer == 4) && (pthis->amplitude == 0) && SkelAnime_Update(&pthis->skelAnime) && (player->stateFlags2 & PLAYER_STATE2_7))
	{
...
void BossSst_HandShake(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	if(HAND_STATE(OTHER_HAND(pthis)) == HAND_DAMAGED)
	{
		if((OTHER_HAND(pthis)->actionFunc == BossSst_HandShake) || (OTHER_HAND(pthis)->actionFunc == BossSst_HandReadyCharge))
		{
...
void BossSst_HandReadyCharge(BossSst* pthis, GlobalContext* globalCtx)
{
	if(!pthis->ready)
	{
		...
		pthis->ready &= Math_ScaledStepToS(&pthis->actor.shape.rot.y, pthis->actor.home.rot.y + (pthis->vParity * 0x1000), 0x800);
...
{
	SkelAnime_Update(&pthis->skelAnime);
	Math_ApproachF(&pthis->actor.world.pos.z, (Math_CosS(sHead->actor.shape.rot.y) * 200.0f) + pthis->actor.home.pos.z, 0.5f, 25.0f);
	Math_ApproachF(&pthis->actor.world.pos.x, (Math_SinS(sHead->actor.shape.rot.y) * 200.0f) + pthis->actor.home.pos.x, 0.5f, 25.0f);
	if(!pthis->ready)
	{
...
void BossSst_HandThrash(BossSst* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.world.pos.y = (((pthis->handAngSpeed / 256.0f) + 0.5f) * 150.0f) * (-1.0f / 0x800) * pthis->actor.shape.rot.x;
	if(pthis->timer == 0)
	{
...
void BossSst_UpdateHand(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		if((pthis->effects[0].move < 5) || (pthis->actor.xzDistToPlayer < ((pthis->effects[2].scale * 0.01f) * sCylinderInitHand.dim.radius)) || (pthis->colliderCyl.base.atFlags & AT_HIT))
		{
	...
	if((sHead->actionFunc != BossSst_HeadLurk) && (sHead->actionFunc != BossSst_HeadIntro) && (pthis->colliderJntSph.base.acFlags & AC_ON))
	{
	...
	if((HAND_STATE(pthis) != HAND_DEATH) && (HAND_STATE(pthis) != HAND_WAIT) && (HAND_STATE(pthis) != HAND_BEAT) && (HAND_STATE(pthis) != HAND_FROZEN))
	{
		pthis->trailCount++;
...
s32 BossSst_OverrideHandDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	BossSst* pthis = (BossSst*)thisx;
...
s32 BossSst_OverrideHandTrailDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* data, Gfx** gfx)
{
...
void BossSst_DrawHand(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, BossSst_OverrideHandDraw, BossSst_PostHandDraw, pthis);
	if(pthis->trailCount >= 2)
	{
		...
		for(i = 0; i < end; i++)
		{
			if(Math3D_Vec3fDistSq(&trail2->world.pos, &trail->world.pos) > 900.0f)
			{
				...
				gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x00, ((3 - i) * 10) + 20, 0, ((3 - i) * 20) + 50, ((3 - i) * 30) + 70);

				POLY_XLU_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, BossSst_OverrideHandTrailDraw, NULL, trail, POLY_XLU_DISP);
			}
			idx = (idx + 5) % 7;
			trail2 = trail;
			trail = &pthis->handTrails[idx];
...
s32 BossSst_OverrideHeadDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	BossSst* pthis = (BossSst*)thisx;
	s32 shakeAmp;
	s32 pad;
	...
	else if(pthis->actionFunc == BossSst_HeadThrash)
	...
	else if((pthis->actionFunc == BossSst_HeadDarken) || (pthis->actionFunc == BossSst_HeadFall) || (pthis->actionFunc == BossSst_HeadMelt))
	{
		if((limbIndex == 3) || (limbIndex == 39) || (limbIndex == 42))
		{
			rot->z -= 0x1000;
...
void BossSst_DrawHead(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc == BossSst_HeadThrash)
	{
		...
		Matrix_Scale((pthis->timer * 0.000375f) + 1.0f, 1.0f - (pthis->timer * 0.00075f), (pthis->timer * 0.000375f) + 1.0f, MTXMODE_APPLY);
		Matrix_RotateX(-randPitch, MTXMODE_APPLY);
		Matrix_RotateY(-randYaw, MTXMODE_APPLY);
	}
	...
	if(!CHECK_FLAG_ALL(pthis->actor.flags, ACTOR_FLAG_7))
	{
		POLY_OPA_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, BossSst_OverrideHeadDraw, BossSst_PostHeadDraw, pthis, POLY_OPA_DISP);
	}
	else
	{
		POLY_XLU_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, BossSst_OverrideHeadDraw, BossSst_PostHeadDraw, pthis, POLY_XLU_DISP);
	}
		...
		if(pthis->effectMode == BONGO_ICE)
		{
			...
			for(i = 0; i < 18; i++)
			{
				...
				if(effect->move)
				{
					...
					{
						Matrix_Translate(effect->pos.x + pthis->actor.world.pos.x, effect->pos.y + pthis->actor.world.pos.y, effect->pos.z + pthis->actor.world.pos.z, MTXMODE_NEW);
					}
