...
void EnAni_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gRoofManSkel, &gRoofManIdleAnim, pthis->jointTable, pthis->morphTable, 0x10);
...
void func_809B064C(EnAni* pthis, GlobalContext* globalCtx)
{
	...
	else if(yawDiff >= -0x36AF && yawDiff < 0 && pthis->actor.xzDistToPlayer < 150.0f && -80.0f < pthis->actor.yDistToPlayer)
	{
...
void func_809B07F8(EnAni* pthis, GlobalContext* globalCtx)
{
	...
	else if(yawDiff > -0x36B0 && yawDiff < 0 && pthis->actor.xzDistToPlayer < 150.0f && -80.0f < pthis->actor.yDistToPlayer)
	{
		if(gSaveContext.itemGetInf[1] & 0x20)
		{
			EnAni_SetText(pthis, globalCtx, 0x5056);
...
void func_809B0994(EnAni* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gRoofManGettingUpAfterKnockbackAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gRoofManGettingUpAfterKnockbackAnim), ANIMMODE_ONCE, -4.0f);
		pthis->unk_2AA++;
...
void func_809B0A6C(EnAni* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gRoofManKnockbackAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gRoofManKnockbackAnim), ANIMMODE_ONCE, 0.0f);
