...
void EnWallmas_SetupDrop(EnWallmas* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, objSegChangee, 0.0f, 20.0f, Animation_GetLastFrame(&gWallmasterLungeAnim), ANIMMODE_ONCE, 0.0f);
...
void EnWallmas_SetupLand(EnWallmas* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, objSegChangee, 1.0f, 41.0f, Animation_GetLastFrame(objSegFrameCount), ANIMMODE_ONCE, -3.0f);
...
void EnWallmas_SetupReturnToCeiling(EnWallmas* pthis)
{
	...
	Animation_Change(&pthis->skelAnime, objSegChangee, 3.0f, 0.0f, Animation_GetLastFrame(objSegFrameCount), ANIMMODE_ONCE, -3.0f);
...
void EnWallmas_SetupDie(EnWallmas* pthis, GlobalContext* globalCtx)
{
	...
	EffectSsDeadDb_Spawn(globalCtx, &pthis->actor.world.pos, &zeroVec_66, &zeroVec_66, 250, -10, 255, 255, 255, 255, 0, 0, 255, 1, 9, true);
...
void EnWallmas_ReturnToCeiling(EnWallmas* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.yDistToPlayer < -900.0f)
	{
		...
		if(pthis->actor.params == WMT_TIMER || Math_Vec3f_DistXZ(&pthis->actor.home.pos, &player->actor.world.pos) < 200.0f)
		{
			EnWallmas_TimerInit(pthis, globalCtx);
		}
		else
...
void EnWallmas_ColUpdate(EnWallmas* pthis, GlobalContext* globalCtx)
{
	if((pthis->collider.base.acFlags & AC_HIT) != 0)
	{
		...
		if((pthis->actor.colChkInfo.damageEffect != 0) || (pthis->actor.colChkInfo.damage != 0))
		{
			...
			if((pthis->actor.colChkInfo.damageEffect == DAMAGE_EFFECT_STUN_WHITE) || (pthis->actor.colChkInfo.damageEffect == DAMAGE_EFFECT_STUN_BLUE))
			{
...
void EnWallmas_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->actionFunc == EnWallmas_WaitToDrop) || (pthis->actionFunc == EnWallmas_WaitForProximity) || (pthis->actionFunc == EnWallmas_TakePlayer) || (pthis->actionFunc == EnWallmas_WaitForSwitchFlag))
	{
		return;
	}
...
void EnWallmas_DrawXlu(EnWallmas* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actionFunc != EnWallmas_WaitToDrop) && (pthis->actionFunc != EnWallmas_ReturnToCeiling) && (pthis->actionFunc != EnWallmas_TakePlayer) && (pthis->actionFunc != EnWallmas_WaitForSwitchFlag))
	{
