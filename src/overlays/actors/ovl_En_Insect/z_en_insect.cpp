...
s32 EnInsect_FoundNearbySoil(EnInsect* pthis, GlobalContext* globalCtx)
{
	...
	while(currentActor != NULL)
	{
		...
		{
			currentDistance = Math3D_Dist2DSq(pthis->actor.world.pos.x, pthis->actor.world.pos.z, currentActor->world.pos.x, currentActor->world.pos.z);
...
void EnInsect_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(temp_s2 == 2 || temp_s2 == 3)
	{
		...
		if(temp_s2 == 2)
		{
			...
			{
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_INSECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z, 3);
			}
		}
...
void func_80A7CC3C(EnInsect* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_31A > 20 && Rand_ZeroOne() < 0.1f)
	{
		...
		func_800286CC(globalCtx, &pthis->actor.world.pos, &velocity, &accel_64, Rand_ZeroOne() * 5.0f + 8.0f, Rand_ZeroOne() * 5.0f + 8.0f);
	}
...
void func_80A7D460(EnInsect* pthis, GlobalContext* globalCtx)
{
	...
	if(sp3A == 2 && (pthis->unk_314 & 0x10) && !(pthis->unk_314 & 0x80))
	{
		if(pthis->unk_32A >= 15)
		{
			if(pthis->soilActor != NULL)
			{
				if(!(GET_GS_FLAGS(((pthis->soilActor->actor.params >> 8) & 0x1F) - 1) & (pthis->soilActor->actor.params & 0xFF)))
				{
