...
void EnRr_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	for(i = 0; i < 5; i++)
	{
		pthis->bodySegs[i].height = pthis->bodySegs[i].heightTarget = pthis->bodySegs[i].scaleMod.x = pthis->bodySegs[i].scaleMod.y = pthis->bodySegs[i].scaleMod.z = 0.0f;
	}
...
void EnRr_SetupStunned(EnRr* pthis)
{
	...
	for(i = 0; i < 5; i++)
	{
		...
		pthis->bodySegs[i].rotTarget.z = 0.0f;
		pthis->bodySegs[i].scale.x = pthis->bodySegs[i].scale.y = pthis->bodySegs[i].scale.z = pthis->bodySegs[i].scaleTarget.x = pthis->bodySegs[i].scaleTarget.y = pthis->bodySegs[i].scaleTarget.z = 1.0f;
	}
	...
	else
	{
		if(pthis->collider1.base.acFlags & AC_HIT)
		{
			...
			switch(pthis->actor.colChkInfo.damageEffect)
			{
				...
				case RR_DMG_NORMAL:
					// "ouch"
					osSyncPrintf(VT_FGCOL(RED) "いてっ( %d : LIFE %d : DAMAGE %d : %x )！！" VT_RST "\n", pthis->frameCount, pthis->actor.colChkInfo.health, pthis->actor.colChkInfo.damage, pthis->actor.colChkInfo.damageEffect);
					pthis->stopScroll = false;
		...
		if((pthis->ocTimer == 0) && (pthis->actor.colorFilterTimer == 0) && (player->invincibilityTimer == 0) && !(player->stateFlags2 & PLAYER_STATE2_7) && ((pthis->collider1.base.ocFlags1 & OC1_HIT) || (pthis->collider2.base.ocFlags1 & OC1_HIT)))
		{
...
void EnRr_InitBodySegments(EnRr* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 5; i++)
	{
		pthis->bodySegs[i].scaleMod.x = pthis->bodySegs[i].scaleMod.z = Math_CosS(i * (u32)(s16)pthis->segPulsePhaseDiff * 0x1000) * pthis->pulseSize;
	}
	for(i = 1; i < 5; i++)
	{
...
void EnRr_UpdateBodySegments(EnRr* pthis, GlobalContext* globalCtx)
{
	...
	if(!pthis->isDead)
	{
		for(i = 0; i < 5; i++)
		{
			pthis->bodySegs[i].scaleMod.x = pthis->bodySegs[i].scaleMod.z = Math_CosS(phase + i * (s16)pthis->segPulsePhaseDiff * 0x1000) * pthis->pulseSize;
		}
		...
		if(!pthis->isDead && (pthis->reachState == 0))
		{
			for(i = 1; i < 5; i++)
			{
				pthis->bodySegs[i].rotTarget.x = Math_CosS(phase + i * (s16)pthis->segWobblePhaseDiffX * 0x1000) * pthis->wobbleSize;
				pthis->bodySegs[i].rotTarget.z = Math_SinS(phase + i * (s16)pthis->segWobblePhaseDiffZ * 0x1000) * pthis->wobbleSize;
			}
		}
	}
	if(!pthis->stopScroll)
...
void EnRr_Death(EnRr* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->frameCount < 40)
	{
		for(i = 0; i < 5; i++)
		{
			...
			pthis->bodySegs[i].scaleTarget.x = pthis->bodySegs[i].scaleTarget.z = (SQ(4 - i) * (f32)pthis->frameCount * 0.003f) + 1.0f;
		}
	}
...
void EnRr_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(!pthis->stopScroll)
	{
		...
		for(i = 0; i < 5; i++)
		{
			Math_SmoothStepToS(&pthis->bodySegs[i].rot.x, pthis->bodySegs[i].rotTarget.x, 5, pthis->segMoveRate * 1000.0f, 0);
			Math_SmoothStepToS(&pthis->bodySegs[i].rot.z, pthis->bodySegs[i].rotTarget.z, 5, pthis->segMoveRate * 1000.0f, 0);
...
void EnRr_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	for(i = 1; i < 5; i++)
	{
		...
		Matrix_Push();
		Matrix_Scale((1.0f + pthis->bodySegs[i].scaleMod.x) * pthis->bodySegs[i].scale.x, (1.0f + pthis->bodySegs[i].scaleMod.y) * pthis->bodySegs[i].scale.y, (1.0f + pthis->bodySegs[i].scaleMod.z) * pthis->bodySegs[i].scale.z, MTXMODE_APPLY);
	...
	if(pthis->effectTimer != 0)
	{
		...
		if((effectTimer & 1) == 0)
		{
			...
			{
				EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &effectPos, 150, 150, 150, 250, 235, 245, 255, 3.0f);
			}
		}
	}
}
