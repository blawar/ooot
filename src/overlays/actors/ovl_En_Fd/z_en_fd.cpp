...
ActorInit En_Fd_InitVars = {
    ACTOR_EN_FD, ACTORCAT_ENEMY, FLAGS, OBJECT_FW, ACTOR_FACTORY(EnFd), (ActorFunc)EnFd_Init, (ActorFunc)EnFd_Destroy, (ActorFunc)EnFd_Update, (ActorFunc)EnFd_Draw, (ActorFunc)EnFd_Reset,
};
...
static struct_80034EC0_Entry sAnimations[] = {
    {&gFlareDancerCastingFireAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, 0.0f}, {&gFlareDancerBackflipAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, -10.0f}, {&gFlareDancerGettingUpAnim, 0.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, -10.0f},
    {&gFlareDancerChasingAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP_INTERP, -10.0f},   {&gFlareDancerTwirlAnim, 0.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, -10.0f},
};
...
s32 EnFd_SpawnCore(EnFd* pthis, GlobalContext* globalCtx)
{
	...
	if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FW, pthis->corePos.x, pthis->corePos.y, pthis->corePos.z, 0, pthis->actor.shape.rot.y, 0, pthis->runDir) == NULL)
	{
		return false;
	}
...
void EnFd_SpawnChildFire(EnFd* pthis, GlobalContext* globalCtx, s16 fireCnt, s16 color)
{
	...
	for(i = 0; i < fireCnt; i++)
	{
		...
		Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_FD_FIRE, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, angle, 0, (color << 0xF) | i);
	}
}
...
{
	if(pthis->actionFunc == EnFd_Reappear || pthis->actionFunc == EnFd_SpinAndGrow || pthis->actionFunc == EnFd_JumpToGround || pthis->actionFunc == EnFd_WaitForCore)
	{
		return false;
	}
	else if(globalCtx->actorCtx.unk_02 != 0 && pthis->actor.xzDistToPlayer < 300.0f && pthis->actor.yDistToPlayer < 60.0f)
	{
		return true;
	}
	else
...
s32 EnFd_CanSeeActor(EnFd* pthis, Actor* actor, GlobalContext* globalCtx)
{
	...
	if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.world.pos, &actor->world.pos, &colPoint, &colPoly, true, false, false, true, &bgId))
	{
		return false;
	}
...
s32 EnFd_ShouldStopRunning(EnFd* pthis, GlobalContext* globalCtx, f32 radius, s16* runDir)
{
	...
	if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.world.pos, &pos, &colPoint, &poly, true, false, false, true, &bgId))
	{
		*runDir = -*runDir;
		return true;
	}
...
void EnFd_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnFd_Reappear && pthis->actionFunc != EnFd_SpinAndGrow && pthis->actionFunc != EnFd_WaitForCore)
	{
...
void EnFd_DrawDots(EnFd* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(pthis->effects); i++, eff++)
	{
		if(eff->type == FD_EFFECT_DOT)
		{
			...
			gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, eff->color.r, eff->color.g, eff->color.b, (u8)(eff->color.a * (pthis->fadeAlpha / 255.0f)));
			gDPPipeSync(POLY_XLU_DISP++);
			if(1)
			{
			}
