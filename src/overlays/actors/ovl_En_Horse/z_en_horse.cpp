...
static RaceWaypoint sIngoRaceWaypoints[] = {
    {1056, 1, -1540, 11, 0x2A8D}, {1593, 1, -985, 10, 0xFC27}, {1645, 1, -221, 11, 0xE891}, {985, 1, 403, 10, 0xBB9C}, {-1023, 1, 354, 11, 0xA37D}, {-1679, 1, -213, 10, 0x889C}, {-1552, 1, -1008, 11, 0x638D}, {-947, -1, -1604, 10, 0x4002},
};
...
static RaceWaypoint sHbaWaypoints[] = {
    {3600, 1413, -5055, 11, 0x8001}, {3360, 1413, -5220, 5, 0xC000}, {3100, 1413, -4900, 5, 0x0000}, {3600, 1413, -4100, 11, 0x0000}, {3600, 1413, 360, 11, 0x0000},
};
...
s32 EnHorse_BgCheckBridgeJumpPoint(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 2; i++)
	{
		...
		if(sBridgeJumps[i].zMin < pthis->actor.world.pos.z && pthis->actor.world.pos.z < sBridgeJumps[i].zMax)
		{
			...
			{
				if(sBridgeJumps[i].angle - sBridgeJumps[i].angleRange < pthis->actor.world.rot.y && pthis->actor.world.rot.y < sBridgeJumps[i].angle + sBridgeJumps[i].angleRange)
				{
					return true;
				}
			}
...
void EnHorse_UpdateIngoRaceInfo(EnHorse* pthis, GlobalContext* globalCtx, RaceInfo* raceInfo)
{
	...
	Math3D_PointDistToLine2D(pthis->actor.world.pos.x, pthis->actor.world.pos.z, prevWaypointPos.x, prevWaypointPos.z, curWaypointPos.x, curWaypointPos.z, &dist);
...
void EnHorse_PlayWalkingSound(EnHorse* pthis)
{
	if(sAnimSoundFrames[pthis->soundTimer] < pthis->curFrame)
	{
		...
		Audio_PlaySoundGeneral(NA_SE_EV_HORSE_WALK, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
{
	if(pthis->animationIdx == ENHORSE_ANIM_IDLE && ((pthis->curFrame > 35.0f && pthis->type == HORSE_EPONA) || (pthis->curFrame > 28.0f && pthis->type == HORSE_HNI)) && !(pthis->stateFlags & ENHORSE_SANDDUST_SOUND))
	{
		pthis->stateFlags |= ENHORSE_SANDDUST_SOUND;
		Audio_PlaySoundGeneral(NA_SE_EV_HORSE_SANDDUST, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	else if(pthis->animationIdx == ENHORSE_ANIM_REARING && pthis->curFrame > 25.0f && !(pthis->stateFlags & ENHORSE_LAND2_SOUND))
	{
		pthis->stateFlags |= ENHORSE_LAND2_SOUND;
		Audio_PlaySoundGeneral(NA_SE_EV_HORSE_LAND2, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
	...
	for(i = 0; i < 169; i++)
	{
		if(sHorseSpawns[i].scene == globalCtx->sceneNum)
		{
			...
			if(globalCtx->sceneNum != SCENE_SPOT20 ||
			   ...
			   ((sHorseSpawns[i].pos.x == 856 && sHorseSpawns[i].pos.y == 0 && sHorseSpawns[i].pos.z == -918) || (sHorseSpawns[i].pos.x == -1003 && sHorseSpawns[i].pos.y == 0 && sHorseSpawns[i].pos.z == -755)))
			{
				...
				if(!(minDist < dist) && !func_80A5BBBC(globalCtx, pthis, &spawnPos))
				{
					...
					spawn = true;
					SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->viewProjectionMtxF, &pthis->actor.world.pos, &pthis->actor.projectedPos, &pthis->actor.projectedW);
				}
			}
		}
	}
...
void EnHorse_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(globalCtx->sceneNum == SCENE_SPOT20 && (gSaveContext.eventInf[0] & 0xF) == 6 && Flags_GetEventChkInf(0x18) == 0 && !DREG(1))
	{
	...
	func_800A663C(globalCtx, &pthis->skin, sSkeletonHeaders[pthis->type], sAnimationHeaders[pthis->type][ENHORSE_ANIM_IDLE]);
	...
	else if(pthis->actor.params == 3)
	{
		...
		pthis->rider = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_IN, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, 1, 1);
		if(pthis->rider == NULL)
		{
...
void EnHorse_UpdateSpeed(EnHorse* pthis, GlobalContext* globalCtx, f32 brakeDecel, f32 brakeAngle, f32 minStickMag, f32 decel, f32 baseSpeed, s16 turnSpeed)
{
	...
	if(pthis->stateFlags & ENHORSE_BOOST)
	{
		...
		{
			pthis->actor.speedXZ = (EnHorse_SlopeSpeedMultiplier(pthis, globalCtx) * pthis->boostSpeed - pthis->actor.speedXZ) / (16 - pthis->boostTimer) + pthis->actor.speedXZ;
		}
		else
		{
	...
	{
		pthis->actor.speedXZ += (pthis->actor.speedXZ <= baseSpeed * (1.0f / 54.0f) * stickMag ? 1.0f : -1.0f) * 50.0f * 0.01f;
		if(baseSpeed < pthis->actor.speedXZ)
		{
...
void EnHorse_StartMountedIdle(EnHorse* pthis)
{
	...
	Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.0f, curFrame, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, -3.0f);
}
...
void EnHorse_MountedWalk(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	EnHorse_PlayWalkingSound(pthis);
	EnHorse_StickDirection(&pthis->curStick, &stickMag, &stickAngle);
	if(pthis->noInputTimerMax == 0.0f || (pthis->noInputTimer > 0.0f && pthis->noInputTimer < pthis->noInputTimerMax - 20.0f))
	{
...
void EnHorse_StartBraking(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.5f, 0.0f, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, -3.0f);
...
void EnHorse_StartReversing(EnHorse* pthis)
{
	...
	pthis->animationIdx = ENHORSE_ANIM_WALK;
	pthis->soundTimer = 0;
	Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.0f, 0.0f, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_LOOP, -3.0f);
}
...
void EnHorse_Reverse(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime) && (f32)pthis->noInputTimer <= 0.0f && EnHorse_PlayerCanMove(pthis, globalCtx) == true)
	{
		if(stickMag > 10.0f && Math_CosS(stickAngle) <= -0.5f)
		{
			pthis->noInputTimerMax = 0;
...
void EnHorse_StartLowJump(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	curFrame = pthis->skin.skelAnime.curFrame;
	Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.5f, curFrame, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, -3.0f);
...
void EnHorse_LowJump(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime) || (curFrame > 17.0f && pthis->actor.world.pos.y < pthis->actor.floorHeight - pthis->actor.velocity.y + 80.0f))
	{
		Audio_PlaySoundGeneral(NA_SE_EV_HORSE_LAND, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
void EnHorse_HighJump(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime) || (curFrame > 23.0f && pthis->actor.world.pos.y < pthis->actor.floorHeight - pthis->actor.velocity.y + 80.0f))
	{
		Audio_PlaySoundGeneral(NA_SE_EV_HORSE_LAND, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		Rumble_Shake(0.0f, 255, 10, 80);
		pthis->stateFlags &= ~ENHORSE_JUMPING;
...
void EnHorse_Inactive(EnHorse* pthis, GlobalContext* globalCtx2)
{
	...
	if(DREG(53) != 0 && pthis->type == HORSE_EPONA)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_EV_HORSE_NEIGH, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
			pthis->stateFlags &= ~ENHORSE_INACTIVE;
...
void EnHorse_PlayIdleAnimation(EnHorse* pthis, s32 anim, f32 morphFrames, f32 startFrame)
{
	...
	if(anim != pthis->animationIdx)
	{
		...
		Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.0f, startFrame, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, morphFrames);
	}
}
...
void EnHorse_StartMovingAnimation(EnHorse* pthis, s32 animId, f32 morphFrames, f32 startFrame)
{
	...
	{
		Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.0f, startFrame, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, 0.0f);
	}
}
...
void EnHorse_FollowPlayer(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->playerDir == PLAYER_DIR_BACK_R || pthis->playerDir == PLAYER_DIR_BACK_L) && (distToPlayer > 300.0f && !(pthis->stateFlags & ENHORSE_TURNING_TO_PLAYER)))
	{
		pthis->animationIdx = ENHORSE_ANIM_REARING;
		pthis->stateFlags |= ENHORSE_TURNING_TO_PLAYER;
		...
		pthis->followPlayerTurnSpeed = angleDiff / Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]);
		Animation_PlayOnce(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx]);
...
void EnHorse_UpdateIngoHorseAnim(EnHorse* pthis)
{
	...
	{
		Animation_Change(
		    &pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], sPlaybackSpeeds[pthis->animationIdx] * animSpeed * 1.5f, 0, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, -3);
	}
	else
	{
		Animation_Change(
		    &pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], sPlaybackSpeeds[pthis->animationIdx] * animSpeed * 1.5f, 0, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, 0);
	}
}
...
void EnHorse_UpdateIngoRace(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime) || (pthis->animationIdx == ENHORSE_ANIM_IDLE && pthis->actor.speedXZ != 0.0f))
	{
		EnHorse_UpdateIngoHorseAnim(pthis);
	}
	...
	EnHorse_SetIngoAnimation(pthis->animationIdx, pthis->skin.skelAnime.curFrame, pthis->ingoRaceFlags & 1, &((EnIn*)pthis->rider)->animationIdx, &((EnIn*)pthis->rider)->unk_1E0);
}
...
void EnHorse_CsMoveInit(EnHorse* pthis, GlobalContext* globalCtx, CsCmdActorAction* action)
{
	pthis->animationIdx = ENHORSE_ANIM_GALLOP;
	pthis->cutsceneAction = 1;
	Animation_PlayOnceSetSpeed(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], pthis->actor.speedXZ * 0.3f);
}
...
void EnHorse_CsJump(EnHorse* pthis, GlobalContext* globalCtx, CsCmdActorAction* action)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime) || (temp_f2 > 19.0f && pthis->actor.world.pos.y < (pthis->actor.floorHeight - pthis->actor.velocity.y) + 80.0f))
	{
		Vec3s* jointTable;
		f32 y;
		...
		func_80028A54(globalCtx, 25.0f, &pthis->actor.world.pos);
		pthis->animationIdx = ENHORSE_ANIM_GALLOP;
		Animation_PlayOnceSetSpeed(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], sPlaybackSpeeds[6]);
		jointTable = pthis->skin.skelAnime.jointTable;
		y = jointTable->y;
		pthis->riderPos.y += y * 0.01f;
		pthis->postDrawFunc = NULL;
...
void EnHorse_CsRearing(EnHorse* pthis, GlobalContext* globalCtx, CsCmdActorAction* action)
{
	...
	if(SkelAnime_Update(&pthis->skin.skelAnime))
	{
		...
		{
			Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.0f, 0.0f, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), 0, 0.0f);
		}
	}
}
...
s32 EnHorse_UpdateHbaRaceInfo(EnHorse* pthis, GlobalContext* globalCtx, RaceInfo* raceInfo)
{
	...
	if(pthis->curRaceWaypoint >= raceInfo->numWaypoints - 1 && Math3D_Vec3f_DistXYZ(&pos, &pthis->actor.world.pos) < DREG(8))
	{
		pthis->hbaFlags |= 2;
	}
...
void EnHorse_UpdateHbaAnim(EnHorse* pthis)
{
	...
	{
		Animation_Change(
		    &pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], sPlaybackSpeeds[pthis->animationIdx] * animSpeed * 1.5f, 0, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, -3.0f);
	}
	else
	{
		Animation_Change(
...
void EnHorse_FleePlayer(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	else if(pthis->actor.speedXZ > 0.1f)
	{ // walk
		pthis->skin.skelAnime.playSpeed = pthis->actor.speedXZ * 0.75f;
	...
	if(animFinished)
	{
		...
		{
			Animation_Change(&pthis->skin.skelAnime, sAnimationHeaders[pthis->type][pthis->animationIdx], 1.0f, 0.0f, Animation_GetLastFrame(sAnimationHeaders[pthis->type][pthis->animationIdx]), ANIMMODE_ONCE, 0.0f);
		}
		return;
	}
...
void EnHorse_BridgeJumpInit(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	pthis->bridgeJumpYVel = (((sBridgeJumps[pthis->bridgeJumpIdx].pos.y + 48.7f) - pthis->bridgeJumpStart.y) - -360.0f) / 30.0f;
	pthis->riderPos.y -= y;
	pthis->stateFlags |= ENHORSE_CALC_RIDER_POS;
...
void EnHorse_BridgeJumpMove(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.world.pos.x = ((sBridgeJumps[pthis->bridgeJumpIdx].pos.x - pthis->bridgeJumpStart.x) * interp) + pthis->bridgeJumpStart.x;
	pthis->actor.world.pos.z = ((sBridgeJumps[pthis->bridgeJumpIdx].pos.z - pthis->bridgeJumpStart.z) * interp) + pthis->bridgeJumpStart.z;

	pthis->actor.world.pos.y = (pthis->bridgeJumpStart.y + (pthis->bridgeJumpYVel * pthis->bridgeJumpTimer) + (-0.4f * timeSq));

	pthis->actor.world.rot.y = pthis->actor.shape.rot.y = (sBridgeJumps[pthis->bridgeJumpIdx].angle + ((1.0f - interp) * pthis->bridgeJumpRelAngle));
...
s32 EnHorse_CalcFloorHeight(EnHorse* pthis, GlobalContext* globalCtx, Vec3f* pos, CollisionPoly** floorPoly, f32* floorHeight)
{
	s32 bgId;
	f32 waterY;
	WaterBox* waterBox;
	...
	*floorHeight = BgCheck_EntityRaycastFloor3(&globalCtx->colCtx, floorPoly, &bgId, pos);
	...
	if(WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pos->x, pos->z, &waterY, &waterBox) == 1 && *floorHeight < waterY)
	{
		return 2; // Water
	}

	if((*floorPoly)->normal.y * COLPOLY_NORMAL_FRAC < 0.81915206f || // cos(35 degrees)
	   SurfaceType_IsHorseBlocked(&globalCtx->colCtx, *floorPoly, bgId) || func_80041D4C(&globalCtx->colCtx, *floorPoly, bgId) == 7)
	{
...
void EnHorse_CheckFloors(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	if(WaterBox_GetSurfaceImpl(globalCtx, &globalCtx->colCtx, pthis->actor.world.pos.x, pthis->actor.world.pos.z, &waterHeight, &waterBox) == 1 && pthis->actor.floorHeight < waterHeight)
	{
	...
	if(pthis->actor.floorPoly != 0)
	{
		...
		{
			if((!(pthis->stateFlags & ENHORSE_JUMPING) && dist < -40.0f) || (pthis->stateFlags & ENHORSE_JUMPING && dist < -200.0f))
			{
				EnHorse_ObstructMovement(pthis, globalCtx, 4, galloping);
				return;
			}
		...
		pos.y = pthis->yBack;
		dist = Math3D_DistPlaneToPos(nx, ny, nz, pthis->actor.floorPoly->dist, &pos);
		if(((backFloor != pthis->actor.floorPoly) && (pthis->actor.speedXZ <= 0.0f) && !(pthis->stateFlags & ENHORSE_JUMPING) && (dist < -40.0f)) || (pthis->stateFlags & ENHORSE_JUMPING && dist < -200.0f))
		{
			EnHorse_ObstructMovement(pthis, globalCtx, 5, galloping);
			return;
		}
		...
		if(ny < 0.81915206f || // cos(35 degrees)
		   SurfaceType_IsHorseBlocked(&globalCtx->colCtx, pthis->actor.floorPoly, pthis->actor.floorBgId) || func_80041D4C(&globalCtx->colCtx, pthis->actor.floorPoly, pthis->actor.floorBgId) == 7)
		{
			if((pthis->actor.speedXZ >= 0.0f))
			{
				EnHorse_ObstructMovement(pthis, globalCtx, 4, galloping);
		...
		pthis->actor.shape.yOffset = (pthis->yFront + (((pthis->yBack - pthis->yFront) * 20.0f) / 45.0f)) - pthis->actor.floorHeight;
	}
}
...
void EnHorse_ResolveCollision(EnHorse* pthis, GlobalContext* globalCtx, CollisionPoly* colPoly)
{
	...
	if(!(Math_CosS(pthis->actor.world.rot.y - (s16)(Math_FAtan2F(colPoly->normal.x, colPoly->normal.z) * (0x8000 / M_PI)) - 0x7FFF) < 0.7071f))
	{ // cos(45 degrees)
...
void EnHorse_UpdateBgCheckInfo(EnHorse* pthis, GlobalContext* globalCtx)
{
	...
	Actor_UpdateBgCheckInfo(globalCtx, &pthis->actor, globalCtx->sceneNum == SCENE_SPOT20 ? 19.0f : 40.0f, 35.0f, 100.0f, 29);
	...
	if(wall != NULL)
	{
		...
		if((Math_CosS(pthis->actor.world.rot.y - (s16)(Math_FAtan2F(wall->normal.x, wall->normal.z) * (0x8000 / M_PI)) - 0x7FFF) < 0.5f) || SurfaceType_IsHorseBlocked(&globalCtx->colCtx, wall, bgId) != 0)
		{
			return;
		}
	...
	if(Math3D_DistPlaneToPos(pthis->actor.floorPoly->normal.x * COLPOLY_NORMAL_FRAC, pthis->actor.floorPoly->normal.y * COLPOLY_NORMAL_FRAC, pthis->actor.floorPoly->normal.z * COLPOLY_NORMAL_FRAC, pthis->actor.floorPoly->dist, &obstacleTop) < -40.0f &&
	   Math3D_DistPlaneToPos(obstacleFloor->normal.x * COLPOLY_NORMAL_FRAC, obstacleFloor->normal.y * COLPOLY_NORMAL_FRAC, obstacleFloor->normal.z * COLPOLY_NORMAL_FRAC, obstacleFloor->dist, &pthis->actor.world.pos) > 40.0f)
	{
	...
	if(ny < 0.81915206f || // cos(35 degrees)
	   (SurfaceType_IsHorseBlocked(&globalCtx->colCtx, obstacleFloor, bgId) != 0) || (func_80041D4C(&globalCtx->colCtx, obstacleFloor, bgId) == 7))
	{
		if(movingFast == true && pthis->action != ENHORSE_ACT_STOPPING)
		{
			pthis->stateFlags |= ENHORSE_FORCE_REVERSING;
	...
	ny = obstacleFloor->normal.y * COLPOLY_NORMAL_FRAC;
	if(ny < 0.81915206f || // cos(35 degrees)
	   SurfaceType_IsHorseBlocked(&globalCtx->colCtx, obstacleFloor, bgId) || func_80041D4C(&globalCtx->colCtx, obstacleFloor, bgId) == 7)
	{
		if(movingFast == true && pthis->action != ENHORSE_ACT_STOPPING)
		{
			pthis->stateFlags |= ENHORSE_FORCE_REVERSING;
	...
	else if(behindObstacleHeight < -DREG(4))
	{ // -70
		if(movingFast == true && pthis->action != ENHORSE_ACT_STOPPING)
		{
			pthis->stateFlags |= ENHORSE_FORCE_REVERSING;
	...
	else if((movingFast == true && pthis->actor.speedXZ < 13.8f && obstacleHeight > 19.0f && obstacleHeight <= 72.0f) || (pthis->actor.speedXZ > 13.8f && obstacleHeight <= 112.0f))
	{
		EnHorse_Stub2(pthis);
...
void EnHorse_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->stateFlags & ENHORSE_FLAG_19)
	{
		...
		else if(!(pthis->stateFlags & ENHORSE_FLAG_20) && pthis->stateFlags & ENHORSE_FLAG_21 && pthis->action != ENHORSE_ACT_MOUNTED_REARING && pthis->inRace == true)
		{
	...
	if(!(pthis->stateFlags & ENHORSE_INACTIVE))
	{
		if(pthis->action == ENHORSE_ACT_MOUNTED_GALLOP || pthis->action == ENHORSE_ACT_MOUNTED_TROT || pthis->action == ENHORSE_ACT_MOUNTED_WALK)
		{
...
s32 EnHorse_MountSideCheck(EnHorse* pthis, GlobalContext* globalCtx, Player* player)
{
	...
	else if(Math_CosS(Actor_WorldYawTowardActor(&player->actor, &pthis->actor) - player->actor.world.rot.y) < 0.17364818f)
	{ // cos(80 degrees)
		return 0;
	}
	else
...
void EnHorse_SkinCallback1(Actor* thisx, GlobalContext* globalCtx, PSkinAwb* skin)
{
	...
	if((pthis->animationIdx == ENHORSE_ANIM_IDLE && pthis->action != ENHORSE_ACT_FROZEN) && ((frame > 40.0f && frame < 45.0f && pthis->type == HORSE_EPONA) || (frame > 28.0f && frame < 33.0f && pthis->type == HORSE_HNI)))
	{
	...
	else
	{
		...
		else if(pthis->action == ENHORSE_ACT_LOW_JUMP && frame > 6.0f && Rand_ZeroOne() < 1.0f - (frame - 6.0f) * (1.0f / 17.0f))
		{
			if(Rand_ZeroOne() < 0.5f)
			{
				pthis->dustFlags |= 8;
		...
		else if(pthis->action == ENHORSE_ACT_HIGH_JUMP && frame > 5.0f && Rand_ZeroOne() < 1.0f - (frame - 5.0f) * (1.0f / 25.0f))
		{
			if(Rand_ZeroOne() < 0.5f)
			{
				pthis->dustFlags |= 8;
	...
	for(i = 0; i < pthis->jntSph.count; i++)
	{
		...
		pthis->jntSph.elements[i].dim.worldSphere.radius = pthis->jntSph.elements[i].dim.modelSphere.radius * pthis->jntSph.elements[i].dim.scale;
	}
