...
s32 EnFw_PlayerInRange(EnFw* pthis, GlobalContext* globalCtx)
{
	...
	if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.world.pos, &player->actor.world.pos, &collisionPos, &poly, true, false, false, true, &bgId))
	{
		return false;
	}
...
void EnFw_Run(EnFw* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->explosionTimer != 0)
	{
		...
		if(pthis->explosionTimer == 0)
		{
			bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->bompPos.x, pthis->bompPos.y, pthis->bompPos.z, 0, 0, 0x600, 0);
			if(bomb != NULL)
			{
				bomb->timer = 0;
			}
...
s32 EnFw_OverrideLimbDraw(GlobalContext* globalContext, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	return false;
}
...
void EnFw_DrawDust(EnFw* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_fw.c", 1191);
	...
	for(i = 0; i < ARRAY_COUNT(pthis->effects); i++, eff++)
	{
		if(eff->type != 0)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_fw.c", 1229), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_fw.c", 1243);
}
