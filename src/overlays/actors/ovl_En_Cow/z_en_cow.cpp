...
void func_809DF96C(EnCow* pthis, GlobalContext* globalCtx)
{
	if((globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_00) || (globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_04))
	{
		if(DREG(53) != 0)
		{
			...
			{
				if((pthis->actor.xzDistToPlayer < 150.0f) && (ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)) < 0x61A8))
				{
					DREG(53) = 0;
...
void func_809DFA84(EnCow* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.xzDistToPlayer < 150.0f) && (ABS((s16)(pthis->actor.yawTowardsPlayer - pthis->actor.shape.rot.y)) >= 0x61A9) && (!(pthis->unk_276 & 2)))
	{
		pthis->unk_276 |= 2;
...
void EnCow_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime) != 0)
	{
		if(pthis->skelAnime.animation == &gCowBodyChewAnim)
		{
			...
			Animation_Change(&pthis->skelAnime, &gCowBodyMoveHeadAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gCowBodyMoveHeadAnim), ANIMMODE_ONCE, 1.0f);
		}
		else
		{
			Animation_Change(&pthis->skelAnime, &gCowBodyChewAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gCowBodyChewAnim), ANIMMODE_LOOP, 1.0f);
		}
	}
	pthis->actionFunc(pthis, globalCtx);
	if((thisx->xzDistToPlayer < 150.0f) && (ABS(Math_Vec3f_Yaw(&thisx->world.pos, &player->actor.world.pos)) < 0xC000))
	{
...
void func_809DFE98(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime) != 0)
	{
		if(pthis->skelAnime.animation == &gCowTailIdleAnim)
		{
			Animation_Change(&pthis->skelAnime, &gCowTailSwishAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gCowTailSwishAnim), ANIMMODE_ONCE, 1.0f);
		}
		else
		{
			Animation_Change(&pthis->skelAnime, &gCowTailIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gCowTailIdleAnim), ANIMMODE_LOOP, 1.0f);
		}
	}
	pthis->actionFunc(pthis, globalCtx);
}
...
void EnCow_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnCow_OverrideLimbDraw, EnCow_PostLimbDraw, pthis);
}
