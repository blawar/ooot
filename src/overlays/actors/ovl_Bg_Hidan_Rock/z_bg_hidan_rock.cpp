...
void func_8088B69C(BgHidanRock* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer != 0)
	{
		pthis->dyna.actor.world.pos.x = pthis->dyna.actor.home.pos.x + 5.0f * Math_SinS(pthis->dyna.actor.world.rot.y + pthis->timer * 0x4000);
		pthis->dyna.actor.world.pos.z = pthis->dyna.actor.home.pos.z + 5.0f * Math_CosS(pthis->dyna.actor.world.rot.y + pthis->timer * 0x4000);
	}
	else
	{
...
void func_8088B79C(BgHidanRock* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.bgCheckFlags & BG_STATE_1)
	{
		...
		Audio_PlayActorSound2(&pthis->dyna.actor, SurfaceType_GetSfx(&globalCtx->colCtx, pthis->dyna.actor.floorPoly, pthis->dyna.actor.floorBgId) + 0x800);
	}
...
void func_8088B990(BgHidanRock* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->type == 0 && (Math_SmoothStepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y + 1820.0f, 0.25f, 20.0f, 0.5f) < 0.1f)) ||
	   ((pthis->type != 0) && (Math_SmoothStepToF(&pthis->dyna.actor.world.pos.y, pthis->dyna.actor.home.pos.y + 480.0, 0.25f, 20.0f, 0.5f) < 0.1f)))
	{
		if(pthis->type == 0)
		{
			Audio_PlayActorSound2(&pthis->dyna.actor, NA_SE_EV_BLOCK_BOUND);
...
static void* sVerticalFlamesTexs[] = {
    gFireTempleBigVerticalFlame0Tex, gFireTempleBigVerticalFlame1Tex, gFireTempleBigVerticalFlame2Tex, gFireTempleBigVerticalFlame3Tex,
    gFireTempleBigVerticalFlame4Tex, gFireTempleBigVerticalFlame5Tex, gFireTempleBigVerticalFlame6Tex, gFireTempleBigVerticalFlame7Tex,
};
...
void func_8088BC40(GlobalContext* globalCtx, BgHidanRock* pthis)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_hidan_rock.c", 808);
	...
	{
		Matrix_Translate(pthis->dyna.actor.home.pos.x, pthis->dyna.actor.home.pos.y - 40.0f, pthis->dyna.actor.home.pos.z, MTXMODE_NEW);
	}
	...
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_hidan_rock.c", 853), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, gFireTempleBigVerticalFlameDL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_hidan_rock.c", 857);
}
