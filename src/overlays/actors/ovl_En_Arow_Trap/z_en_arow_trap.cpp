...
void EnArowTrap_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.xzDistToPlayer <= 400)
	{
		...
		{
			Actor_Spawn(
			    &globalCtx->actorCtx, globalCtx, ACTOR_EN_ARROW, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z, ARROW_NORMAL_SILENT);
			pthis->attackTimer = 80;
		}
	}
}
