...
void EnBrob_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_brob_Skel_0015D8, &object_brob_Anim_001750, pthis->jointTable, pthis->morphTable, 10);
...
void func_809CB458(EnBrob* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		EffectSsLightning_Spawn(globalCtx, &pos, &primColor_44, &envColor_44, pthis->dyna.actor.scale.y * 8000.0f, Rand_ZeroOne() * 65536.0f, 4, 1);
	}
...
void EnBrob_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if((acHits[0] && (pthis->colliders[0].info.acHitInfo->toucher.dmgFlags & 0x10)) || (acHits[1] && (pthis->colliders[1].info.acHitInfo->toucher.dmgFlags & 0x10)))
	{
		for(i = 0; i < 2; i++)
		{
	    ...
	    (pthis->colliders[0].base.atFlags & AT_HIT) || (pthis->colliders[1].base.atFlags & AT_HIT) || (acHits[0] && (pthis->colliders[0].info.acHitInfo->toucher.dmgFlags & 0x100)) ||
	    (acHits[1] && (pthis->colliders[1].info.acHitInfo->toucher.dmgFlags & 0x100)))
	{
		if(pthis->actionFunc == func_809CB114 && !(pthis->colliders[0].base.atFlags & AT_BOUNCED) && !(pthis->colliders[1].base.atFlags & AT_BOUNCED))
		{
			func_8002F71C(globalCtx, &pthis->dyna.actor, 5.0f, pthis->dyna.actor.yawTowardsPlayer, 1.0f);
		}
