...
static Vec3f D_80A78514_115[] = {
    {1000.0, -1000.0, 1000.0}, {0.0, -1000.0, 0.0},	    {-1000.0, -5000.0, -4000.0}, {1000.0, -5000.0, -3000.0},  {-1000.0, 1000.0, -6000.0}, {-1000.0, 3000.0, -5000.0}, {-800.0, 1000.0, -3000.0},
    {0.0, -4000.0, -2000.0},   {-1000.0, -2000.0, -6000.0}, {1000.0, -3000.0, 0.0},	 {2000.0, -2000.0, -4000.0},  {-1000.0, 0.0, -6000.0},	  {1000.0, -2000.0, -2000.0}, {0.0, -2000.0, 2100.0},
    {0.0, 0.0, 0.0},	       {1000.0, -1000.0, -6000.0},  {2000.0, 0.0, -3000.0},	 {-1000.0, -1000.0, -4000.0}, {900.0, -800.0, 2700.0},	  {720.0f, 900.0f, 2500.0f},
};
...
void func_80A74398(Actor* thisx, GlobalContext* globalCtx)
{
	...
	blureInit.p1StartColor[0] = blureInit.p1StartColor[1] = blureInit.p2StartColor[0] = blureInit.p2StartColor[1] = blureInit.p2StartColor[2] = blureInit.p1EndColor[0] = blureInit.p1EndColor[1] = blureInit.p2EndColor[0] = blureInit.p2EndColor[1] =
...
{
	if(((pthis->unk_2FB != 0) || (pthis->actor.params == 0)) && (func_800354B4(globalCtx, &pthis->actor, 100.0f, 0x2710, 0x4000, pthis->actor.shape.rot.y) != 0) && (globalCtx->gameplayFrames & 1))
	{
		func_80A755F0(pthis);
		return true;
	}
...
void func_80A74AAC(EnIk* pthis)
{
	...
	if(pthis->unk_2FB == 0)
	{
		Animation_Change(&pthis->skelAnime, &object_ik_Anim_00ED24, 1.0f, 0.0f, Animation_GetLastFrame(&object_ik_Anim_00ED24), ANIMMODE_LOOP, -4.0f);
		pthis->actor.speedXZ = 0.9f;
	}
	else
	{
		Animation_Change(&pthis->skelAnime, &object_ik_Anim_006734, 1.0f, 0.0f, Animation_GetLastFrame(&object_ik_Anim_006734), ANIMMODE_LOOP, -4.0f);
...
void func_80A75260(EnIk* pthis, GlobalContext* globalCtx)
{
	...
	if(((pthis->skelAnime.curFrame > 1.0f) && (pthis->skelAnime.curFrame < 9.0f)) || ((pthis->skelAnime.curFrame > 13.0f) && (pthis->skelAnime.curFrame < 18.0f)))
	{
...
void func_80A75790(EnIk* pthis)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &object_ik_Anim_006194, 1.0f, 0.0f, Animation_GetLastFrame(&object_ik_Anim_006194), ANIMMODE_ONCE, -4.0f);
		pthis->actor.speedXZ = -6.0f;
	}
	else
	{
		Animation_Change(&pthis->skelAnime, &object_ik_Anim_0045BC, 1.0f, 0.0f, Animation_GetLastFrame(&object_ik_Anim_0045BC), ANIMMODE_ONCE, -4.0f);
		pthis->actor.speedXZ = 6.0f;
	}
	pthis->unk_2FE = 0;
...
void EnIk_PostLimbDraw3(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	else if(limbIndex == 17)
	{
		...
		Collider_SetQuadVertices(&pthis->axeCollider, &pthis->axeCollider.dim.quad[0], &pthis->axeCollider.dim.quad[1], &pthis->axeCollider.dim.quad[2], &pthis->axeCollider.dim.quad[3]);
...
void func_80A76C14(EnIk* pthis)
{
	...
	{
		Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_WAKEUP, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	else if(Animation_OnFrame(&pthis->skelAnime, 33.0f))
	{
		Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_WALK, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	else if(Animation_OnFrame(&pthis->skelAnime, 68.0f) || Animation_OnFrame(&pthis->skelAnime, 80.0f))
	{
		Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_ARMOR_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	else if(Animation_OnFrame(&pthis->skelAnime, 107.0f))
	{
		Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_FINGER_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
	...
	{
		Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_WAVE_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}

void func_80A76DDC(EnIk* pthis, GlobalContext* globalCtx, Vec3f* pos)
{
	Audio_PlaySoundGeneral(NA_SE_EN_TWINROBA_TRANSFORM, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
}
...
void func_80A76E2C(EnIk* pthis, GlobalContext* globalCtx, Vec3f* pos)
{
	if(pthis->unk_4D4 == 0)
	{
		...
		for(i = ARRAY_COUNT(D_80A78514_115) - 1; i >= 0; i--)
		{
			...
			envColor.g += temp_v0;
			envColor.b += temp_v0;
			func_8002829C(globalCtx, &effectPos, &effectVelocity, &effectAccel, &primColor, &envColor, (Rand_ZeroOne() * 60.0f) + 300.0f, 0);
		}
...
{
	Animation_Change(&pthis->skelAnime, &object_ik_Anim_00C114, 1.0f, 0.0f, Animation_GetLastFrame(&object_ik_Anim_00C114), ANIMMODE_ONCE, 0.0f);
...
{
	Audio_PlaySoundGeneral(NA_SE_EN_IRONNACK_STAGGER_DEMO, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
}
...
void func_80A7735C(EnIk* pthis, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_ik_Skel_0205C0, NULL, pthis->jointTable, pthis->morphTable, 30);
...
void EnIk_PostLimbDraw2(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	switch(limbIndex)
	{
		case 13:
		{
			...
			if(EnIk_curFrame(&pthis->actor) < 30.0f)
			{
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inAwake.c", 267), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016D88);
			}
		}
		break;
		case 22:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inAwake.c", 274), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016F88);
			break;
		case 24:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inAwake.c", 280), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016EE8);
			break;
		case 26:
		{
			...
			if(EnIk_curFrame(&pthis->actor) < 30.0f)
			{
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inAwake.c", 288), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016BE0);
			}
		}
		break;
		case 27:
		{
			...
			if(EnIk_curFrame(&pthis->actor) < 30.0f)
			{
				gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inAwake.c", 297), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
				gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016CD8);
			}
		}
		break;
...
void func_80A77844(EnIk* pthis, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, EnIk_OverrideLimbDraw2, EnIk_PostLimbDraw2, pthis);
...
void EnIk_PostLimbDraw1(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx)
{
	...
	{
		case 12:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inConfrontion.c", 575), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016D88);
			break;
		case 22:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inConfrontion.c", 581), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016F88);
			break;
		case 24:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inConfrontion.c", 587), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016EE8);
			break;
		case 26:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inConfrontion.c", 593), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016BE0);
			break;
		case 27:
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_ik_inConfrontion.c", 599), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_ik_DL_016CD8);
			break;
	}
...
void func_80A77EDC(EnIk* pthis, GlobalContext* globalCtx)
{
	...
	gSPSegment(POLY_OPA_DISP++, 0x09, func_80A761B0(gfxCtx, 255, 40, 0, 40, 0, 0));
	gSPSegment(POLY_OPA_DISP++, 0x0A, func_80A761B0(gfxCtx, 255, 255, 255, 20, 40, 30));
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, EnIk_OverrideLimbDraw1, EnIk_PostLimbDraw1, pthis);
...
void EnIk_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(((pthis->actor.params & 0xFF) == 0 && (gSaveContext.eventChkInf[3] & 0x1000)) || (flag != 0 && Flags_GetSwitch(globalCtx, flag >> 8)))
	{
		Actor_Kill(&pthis->actor);
	}
	else
	{
		...
		SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_ik_Skel_01E178, &object_ik_Anim_00C114, pthis->jointTable, pthis->morphTable, 30);
