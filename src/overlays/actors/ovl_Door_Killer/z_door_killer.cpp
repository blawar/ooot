...
void DoorKiller_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	switch((u8)(pthis->actor.params & 0xFF))
	{
		case DOOR_KILLER_DOOR:
			...
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &object_door_killer_Skel_001BC8, NULL, pthis->jointTable, pthis->jointTable, 9);
			...
			if((((pthis->actor.params >> 8) & 0x3F) != 0x3F) && Flags_GetSwitch(globalCtx, ((pthis->actor.params >> 8) & 0x3F)))
			{
		...
		case DOOR_KILLER_RUBBLE_PIECE_4:
			...
			pthis->actor.velocity.x = (Math_CosS(pthis->actor.world.rot.y) * randF) + (Math_SinS(pthis->actor.world.rot.y) * pthis->actor.velocity.z);
			pthis->actor.velocity.z = (-Math_SinS(pthis->actor.world.rot.y) * randF) + (Math_CosS(pthis->actor.world.rot.y) * pthis->actor.velocity.z);
...
{
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_KILLER, thisx->world.pos.x, thisx->world.pos.y + 9.0f, thisx->world.pos.z, thisx->shape.rot.x, thisx->shape.rot.y, thisx->shape.rot.z, DOOR_KILLER_RUBBLE_PIECE_1);
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_KILLER, thisx->world.pos.x + 7.88f, thisx->world.pos.y + 39.8f, thisx->world.pos.z, thisx->shape.rot.x, thisx->shape.rot.y, thisx->shape.rot.z, DOOR_KILLER_RUBBLE_PIECE_2);
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_KILLER, thisx->world.pos.x - 15.86f, thisx->world.pos.y + 61.98f, thisx->world.pos.z, thisx->shape.rot.x, thisx->shape.rot.y, thisx->shape.rot.z, DOOR_KILLER_RUBBLE_PIECE_3);
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_KILLER, thisx->world.pos.x + 3.72f, thisx->world.pos.y + 85.1f, thisx->world.pos.z, thisx->shape.rot.x, thisx->shape.rot.y, thisx->shape.rot.z, DOOR_KILLER_RUBBLE_PIECE_4);
}
...
void DoorKiller_Wait(DoorKiller* pthis, GlobalContext* globalCtx)
{
	...
	else if(!Player_InCsMode(globalCtx) && (fabsf(playerPosRelToDoor.y) < 20.0f) && (fabsf(playerPosRelToDoor.x) < 20.0f) && (playerPosRelToDoor.z < 50.0f) && (playerPosRelToDoor.z > 0.0f))
	{
