...
void EnJs_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gCarpetMerchantSkel, &gCarpetMerchantSlappingKneeAnim, pthis->jointTable, pthis->morphTable, 13);
	...
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_JSJUTAN, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
}
...
{
	En_Js_SetupAction(pthis, func_80A89304);
	Animation_Change(&pthis->skelAnime, &gCarpetMerchantSlappingKneeAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gCarpetMerchantSlappingKneeAnim), ANIMMODE_ONCE, -4.0f);
}
...
void func_80A89294(EnJs* pthis)
{
	...
	Animation_Change(&pthis->skelAnime, &gCarpetMerchantIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gCarpetMerchantIdleAnim), ANIMMODE_ONCE, -4.0f);
}
