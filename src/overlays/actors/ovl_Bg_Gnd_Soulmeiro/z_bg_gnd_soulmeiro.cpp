...
void BgGndSoulmeiro_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	switch(pthis->actor.params & 0xFF)
	{
		case 0:
			...
			{
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_MIR_RAY, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 9);
...
void func_8087AF38(BgGndSoulmeiro* pthis, GlobalContext* globalCtx)
{
	...
	if(!pthis->unk_198)
	{
		Flags_SetSwitch(globalCtx, (thisx->params >> 8) & 0x3F);
		Actor_Kill(&pthis->actor);
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_MIR_RAY, thisx->world.pos.x, thisx->world.pos.y, thisx->world.pos.z, 0, 0, 0, 9);
	}
	else if((pthis->unk_198 % 6) == 0)
	{
		...
		for(i = 0; i < 6; i++)
		{
			...
			EffectSsDeadDb_Spawn(globalCtx, &thisx->home.pos, &vecA, &zeroVec, 60, 6, 255, 255, 150, 170, 255, 0, 0, 1, 14, true);
			temp_2 += 0x2AAA;
		}
	}
}
...
void BgGndSoulmeiro_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		case 0:
			OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_gnd_soulmeiro.c", 398);
			func_80093D84(globalCtx->gfxCtx);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_gnd_soulmeiro.c", 400), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, dLists_35[params]);
			CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_gnd_soulmeiro.c", 403);
			break;
		case 1:
