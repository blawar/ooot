...
void func_80A58DD4(EnHoll* pthis, GlobalContext* globalCtx)
{
	...
	absZ = fabsf(vec.z);
	if(vec.y > PLANE_Y_MIN && vec.y < PLANE_Y_MAX && fabsf(vec.x) < PLANE_HALFWIDTH && absZ < sHorizTriggerDists[phi_t0][0])
	{
		...
		else
		{
			...
			{
				pthis->planeAlpha = (255.0f / (sHorizTriggerDists[phi_t0][2] - sHorizTriggerDists[phi_t0][3])) * (absZ - sHorizTriggerDists[phi_t0][3]);
...
void func_80A59014(EnHoll* pthis, GlobalContext* globalCtx)
{
	...
	if(temp || (PLANE_Y_MIN < vec.y && vec.y < PLANE_Y_MAX && fabsf(vec.x) < planeHalfWidth && (absZ = fabsf(vec.z), 100.0f > absZ && absZ > 50.0f)))
	{
...
void func_80A591C0(EnHoll* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.xzDistToPlayer < 500.0f && absY < 700.0f)
	{
		...
		if(absY < 95.0f)
		{
			...
			if(pthis->actor.room != globalCtx->roomCtx.curRoom.num && func_8009728C(globalCtx, &globalCtx->roomCtx, pthis->actor.room) != 0)
			{
				EnHoll_SetupAction(pthis, EnHoll_NextAction);
				pthis->unk_14F = 1;
