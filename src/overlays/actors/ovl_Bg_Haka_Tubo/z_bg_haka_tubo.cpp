...
void BgHakaTubo_Idle(BgHakaTubo* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->potCollider.base.acFlags & AC_HIT)
	{
		...
		if(Actor_WorldDistXZToPoint(&pthis->dyna.actor, &pthis->potCollider.base.ac->world.pos) < 50.0f && (pthis->potCollider.base.ac->world.pos.y - pthis->dyna.actor.world.pos.y) < 50.0f)
		{
			...
			EffectSsHahen_SpawnBurst(globalCtx, &pos, 20.0f, 0, 350, 100, 50, OBJECT_HAKA_OBJECTS, 40, gEffFragments2DL);
			pthis->dropTimer = 5;
	...
	{ // Creates a delay between destroying the pot and dropping the collectible
		...
		{ // 3 spinning pots room in Shadow Temple
			...
			else if(rnd < 0.2f)
			{
				...
				collectibleParams = -1;
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_FIREFLY, pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y + 80.0f, pthis->dyna.actor.world.pos.z, 0, pthis->dyna.actor.shape.rot.y, 0, 2);
...
void BgHakaTubo_DrawFlameCircle(BgHakaTubo* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_bg_haka_tubo.c", 476);

	func_80093D84(globalCtx->gfxCtx);
	Matrix_Translate(pthis->dyna.actor.world.pos.x, pthis->dyna.actor.world.pos.y + 235.0f, pthis->dyna.actor.world.pos.z, MTXMODE_NEW);
	...
	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, pthis->fireScroll & 127, 0, 32, 64, 1, 0, (pthis->fireScroll * -15) & 0xFF, 32, 64));
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_bg_haka_tubo.c", 497), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, gEffFireCircleDL);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_bg_haka_tubo.c", 501);
}
