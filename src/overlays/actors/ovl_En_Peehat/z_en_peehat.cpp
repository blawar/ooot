...
void EnPeehat_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gPeehatSkel, &gPeehatRisingAnim, pthis->jointTable, pthis->morphTable, 24);
...
void EnPeehat_Larva_StateSeekPlayer(EnPeehat* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->colQuad.base.atFlags & AT_HIT) || (pthis->colCylinder.base.acFlags & AC_HIT) || (pthis->actor.bgCheckFlags & BG_STATE_0))
	{
		Player* player = GET_PLAYER(globalCtx);
		...
		else if(pthis->colCylinder.base.acFlags & AC_HIT || pthis->actor.bgCheckFlags & BG_STATE_0)
		{
			Vec3f zeroVec = {0, 0, 0};
			s32 i;
			for(i = 4; i >= 0; i--)
			{
				...
				EffectSsDeadDb_Spawn(globalCtx, &pos, &zeroVec, &zeroVec, 40, 7, 255, 255, 255, 255, 255, 0, 0, 1, 9, 1);
			}
		}
		if(&player->actor != pthis->colQuad.base.at || pthis->colCylinder.base.acFlags & AC_HIT)
		{
			...
			{
				EffectSsDeadSound_SpawnStationary(globalCtx, &pthis->actor.projectedPos, NA_SE_EN_PIHAT_SM_DEAD, 1, 1, 40);
			}
...
void EnPeehat_Adult_StateDie(EnPeehat* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(Math_SmoothStepToF(&pthis->actor.world.pos.y, pthis->actor.floorHeight + 88.5f, 1.0f, 3.0f, 0.0f) == 0.0f && pthis->actor.world.pos.y - pthis->actor.floorHeight < 59.0f)
		{
			Vec3f pos = pthis->actor.world.pos;
			pos.y = pthis->actor.floorHeight;
			func_80033480(globalCtx, &pos, 80.0f, 1, 150, 100, 1);
...
void EnPeehat_StateExplode(EnPeehat* pthis, GlobalContext* globalCtx)
{
	...
	{
		bomb = (EnBom*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOM, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0x602, 0);
		if(bomb != NULL)
		{
			bomb->timer = 0;
		}
...
void EnPeehat_Adult_CollisionCheck(EnPeehat* pthis, GlobalContext* globalCtx)
{
	...
	else if(pthis->colJntSph.base.acFlags & AC_HIT)
	{
		...
		if(pthis->actor.colChkInfo.damageEffect == PEAHAT_DMG_EFF_NUT || pthis->actor.colChkInfo.damageEffect == PEAHAT_DMG_EFF_LIGHT_ICE_ARROW)
		{
			return;
		}
...
void EnPeehat_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->state == PEAHAT_STATE_15 || pthis->state == PEAHAT_STATE_SEEK_PLAYER || pthis->state == PEAHAT_STATE_FLY || pthis->state == PEAHAT_STATE_RETURN_HOME || pthis->state == PEAHAT_STATE_EXPLODE)
	{
		...
		if(thisx->params < 0 && (thisx->flags & ACTOR_FLAG_UNCULLED))
		{
			for(i = 1; i >= 0; i--)
			{
				...
				if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &thisx->world.pos, posB, &posResult, &poly, true, true, false, true, &bgId) == true)
				{
...
s32 EnPeehat_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnPeehat* pthis = (EnPeehat*)thisx;
...
void EnPeehat_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.speedXZ != 0.0f || pthis->actor.velocity.y != 0.0f)
	{
		...
		Collider_SetQuadVertices(&pthis->colQuad, &pthis->colQuad.dim.quad[0], &pthis->colQuad.dim.quad[1], &pthis->colQuad.dim.quad[2], &pthis->colQuad.dim.quad[3]);
	}
}
