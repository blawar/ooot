...
{
	Animation_Change(&pthis->skelAnime, &object_rl_Anim_000A3C, 1.0f, 0.0f, Animation_GetLastFrame(&object_rl_Anim_000A3C), ANIMMODE_LOOP, 0.0f);
	pthis->action = 4;
	pthis->drawConfig = 0;
	pthis->alpha = 0;
...
void func_80AE7590(EnRl* pthis, GlobalContext* globalCtx)
{
	...
	if(gSaveContext.sceneSetupIndex == 4 && sceneNum == SCENE_KENJYANOMA && globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[6] != NULL && globalCtx->csCtx.npcActions[6]->action == 2 && !pthis->lightMedallionGiven)
	{
...
void func_80AE7698(EnRl* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		...
		{
			Animation_Change(&pthis->skelAnime, &object_rl_Anim_00040C, 1.0f, 0.0f, Animation_GetLastFrame(&object_rl_Anim_00040C), ANIMMODE_ONCE, 0.0f);
			pthis->action = 2;
		}
	}
}
...
void func_80AE772C(EnRl* pthis, s32 arg1)
{
	if(arg1)
	{
		Animation_Change(&pthis->skelAnime, &object_rl_Anim_000830, 1.0f, 0.0f, Animation_GetLastFrame(&object_rl_Anim_000830), ANIMMODE_LOOP, 0.0f);
		pthis->action = 3;
	}
}
...
{
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, kREG(18) + 22.0f + pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 5);
}
...
void func_80AE7AF8(EnRl* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &object_rl_Anim_00040C, 1.0f, 0.0f, Animation_GetLastFrame(&object_rl_Anim_00040C), ANIMMODE_ONCE, -8.0f);
		pthis->action = 6;
	}
...
static EnRlActionFunc sActionFuncs[] = {
    func_80AE7798, func_80AE77B8, func_80AE77F8, func_80AE7838, func_80AE7C64, func_80AE7C94, func_80AE7CE8, func_80AE7D40,
};
...
ActorInit En_Rl_InitVars = {
    ACTOR_EN_RL, ACTORCAT_NPC, FLAGS, OBJECT_RL, ACTOR_FACTORY(EnRl), (ActorFunc)EnRl_Init, (ActorFunc)EnRl_Destroy, (ActorFunc)EnRl_Update, (ActorFunc)EnRl_Draw, (ActorFunc)EnRl_Reset,
};
