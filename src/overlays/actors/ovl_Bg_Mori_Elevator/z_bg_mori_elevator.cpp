...
{
	return ((pthis->dyna.unk_160 & 2) && !(pthis->unk_170 & 2) && ((GET_PLAYER(globalCtx)->actor.world.pos.y - pthis->dyna.actor.world.pos.y) < 80.0f));
}
...
void BgMoriElevator_SetPosition(BgMoriElevator* pthis, GlobalContext* globalCtx)
{
	...
	else if((globalCtx->roomCtx.curRoom.num == 2) && Flags_GetSwitch(globalCtx, pthis->dyna.actor.params & 0x3F) && (pthis->unk_16C == 0))
	{
		pthis->targetY = 73.0f;
		func_808A1C30(pthis);
	}
