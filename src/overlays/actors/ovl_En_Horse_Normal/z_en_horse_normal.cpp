...
static AnimationHeader* sAnimations[] = {
    &gHorseNormalIdleAnim, &gHorseNormalWhinnyAnim, &gHorseNormalRefuseAnim, &gHorseNormalRearingAnim, &gHorseNormalWalkingAnim, &gHorseNormalTrottingAnim, &gHorseNormalGallopingAnim, &gHorseNormalJumpingAnim, &gHorseNormalJumpingHighAnim,
};
...
static EnHorseNormalUnkStruct1 D_80A6D428[] = {
    {{1058, 1, 384}, 7}, {{1653, 39, -381}, 6}, {{1606, 1, -1048}, 6}, {{1053, 1, -1620}, 6}, {{-1012, 1, -1633}, 7}, {{-1655, 1, -918}, 6}, {{-1586, 1, -134}, 6}, {{-961, 1, 403}, 7},
};
...
static EnHorseNormalUnkStruct1 D_80A6D470[] = {
    {{88, 0, 2078}, 10}, {{2482, 376, 4631}, 7}, {{2228, -28, 6605}, 12}, {{654, -100, 8864}, 7}, {{-297, -500, 10667}, 12}, {{-5303, -420, 10640}, 10}, {{-6686, -500, 7760}, 10}, {{-5260, 100, 5411}, 7}, {{-3573, -269, 3893}, 10},
};
...
{
	if(D_80A6D4C0_50[pthis->unk_200] < pthis->skin.skelAnime.curFrame && ((pthis->unk_200 != 0) || !(D_80A6D4C0_50[1] < pthis->skin.skelAnime.curFrame)))
	{
...
void EnHorseNormal_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(globalCtx->sceneNum == SCENE_SPOT20)
	{
		...
		if((pthis->actor.world.pos.x == -730.0f && pthis->actor.world.pos.y == 0.0f && pthis->actor.world.pos.z == -1100.0f) || (pthis->actor.world.pos.x == 880.0f && pthis->actor.world.pos.y == 0.0f && pthis->actor.world.pos.z == -1170.0f))
		{
			func_80A6C6B0(pthis);
			return;
		}
	...
	{
		if(pthis->actor.world.pos.x == 3707.0f && pthis->actor.world.pos.y == 1413.0f && pthis->actor.world.pos.z == -665.0f)
		{
			func_800A663C(globalCtx, &pthis->skin, &gHorseNormalSkel, &gHorseNormalIdleAnim);
			Animation_PlayOnce(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx]);
			func_80A6C4CC(pthis);
...
void func_80A6B91C(EnHorseNormal* pthis, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], func_80A6B30C(pthis), 0.0f, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), ANIMMODE_ONCE, 0.0f);
}
...
void EnHorseNormal_Wander(EnHorseNormal* pthis, GlobalContext* globalCtx)
{
	...
	switch(D_80A6D510_62[pthis->animationIdx])
	{
		...
		case 1:
			...
			if(Rand_ZeroOne() < 0.1f || (pthis->unk_21E == 0 && ((pthis->actor.bgCheckFlags & BG_STATE_3) || (pthis->bodyCollider.base.ocFlags1 & OC1_HIT) || (pthis->headCollider.base.ocFlags1 & OC1_HIT))))
			{
	...
	if(phi_t0 != pthis->animationIdx || SkelAnime_Update(&pthis->skin.skelAnime))
	{
		if(phi_t0 != pthis->animationIdx)
		{
			...
			Animation_Change(&pthis->skin.skelAnime, sAnimations[pthis->animationIdx], func_80A6B30C(pthis), 0.0f, Animation_GetLastFrame(sAnimations[pthis->animationIdx]), ANIMMODE_ONCE, -3.0f);
		}
		else
		{
			switch(D_80A6D510_62[pthis->animationIdx])
...
static EnHorseNormalActionFunc sActionFuncs[] = {
    EnHorseNormal_CycleAnimations, EnHorseNormal_Wander, EnHorseNormal_Wait, EnHorseNormal_WaitClone, EnHorseNormal_FollowPath,
};
...
void func_80A6CAFC(Actor* thisx, GlobalContext* globalCtx, PSkinAwb* skin)
{
	...
	for(i = 0; i < pthis->headCollider.count; i++)
	{
		...
		pthis->headCollider.elements[i].dim.worldSphere.radius = pthis->headCollider.elements[i].dim.modelSphere.radius * pthis->headCollider.elements[i].dim.scale;
	}
...
void EnHorseNormal_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->action == HORSE_WAIT_CLONE)
	{
		...
		{
			if(pthis->actor.world.pos.x == 355.0f && pthis->actor.world.pos.y == 0.0f && pthis->actor.world.pos.z == -245.0f)
			{
				clonePos.x = 235.0f;
				clonePos.y = 0.0f;
				clonePos.z = 100.0f;
			...
			else if(pthis->actor.world.pos.x == 238.0f && pthis->actor.world.pos.y == 0.0f && pthis->actor.world.pos.z == -245.0f)
			{
				clonePos.x = 478.0f;
				clonePos.y = 0.0f;
				clonePos.z = 100.0f;
		...
		{
			if(pthis->actor.world.pos.x == -730.0f && pthis->actor.world.pos.y == 0.0f && pthis->actor.world.pos.z == -1100.0f)
			{
				clonePos.x = 780.0f;
				clonePos.y = 0.0f;
				clonePos.z = -80.0f;
			...
			else if(pthis->actor.world.pos.x == 880.0f && pthis->actor.world.pos.y == 0.0f && pthis->actor.world.pos.z == -1170.0f)
			{
				clonePos.x = -1000.0f;
				clonePos.y = 0.0f;
				clonePos.z = -70.0f;
