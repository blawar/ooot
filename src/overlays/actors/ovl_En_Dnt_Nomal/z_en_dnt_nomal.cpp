...
static Color_RGBA8 sLeafColors[] = {
    {255, 255, 255, 255}, {255, 195, 175, 255}, {210, 255, 0, 255}, {255, 255, 255, 255}, {210, 255, 0, 255}, {255, 195, 175, 255}, {255, 255, 255, 255}, {255, 195, 175, 255}, {210, 255, 0, 255},
};
...
void EnDntNomal_WaitForObject(EnDntNomal* pthis, GlobalContext* globalCtx)
{
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->objIndex))
	{
		...
		if(pthis->type == ENDNTNOMAL_TARGET)
		{
			SkelAnime_Init(globalCtx, &pthis->skelAnime, &gHintNutsSkel, &gHintNutsBurrowAnim, pthis->jointTable, pthis->morphTable, 10);
			pthis->actor.draw = EnDntNomal_DrawTargetScrub;
		}
		else
		{
			SkelAnime_Init(globalCtx, &pthis->skelAnime, &gDntStageSkel, &gDntStageHideAnim, pthis->jointTable, pthis->morphTable, 11);
...
void EnDntNomal_TargetGivePrize(EnDntNomal* pthis, GlobalContext* globalCtx)
{
	...
	if((frame >= 8.0f) && !pthis->spawnedItem)
	{
		...
		if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_ITEM, itemX, itemY, itemZ, 0, 0, 0, EXITEM_BULLET_BAG) == NULL)
		{
	...
	if(frame >= pthis->endFrame)
	{
		...
		switch(pthis->action)
		{
			...
			case DNT_ACTION_HIGH_RUPEES:
				rupee = (EnExRuppy*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_EX_RUPPY, pthis->actor.world.pos.x, pthis->actor.world.pos.y + 20.0f, pthis->actor.world.pos.z, 0, 0, 0, 3);
				if(rupee != NULL)
				{
...
void EnDntNomal_StageAttack(EnDntNomal* pthis, GlobalContext* globalCtx)
{
	...
	Math_SmoothStepToS(&pthis->actor.shape.rot.x, -(s16)(Math_FAtan2F(dy, sqrtf(SQ(dx) + SQ(dz))) * (0x8000 / M_PI)), 3, 0x1388, 0);
	...
	else if((frame >= 8.0f) && (!pthis->spawnedItem))
	{
		...
		nut = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_NUTSBALL, spawnX, spawnY, spawnZ, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z, 4);
		if(nut != NULL)
		{
...
void EnDntNomal_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->type == ENDNTNOMAL_TARGET)
	{
		Collider_SetQuadVertices(&pthis->targetQuad, &pthis->targetVtx[0], &pthis->targetVtx[1], &pthis->targetVtx[2], &pthis->targetVtx[3]);
...
s32 EnDntNomal_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	...
	{
		OPEN_DISPS(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1733);
		gDPPipeSync(POLY_OPA_DISP++);
		gDPSetEnvColor(POLY_OPA_DISP++, sLeafColors[pthis->type - ENDNTNOMAL_STAGE].r, sLeafColors[pthis->type - ENDNTNOMAL_STAGE].g, sLeafColors[pthis->type - ENDNTNOMAL_STAGE].b, 255);
		CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1743);
	}
	return false;
}
...
void EnDntNomal_DrawStageScrub(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1790);
	func_80093D18(globalCtx->gfxCtx);
	gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(blinkTex_104[pthis->eyeState]));
	SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnDntNomal_OverrideLimbDraw, EnDntNomal_PostLimbDraw, pthis);
	Matrix_Translate(pthis->flowerPos.x, pthis->flowerPos.y, pthis->flowerPos.z, MTXMODE_NEW);
	Matrix_Scale(0.01f, 0.01f, 0.01f, MTXMODE_APPLY);
	gDPPipeSync(POLY_OPA_DISP++);
	gDPSetEnvColor(POLY_OPA_DISP++, sLeafColors[pthis->type - ENDNTNOMAL_STAGE].r, sLeafColors[pthis->type - ENDNTNOMAL_STAGE].g, sLeafColors[pthis->type - ENDNTNOMAL_STAGE].b, 255);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1814), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, gDntStageFlowerDL);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1817);
	if(pthis->actionFunc == EnDntNomal_StageCelebrate)
	{
...
void EnDntNomal_DrawTargetScrub(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1833);
	func_80093D18(globalCtx->gfxCtx);
	SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, NULL, EnDntNomal_PostLimbDraw, pthis);
	Matrix_Translate(pthis->flowerPos.x, pthis->flowerPos.y, pthis->flowerPos.z, MTXMODE_NEW);
	Matrix_Scale(0.01f, 0.01f, 0.01f, MTXMODE_APPLY);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1848), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, gHintNutsFlowerDL);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_dnt_nomal.c", 1851);
}
