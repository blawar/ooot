...
static Gfx* D_80B3BF70[] = {
    object_wood02_DL_007968, object_wood02_DL_007D38, object_wood02_DL_0081A8, NULL, NULL, NULL, object_wood02_DL_007AD0, object_wood02_DL_007E20, object_wood02_DL_008350, object_wood02_DL_000160, object_wood02_DL_000440, object_wood02_DL_000700,
};
...
s32 EnWood02_SpawnZoneCheck(EnWood02* pthis, GlobalContext* globalCtx, Vec3f* pos)
{
	...
	SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->viewProjectionMtxF, pos, &pthis->actor.projectedPos, &pthis->actor.projectedW);

	phi_f12 = ((pthis->actor.projectedW == 0.0f) ? 1000.0f : fabsf(1.0f / pthis->actor.projectedW));

	if((-pthis->actor.uncullZoneScale < pthis->actor.projectedPos.z) && (pthis->actor.projectedPos.z < (pthis->actor.uncullZoneForward + pthis->actor.uncullZoneScale)) &&
	   (((fabsf(pthis->actor.projectedPos.x) - pthis->actor.uncullZoneScale) * phi_f12) < 1.0f) && (((pthis->actor.projectedPos.y + pthis->actor.uncullZoneDownward) * phi_f12) > -1.0f) &&
	...
	for(i = 4; i >= 0; i--)
	{
		if((pthis->unk_14E[i] & 0x7F) == 0)
		{
			...
			if(EnWood02_SpawnZoneCheck(pthis, globalCtx, &childPos))
			{
				...
				childWood = (EnWood02*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_WOOD02, childPos.x, childPos.y, childPos.z, pthis->actor.world.rot.x, *childSpawnAngle, 0, childParams);
				if(childWood != NULL)
				{
...
void EnWood02_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	else if(pthis->actor.params <= WOOD_LEAF_GREEN)
...
void EnWood02_Update(Actor* thisx, GlobalContext* globalCtx2)
{
		...
		if(pthis->actor.home.rot.y != 0)
		{
			...
			else
			{
				if(pthis->actor.home.rot.z != 0)
				{
					...
					Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_SW, dropsSpawnPt.x, dropsSpawnPt.y, dropsSpawnPt.z, 0, pthis->actor.world.rot.y, 0, pthis->actor.home.rot.z);
			...
			if(pthis->unk_14C >= -1)
			{
				...
				if((pthis->actor.params == WOOD_TREE_OVAL_YELLOW_SPAWNER) || (pthis->actor.params == WOOD_TREE_OVAL_YELLOW_SPAWNED))
				{
				...
				for(i = 3; i >= 0; i--)
				{
					Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_WOOD02, dropsSpawnPt.x, dropsSpawnPt.y, dropsSpawnPt.z, 0, Rand_CenteredFloat(65535.0f), 0, leavesParams);
				}
			}
			pthis->unk_14C = -0x15;
			pthis->actor.home.rot.y = 0;
	...
	else if(pthis->actor.params < 0x17)
	{ // Bush
		Player* player = GET_PLAYER(globalCtx);

		if(pthis->unk_14C >= -1)
		{
			if(((player->rideActor == NULL) && (sqrt(pthis->actor.xyzDistToPlayerSq) < 20.0) && (player->linearVelocity != 0.0f)) ||
			   ((player->rideActor != NULL) && (sqrt(pthis->actor.xyzDistToPlayerSq) < 60.0) && (player->rideActor->speedXZ != 0.0f)))
			{
				if((pthis->unk_14C >= 0) && (pthis->unk_14C < 0x64))
				{
					Item_DropCollectibleRandom(globalCtx, &pthis->actor, &pthis->actor.world.pos, ((pthis->unk_14C << 4) | 0x8000));
				}
				pthis->unk_14C = -0x15;
				Audio_PlayActorSound2(&pthis->actor, NA_SE_EV_TREE_SWING);
			}
...
void EnWood02_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((type == WOOD_TREE_OVAL_GREEN_SPAWNER) || (type == WOOD_TREE_OVAL_GREEN_SPAWNED) || (type == WOOD_TREE_OVAL_GREEN) || (type == WOOD_LEAF_GREEN))
	{
		red = 50;
		green = 170;
		blue = 70;
	}
	else if((type == WOOD_TREE_OVAL_YELLOW_SPAWNER) || (type == WOOD_TREE_OVAL_YELLOW_SPAWNED) || (type == WOOD_LEAF_YELLOW))
	{
		red = 180;
		green = 155;
		blue = 0;
	...
	else if(D_80B3BF70[pthis->drawType & 0xF] != NULL)
	{
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_wood02.c", 808), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, D_80B3BF70[pthis->drawType & 0xF]);
	}
	else
	{
		func_80093D84(gfxCtx);
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_wood02.c", 814), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
