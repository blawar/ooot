...
s32 EnTorch2_SwingSword(GlobalContext* globalCtx, Input* input, Player* pthis)
{
	...
	if((((globalCtx->gameplayFrames & attackDelay) == 0) || (sSwordJumpState != 0)) && (noAttackChance <= Rand_ZeroOne()))
	{
...
void EnTorch2_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	switch(sActionState)
	{
		case ENTORCH2_WAIT:
			...
			if((pthis->actor.xzDistToPlayer <= 120.0f) || Actor_IsTargeted(globalCtx, &pthis->actor) || (attackItem != NULL))
			{
				if(attackItem != NULL)
				{
					sDodgeRollState = 1;
		...
		case ENTORCH2_ATTACK:
			...
			if((pthis->invincibilityTimer < 0) && (sActionState != ENTORCH2_DAMAGE) && (pthis->cylinder.base.acFlags & AC_HIT))
			{
				pthis->cylinder.base.acFlags &= ~AC_HIT;
			}
			...
			else if(sJumpslashTimer == 0)
			{
				...
				if(((player->swordState != 0) || (player->actor.velocity.y > -3.0f)) && (player->swordAnimation == JUMPSLASH_START))
				{
				...
				else
				{
					...
					if(func_800354B4(globalCtx, &pthis->actor, 120.0f, 0x7FFF, 0x7FFF, pthis->actor.world.rot.y))
					{
						if((player->swordAnimation == STAB_1H) && (pthis->actor.xzDistToPlayer < 90.0f))
						{
							...
							if((pthis->swordState == 0) && (sCounterState == 0) && (player->invincibilityTimer == 0) && (player->swordAnimation == STAB_1H) && (pthis->actor.xzDistToPlayer <= 85.0f) &&
						...
						else
						{
							...
							if(!CHECK_BTN_ANY(input->cur.button, BTN_A | BTN_R) && (pthis->swordState == 0) && (player->swordState != 0))
							{
								sCounterState = 1;
							}
						}
					...
					else
					{
						...
						sStickAngle = thisx->yawTowardsPlayer;
						sp50 = 0.0f;
						if((90.0f >= pthis->actor.xzDistToPlayer) && (pthis->actor.xzDistToPlayer > 70.0f) && (ABS(sp5A) >= 0x7800) && (pthis->actor.isTargeted || !(player->stateFlags1 & PLAYER_STATE1_22)))
						{
							EnTorch2_SwingSword(globalCtx, input, pthis);
						}
						else if(((pthis->actor.xzDistToPlayer <= 70.0f) || ((pthis->actor.xzDistToPlayer <= 80.0f + sp50) && (player->swordState != 0))) && (pthis->swordState == 0))
						{
							if(!EnTorch2_SwingSword(globalCtx, input, pthis) && (pthis->swordState == 0) && (sCounterState == 0))
							{
								EnTorch2_Backflip(pthis, input, &pthis->actor);
							}
						}
						...
						{
							if((player->swordState == 0) || (player->swordAnimation < SPIN_ATTACK_1H) || (player->swordAnimation > BIG_SPIN_2H) || (pthis->actor.xzDistToPlayer >= 280.0f))
							{
								sStickTilt = 127.0f;
								sStickAngle = pthis->actor.yawTowardsPlayer;
								if(!pthis->actor.isTargeted)
						...
						else if(((ABS(sp5A) < 0x7800) && (ABS(sp5A) >= 0x3000)) || !EnTorch2_SwingSword(globalCtx, input, pthis))
						{
							sStickAngle = pthis->actor.yawTowardsPlayer;
							sStickTilt = 127.0f;
							if(!pthis->actor.isTargeted)
		...
		case ENTORCH2_DAMAGE:
			...
			if((pthis->invincibilityTimer > 0) && (pthis->actor.world.pos.y < (pthis->actor.floorHeight - 160.0f)))
			{
				...
				if(Actor_WorldDistXYZToPoint(&pthis->actor, &sSpawnPoint) > 800.0f)
				{
					...
					pthis->actor.shape.rot.y = pthis->actor.world.rot.y = Math_Vec3f_Yaw(&sSpawnPoint, &player->actor.world.pos);
					pthis->actor.world.pos.x = (Math_SinS(pthis->actor.world.rot.y + sp4E) * (25.0f + sp50)) + sSpawnPoint.x;
					pthis->actor.world.pos.z = (Math_CosS(pthis->actor.world.rot.y + sp4E) * (25.0f + sp50)) + sSpawnPoint.z;
	...
	if((gSaveContext.unk_13F0 == 3) && (player->swordState == 0 || (player->swordAnimation < SPIN_ATTACK_1H) || (player->swordAnimation > BIG_SPIN_2H)))
	{
		sStickTilt = 0.0f;
		input->cur.stick_x = 0;
		input->cur.stick_y = 0;
	...
	if((pthis->invincibilityTimer == 0) && (pthis->actor.colChkInfo.health != 0) && (pthis->cylinder.base.acFlags & AC_HIT) && !(pthis->stateFlags1 & PLAYER_STATE1_26) && !(pthis->swordQuads[0].base.atFlags & AT_HIT) &&
...
s32 EnTorch2_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	Player* pthis = (Player*)thisx;
...
void EnTorch2_Draw(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(sAlpha == 255)
	{
		...
		POLY_OPA_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnTorch2_OverrideLimbDraw, EnTorch2_PostLimbDraw, pthis, POLY_OPA_DISP);
	}
	else
	{
