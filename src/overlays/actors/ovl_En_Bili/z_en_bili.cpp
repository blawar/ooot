...
void EnBili_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gBiriSkel, &gBiriDefaultAnim, pthis->jointTable, pthis->morphTable, EN_BILI_LIMB_MAX);
...
void EnBili_SetupFrozen(EnBili* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 8; i++)
	{
		...
		EffectSsEnIce_SpawnFlyingVec3f(globalCtx, &pthis->actor, &effectPos, 150, 150, 150, 250, 235, 245, 255, (Rand_ZeroOne() * 0.2f) + 0.7f);
	}
...
void EnBili_Climb(EnBili* pthis, GlobalContext* globalCtx)
{
	...
	if(curFrame > 9.0f)
	{
		Math_ApproachF(&pthis->actor.world.pos.y, pthis->actor.world.pos.y + pthis->actor.yDistToPlayer + 100.0f, 0.5f, 5.0f);
	}
...
void EnBili_Die(EnBili* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer != 0)
	{
		for(i = 0; i < 2; i++)
		{
			...
			{
				EffectSsDtBubble_SpawnColorProfile(globalCtx, &effectPos, &effectVelocity_82, &effectAccel_82, effectScale, 25, 2, 1);
			}
			else
			{
				EffectSsDtBubble_SpawnColorProfile(globalCtx, &effectPos, &effectVelocity_82, &effectAccel_82, effectScale, 25, 0, 1);
			}
		}
	}
	else
...
void EnBili_UpdateDamage(EnBili* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->actor.colChkInfo.health != 0) && (pthis->collider.base.acFlags & AC_HIT))
	{
		...
		if((pthis->actor.colChkInfo.damageEffect != 0) || (pthis->actor.colChkInfo.damage != 0))
		{
			...
			if(pthis->collider.info.acHitInfo->toucher.dmgFlags & 0x1F820)
			{ // DMG_ARROW
				pthis->actor.flags |= ACTOR_FLAG_4;
			}
		}
...
void EnBili_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->actionFunc != EnBili_Die)
	{
		...
		if(Animation_OnFrame(&pthis->skelAnime, 9.0f))
		{
			if((pthis->actionFunc == EnBili_FloatIdle) || (pthis->actionFunc == EnBili_SetNewHomeHeight) || (pthis->actionFunc == EnBili_ApproachPlayer) || (pthis->actionFunc == EnBili_Recoil))
			{
...
s32 EnBili_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	EnBili* pthis = (EnBili*)thisx;
...
static void* sTentaclesTextures[] = {
    gBiriTentacles0Tex, gBiriTentacles1Tex, gBiriTentacles2Tex, gBiriTentacles3Tex, gBiriTentacles4Tex, gBiriTentacles5Tex, gBiriTentacles6Tex, gBiriTentacles7Tex,
};
