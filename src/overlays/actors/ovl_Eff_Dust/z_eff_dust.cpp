...
void EffDust_DrawFunc_8099E4F4(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	for(i = 0; i < 64; i++)
	{
		if(*distanceTraveled < 1.0f)
		{
			...
			Matrix_Translate(
			    pthis->actor.world.pos.x + (initialPositions->x * ((pthis->dx * aux) + (1.0f - pthis->dx))), pthis->actor.world.pos.y + (initialPositions->y * ((pthis->dy * aux) + (1.0f - pthis->dy))),
			    pthis->actor.world.pos.z + (initialPositions->z * ((pthis->dz * aux) + (1.0f - pthis->dz))), MTXMODE_NEW);

			Matrix_Scale(pthis->scalingFactor, pthis->scalingFactor, pthis->scalingFactor, MTXMODE_APPLY);
			Matrix_Mult(&globalCtx->billboardMtxF, MTXMODE_APPLY);

			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_eff_dust.c", 449), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EffDust_DrawFunc_8099E784(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	for(i = 0; i < 64; i++)
	{
		if(*distanceTraveled < 1.0f)
		{
			...
			Matrix_Translate(initialPositions->x * ((pthis->dx * aux) + (1.0f - pthis->dx)), initialPositions->y * (1.0f - *distanceTraveled) + 320.0f, initialPositions->z * (1.0f - *distanceTraveled) + -20.0f, MTXMODE_APPLY);

			Matrix_Scale(*distanceTraveled * pthis->scalingFactor, *distanceTraveled * pthis->scalingFactor, *distanceTraveled * pthis->scalingFactor, MTXMODE_APPLY);

			func_800D1FD4(&globalCtx->billboardMtxF);

			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_eff_dust.c", 506), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, SEGMENTED_TO_VIRTUAL(gEffSparklesDL));
		}
