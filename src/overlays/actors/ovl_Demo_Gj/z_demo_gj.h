...
struct DemoGj
{
	...
	/* 0x0268 */ s32 killFlag; // This actor never sets this flag, but it reads it. If set to `true` and the actor type is DEMOGJ_TYPE_DESTRUCTABLE_RUBBLE_1, DEMOGJ_TYPE_DESTRUCTABLE_RUBBLE_2 or DEMOGJ_TYPE_DESTRUCTABLE_RUBBLE_TALL, then the actor will
				   // be killed and will drop the specified amount of collectibles.
	/* 0x026C */ Vec3f explosionDirection; // This actor never sets this. Specifies which direction will this actor explode when killed using `killFlag`.
};
