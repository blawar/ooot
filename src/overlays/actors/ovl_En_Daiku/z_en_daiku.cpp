...
static Gfx* hairDLists_80[] = {object_daiku_DL_005BD0, object_daiku_DL_005AC0, object_daiku_DL_005990, object_daiku_DL_005880};
...
static EnDaikuAnimation sAnimations[] = {
    {&object_daiku_Anim_001AB0, 1.0f, 0, 0}, {&object_daiku_Anim_007DE0, 1.0f, 0, 0}, {&object_daiku_Anim_00885C, 1.0f, 0, 0}, {&object_daiku_Anim_000C44, 1.0f, 0, 0}, {&object_daiku_Anim_008164, 1.0f, 0, 0},
};
...
void EnDaiku_Change(EnDaiku* pthis, s32 animIndex, s32* currentAnimIndex)
{
	...
	Animation_Change(&pthis->skelAnime, sAnimations[animIndex].anim, 1.0f, 0.0f, Animation_GetLastFrame(sAnimations[animIndex].anim), sAnimations[animIndex].mode, transitionRate);
...
void EnDaiku_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Animation_Change(&pthis->skelAnime, sAnimations[0].anim, 1.0f, 0.0f, Animation_GetLastFrame(sAnimations[0].anim), sAnimations[0].mode, sAnimations[0].transitionRate);
...
void EnDaiku_UpdateText(EnDaiku* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if(sp2E >= 0 && sp2E <= 320 && sp2C >= 0 && sp2C <= 240 && pthis->talkState == ENDAIKU_STATE_CAN_TALK && func_8002F2CC(&pthis->actor, globalCtx, 100.0f) == 1)
		{
			if(globalCtx->sceneNum == SCENE_GERUDOWAY)
			{
				...
				else if(!(pthis->stateFlags & (ENDAIKU_STATEFLAG_GERUDOFIGHTING | ENDAIKU_STATEFLAG_GERUDODEFEATED)))
				{
...
void EnDaiku_EscapeSuccess(EnDaiku* pthis, GlobalContext* globalCtx)
{
	...
	if((gSaveContext.eventChkInf[9] & 0xF) == 0xF)
	{
		...
		gerudoGuard = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_GE3, pthis->initPos.x + vec.x, pthis->initPos.y + vec.y, pthis->initPos.z + vec.z, 0, Math_FAtan2F(-vec.x, -vec.z) * (0x8000 / M_PI), 0, 2);
