...
void BgDyYoseizo_ParticleInit(BgDyYoseizo* pthis, Vec3f* initPos, Vec3f* initVelocity, Vec3f* accel, Color_RGB8* primColor, Color_RGB8* envColor, f32 scale, s16 life, s16 type);
...
void BgDyYoseizo_Init(Actor* pthisx, GlobalContext* globalCtx2)
{
	...
	if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
	{
		...
		SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gGreatFairySkel, &gGreatFairySittingTransitionAnim, pthis->jointTable, pthis->morphTable, 28);
	}
	else
	{
		// "Stone/Jewel Fairy Fountain"
		osSyncPrintf(VT_FGCOL(GREEN) "☆☆☆☆☆ 石妖精の泉 ☆☆☆☆☆ %d\n" VT_RST, globalCtx->curSpawn);
		SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gGreatFairySkel, &gGreatFairyLayingDownTransitionAnim, pthis->jointTable, pthis->morphTable, 28);
	}
...
static Color_RGB8 sParticlePrimColors[] = {
    {255, 255, 255}, {255, 255, 100}, {100, 255, 100}, {255, 100, 100}, {255, 255, 170}, {255, 255, 100}, {100, 255, 100}, {255, 100, 100}, {255, 255, 170},
};

static Color_RGB8 sParticleEnvColors[] = {
    {155, 255, 255}, {255, 255, 100}, {100, 255, 100}, {255, 100, 100}, {255, 100, 255}, {255, 255, 100}, {100, 255, 100}, {255, 100, 100}, {100, 255, 255},
};
	...
	if(!(pthis->scale < 0.01f))
	{
		...
		for(i = 0; i < 2; i++)
		{
			if(type == 0)
			{
				...
				particleInitPos.y = pthis->actor.world.pos.y + spawnPosVariation + ((Rand_ZeroOne() - 0.5f) * (spawnPosVariation * 0.5f));
			...
			else
			{
				...
				if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
				{
					particleInitPos.y = pthis->actor.world.pos.y + spawnPosVariation + 50.0f + ((Rand_ZeroOne() - 0.5f) * (spawnPosVariation * 0.1f));
					particleInitPos.z = pthis->actor.world.pos.z + 30.0f;
				}
				else
				{
					particleInitPos.y = pthis->actor.world.pos.y + spawnPosVariation - 30.0f + ((Rand_ZeroOne() - 0.5f) * (spawnPosVariation * 0.1f));
			...
			BgDyYoseizo_ParticleInit(pthis, &particleInitPos, &particleInitVelocity, &particleAccel, &particlePrimColor, &particleEnvColor, particleScale, particleLife, particleType);
		}
	}
}
...
void BgDyYoseizo_SetupSpinGrow_NoReward(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
	{
		...
		Animation_Change(&pthis->skelAnime, &gGreatFairySittingTransitionAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_ONCE, -10.0f);
	}
	else
	{
		pthis->frameCount = Animation_GetLastFrame(&gGreatFairyLayingDownTransitionAnim);
		Animation_Change(&pthis->skelAnime, &gGreatFairyLayingDownTransitionAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_ONCE, -10.0f);
	}
...
void BgDyYoseizo_SetupGreetPlayer_NoReward(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
	{
		...
		Animation_Change(&pthis->skelAnime, &gGreatFairySittingAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_LOOP, -10.0f);
	}
	else
	{
		pthis->frameCount = Animation_GetLastFrame(&gGreatFairyLayingSidewaysAnim);
		Animation_Change(&pthis->skelAnime, &gGreatFairyLayingSidewaysAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_LOOP, -10.0f);
	}
...
void BgDyYoseizo_SetupHealPlayer_NoReward(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
	{
		...
		Animation_Change(&pthis->skelAnime, &gGreatFairyGivingUpgradeAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_ONCE, -10.0f);
	}
	else
	{
		pthis->frameCount = Animation_GetLastFrame(&gGreatFairyAnim_005810);
		Animation_Change(&pthis->skelAnime, &gGreatFairyAnim_005810, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_ONCE, -10.0f);
	}
...
void BgDyYoseizo_HealPlayer_NoReward(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->frameCount <= curFrame) && !(pthis->animationChanged))
	{
		if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
		{
			...
			Animation_Change(&pthis->skelAnime, &gGreatFairyAfterUpgradeAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_LOOP, -10.0f);
		}
		else
		{
			pthis->frameCount = Animation_GetLastFrame(&gGreatFairyAfterSpellAnim);
			Animation_Change(&pthis->skelAnime, &gGreatFairyAfterSpellAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_LOOP, -10.0f);
		}
		...
		if(!pthis->givingSpell)
		{
			...
			pthis->beam = (EnDyExtra*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_DY_EXTRA, beamPos.x, beamPos.y, beamPos.z, 0, 0, 0, beamParams);
		}
	}
	...
	if(((gSaveContext.healthCapacity == gSaveContext.health) && (gSaveContext.magic == gSaveContext.unk_13F4)) || (pthis->refillTimer == 1))
	{
		pthis->healingTimer--;
...
void BgDyYoseizo_SetupSpinShrink(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
	{
		...
		Animation_Change(&pthis->skelAnime, &gGreatFairyJewelFountainSpinShrinkAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_ONCE, -10.0f);
	}
	else
	{
		pthis->frameCount = Animation_GetLastFrame(&gGreatFairySpellFountainSpinShrinkAnim);
		Animation_Change(&pthis->skelAnime, &gGreatFairySpellFountainSpinShrinkAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_ONCE, -10.0f);
	}
...
void BgDyYoseizo_SpinGrowSetupGive_Reward(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		if((globalCtx->csCtx.state != CS_STATE_IDLE) && ((globalCtx->csCtx.npcActions[0] != NULL) && (globalCtx->csCtx.npcActions[0]->action == 3)))
		{
...
void BgDyYoseizo_Give_Reward(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	...
	if((globalCtx->csCtx.npcActions[0]->action >= 4) && (globalCtx->csCtx.npcActions[0]->action < 7))
	{
		...
		else if(!pthis->lightBallSpawned)
		{
			...
			Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_EFFECT, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, (s32)demoEffectParams);
	...
	if((globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI) && (globalCtx->csCtx.npcActions[0]->action >= 10) && (globalCtx->csCtx.npcActions[0]->action < 13))
	{
	...
	if((globalCtx->sceneNum != SCENE_DAIYOUSEI_IZUMI) && (globalCtx->csCtx.npcActions[0]->action >= 14) && (globalCtx->csCtx.npcActions[0]->action < 17))
	{
		...
		if(!pthis->itemSpawned)
		{
			...
			pthis->item = (EnExItem*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_ITEM, itemPos.x, itemPos.y, itemPos.z, 0, 0, 0, sExItemTypes[actionIndex]);
		...
		else
		{
			...
			pthis->item->actor.world.pos.y = (LINK_IS_ADULT ? player->actor.world.pos.y + 73.0f : player->actor.world.pos.y + 53.0f);
			pthis->item->actor.world.pos.z = player->actor.world.pos.z;
			pthis->item->scale = 0.3f;
		}
	}

	if((globalCtx->sceneNum != SCENE_DAIYOUSEI_IZUMI) && (globalCtx->csCtx.npcActions[0]->action == 17) && (pthis->item != NULL))
	{
	...
	if((globalCtx->csCtx.npcActions[0]->action >= 19) && (globalCtx->csCtx.npcActions[0]->action < 22) && !pthis->warpEffectSpawned)
	{
		actionIndex = globalCtx->csCtx.npcActions[0]->action - 11;
		Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DOOR_WARP1, player->actor.world.pos.x, player->actor.world.pos.y, player->actor.world.pos.z, 0, 0, 0, actionIndex);
...
void BgDyYoseizo_Update(Actor* pthisx, GlobalContext* globalCtx2)
{
	...
	if(globalCtx->csCtx.state != CS_STATE_IDLE)
	{
		...
		if(globalCtx->sceneNum == SCENE_DAIYOUSEI_IZUMI)
		{
			if((globalCtx->csCtx.frames == 32) || (globalCtx->csCtx.frames == 291) || (globalCtx->csCtx.frames == 426) || (globalCtx->csCtx.frames == 851))
			{
				phi_v1 = 1;
			}
		...
		{
			if((globalCtx->csCtx.frames == 35) || (globalCtx->csCtx.frames == 181) || (globalCtx->csCtx.frames == 462) || (globalCtx->csCtx.frames == 795))
			{
				phi_v1 = 1;
			}
...
s32 BgDyYoseizo_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* pthisx)
{
	BgDyYoseizo* pthis = (BgDyYoseizo*)pthisx;
...
void BgDyYoseizo_ParticleDraw(BgDyYoseizo* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 200; i++, particle++)
	{
		if(particle->alive == 1)
		{
			...
			gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, particle->primColor.r, particle->primColor.g, particle->primColor.b, particle->alpha);
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_bg_dy_yoseizo.c", 1810), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
