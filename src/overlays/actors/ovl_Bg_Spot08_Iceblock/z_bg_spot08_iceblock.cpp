...
void BgSpot08Iceblock_InitDynaPoly(BgSpot08Iceblock* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 flags)
{
	s32 pad;
	...
	if(pthis->dyna.bgId == BG_ACTOR_MAX)
	{
		...
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_bg_spot08_iceblock.c", 0xD9, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void BgSpot08Iceblock_Roll(BgSpot08Iceblock* pthis, GlobalContext* globalCtx)
{
	...
	if(func_8004356C(&pthis->dyna) && (playerCentroidDist > 3.0f))
	{
		...
		BgSpot08Iceblock_MultVectorScalar(&playerMoment, &playerMoment, (sInertias[rollDataIndex] * playerCentroidDist) / pthis->dyna.actor.scale.x);
	}
	else
	{
	...
	Matrix_RotateAxis(Math_FAcosF(Math3D_Cos(&sVerticalVector, &pthis->surfaceNormal)), &pthis->rotationAxis, MTXMODE_NEW);
...
void BgSpot08Iceblock_SpawnTwinFloe(BgSpot08Iceblock* pthis, GlobalContext* globalCtx)
{
	...
	{
		Actor_SpawnAsChild(
		    &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_BG_SPOT08_ICEBLOCK, pthis->dyna.actor.home.pos.x, pthis->dyna.actor.home.pos.y, pthis->dyna.actor.home.pos.z, pthis->dyna.actor.home.rot.x, pthis->dyna.actor.home.rot.y,
		    pthis->dyna.actor.home.rot.z, 0x123);
