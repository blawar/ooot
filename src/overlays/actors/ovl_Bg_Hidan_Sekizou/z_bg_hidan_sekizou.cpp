...
static void* sFireballsTexs[] = {
    gFireTempleFireball0Tex, gFireTempleFireball1Tex, gFireTempleFireball2Tex, gFireTempleFireball3Tex, gFireTempleFireball4Tex, gFireTempleFireball5Tex, gFireTempleFireball6Tex, gFireTempleFireball7Tex,
};
...
void func_8088CEC0(BgHidanSekizou* pthis, s32 arg1, s16 arg2)
{
	...
	for(i = start; i < end; i++)
	{
		...
		element->dim.worldSphere.center.x = pthis->dyna.actor.home.pos.x + (sp2C * element->dim.modelSphere.center.x) + (sp30 * element->dim.modelSphere.center.z);
		element->dim.worldSphere.center.y = (s16)pthis->dyna.actor.home.pos.y + element->dim.modelSphere.center.y;
		element->dim.worldSphere.center.z = pthis->dyna.actor.home.pos.z - (sp30 * element->dim.modelSphere.center.x) + (sp2C * element->dim.modelSphere.center.z);
...
Gfx* func_8088D9F4(GlobalContext* globalCtx, BgHidanSekizou* pthis, s16 arg2, MtxF* arg3, f32 arg4, f32 arg5, s16 arg6, Gfx* arg7)
{
	f32 temp_f0;
	f32 temp_f2;
	f32 phi_f12;
...
void BgHidanSekizou_Draw(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->dyna.actor.params == 0)
	{
		if(pthis->unk_168[0] > 0)
		{
			...
			{
				POLY_XLU_DISP = func_8088DC50(globalCtx, pthis, pthis->dyna.actor.shape.rot.y + 0x2000, pthis->unk_168[0], POLY_XLU_DISP);
				POLY_XLU_DISP = func_8088DC50(globalCtx, pthis, pthis->dyna.actor.shape.rot.y - 0x2000, pthis->unk_168[0], POLY_XLU_DISP);
			}
			else
			{
	...
	else
	{
		...
		for(i = 0; i < 4; i++)
		{
			...
			{
				POLY_XLU_DISP = func_8088DC50(globalCtx, pthis, pthis->dyna.actor.shape.rot.y + index * 0x4000, pthis->unk_168[index], POLY_XLU_DISP);
			}
		}
	}
...
void BgHidanSekizou_Reset(Actor* pthisx, GlobalContext* globalCtx)
{
	...
	sColChkInfoInit = {1, 40, 240, MASS_IMMOVABLE};
}
