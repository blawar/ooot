...
void EnHeishi4_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->type == HEISHI4_AT_MARKET_DYING)
	{
		...
		SkelAnime_Init(globalCtx, &pthis->skelAnime, &gEnHeishiSkel, &gEnHeishiDyingGuardAnim_00C444, pthis->jointTable, pthis->morphTable, 17);
	}
	else
	{
		pthis->height = 60.0f;
...
void func_80A5673C(EnHeishi4* pthis, GlobalContext* globalCtx)
{
	...
	if(gSaveContext.eventChkInf[8] & 1)
	{
		if(!(gSaveContext.infTable[6] & 0x1000))
		{
			...
			Animation_Change(&pthis->skelAnime, &gEnHeishiDyingGuardAnim_00C444, 1.0f, 0.0f, (s16)frames, ANIMMODE_LOOP, -10.0f);
...
s32 EnHeishi_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnHeishi4* pthis = (EnHeishi4*)thisx;
