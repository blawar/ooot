...
void EnMm2_ChangeAnimation(EnMm2* pthis, s32 animationIndex, s32* previousAnimation)
{
	...
	{
		Animation_Change(
		    &pthis->skelAnime, sAnimationEntries[animationIndex].animation, sAnimationEntries[animationIndex].playSpeed, 0.0f, (f32)Animation_GetLastFrame(sAnimationEntries[animationIndex].animation), sAnimationEntries[animationIndex].mode,
		    phi_f0);
	}
	else
	{
		Animation_Change(
		    &pthis->skelAnime, sAnimationEntries[animationIndex].animation, sAnimationEntries[animationIndex].playSpeed, (f32)Animation_GetLastFrame(sAnimationEntries[animationIndex].animation), 0.0f, sAnimationEntries[animationIndex].mode,
		    phi_f0);
	}
...
void EnMm2_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	Animation_Change(
	    &pthis->skelAnime, sAnimationEntries[RM2_ANIM_SIT_WAIT].animation, 1.0f, 0.0f, Animation_GetLastFrame(sAnimationEntries[RM2_ANIM_SIT_WAIT].animation), sAnimationEntries[RM2_ANIM_SIT_WAIT].mode, sAnimationEntries[RM2_ANIM_SIT_WAIT].morphFrames);
...
void EnMm2_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_mm2.c", 634);
	func_80093D18(globalCtx->gfxCtx);
	gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(mouthTextures_46[pthis->mouthTexIndex]));
	SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnMm2_OverrideLimbDraw, EnMm2_PostLimbDraw, pthis);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_mm2.c", 654);
}
