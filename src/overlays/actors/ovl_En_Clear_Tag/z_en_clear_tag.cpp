...
void EnClearTag_CreateDebrisEffect(GlobalContext* globalCtx, Vec3f* position, Vec3f* velocity, Vec3f* acceleration, f32 scale, f32 floorHeight);
...
void EnClearTag_CreateFlashEffect(GlobalContext* globalCtx, Vec3f* position, f32 scale, f32 floorHeight, Vec3f* floorTangent);
...
void EnClearTag_Update(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	if(pthis->drawMode != CLEAR_TAG_DRAW_MODE_EFFECT)
	{
		...
		switch(pthis->state)
		{
			...
			case CLEAR_TAG_STATE_FLYING:
			{
				...
				if(pthis->timers[CLEAR_TAG_TIMER_ARWING_UPDATE_STATE] == 0)
				{
					...
					else
					{
						...
						if(pthis->actor.params == CLEAR_TAG_ARWING)
						{
							...
							pthis->targetPosition.x = Rand_CenteredFloat(700.0f) + (player->actor.world.pos.x + targetCircleX);
							pthis->targetPosition.y = Rand_ZeroFloat(200.0f) + player->actor.world.pos.y + 150.0f;
							pthis->targetPosition.z = Rand_CenteredFloat(700.0f) + (player->actor.world.pos.z + targetCircleZ);
						}
						else
						{
				...
				worldRotationTargetX = Math_FAtan2F(vectorToTargetY, sqrtf(SQ(vectorToTargetX) + SQ(vectorToTargetZ))) * (0x8000 / M_PI);
				...
				worldRotationTargetZ = Math_SmoothStepToS(&pthis->actor.world.rot.y, worldRotationTargetY, rotationScale, pthis->targetDirection.y, 0);
				...
				if(ABS(worldRotationTargetZ) < 0x1000)
				{
					...
					if((pthis->frameCounter % 4) == 0 && (Rand_ZeroOne() < 0.75f) && (pthis->state == CLEAR_TAG_STATE_TARGET_LOCKED))
					{
				...
				else
				{
					...
					Math_ApproachS(&pthis->actor.world.rot.z, worldRotationTargetZ, rotationScale, pthis->targetDirection.z);
				...
				if(pthis->shouldShootLaser)
				{
					...
					Actor_Spawn(
					    &globalCtx->actorCtx, globalCtx, ACTOR_EN_CLEAR_TAG, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.world.rot.x, pthis->actor.world.rot.y, pthis->actor.world.rot.z,
					    CLEAR_TAG_STATE_LASER);
				}
			}
		...
		if(pthis->state < CLEAR_TAG_STATE_LASER)
		{
			...
			if(pthis->cutsceneMode != CLEAR_TAG_CUTSCENE_MODE_NONE)
			{
				...
				if(pthis->cameraId != SUBCAM_FREE)
				{
					...
					Gameplay_CameraSetAtEye(globalCtx, pthis->cameraId, &pthis->cutsceneCameraEye, &pthis->cutsceneCameraAt);
				}
	...
	if(pthis->shouldExplode)
	{
		...
		EnClearTag_CreateFlashEffect(globalCtx, &crashEffectLocation, 6.0f, pthis->actor.floorHeight, &pthis->floorTangent);
		...
		for(i = 0; i < 15; i++)
		{
			...
			EnClearTag_CreateDebrisEffect(globalCtx, &crashEffectLocation, &crashEffectVelocity, &debrisEffectAcceleration, Rand_ZeroFloat(0.15f) + 0.075f, pthis->actor.floorHeight);
		}
	}
...
void EnClearTag_DrawEffects(GlobalContext* globalCtx)
{
	...
	for(i = 0; i < CLEAR_TAG_EFFECT_MAX_COUNT; i++, effect++)
	{
		if(effect->type == CLEAR_TAG_EFFECT_DEBRIS)
		{
			...
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_clear_tag.c", 1307), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	for(i = 0; i < CLEAR_TAG_EFFECT_MAX_COUNT; i++, effect++)
	{
		if(effect->type == CLEAR_TAG_EFFECT_FLASH)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_clear_tag.c", 1342), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	for(i = 0; i < CLEAR_TAG_EFFECT_MAX_COUNT; i++, effect++)
	{
		if(effect->type == CLEAR_TAG_EFFECT_SMOKE)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_clear_tag.c", 1392), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	for(i = 0; i < CLEAR_TAG_EFFECT_MAX_COUNT; i++, effect++)
	{
		if(effect->type == CLEAR_TAG_EFFECT_FIRE)
		{
			...
			func_800D1FD4(&globalCtx->billboardMtxF);
			Matrix_Scale(effect->scale, effect->scale, 1.0f, MTXMODE_APPLY);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_clear_tag.c", 1439), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, gArwingFireEffectDL);
		}
	}
	...
	for(i = 0; i < CLEAR_TAG_EFFECT_MAX_COUNT; i++, effect++)
	{
		if(effect->type == CLEAR_TAG_EFFECT_FLASH)
		{
			...
			func_800D1FD4(&globalCtx->billboardMtxF);
			Matrix_Scale(effect->scale, effect->scale, 1.0f, MTXMODE_APPLY);
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_clear_tag.c", 1470), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
