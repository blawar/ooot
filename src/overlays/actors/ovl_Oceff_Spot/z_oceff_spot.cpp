...
ActorInit Oceff_Spot_InitVars = {
    ACTOR_OCEFF_SPOT, ACTORCAT_ITEMACTION, FLAGS, OBJECT_GAMEPLAY_KEEP, sizeof(OceffSpot), (ActorFunc)OceffSpot_Init, (ActorFunc)OceffSpot_Destroy, (ActorFunc)OceffSpot_Update, (ActorFunc)OceffSpot_Draw, (ActorFunc)OceffSpot_Reset,
};
...
void OceffSpot_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo1, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
	pthis->lightNode1 = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &pthis->lightInfo1);

	Lights_PointNoGlowSetInfo(&pthis->lightInfo2, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
...
void OceffSpot_End(OceffSpot* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		{
			if(globalCtx->msgCtx.ocarinaAction != OCARINA_ACTION_CHECK_NOWARP_DONE || globalCtx->msgCtx.ocarinaMode != OCARINA_MODE_08)
			{
...
void OceffSpot_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo1, (s16)pthis->actor.world.pos.x, (s16)pthis->actor.world.pos.y + 55.0f, (s16)pthis->actor.world.pos.z, (s32)(255.0f * temp), (s32)(255.0f * temp), (s32)(200.0f * temp), (s16)(100.0f * temp));

	Lights_PointNoGlowSetInfo(
	    &pthis->lightInfo2, (s16)pthis->actor.world.pos.x + Math_SinS(player->actor.shape.rot.y) * 20.0f, (s16)pthis->actor.world.pos.y + 20.0f, (s16)pthis->actor.world.pos.z + Math_CosS(player->actor.shape.rot.y) * 20.0f, (s32)(255.0f * temp),
...
void OceffSpot_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_oceff_spot.c", 469), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_XLU_DISP++, sCylinderMaterialDL);
	gSPDisplayList(POLY_XLU_DISP++, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, scroll * 2, scroll * (-2), 32, 32, 1, 0, scroll * (-8), 32, 32));
