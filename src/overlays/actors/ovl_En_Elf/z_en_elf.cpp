...
static FairyColorFlags sColorFlags[] = {
    {0, 0, 0}, {1, 0, 0}, {1, 2, 0}, {1, 0, 2}, {0, 1, 0}, {2, 1, 0}, {0, 1, 2}, {0, 0, 1}, {2, 0, 1}, {0, 2, 1}, {1, 1, 0}, {1, 0, 1}, {0, 1, 1},
};
...
void EnElf_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointGlowSetInfo(&pthis->lightInfoGlow, thisx->world.pos.x, thisx->world.pos.y, thisx->world.pos.z, 255, 255, 255, 0);
	pthis->lightNodeGlow = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &pthis->lightInfoGlow);

	Lights_PointNoGlowSetInfo(&pthis->lightInfoNoGlow, thisx->world.pos.x, thisx->world.pos.y, thisx->world.pos.z, 255, 255, 255, 0);
	...
	switch(thisx->params)
	{
		...
		case FAIRY_SPAWNER:
			...
			for(i = 0; i < 8; i++)
			{
				Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ELF, thisx->world.pos.x, thisx->world.pos.y - 30.0f, thisx->world.pos.z, 0, 0, 0, FAIRY_HEAL);
			}
			break;
		default:
...
void func_80A03018(EnElf* pthis, GlobalContext* globalCtx)
{
	...
	switch(pthis->unk_2A8)
	{
		...
		case 3:
			targetYaw = Math_Atan2S(-(pthis->actor.world.pos.z - player->actor.world.pos.z), -(pthis->actor.world.pos.x - player->actor.world.pos.x));
			break;
...
void EnElf_UpdateLights(EnElf* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->fairyFlags & 0x20)
	{
		...
		Lights_PointNoGlowSetInfo(&pthis->lightInfoNoGlow, player->actor.world.pos.x, (s16)(player->actor.world.pos.y) + 60.0f, player->actor.world.pos.z, 255, 255, 255, 200);
	}
	else
	{
		Lights_PointNoGlowSetInfo(&pthis->lightInfoNoGlow, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 255, 255, 255, -1);
	}

	Lights_PointGlowSetInfo(&pthis->lightInfoGlow, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 255, 255, 255, glowLightRadius);
...
void func_80A04414(EnElf* pthis, GlobalContext* globalCtx)
{
	...
	else
	{
		if(pthis->unk_2C6 == 0)
		{
			if((arrowPointedActor == NULL) || (Math_Vec3f_DistXYZ(&pthis->actor.world.pos, &globalCtx->actorCtx.targetCtx.naviRefPos) < 50.0f))
			{
				pthis->unk_2C6 = 1;
			}
		}
		else if(pthis->unk_29C != 0.0f)
		{
			...
			else
			{
				...
				EnElf_ChangeColor(&pthis->innerColor, &globalCtx->actorCtx.targetCtx.naviInner, &pthis->innerColor, transitionRate);
				EnElf_ChangeColor(&pthis->outerColor, &globalCtx->actorCtx.targetCtx.naviOuter, &pthis->outerColor, transitionRate);
			}
		}
	}
...
void EnElf_SpawnSparkles(EnElf* pthis, GlobalContext* globalCtx, s32 sparkleLife)
{
	...
	EffectSsKiraKira_SpawnDispersed(globalCtx, &sparklePos, &sparkleVelocity_105, &sparkleAccel_105, &primColor, &envColor, 1000, sparkleLife);
}
...
void func_80A04D90(EnElf* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.floorHeight = BgCheck_EntityRaycastFloor5(globalCtx, &globalCtx->colCtx, &pthis->actor.floorPoly, &bgId, &pthis->actor, &pthis->actor.world.pos);
...
void func_80A04DE4(EnElf* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->fairyFlags & 0x10)
	{
		...
		if((player->targetedActor == NULL) || (&player->actor == player->targetedActor) || (&pthis->actor == player->targetedActor))
		{
...
void func_80A053F0(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->unk_2A4 > 0.0f)
	{
		...
		Environment_AdjustLights(globalCtx, SQ(pthis->unk_2A4) * pthis->unk_2A4, player->actor.projectedPos.z + 780.0f, 0.2f, 0.5f);
	}
...
s32 EnElf_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx)
{
	s32 pad;
	f32 scale;
	Vec3f mtxMult;
...
void EnElf_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->unk_2A8 != 8) && !(pthis->fairyFlags & 8))
	{
		if(!(player->stateFlags1 & PLAYER_STATE1_20) || (kREG(90) < pthis->actor.projectedPos.z))
		{
			...
			gDPSetPrimColor(dListHead++, 0, 0x01, (u8)pthis->innerColor.r, (u8)pthis->innerColor.g, (u8)pthis->innerColor.b, (u8)(pthis->innerColor.a * alphaScale));
