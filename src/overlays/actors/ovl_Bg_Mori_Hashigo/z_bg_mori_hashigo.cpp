...
static InitChainEntry sInitChainClasp[] = {
    ICHAIN_F32(uncullZoneForward, 1000, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneScale, 400, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneDownward, 1000, ICHAIN_CONTINUE), ICHAIN_U8(targetMode, 3, ICHAIN_CONTINUE),
...
void BgMoriHashigo_InitDynapoly(BgMoriHashigo* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 moveFlag)
{
	s32 pad;
	...
	if(pthis->dyna.bgId == BG_ACTOR_MAX)
	{
		...
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_bg_mori_hashigo.c", 164, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
s32 BgMoriHashigo_SpawnLadder(BgMoriHashigo* pthis, GlobalContext* globalCtx)
{
	...
	ladder = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_BG_MORI_HASHIGO, pos.x, pos.y, pos.z, pthis->dyna.actor.world.rot.x, pthis->dyna.actor.world.rot.y, pthis->dyna.actor.world.rot.z, 0);
	if(ladder != NULL)
	{
		return true;
	}
	...
	{
		// "Ladder failure"
		osSyncPrintf("Error : 梯子の発生失敗(%s %d)(arg_data 0x%04x)\n", "../z_bg_mori_hashigo.c", 220, pthis->dyna.actor.params);
		return false;
	}
}
...
void BgMoriHashigo_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	{
		// "Bank danger!"
		osSyncPrintf("Error : バンク危険！(arg_data 0x%04x)(%s %d)\n", pthis->dyna.actor.params, "../z_bg_mori_hashigo.c", 312);
		Actor_Kill(&pthis->dyna.actor);
	}
	else
	{
