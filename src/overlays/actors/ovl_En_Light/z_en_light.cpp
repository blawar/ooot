...
static FlameParams D_80A9E840[] = {
    {{255, 200, 0, 255}, {255, 0, 0}, 75},     {{255, 200, 0, 255}, {255, 0, 0}, 75},	  {{0, 170, 255, 255}, {0, 0, 255}, 75},     {{170, 255, 0, 255}, {0, 150, 0}, 75},
    {{255, 200, 0, 255}, {255, 0, 0}, 40},     {{255, 200, 0, 255}, {255, 0, 0}, 75},	  {{170, 255, 0, 255}, {0, 150, 0}, 75},     {{0, 170, 255, 255}, {0, 0, 255}, 75},
    {{255, 0, 170, 255}, {200, 0, 0}, 75},     {{255, 255, 170, 255}, {255, 50, 0}, 75},  {{255, 255, 170, 255}, {255, 255, 0}, 75}, {{255, 255, 170, 255}, {100, 255, 0}, 75},
    {{255, 170, 255, 255}, {255, 0, 100}, 75}, {{255, 170, 255, 255}, {100, 0, 255}, 75}, {{170, 255, 255, 255}, {0, 0, 255}, 75},   {{170, 255, 255, 255}, {0, 150, 255}, 75},
};
...
void EnLight_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(gSaveContext.gameMode == 3)
	{
		...
		Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, yOffset + (s16)pthis->actor.world.pos.y, pthis->actor.world.pos.z, 255, 255, 180, -1);
	}
	else
	{
		yOffset = (pthis->actor.params < 0) ? 1 : 40;
		Lights_PointGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, yOffset + (s16)pthis->actor.world.pos.y, pthis->actor.world.pos.z, 255, 255, 180, -1);
	}
...
void EnLight_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	Lights_PointSetColorAndRadius(&pthis->lightInfo, (flameParams->primColor.r * intensity), (flameParams->primColor.g * intensity), (flameParams->primColor.b * intensity), radius);
...
void EnLight_UpdateSwitch(Actor* thisx, GlobalContext* globalCtx)
{
	...
	intensity = (Rand_ZeroOne() * 0.5f) + 0.5f;
	Lights_PointSetColorAndRadius(&pthis->lightInfo, (flameParams->primColor.r * intensity), (flameParams->primColor.g * intensity), (flameParams->primColor.b * intensity), 300.0f * scale);
	EnLight_UpdatePosRot(pthis, globalCtx);
...
void EnLight_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params >= 0)
	{
		gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 0, 0, 32, 64, 1, 0, (pthis->timer.whole() * -20) & 511, 32, 128));
	...
	{
		gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 0, 0, 16, 32, 1, ((pthis->timer.whole() * 2) & 63), (pthis->timer.whole() * -6) & 127 * 1, 16, 32));
	...
	Matrix_RotateY((s16)((Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) - pthis->actor.shape.rot.y) + 0x8000) * (M_PI / 32768.0f), MTXMODE_APPLY);
