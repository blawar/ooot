...
void ArrowFire_Draw(Actor* pthisx, GlobalContext* globalCtx2) {
    ...
    if ((arrow != NULL) && (arrow->actor.update != NULL) && (pthis->timer < 255)) {
        ...
        gSPDisplayList(POLY_XLU_DISP++,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 255 - (uintptr_t)(stateFrames * 2) % 256, 0, 64, 32, 1,
                                        255 - (uintptr_t)stateFrames % 256, 511 - (uintptr_t)(stateFrames * 10) % 512, 64, 64));
