...
s32 func_80B0C9F0(EnSw* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.xyzDistToPlayerSq < SQ(400.0f) && ((pthis->actor.params & 0xE000) >> 0xD) == 0 && globalCtx->actorCtx.unk_02 != 0)
	{
...
void func_80B0CEA8(EnSw* pthis, GlobalContext* globalCtx)
{
	if(!(pthis->actor.scale.x < 0.0139999995f))
	{
		...
		{
			Audio_PlayActorSound2(&pthis->actor, ((pthis->actor.params & 0xE000) >> 0xD) > 0 ? NA_SE_EN_STALGOLD_ROLL : NA_SE_EN_STALWALL_ROLL);
		}
	}
}
...
void func_80B0D878(EnSw* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->unk_394 == 0) && (pthis->unk_392 == 0))
	{
		...
		temp_v0 = Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_SI, pthis->actor.world.pos.x + x, pthis->actor.world.pos.y + y, pthis->actor.world.pos.z + z, 0, 0, 0, pthis->actor.params);
		if(temp_v0 != NULL)
		{
			temp_v0->parent = NULL;
		}
	...
	if((pthis->unk_392 == 0) && (DECR(pthis->unk_394) != 0))
	{
		...
		EffectSsDeadDb_Spawn(globalCtx, &pos, &velAndAccel, &velAndAccel, 42, 0, 255, 255, 255, 255, 255, 0, 0, 1, 9, true);
	}
}
...
void func_80B0DC7C(EnSw* pthis, GlobalContext* globalCtx)
{
	...
	if(DECR(pthis->unk_394) != 0)
	{
		...
		EffectSsDeadDb_Spawn(globalCtx, &pos, &velAndAccel, &velAndAccel, 42, 0, 255, 255, 255, 255, 255, 0, 0, 1, 9, 1);
		pthis->actor.shape.rot.x += 0x1000;
		pthis->actor.shape.rot.z += 0x1000;
	}
	else
...
s32 func_80B0DEA8(EnSw* pthis, GlobalContext* globalCtx, s32 arg2)
{
	...
	else if(!BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.world.pos, &player->actor.world.pos, &sp48, &sp58, true, false, false, true, &sp54))
	{
		return true;
	}
	else
...
s32 func_80B0DFFC(EnSw* pthis, GlobalContext* globalCtx)
{
	...
	if(BgCheck_EntityLineTest1(&globalCtx->colCtx, &pthis->actor.world.pos, &pthis->unk_484, &sp50, &pthis->unk_430, true, false, false, true, &sp5C))
	{
...
void func_80B0E5E0(EnSw* pthis, GlobalContext* globalCtx)
{
	...
	{
		rand = Rand_ZeroOne();
		pthis->unk_444 = ((s16)(20000.0f * rand) + 0x2EE0) * (Rand_ZeroOne() >= 0.5f ? 1.0f : -1.0f) + pthis->actor.world.rot.z;
