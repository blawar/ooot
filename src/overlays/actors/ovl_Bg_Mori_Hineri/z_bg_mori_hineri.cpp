...
void BgMoriHineri_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	else
	{
		...
		{
			moriHineriObjIdx = (pthis->dyna.actor.params == 2) ? Object_GetIndex(&globalCtx->objectCtx, OBJECT_MORI_HINERI2) : Object_GetIndex(&globalCtx->objectCtx, OBJECT_MORI_HINERI2A);
		}
...
void func_808A39FC(BgMoriHineri* pthis, GlobalContext* globalCtx)
{
	...
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->moriHineriObjIdx) && Object_IsLoaded(&globalCtx->objectCtx, pthis->moriTexObjIdx) && ((pthis->boxObjIdx < 0) || Object_IsLoaded(&globalCtx->objectCtx, pthis->boxObjIdx)))
	{
		...
		if(pthis->dyna.actor.params >= 4)
		{
			...
			{
				pthis->moriHineriObjIdx = (pthis->dyna.actor.params == 2) ? Object_GetIndex(&globalCtx->objectCtx, OBJECT_MORI_HINERI2A) : Object_GetIndex(&globalCtx->objectCtx, OBJECT_MORI_HINERI2);
			}
...
void BgMoriHineri_SpawnBossKeyChest(BgMoriHineri* pthis, GlobalContext* globalCtx)
{
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BOX, pthis->dyna.actor.world.pos.x + 147.0f, pthis->dyna.actor.world.pos.y + -245.0f, pthis->dyna.actor.world.pos.z + -453.0f, 0, 0x4000, 0, 0x27EE);
	pthis->actionFunc = BgMoriHineri_DoNothing;
}
...
void func_808A3D58(BgMoriHineri* pthis, GlobalContext* globalCtx)
{
	...
	if((Flags_GetSwitch(globalCtx, pthis->switchFlag) && (pthis->dyna.actor.params == 0 || pthis->dyna.actor.params == 2)) || (!Flags_GetSwitch(globalCtx, pthis->switchFlag) && (pthis->dyna.actor.params == 1 || pthis->dyna.actor.params == 3)))
	{
		pthis->dyna.actor.draw = BgMoriHineri_DrawHallAndRoom;
...
void func_808A3E54(BgMoriHineri* pthis, GlobalContext* globalCtx)
{
	...
	if((sNextCamIdx >= SUBCAM_FIRST) && ((GET_ACTIVE_CAM(globalCtx)->eye.z - pthis->dyna.actor.world.pos.z) < 1100.0f))
	{
