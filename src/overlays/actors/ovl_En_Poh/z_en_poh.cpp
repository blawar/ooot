...
{
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0);
...
void EnPoh_Talk(EnPoh* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params == EN_POH_FLAT || pthis->actor.params == EN_POH_SHARP)
	{
		...
		else if((pthis->actor.params == EN_POH_FLAT && Flags_GetSwitch(globalCtx, 0xA)) || (pthis->actor.params == EN_POH_SHARP && Flags_GetSwitch(globalCtx, 0xB)))
		{
...
void func_80ADFA90(EnPoh* pthis, s32 arg1)
{
	...
	Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->info->lightColor.r, pthis->info->lightColor.g, pthis->info->lightColor.b, pthis->lightColor.a * (200.0f / 255));
}
...
{
	if(pthis->actionFunc != EnPoh_Appear && pthis->actionFunc != EnPoh_Disappear && pthis->actionFunc != func_80ADEF38 && pthis->actionFunc != EnPoh_ComposerAppear)
	{
		...
		else if(pthis->lightColor.a == 0 && pthis->visibilityTimer == 0 && (pthis->actionFunc == func_80ADEAC4 || pthis->actionFunc == EnPoh_Idle || pthis->actionFunc == func_80ADEC9C || pthis->actionFunc == func_80ADF5E0))
		{
...
void EnPoh_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(Object_IsLoaded(&globalCtx->objectCtx, pthis->objectIdx) != 0)
	{
		...
		if(pthis->infoIdx == EN_POH_INFO_NORMAL)
		{
			SkelAnime_Init(globalCtx, &pthis->skelAnime, &gPoeSkel, &gPoeFloatAnim, pthis->jointTable, pthis->morphTable, 21);
			pthis->actor.draw = EnPoh_DrawRegular;
		}
		else
		{
			SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gPoeComposerSkel, &gPoeComposerFloatAnim, pthis->jointTable, pthis->morphTable, 12);
...
void func_80AE089C(EnPoh* pthis)
{
	...
	if((pthis->actionFunc == func_80ADEF38 || pthis->actionFunc == EnPoh_ComposerAppear) && pthis->skelAnime.curFrame < 12.0f)
	{
...
void EnPoh_UpdateLiving(Actor* thisx, GlobalContext* globalCtx)
{
	...
	vec.z = pthis->actor.world.pos.z;
	pthis->actor.floorHeight = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->actor.floorPoly, &sp38, &pthis->actor, &vec);
	func_80AE089C(pthis);
	pthis->actor.shape.shadowAlpha = pthis->lightColor.a;
}

s32 EnPoh_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfxP)
{
	EnPoh* pthis = (EnPoh*)thisx;
...
void EnPoh_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP)
{
	...
	{
		gSPMatrix((*gfxP)++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_poh.c", 2460), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	if(limbIndex == pthis->info->unk_6)
	{
		...
		Lights_PointGlowSetInfo(
		    &pthis->lightInfo, pthis->colliderSph.elements[0].dim.worldSphere.center.x, pthis->colliderSph.elements[0].dim.worldSphere.center.y, pthis->colliderSph.elements[0].dim.worldSphere.center.z, pthis->envColor.r, pthis->envColor.g,
		    pthis->envColor.b, pthis->envColor.a * (200.0f / 255));
	}
}
...
void EnPoh_DrawRegular(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_poh.c", 2629);
	func_80AE067C(pthis);
	...
	if(pthis->lightColor.a == 255 || pthis->lightColor.a == 0)
	{
		...
		POLY_OPA_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnPoh_OverrideLimbDraw, EnPoh_PostLimbDraw, &pthis->actor, POLY_OPA_DISP);
	}
	else
	{
		gDPSetEnvColor(POLY_XLU_DISP++, 255, 255, 255, pthis->lightColor.a);
		gSPSegment(POLY_XLU_DISP++, 0x08, D_80116280);
		POLY_XLU_DISP = SkelAnime_Draw(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, EnPoh_OverrideLimbDraw, EnPoh_PostLimbDraw, &pthis->actor, POLY_XLU_DISP);
	}
	gDPPipeSync(POLY_OPA_DISP++);
	gDPSetEnvColor(POLY_OPA_DISP++, pthis->envColor.r, pthis->envColor.g, pthis->envColor.b, 255);
	Matrix_Put(&pthis->unk_368);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_poh.c", 2676), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, pthis->info->lanternDisplayList);
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_poh.c", 2681);
}
...
void EnPoh_DrawComposer(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_poh.c", 2694);
	func_80AE067C(pthis);
	if(pthis->actor.params == EN_POH_SHARP)
	{
		sp90 = &D_80AE1B4C;
	...
	{
		func_80093D18(globalCtx->gfxCtx);
		gSPSegment(POLY_OPA_DISP++, 0x08, Gfx_EnvColor(globalCtx->gfxCtx, pthis->lightColor.r, pthis->lightColor.g, pthis->lightColor.b, pthis->lightColor.a));
		gSPSegment(POLY_OPA_DISP++, 0x0A, Gfx_EnvColor(globalCtx->gfxCtx, sp90->r, sp90->g, sp90->b, pthis->lightColor.a));
		gSPSegment(POLY_OPA_DISP++, 0x0B, Gfx_EnvColor(globalCtx->gfxCtx, phi_t0->r, phi_t0->g, phi_t0->b, pthis->lightColor.a));
		gSPSegment(POLY_OPA_DISP++, 0x0C, D_80116280 + 2);
		POLY_OPA_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnPoh_OverrideLimbDraw, EnPoh_PostLimbDraw, &pthis->actor, POLY_OPA_DISP);
	}
	else
	{
		func_80093D18(globalCtx->gfxCtx);
		...
		gSPSegment(POLY_XLU_DISP++, 0x0A, Gfx_EnvColor(globalCtx->gfxCtx, sp90->r, sp90->g, sp90->b, pthis->lightColor.a));
		gSPSegment(POLY_XLU_DISP++, 0x0B, Gfx_EnvColor(globalCtx->gfxCtx, phi_t0->r, phi_t0->g, phi_t0->b, pthis->lightColor.a));
		gSPSegment(POLY_XLU_DISP++, 0x0C, D_80116280);
		POLY_XLU_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, pthis->skelAnime.dListCount, EnPoh_OverrideLimbDraw, EnPoh_PostLimbDraw, &pthis->actor, POLY_XLU_DISP);
	}
	gDPPipeSync(POLY_OPA_DISP++);
	gDPSetEnvColor(POLY_OPA_DISP++, pthis->envColor.r, pthis->envColor.g, pthis->envColor.b, 255);
	Matrix_Put(&pthis->unk_368);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_poh.c", 2787), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPDisplayList(POLY_OPA_DISP++, pthis->info->lanternDisplayList);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_poh.c", 2802);
}
...
void EnPoh_DrawSoul(Actor* thisx, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_poh.c", 2833);

	if(pthis->actionFunc == EnPoh_Death)
	{
		func_80093D18(globalCtx->gfxCtx);
		gDPSetEnvColor(POLY_OPA_DISP++, pthis->envColor.r, pthis->envColor.g, pthis->envColor.b, 255);
		Lights_PointGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->envColor.r, pthis->envColor.g, pthis->envColor.b, 200);
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_poh.c", 2854), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_OPA_DISP++, pthis->info->lanternDisplayList);
		if(pthis->infoIdx == EN_POH_INFO_COMPOSER)
		{
	...
	{
		func_80093D84(globalCtx->gfxCtx);
		gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->gfxCtx, 0, 0, 0, 0x20, 0x40, 1, 0, (s32)(pthis->visibilityTimer * pthis->info->unk_8) % 512U, 0x20, 0x80));
		gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, pthis->info->primColor.r, pthis->info->primColor.g, pthis->info->primColor.b, pthis->lightColor.a);
		...
		gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_poh.c", 2910), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		gSPDisplayList(POLY_XLU_DISP++, pthis->info->soulDisplayList);
	}
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_poh.c", 2916);
}
...
void EnPoh_Reset(Actor* pthisx, GlobalContext* globalCtx)
{
	...
	D_80AE1B4C = {75, 20, 25, 255};

	D_80AE1B50 = {80, 110, 90, 255};

	D_80AE1B54 = {90, 85, 50, 255};

	D_80AE1B58 = {100, 90, 100, 255};

	D_80AE1B60 = {0.0f, 3.0f, 0.0f};

	D_80AE1B6C = {0.0f, 0.0f, 0.0f};
}
