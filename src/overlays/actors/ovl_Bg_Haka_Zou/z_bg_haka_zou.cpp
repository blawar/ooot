...
void BgHakaZou_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	pthis->requiredObjBankIndex = (thisx->params == STA_BOMBABLE_RUBBLE) ? Object_GetIndex(&globalCtx->objectCtx, OBJECT_HAKACH_OBJECTS) : Object_GetIndex(&globalCtx->objectCtx, OBJECT_HAKA_OBJECTS);
...
void func_80882CC4(BgHakaZou* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < 3; i++)
	{
		for(j = 0; j < 3; j++)
		{
			...
			Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_HAKA_ZOU, actorSpawnPos.x, actorSpawnPos.y, actorSpawnPos.z, 0, pthis->dyna.actor.shape.rot.y, 0, pthis->dyna.actor.params + 2);
