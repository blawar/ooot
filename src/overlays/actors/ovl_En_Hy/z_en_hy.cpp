...
void EnHy_UpdateEyes(EnHy* pthis) {
    if (DECR(pthis->nextEyeIndexTimer) == 0) {
        ...
        if ((sHeadInfo[headInfoIndex].eyeTextures != NULL) &&
            (sHeadInfo[headInfoIndex].eyeTextures[pthis->curEyeIndex] == NULL)) {
...
s32 EnHy_ShouldSpawn(EnHy* pthis, GlobalContext* globalCtx) {
    switch (globalCtx->sceneNum) {
        case SCENE_SPOT01:
            if (!((pthis->actor.params & 0x7F) == ENHY_TYPE_BOJ_9 || (pthis->actor.params & 0x7F) == ENHY_TYPE_BOJ_10 ||
                  (pthis->actor.params & 0x7F) == ENHY_TYPE_BOJ_12 || (pthis->actor.params & 0x7F) == ENHY_TYPE_AHG_2 ||
        ...
        case SCENE_KAKARIKO:
            ...
            } else if (!((pthis->actor.params & 0x7F) == ENHY_TYPE_BOJ_9 ||
                         (pthis->actor.params & 0x7F) == ENHY_TYPE_AHG_2 ||
                         (pthis->actor.params & 0x7F) == ENHY_TYPE_BJI_7)) {
                return true;
            } else if (IS_DAY) {
                return false;
...
void EnHy_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((pthis->actor.params & 0x7F) >= ENHY_TYPE_MAX || !EnHy_FindOsAnimeObject(pthis, globalCtx) ||
...
void func_80A714C4(EnHy* pthis, GlobalContext* globalCtx) {
    if (Actor_HasParent(&pthis->actor, globalCtx)) {
        ...
        func_8002F434(&pthis->actor, globalCtx, pthis->unkGetItemId, pthis->actor.xzDistToPlayer + 1.0f,
                      fabsf(pthis->actor.yDistToPlayer) + 1.0f);
    }
}
...
void EnHy_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->actionFunc != EnHy_InitImpl) {
        ...
        SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                              pthis->skelAnime.dListCount, EnHy_OverrideLimbDraw, EnHy_PostLimbDraw, &pthis->actor);
    }
