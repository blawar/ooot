...
void EnNiwGirl_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gNiwGirlSkel, &gNiwGirlRunAnim, pthis->jointTable, pthis->morphTable, 17);
	...
	pthis->chasedEnNiw =
	    (EnNiw*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_NIW, pthis->actor.world.pos.x + vec2.x, pthis->actor.world.pos.y + vec2.y, pthis->actor.world.pos.z + vec2.z, 0, pthis->actor.world.rot.y, 0, 0xA);
...
void func_80AB9210(EnNiwGirl* pthis, GlobalContext* globalCtx)
{
	...
	Math_SmoothStepToS(&pthis->actor.shape.rot.y, Math_FAtan2F(xDistBetween, zDistBetween) * (0x8000 / M_PI), 3, pthis->unk_27C, 0);
...
s32 EnNiwGirlOverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnNiwGirl* pthis = (EnNiwGirl*)thisx;
