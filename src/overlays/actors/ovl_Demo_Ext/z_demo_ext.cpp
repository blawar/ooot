...
void DemoExt_PlayVortexSFX(DemoExt* pthis)
{
	...
	{
		Audio_PlaySoundGeneral(NA_SE_EV_FANTOM_WARP_L - SFX_FLAG, &pthis->actor.projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
	}
}
...
void DemoExt_DrawVortex(Actor* thisx, GlobalContext* globalCtx)
{
	...
	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(gfxCtx, 0, curScroll[0], curScroll[1], 0x40, 0x40, 1, curScroll[2], curScroll[3], 0x40, 0x40));
...
void DemoExt_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->drawMode < EXT_DRAW_NOTHING) || (pthis->drawMode > EXT_DRAW_VORTEX) || sDrawFuncs[pthis->drawMode] == NULL)
	{
