...
void EnGeldB_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gGerudoRedSkel, &gGerudoRedNeutralAnim, pthis->jointTable, pthis->morphTable, GELDB_LIMB_MAX);
...
s32 EnGeldB_ReactToPlayer(GlobalContext* globalCtx, EnGeldB* pthis, s16 arg2)
{
	...
	else if(arg2)
	{
		...
		else
		{
			...
			if((thisx->xzDistToPlayer <= 45.0f) && !Actor_OtherIsTargeted(globalCtx, thisx) && ((globalCtx->gameplayFrames & 7) || (ABS(angleToFacingLink) < 0x38E0)))
			{
...
void EnGeldB_Wait(EnGeldB* pthis, GlobalContext* globalCtx)
{
	if((pthis->invisible && !Flags_GetSwitch(globalCtx, pthis->actor.home.rot.z)) || pthis->actor.xzDistToPlayer > 300.0f)
	{
...
void EnGeldB_Ready(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnGeldB_DodgeRanged(globalCtx, pthis))
	{
		...
		else if(--pthis->timer == 0)
		{
			...
			{
				if((210.0f > pthis->actor.xzDistToPlayer) && (pthis->actor.xzDistToPlayer > 150.0f) && (Rand_ZeroOne() < 0.3f))
				{
					if(Actor_OtherIsTargeted(globalCtx, &pthis->actor) || (Rand_ZeroOne() > 0.5f) || (ABS(angleToLink) < 0x38E0))
					{
...
void EnGeldB_Advance(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnGeldB_DodgeRanged(globalCtx, pthis))
	{
		...
		{
			if(!Actor_OtherIsTargeted(globalCtx, &pthis->actor) && (Rand_ZeroOne() > 0.03f || (pthis->actor.xzDistToPlayer <= 45.0f && facingAngletoLink < 0x38E0)))
			{
				EnGeldB_SetupSlash(pthis);
			}
		...
		{
			if((210.0f > pthis->actor.xzDistToPlayer) && (pthis->actor.xzDistToPlayer > 150.0f) && Actor_IsFacingPlayer(&pthis->actor, 0x71C))
			{
...
void EnGeldB_RollForward(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime))
	{
		...
		else if(!Actor_OtherIsTargeted(globalCtx, &pthis->actor) && (Rand_ZeroOne() > 0.5f || (ABS(facingAngleToLink) < 0x3FFC)))
		{
			EnGeldB_SetupSlash(pthis);
		}
		else
...
void EnGeldB_Circle(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnGeldB_DodgeRanged(globalCtx, pthis) && !EnGeldB_ReactToPlayer(globalCtx, pthis, 0))
	{
		...
		if((Math_CosS(angleBehindLink - pthis->actor.shape.rot.y) < -0.85f) && !Actor_OtherIsTargeted(globalCtx, &pthis->actor) && (pthis->actor.xzDistToPlayer <= 45.0f))
		{
			EnGeldB_SetupSlash(pthis);
		}
		else if(--pthis->timer == 0)
...
void EnGeldB_Damaged(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	if(!EnGeldB_DodgeRanged(globalCtx, pthis) && !EnGeldB_ReactToPlayer(globalCtx, pthis, 0) && SkelAnime_Update(&pthis->skelAnime) && (pthis->actor.bgCheckFlags & 1))
	{
		...
		{
			if((pthis->actor.xzDistToPlayer <= 45.0f) && !Actor_OtherIsTargeted(globalCtx, &pthis->actor) && (globalCtx->gameplayFrames & 7))
			{
				EnGeldB_SetupSlash(pthis);
			}
			else
...
void EnGeldB_Block(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	if(SkelAnime_Update(&pthis->skelAnime))
	{
		...
		if((ABS(angleToLink) <= 0x4000) && (pthis->actor.xzDistToPlayer < 40.0f) && (ABS(pthis->actor.yDistToPlayer) < 50.0f))
		{
			...
			{
				angleFacingLink = player->actor.shape.rot.y - pthis->actor.shape.rot.y;
				if(!Actor_OtherIsTargeted(globalCtx, &pthis->actor) && ((globalCtx->gameplayFrames & 1) || (ABS(angleFacingLink) < 0x38E0)))
				{
					EnGeldB_SetupSlash(pthis);
				}
				else
	...
	else if((pthis->timer == 0) && func_800354B4(globalCtx, &pthis->actor, 100.0f, 0x2710, 0x4000, pthis->actor.shape.rot.y))
	{
		if(player->swordAnimation == 0x11)
		{
			EnGeldB_SetupSpinDodge(pthis, globalCtx);
	...
	if(!EnGeldB_DodgeRanged(globalCtx, pthis) && !EnGeldB_ReactToPlayer(globalCtx, pthis, 0))
	{
		if(--pthis->timer == 0)
		{
			...
			else
			{
				...
				pthis->actor.world.rot.y = pthis->actor.shape.rot.y;
				if((pthis->actor.xzDistToPlayer <= 45.0f) && !Actor_OtherIsTargeted(globalCtx, &pthis->actor) && (!(globalCtx->gameplayFrames & 3) || (ABS(angleFacingPlayer2) < 0x38E0)))
				{
					EnGeldB_SetupSlash(pthis);
				}
				else if((210.0f > pthis->actor.xzDistToPlayer) && (pthis->actor.xzDistToPlayer > 150.0f) && !(globalCtx->gameplayFrames & 1))
				{
					if(Actor_OtherIsTargeted(globalCtx, &pthis->actor) || (Rand_ZeroOne() > 0.5f) || (ABS(angleFacingPlayer2) < 0x38E0))
					{
						EnGeldB_SetupRollForward(pthis);
					}
					else
		...
		if((thisKeyFrame != (s32)pthis->skelAnime.curFrame) && (((prevKeyFrame < 0) && (((s32)playSpeed + thisKeyFrame) > 0)) || ((prevKeyFrame < 5) && (((s32)playSpeed + thisKeyFrame) > 5))))
		{
			Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_MUSI_LAND);
		}
		if((globalCtx->gameplayFrames & 0x5F) == 0)
...
void EnGeldB_CollisionCheck(EnGeldB* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->bodyCollider.base.acFlags & AC_HIT) && (pthis->action >= GELDB_READY) && (pthis->spinAttackState < 2))
	{
		pthis->bodyCollider.base.acFlags &= ~AC_HIT;
		if(pthis->actor.colChkInfo.damageEffect != GELDB_DMG_UNK_6)
		{
			...
			Audio_StopSfxByPosAndId(&pthis->actor.projectedPos, NA_SE_EN_GERUDOFT_BREATH);
			if((pthis->actor.colChkInfo.damageEffect == GELDB_DMG_STUN) || (pthis->actor.colChkInfo.damageEffect == GELDB_DMG_FREEZE))
			{
...
void EnGeldB_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->action >= GELDB_READY) && (pthis->spinAttackState < 2) && ((pthis->actor.colorFilterTimer == 0) || !(pthis->actor.colorFilterParams & 0x4000)))
	{
...
void EnGeldB_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((pthis->spinAttackState >= 2) && SkelAnime_Update(&pthis->skelAnime))
	{
		if(pthis->spinAttackState == 2)
		{
			Animation_Change(&pthis->skelAnime, &gGerudoRedSpinAttackAnim, 0.5f, 0.0f, 12.0f, ANIMMODE_ONCE_INTERP, 4.0f);
