...
static ObjLiftFramgentScale sFragmentScales[] = {
    {120, -120}, {120, 0}, {120, 120}, {0, -120}, {0, 0}, {0, 120}, {-120, -120}, {-120, 0}, {-120, 120},
};

static InitChainEntry sInitChain[] = {
    ICHAIN_F32_DIV1000(gravity, -600, ICHAIN_CONTINUE), ICHAIN_F32_DIV1000(minVelocityY, -15000, ICHAIN_CONTINUE), ICHAIN_F32(uncullZoneForward, 2000, ICHAIN_CONTINUE),
    ICHAIN_F32(uncullZoneScale, 500, ICHAIN_CONTINUE),	ICHAIN_F32(uncullZoneDownward, 2000, ICHAIN_STOP),
};
...
void ObjLift_InitDynaPoly(ObjLift* pthis, GlobalContext* globalCtx, CollisionHeader* collision, s32 flags)
{
	...
	{
		osSyncPrintf("Warning : move BG 登録失敗(%s %d)(name %d)(arg_data 0x%04x)\n", "../z_obj_lift.c", 188, pthis->dyna.actor.id, pthis->dyna.actor.params);
	}
}
...
void func_80B96160(ObjLift* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(sFragmentScales); i++)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, temp_s3, -256, (Rand_ZeroOne() < 0.5f) ? 64 : 32, 15, 15, 0, (Rand_ZeroOne() * 50.0f + 50.0f) * pthis->dyna.actor.scale.x, 0, 32, 50, KAKERA_COLOR_NONE, OBJECT_D_LIFT, gCollapsingPlatformDL);
	}
...
void func_80B96840(ObjLift* pthis, GlobalContext* globalCtx)
{
	...
	pthis->dyna.actor.floorHeight = BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &pthis->dyna.actor.floorPoly, &bgId, &pthis->dyna.actor, &sp2C);

	if((pthis->dyna.actor.floorHeight - pthis->dyna.actor.world.pos.y) >= (sMaxFallDistances[(pthis->dyna.actor.params >> 1) & 1] - 0.001f))
	{
