...
void EnBomBowlMan_Init(Actor* thisx, GlobalContext* globalCtx2)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gChuGirlSkel, &gChuGirlNoddingOffAnim, pthis->jointTable, pthis->morphTable, 11);
	...
	for(i = 0; i < 2; i++)
	{
		cucco = (EnSyatekiNiw*)Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_SYATEKI_NIW, cuccoSpawnPos_39[i].x, cuccoSpawnPos_39[i].y, cuccoSpawnPos_39[i].z, 0, 0, 0, 1);
...
void EnBomBowMan_CheckBeatenDC(EnBomBowlMan* pthis, GlobalContext* globalCtx)
{
	...
	if((Message_GetState(&globalCtx->msgCtx) == pthis->dialogState) && Message_ShouldAdvance(globalCtx))
	{
		...
		Animation_Change(&pthis->skelAnime, &gChuGirlLeanOverCounterAnim, 1.0f, 0.0f, pthis->frameCount, ANIMMODE_LOOP, -10.0f);
...
void EnBomBowMan_RunGame(EnBomBowlMan* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->bowlPit != NULL)
	{
		...
		if((globalCtx->bombchuBowlingStatus == -1) && (globalCtx->actorCtx.actorLists[ACTORCAT_EXPLOSIVE].length == 0) && (pthis->bowlPit->status == 0) && (pthis->wallStatus[0] != 1) && (pthis->wallStatus[1] != 1))
		{
...
void EnBomBowMan_ChooseShowPrize(EnBomBowlMan* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->prizeRevealTimer == 0)
	{
		...
		pthis->exItem = (EnExItem*)Actor_SpawnAsChild(
		    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EX_ITEM, sPrizePosOffset[pthis->prizeIndex].x + 148.0f, sPrizePosOffset[pthis->prizeIndex].y + 40.0f, sPrizePosOffset[pthis->prizeIndex].z + 300.0f, 0,
		    sPrizeRot[pthis->prizeIndex], 0, pthis->prizeIndex + EXITEM_COUNTER);

		if(!pthis->startedPlaying)
		{
			pthis->bowlPit = (EnBomBowlPit*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_BOM_BOWL_PIT, 0.0f, 90.0f, -860.0f, 0, 0, 0, 0);
			if(pthis->bowlPit != NULL)
			{
...
s32 EnBomBowlMan_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx)
{
	EnBomBowlMan* pthis = (EnBomBowlMan*)thisx;
