...
void BgSpot00Hanebasi_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.params == DT_DRAWBRIDGE)
	{
		...
		if((gSaveContext.sceneSetupIndex != 6) && ((gSaveContext.sceneSetupIndex == 4) || (gSaveContext.sceneSetupIndex == 5) || (!LINK_IS_ADULT && !IS_DAY)))
		{
		...
		{
			if(CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) && CHECK_QUEST_ITEM(QUEST_GORON_RUBY) && CHECK_QUEST_ITEM(QUEST_ZORA_SAPPHIRE) && !(gSaveContext.eventChkInf[8] & 1))
			{
				pthis->dyna.actor.shape.rot.x = -0x4000;
			}
		}

		chainPos.y = (10.0f * Math_CosS(pthis->dyna.actor.shape.rot.x)) - (Math_SinS(pthis->dyna.actor.shape.rot.x) * 400.0f);
		chainPos.z = (10.0f * Math_SinS(pthis->dyna.actor.shape.rot.x)) - (Math_CosS(pthis->dyna.actor.shape.rot.x) * 400.0f);
		chainPos.x = (158.0f * Math_CosS(pthis->dyna.actor.shape.rot.y)) + (Math_SinS(pthis->dyna.actor.shape.rot.y) * chainPos.z);
		chainPos.z = (-158.0f * Math_SinS(pthis->dyna.actor.shape.rot.y)) + (Math_CosS(pthis->dyna.actor.shape.rot.y) * chainPos.z);

		if(Actor_SpawnAsChild(
		       &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_BG_SPOT00_HANEBASI, pthis->dyna.actor.world.pos.x + chainPos.x, pthis->dyna.actor.world.pos.y + chainPos.y, pthis->dyna.actor.world.pos.z + chainPos.z,
		       ((pthis->dyna.actor.shape.rot.x == 0) ? 0 : 0xF020), pthis->dyna.actor.shape.rot.y, 0, DT_CHAIN_1) == NULL)
		{
			Actor_Kill(&pthis->dyna.actor);
		}
	...
	{
		if(Actor_SpawnAsChild(
		       &globalCtx->actorCtx, &pthis->dyna.actor, globalCtx, ACTOR_BG_SPOT00_HANEBASI, pthis->dyna.actor.world.pos.x - (Math_CosS(pthis->dyna.actor.shape.rot.y) * 316.0f), pthis->dyna.actor.world.pos.y,
		       pthis->dyna.actor.world.pos.z + (Math_SinS(pthis->dyna.actor.shape.rot.y) * 316.0f), pthis->dyna.actor.shape.rot.x, pthis->dyna.actor.shape.rot.y, 0, DT_CHAIN_2) == NULL)
		{
			Actor_Kill(&pthis->dyna.actor);
	...
	if(pthis->dyna.actor.params >= DT_CHAIN_1)
	{
		...
		Lights_PointGlowSetInfo(&pthis->lightInfo, ((pthis->dyna.actor.params == DT_CHAIN_1) ? 260.0f : -260.0f), 168, 690, 255, 255, 0, 0);
	}
}
...
void BgSpot00Hanebasi_DrawbridgeWait(BgSpot00Hanebasi* pthis, GlobalContext* globalCtx)
{
	...
	if((gSaveContext.sceneSetupIndex >= 4) || !CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) || !CHECK_QUEST_ITEM(QUEST_GORON_RUBY) || !CHECK_QUEST_ITEM(QUEST_ZORA_SAPPHIRE) || (gSaveContext.eventChkInf[8] & 1))
	{
		...
		if((pthis->dyna.actor.shape.rot.x == 0) && (gSaveContext.sceneSetupIndex < 4) && !LINK_IS_ADULT && (gSaveContext.nightFlag != 0))
		{
			pthis->actionFunc = BgSpot00Hanebasi_DrawbridgeRiseAndFall;
...
void BgSpot00Hanebasi_SetTorchLightInfo(BgSpot00Hanebasi* pthis, GlobalContext* globalCtx)
{
	...
	Lights_PointGlowSetInfo(&pthis->lightInfo, (pthis->dyna.actor.params == DT_CHAIN_1) ? 260.0f : -260.0f, (5000.0f * sTorchFlameScale) + 128.0f, 690, lightColor, lightColor, 0, sTorchFlameScale * 37500.0f);
}
...
void BgSpot00Hanebasi_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->dyna.actor.params == DT_DRAWBRIDGE)
	{
		...
		{
			if(CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) && CHECK_QUEST_ITEM(QUEST_GORON_RUBY) && CHECK_QUEST_ITEM(QUEST_ZORA_SAPPHIRE) && !(gSaveContext.eventChkInf[8] & 1) && LINK_IS_CHILD)
			{
				Player* player = GET_PLAYER(globalCtx);

				if((player->actor.world.pos.x > -450.0f) && (player->actor.world.pos.x < 450.0f) && (player->actor.world.pos.z > 1080.0f) && (player->actor.world.pos.z < 1700.0f) && (!(Gameplay_InCsMode(globalCtx))))
				{
