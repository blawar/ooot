...
{
	Animation_Change(&pthis->skelAnime, &gFishInWaterAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gFishInWaterAnim), ANIMMODE_LOOP_INTERP, 2.0f);
}

void EnFish_SetOutOfWaterAnimation(EnFish* pthis)
{
	Animation_Change(&pthis->skelAnime, &gFishOutOfWaterAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gFishOutOfWaterAnim), ANIMMODE_LOOP_INTERP, 2.0f);
}
...
void EnFish_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gFishSkel, &gFishInWaterAnim, pthis->jointTable, pthis->morphTable, 7);
...
void EnFish_Respawning_FollowChild(EnFish* pthis, GlobalContext* globalCtx)
{
	...
	{
		Math_StepToAngleS(&pthis->actor.world.rot.y, Math_Vec3f_Yaw(&pthis->actor.world.pos, &pthis->actor.home.pos), 3000);
	}
	else if((pthis->actor.child != NULL) && (&pthis->actor != pthis->actor.child))
	{
		Math_StepToAngleS(&pthis->actor.world.rot.y, Math_Vec3f_Yaw(&pthis->actor.world.pos, &pthis->actor.child->world.pos), 3000);
	}
...
void EnFish_Dropped_Fall(EnFish* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->timer <= 0) && (pthis->actor.params == FISH_DROPPED) && (pthis->actor.floorHeight < BGCHECK_Y_MIN + 10.0f))
	{
...
void EnFish_Dropped_FlopOnGround(EnFish* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.world.rot.y += (s16)(((Math_SinS(pthis->slowPhase) * 2000.0f) + (Math_SinS(pthis->fastPhase) * 1000.0f)) * Rand_ZeroOne());
...
void EnFish_Unique_SwimIdle(EnFish* pthis, GlobalContext* globalCtx)
{
	...
	{
		if(!Math_ScaledStepToS(&pthis->actor.world.rot.y, Math_Vec3f_Yaw(&pthis->actor.world.pos, &pthis->actor.home.pos), 200))
		{
			extraPlaySpeed = 0.5f;
		}
	}
...
void EnFish_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if((D_80A17010 == NULL) && (pthis->actor.params == FISH_DROPPED) && (globalCtx->csCtx.state != 0) && (globalCtx->csCtx.npcActions[1] != NULL))
	{
