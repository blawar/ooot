...
void EnTuboTrap_SpawnEffectsOnLand(EnTuboTrap* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, var = 0; i < 15; i++, var += 20000)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, actorPos, -240, arg5, 10, 10, 0, (Rand_ZeroOne() * 65.0f) + 15.0f, 0, 32, 60, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gPotFragmentDL);
	}
...
void EnTuboTrap_SpawnEffectsInWater(EnTuboTrap* pthis, GlobalContext* globalCtx)
{
	...
	for(i = 0, var = 0; i < 15; i++, var += 20000)
	{
		...
		EffectSsKakera_Spawn(globalCtx, &pos, &velocity, actorPos, -180, arg5, 30, 30, 0, (Rand_ZeroOne() * 65.0f) + 15.0f, 0, 32, 70, KAKERA_COLOR_NONE, OBJECT_GAMEPLAY_DANGEON_KEEP, gPotFragmentDL);
	}
}
