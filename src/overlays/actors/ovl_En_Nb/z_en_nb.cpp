...
void EnNb_UpdatePath(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	if(pathList != NULL)
	{
		...
		pthis->pathYaw = (Math_FAtan2F(pthis->finalPos.x - pthis->initialPos.x, pthis->finalPos.z - pthis->initialPos.z) * (0x8000 / M_PI));
...
s32 func_80AB1390(EnNb* pthis, GlobalContext* globalCtx, u16 arg2, s32 npcActionIdx)
{
	...
	if((globalCtx->csCtx.state != CS_STATE_IDLE) && (csCmdNPCAction = globalCtx->csCtx.npcActions[npcActionIdx], csCmdNPCAction != NULL) && (csCmdNPCAction->action == arg2))
	{
		return true;
	}
	return false;
...
void EnNb_GiveMedallion(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_EFFECT, posX, posY, posZ, 0, 0, 0, 0xC);
...
void EnNb_SetupRaisedArmTransition(EnNb* pthis, s32 animFinished)
{
	...
	if(animFinished)
	{
		Animation_Change(&pthis->skelAnime, animation, 1.0f, 0.0f, Animation_GetLastFrame(animation), ANIMMODE_LOOP, 0.0f);
...
{
	Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, kREG(21) + 22.0f + pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 7);
}
...
void EnNb_DrawTransparency(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_nb_inKenjyanomaDemo02.c", 263);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_nb_inKenjyanomaDemo02.c", 290);
}
...
void EnNb_PlayCrySFX(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	{
		Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_VO_NB_CRY_0);
	}
}
...
void EnNb_PlayAgonySFX(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	{
		Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_VO_NB_AGONY);
	}
}
...
void EnNb_PlayKnuckleDefeatSFX(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	{
		Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_VO_NB_CRY_0);
		Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_EN_FANTOM_HIT_THUNDER);
	}
}
...
void EnNb_PlayKneelingOnGroundSFX(EnNb* pthis)
{
	...
	if((pthis->skelAnime.mode == 2) && (Animation_OnFrame(&pthis->skelAnime, 18.0f) || Animation_OnFrame(&pthis->skelAnime, 25.0f)))
	{
		Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_EV_HUMAN_BOUND);
	}
}
...
{
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, kREG(21) + 22.0f + pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, 0xB);
}
...
void func_80AB2E70(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_nb_inConfrontion.c", 572);
	...
	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_nb_inConfrontion.c", 593);
}
...
void func_80AB2FE4(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_nb_inConfrontion.c", 623);
	...
	gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 255);
	gSPSegment(POLY_OPA_DISP++, 0x0C, &D_80116280[2]);
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, func_80AB2FC0, NULL, &pthis->actor);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_nb_inConfrontion.c", 644);
}
...
{
	Common_PlaySfxAtPos(&pthis->actor.projectedPos, NA_SE_VO_NB_NOTICE);
}
...
void EnNb_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gNabooruSkel, NULL, pthis->jointTable, pthis->morphTable, NB_LIMB_MAX);
...
void EnNb_DrawDefault(EnNb* pthis, GlobalContext* globalCtx)
{
	...
	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_nb.c", 992);
	...
	gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 255);
	gSPSegment(POLY_OPA_DISP++, 0x0C, &D_80116280[2]);
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, EnNb_OverrideLimbDraw, EnNb_PostLimbDraw, &pthis->actor);

	CLOSE_DISPS(globalCtx->gfxCtx, "../z_en_nb.c", 1013);
}
