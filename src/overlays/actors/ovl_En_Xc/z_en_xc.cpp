...
void EnXc_SpawnNut(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ARROW, x, y, z, 0xFA0, pthis->actor.shape.rot.y, 0, ARROW_CS_NUT);
}
...
{
	Animation_Change(&pthis->skelAnime, &gSheikIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikIdleAnim), ANIMMODE_LOOP, 0.0f);
...
s32 EnXc_BoleroCS(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params == SHEIK_TYPE_BOLERO)
	{
		...
		if((posRot->pos.x > -784.0f) && (posRot->pos.x < -584.0f) && (posRot->pos.y > 447.0f) && (posRot->pos.y < 647.0f) && (posRot->pos.z > -446.0f) && (posRot->pos.z < -246.0f) && !Gameplay_InCsMode(globalCtx))
		{
...
s32 EnXc_SerenadeCS(EnXc* pthis, GlobalContext* globalCtx)
{
	if(pthis->actor.params == SHEIK_TYPE_SERENADE)
	{
		...
		if(CHECK_OWNED_EQUIP(EQUIP_BOOTS, 1) && !(gSaveContext.eventChkInf[5] & 4) && !(stateFlags & 0x20000000) && !Gameplay_InCsMode(globalCtx))
		{
...
void func_80B3DAF0(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	if(npcAction && (action = npcAction->action, action == 3 || action == 11 || action == 12 || action == 13 || action == 23))
	{
		f32 frameCount;
...
void func_80B3DCA8(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	if(globalCtx->csCtx.state != 0)
	{
		...
		{
			frameCount = Animation_GetLastFrame(&gSheikInitialHarpAnim);
			Animation_Change(&pthis->skelAnime, &gSheikInitialHarpAnim, 0.0f, frameCount, frameCount, ANIMMODE_LOOP, -8.0f);
...
void EnXc_SetupHarpPutawayAction(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	if(EnXc_CompareCsAction(pthis, globalCtx, 5, 4))
	{
		...
		{
			Animation_Change(&pthis->skelAnime, &gSheikInitialHarpAnim, -1.0f, Animation_GetLastFrame(&gSheikInitialHarpAnim), 0.0f, ANIMMODE_ONCE, 0.0f);
...
void func_80B3DE00(EnXc* pthis, s32 animFinished)
{
	if(animFinished)
	{
		Animation_Change(&pthis->skelAnime, &gSheikPullingOutHarpAnim, -1.0f, Animation_GetLastFrame(&gSheikPullingOutHarpAnim), 0.0f, ANIMMODE_ONCE, 0.0f);
...
void EnXc_SetupReverseAccel(EnXc* pthis, GlobalContext* globalCtx)
{
	if(globalCtx->csCtx.state != 0)
	{
		...
		{
			Animation_Change(&pthis->skelAnime, &gSheikWalkingAnim, -1.0f, Animation_GetLastFrame(&gSheikWalkingAnim), 0.0f, ANIMMODE_LOOP, -12.0f);
...
void EnXc_SetupReverseHaltAction(EnXc* pthis)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gSheikIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikIdleAnim), ANIMMODE_LOOP, -12.0f);
...
void EnXc_SetupNutThrow(EnXc* pthis)
{
	...
	if(pthis->timer >= 12.0f)
	{
		Animation_Change(&pthis->skelAnime, &gSheikThrowingNutAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikThrowingNutAnim), ANIMMODE_ONCE, 0.0f);
...
void EnXc_DrawPullingOutHarp(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, EnXc_PullingOutHarpOverrideLimbDraw, NULL, pthis);
...
void EnXc_DrawHarp(Actor* thisx, GlobalContext* globalCtx)
{
	...
	func_80093D18(gfxCtx);
	func_8002EBCC(&pthis->actor, globalCtx, 0);
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, EnXc_HarpOverrideLimbDraw, NULL, pthis);
...
void EnXc_SetupShowTriforceAction(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	{
		Animation_Change(&pthis->skelAnime, &gSheikShowingTriforceOnHandAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikShowingTriforceOnHandAnim), ANIMMODE_ONCE, -8.0f);
...
void EnXc_SetupShowTriforceIdleAction(EnXc* pthis, s32 animFinished)
{
	if(animFinished)
	{
		Animation_Change(&pthis->skelAnime, &gSheikShowingTriforceOnHandIdleAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikShowingTriforceOnHandIdleAnim), ANIMMODE_LOOP, 0.0f);
...
void EnXc_SetupContortions(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	func_80B3C588(pthis, globalCtx, 4);
	func_80B3C964(pthis, globalCtx);
	Animation_Change(skelAnime, &gSheikContortionsAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikContortionsAnim), ANIMMODE_ONCE, 0.0f);
...
void EnXc_SetupFallInNocturne(EnXc* pthis, GlobalContext* globalCtx)
{
	...
	func_80B3C588(pthis, globalCtx, 4);
	func_80B3C964(pthis, globalCtx);
	Animation_Change(skelAnime, &gSheikFallingFromContortionsAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gSheikFallingFromContortionsAnim), ANIMMODE_ONCE, 0.0f);
...
void EnXc_DrawSquintingEyes(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, NULL, NULL, NULL);
...
void EnXc_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gSheikSkel, &gSheikIdleAnim, pthis->jointTable, pthis->morphTable, ARRAY_COUNT(pthis->jointTable));
...
void EnXc_DrawDefault(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, EnXc_OverrideLimbDraw, EnXc_PostLimbDraw, pthis);
...
ActorInit En_Xc_InitVars = {
    ACTOR_EN_XC, ACTORCAT_NPC, FLAGS, OBJECT_XC, ACTOR_FACTORY(EnXc), (ActorFunc)EnXc_Init, (ActorFunc)EnXc_Destroy, (ActorFunc)EnXc_Update, (ActorFunc)EnXc_Draw, (ActorFunc)EnXc_Reset,
};
