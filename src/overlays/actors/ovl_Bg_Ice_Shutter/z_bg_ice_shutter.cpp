...
ActorInit Bg_Ice_Shutter_InitVars = {
    ACTOR_BG_ICE_SHUTTER, ACTORCAT_PROP, FLAGS, OBJECT_ICE_OBJECTS, ACTOR_FACTORY(BgIceShutter), (ActorFunc)BgIceShutter_Init, (ActorFunc)BgIceShutter_Destroy, (ActorFunc)BgIceShutter_Update, (ActorFunc)BgIceShutter_Draw, (ActorFunc)BgIceShutter_Reset,
};
...
void func_80891AC0(BgIceShutter* pthis)
{
	...
	pthis->dyna.actor.world.pos.y = (Math_CosS(pthis->dyna.actor.shape.rot.x) * pthis->dyna.actor.velocity.y) + pthis->dyna.actor.home.pos.y;
...
void BgIceShutter_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(sp28 == 2)
	{
		...
		pthis->dyna.actor.focus.pos.x = (Math_SinS(pthis->dyna.actor.shape.rot.y) * temp_f6) + pthis->dyna.actor.home.pos.x;
		pthis->dyna.actor.focus.pos.y = pthis->dyna.actor.home.pos.y;
		pthis->dyna.actor.focus.pos.z = pthis->dyna.actor.home.pos.z + (Math_CosS(pthis->dyna.actor.shape.rot.y) * temp_f6);
	}
	else
	{
