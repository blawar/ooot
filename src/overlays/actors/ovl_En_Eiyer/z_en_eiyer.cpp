...
void EnEiyer_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actor.params < 3)
	{
		...
		if(Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_EIYER, pthis->actor.home.pos.x, pthis->actor.home.pos.y, pthis->actor.home.pos.z, 0, pthis->actor.shape.rot.y + 0x4000, 0, pthis->actor.params + 1) == NULL)
		{
...
void EnEiyer_WanderUnderground(EnEiyer* pthis, GlobalContext* globalCtx)
{
	...
	{
		pthis->targetYaw = pthis->actor.world.rot.y + (Rand_ZeroOne() < 0.5f ? -1 : 1) * (Rand_ZeroOne() * 0x2000 + 0x2000);
	}
...
void EnEiyer_Update(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc == EnEiyer_Glide || pthis->actionFunc == EnEiyer_DiveAttack || pthis->actionFunc == EnEiyer_Stunned || pthis->actionFunc == EnEiyer_Die || pthis->actionFunc == EnEiyer_Hurt ||
	   (pthis->actionFunc == EnEiyer_Land && pthis->timer == -1))
	{
		Actor_UpdateBgCheckInfo(globalCtx, &pthis->actor, 5.0f, 27.0f, 30.0f, 7);
	}

	if(pthis->actor.params == 0xA || (pthis->actionFunc != EnEiyer_AppearFromGround && pthis->actionFunc != EnEiyer_CircleUnderground))
	{
...
void EnEiyer_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnEiyer_Dead)
	{
		...
		POLY_OPA_DISP = SkelAnime_Draw(globalCtx, pthis->skelanime.skeleton, pthis->skelanime.jointTable, EnEiyer_OverrideLimbDraw, NULL, pthis, POLY_OPA_DISP);
	}
	else
	{
