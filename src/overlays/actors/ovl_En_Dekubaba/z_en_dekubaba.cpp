...
void EnDekubaba_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_Init(globalCtx, &pthis->skelAnime, &gDekuBabaSkel, &gDekuBabaFastChompAnim, pthis->jointTable, pthis->morphTable, 8);
	...
	if(pthis->actor.params == DEKUBABA_BIG)
	{
		...
		{
			pthis->collider.elements[i].dim.worldSphere.radius = pthis->collider.elements[i].dim.modelSphere.radius = (sJntSphElementsInit[i].dim.modelSphere.radius * 2.50f);
		}
...
void EnDekubaba_SetupGrow(EnDekubaba* pthis)
{
	...
	Animation_Change(&pthis->skelAnime, &gDekuBabaFastChompAnim, Animation_GetLastFrame(&gDekuBabaFastChompAnim) * (1.0f / 15), 0.0f, Animation_GetLastFrame(&gDekuBabaFastChompAnim), ANIMMODE_ONCE, 0.0f);
...
void EnDekubaba_SetupRetract(EnDekubaba* pthis)
{
	...
	Animation_Change(&pthis->skelAnime, &gDekuBabaFastChompAnim, -1.5f, Animation_GetLastFrame(&gDekuBabaFastChompAnim), 0.0f, ANIMMODE_ONCE, -3.0f);
...
{
	Animation_Change(&pthis->skelAnime, &gDekuBabaPauseChompAnim, 1.0f, 15.0f, Animation_GetLastFrame(&gDekuBabaPauseChompAnim), ANIMMODE_ONCE, -3.0f);
	pthis->timer = 0;
...
void EnDekubaba_SetupStunnedVertical(EnDekubaba* pthis)
{
	...
	if(pthis->timer == 1)
	{
		Animation_Change(&pthis->skelAnime, &gDekuBabaFastChompAnim, 4.0f, 0.0f, Animation_GetLastFrame(&gDekuBabaFastChompAnim), ANIMMODE_LOOP, -3.0f);
		pthis->timer = 40;
	}
	else
	{
		Animation_Change(&pthis->skelAnime, &gDekuBabaFastChompAnim, 0.0f, 0.0f, Animation_GetLastFrame(&gDekuBabaFastChompAnim), ANIMMODE_LOOP, -3.0f);
		pthis->timer = 60;
	}
...
void EnDekubaba_Wait(EnDekubaba* pthis, GlobalContext* globalCtx)
{
	...
	if((pthis->timer == 0) && (pthis->actor.xzDistToPlayer < 200.0f * pthis->size) && (fabsf(pthis->actor.yDistToPlayer) < 30.0f * pthis->size))
	{
...
void EnDekubaba_Grow(EnDekubaba* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.scale.x = pthis->actor.scale.y = pthis->actor.scale.z = pthis->size * 0.01f * (0.5f + (15 - pthis->timer) * 0.5f / 15.0f);
	...
	else if(pthis->actor.shape.rot.x < -0xE38)
	{
		...
		headDistHorizontal = 20.0f * (Math_CosS(pthis->stemSectionAngle[0]) + Math_CosS(pthis->stemSectionAngle[1])) +
				     (headDistVertical - 20.0f * (-Math_SinS(pthis->stemSectionAngle[0]) - Math_SinS(pthis->stemSectionAngle[1]))) * Math_CosS(pthis->stemSectionAngle[2]) / -Math_SinS(pthis->stemSectionAngle[2]);
	}
	else
	{
		Math_ScaledStepToS(&pthis->stemSectionAngle[0], -0xAAA, 0x38E);
	...
	EffectSsHahen_SpawnBurst(globalCtx, &pthis->actor.home.pos, pthis->size * 3.0f, 0, pthis->size * 12.0f, pthis->size * 5.0f, 1, HAHEN_OBJECT_DEFAULT, 10, NULL);
...
void EnDekubaba_Retract(EnDekubaba* pthis, GlobalContext* globalCtx)
{
	...
	pthis->actor.scale.x = pthis->actor.scale.y = pthis->actor.scale.z = pthis->size * 0.01f * (0.5f + pthis->timer * (1.0f / 30));
	...
	EffectSsHahen_SpawnBurst(globalCtx, &pthis->actor.home.pos, pthis->size * 3.0f, 0, pthis->size * 12.0f, pthis->size * 5.0f, 1, HAHEN_OBJECT_DEFAULT, 0xA, NULL);
...
{
	f32 horizontalHeadShift = (Math_CosS(pthis->stemSectionAngle[0]) + Math_CosS(pthis->stemSectionAngle[1]) + Math_CosS(pthis->stemSectionAngle[2])) * 20.0f;

	pthis->actor.world.pos.x = pthis->actor.home.pos.x + Math_SinS(pthis->actor.shape.rot.y) * (horizontalHeadShift * pthis->size);
	pthis->actor.world.pos.y = pthis->actor.home.pos.y - (Math_SinS(pthis->stemSectionAngle[0]) + Math_SinS(pthis->stemSectionAngle[1]) + Math_SinS(pthis->stemSectionAngle[2])) * 20.0f * pthis->size;
	pthis->actor.world.pos.z = pthis->actor.home.pos.z + Math_CosS(pthis->actor.shape.rot.y) * (horizontalHeadShift * pthis->size);
}
...
void EnDekubaba_DecideLunge(EnDekubaba* pthis, GlobalContext* globalCtx)
{
	...
	Math_ApproachS(&pthis->actor.shape.rot.y, Math_Vec3f_Yaw(&pthis->actor.home.pos, &player->actor.world.pos), 2, (pthis->timer % 5) * 0x222);
...
void EnDekubaba_Lunge(EnDekubaba* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->timer == 0)
	{
		...
		if(allStepsDone)
		{
			...
			func_8002829C(globalCtx, &pthis->actor.world.pos, &velocity, &sZeroVec, &primColor_49, &envColor_49, 1, pthis->size * 100.0f);
			pthis->timer = 1;
			pthis->collider.base.acFlags |= AC_ON;
		}
	}
...
void EnDekubaba_UpdateDamage(EnDekubaba* pthis, GlobalContext* globalCtx)
{
	...
	if(pthis->collider.base.acFlags & AC_HIT)
	{
		...
		if((pthis->collider.base.colType != COLTYPE_HARD) && ((pthis->actor.colChkInfo.damageEffect != DEKUBABA_DMGEFF_NONE) || (pthis->actor.colChkInfo.damage != 0)))
		{
			phi_s0 = pthis->actor.colChkInfo.health - pthis->actor.colChkInfo.damage;

			if(pthis->actionFunc != EnDekubaba_StunnedVertical)
			{
				if((pthis->actor.colChkInfo.damageEffect == DEKUBABA_DMGEFF_BOOMERANG) || (pthis->actor.colChkInfo.damageEffect == DEKUBABA_DMGEFF_DEKUNUT))
				{
			...
			else if((pthis->actor.colChkInfo.damageEffect == DEKUBABA_DMGEFF_BOOMERANG) || (pthis->actor.colChkInfo.damageEffect == DEKUBABA_DMGEFF_SWORD))
			{
				if(phi_s0 > 0)
				{
	...
	else if((globalCtx->actorCtx.unk_02 != 0) && (pthis->collider.base.colType != COLTYPE_HARD) && (pthis->actionFunc != EnDekubaba_StunnedVertical) && (pthis->actionFunc != EnDekubaba_Hit) && (pthis->actor.colChkInfo.health != 0))
	{
...
void EnDekubaba_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	if(pthis->actionFunc != EnDekubaba_DeadStickDrop)
	{
		SkelAnime_DrawOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable, NULL, EnDekubaba_PostLimbDraw, pthis);
