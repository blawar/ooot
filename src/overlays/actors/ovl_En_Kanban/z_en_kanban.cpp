	...
	switch(pthis->actionState)
	{
		case ENKANBAN_SIGN:
			...
			if((pthis->invincibilityTimer == 0) && (pthis->collider.base.acFlags & AC_HIT))
			{
				...
				piece = (EnKanban*)Actor_SpawnAsChild(
				    &globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EN_KANBAN, pthis->actor.world.pos.x, pthis->actor.world.pos.y, pthis->actor.world.pos.z, pthis->actor.shape.rot.x, pthis->actor.shape.rot.y, pthis->actor.shape.rot.z,
				    ENKANBAN_PIECE);
				if(piece != NULL)
				{
		...
		case ENKANBAN_WATER:
			...
			if(pthis->actionState == ENKANBAN_WATER)
			{
				...
				if((player->actor.speedXZ > 0.0f) && (player->actor.world.pos.y < pthis->actor.world.pos.y) && (pthis->actor.xyzDistToPlayerSq < 2500.0f))
				{
					...
					if(Math_SmoothStepToS(&pthis->actor.world.rot.y, pthis->actor.yawTowardsPlayer + 0x8000, 1, 0x1000, 0) > 0)
					{
			...
			switch(pthis->ocarinaFlag)
			{
				case 0:
					...
					break;
				case 1:
					if((globalCtx->msgCtx.ocarinaMode == OCARINA_MODE_04) && (globalCtx->msgCtx.unk_E3F2 == OCARINA_SONG_LULLABY))
					{
		...
		case ENKANBAN_REPAIR:
		{
			...
			distX = Math_SmoothStepToF(&pthis->actor.world.pos.x, signpost->actor.world.pos.x + offset.x, 1.0f, 3.0f, 0.0f);
			distY = Math_SmoothStepToF(&pthis->actor.world.pos.y, signpost->actor.world.pos.y + offset.y, 1.0f, 3.0f, 0.0f);
			distZ = Math_SmoothStepToF(&pthis->actor.world.pos.z, signpost->actor.world.pos.z + offset.z, 1.0f, 3.0f, 0.0f);
...
void EnKanban_Draw(Actor* thisx, GlobalContext* globalCtx)
{
	...
	f32 zShift2;
	s16 i;
	u8* shadowTex = (u8*)Graph_Alloc(globalCtx->gfxCtx, 0x400);

	OPEN_DISPS(globalCtx->gfxCtx, "../z_en_kanban.c", 1659);
	...
	if(pthis->actionState != ENKANBAN_SIGN)
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_kanban.c", 1715), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		for(i = 0; i < ARRAY_COUNT(sPartFlags); i++)
		{
	...
	else
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_kanban.c", 1725), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		if(pthis->partFlags == 0xFFFF)
		{
		...
		if(pthis->cutMarkAlpha != 0)
		{
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_kanban.c", 1773), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			gSPDisplayList(POLY_XLU_DISP++, object_kanban_DL_001630);
		}
	}
	if((pthis->actor.projectedPos.z <= 400.0f) && (pthis->actor.projectedPos.z > 0.0f) && (pthis->actor.floorHeight > -3000.0f))
	{
		if((pthis->bounceX != 0) || (pthis->bounceZ != 0))
		{
			...
			Matrix_Translate(pthis->actor.world.pos.x, pthis->actor.floorHeight, pthis->actor.world.pos.z + zShift, MTXMODE_NEW);
			Matrix_RotateX(pthis->floorRot.x, MTXMODE_APPLY);
			Matrix_RotateZ(pthis->floorRot.z, MTXMODE_APPLY);
			...
			gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->gfxCtx, "../z_en_kanban.c", 1833), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
