...
void EnMs_Init(Actor* thisx, GlobalContext* globalCtx)
{
	...
	SkelAnime_InitFlex(globalCtx, &pthis->skelAnime, &gBeanSalesmanSkel, &gBeanSalesmanEatingAnim, pthis->jointTable, pthis->morphTable, 9);
...
void EnMs_Wait(EnMs* pthis, GlobalContext* globalCtx)
{
	...
	else if((pthis->actor.xzDistToPlayer < 90.0f) && (ABS(yawDiff) < 0x2000))
	{ // talk range
...
void EnMs_Talk(EnMs* pthis, GlobalContext* globalCtx)
{
	...
	{
		if((dialogState == TEXT_STATE_DONE) && Message_ShouldAdvance(globalCtx))
