...
void EffectSsDtBubble_Draw(GlobalContext* globalCtx, u32 index, EffectSs* pthis)
{
	...
	gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_eff_ss_dt_bubble.c", 213), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	func_80093C14(gfxCtx);
	gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, pthis->rPrimColorR, pthis->rPrimColorG, pthis->rPrimColorB, (pthis->rPrimColorA * pthis->life) / pthis->rLifespan);
	gDPSetEnvColor(POLY_XLU_DISP++, pthis->rEnvColorR, pthis->rEnvColorG, pthis->rEnvColorB, (pthis->rEnvColorA * pthis->life) / pthis->rLifespan);
