...
void EffectSsFireTail_Draw(GlobalContext* globalCtx, u32 index, EffectSs* pthis)
{
	...
	if(pthis->actor != NULL)
	{
		...
		{
			Matrix_Translate(pthis->pos.x + pthis->actor->world.pos.x, pthis->pos.y + pthis->actor->world.pos.y, pthis->pos.z + pthis->actor->world.pos.z, MTXMODE_NEW);
		}
		else
		{
			...
			pthis->pos.x = player->bodyPartsPos[bodyPart].x - (Math_SinS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx))) * 5.0f);
			pthis->pos.y = player->bodyPartsPos[bodyPart].y;
			pthis->pos.z = player->bodyPartsPos[bodyPart].z - (Math_CosS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx))) * 5.0f);
	...
	gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0, (uintptr_t)(globalCtx->state.frames * -0x14) & 0x1FF, 32, 128));
