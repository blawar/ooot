...
void FileChoose_SetWindowVtx(GameState* pthisx)
{
	...
	for(x = 0, i = 0; i < 4; i++)
	{
		...
		for(j = 0, tmp3 = 0x50; j < 5; j++, x += 4, tmp3 -= 0x20)
		{
			...
			pthis->windowVtx[x].v.cn[0] = pthis->windowVtx[x + 2].v.cn[0] = pthis->windowVtx[x].v.cn[1] = pthis->windowVtx[x + 2].v.cn[1] = pthis->windowVtx[x].v.cn[2] = pthis->windowVtx[x + 2].v.cn[2] = pthis->windowVtx[x + 1].v.cn[0] =
			    pthis->windowVtx[x + 3].v.cn[0] = pthis->windowVtx[x + 1].v.cn[1] = pthis->windowVtx[x + 3].v.cn[1] = pthis->windowVtx[x + 1].v.cn[2] = pthis->windowVtx[x + 3].v.cn[2] = pthis->windowVtx[x].v.cn[3] =
				pthis->windowVtx[x + 2].v.cn[3] = pthis->windowVtx[x + 1].v.cn[3] = pthis->windowVtx[x + 3].v.cn[3] = 255;
		}
	}
}
...
void FileChoose_SetWindowContentVtx(GameState* pthisx)
{
	...
	for(phi_t2 = 0; phi_t2 < 0x288; phi_t2 += 4)
	{
		...
		pthis->windowContentVtx[phi_t2].v.cn[0] = pthis->windowContentVtx[phi_t2 + 1].v.cn[0] = pthis->windowContentVtx[phi_t2 + 2].v.cn[0] = pthis->windowContentVtx[phi_t2 + 3].v.cn[0] = pthis->windowContentVtx[phi_t2].v.cn[1] =
			...
			pthis->windowContentVtx[phi_t2 + 2].v.cn[2] = pthis->windowContentVtx[phi_t2 + 3].v.cn[2] = pthis->windowContentVtx[phi_t2].v.cn[3] = pthis->windowContentVtx[phi_t2 + 1].v.cn[3] = pthis->windowContentVtx[phi_t2 + 2].v.cn[3] =
...
void FileChoose_DrawFileInfo(GameState* pthisx, s16 fileIndex, s16 isActive)
{
	...
	if(pthis->nameAlpha[fileIndex] != 0)
	{
		...
		{
			FileChoose_DrawCharacter(pthis->state.gfxCtx, sp54->fontBuf + pthis->fileNames[fileIndex][i] * FONT_CHAR_TEX_SIZE, vtxOffset);
		}
	}
	...
	if((fileIndex == pthis->selectedFileIndex) || (fileIndex == pthis->copyDestFileIndex))
	{
		...
		{
			FileChoose_DrawCharacter(pthis->state.gfxCtx, sp54->fontBuf + deathCountSplit[i] * FONT_CHAR_TEX_SIZE, vtxOffset);
		}
...
static void* sTitleLabels[3][9] = {
    ...
    {gFileSelPleaseSelectAFileGERTex, gFileSelOpenThisFileGERTex, gFileSelWhichFile1GERTex, gFileSelCopyToWhichFileGERTex, gFileSelAreYouSureGERTex, gFileSelFileCopiedGERTex, gFileSelWhichFile2GERTex, gFileSelAreYouSure2GERTex, gFileSelFileErasedGERTex},
...
void FileChoose_DrawWindowContents(GameState* pthisx)
{
	...
	for(fileIndex = 0; fileIndex < 3; fileIndex++, temp += 20)
	{
		...
		{
			gDPLoadTextureBlock(POLY_OPA_DISP++, sFileInfoBoxTextures[i], G_IM_FMT_IA, G_IM_SIZ_16b, sFileInfoBoxPartWidths[i], 56, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
	...
	if(pthis->warningLabel > FS_WARNING_NONE)
	{
		...
		gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 0);
		gDPLoadTextureBlock(POLY_OPA_DISP++, sWarningLabels[gSaveContext.language][pthis->warningLabel], G_IM_FMT_IA, G_IM_SIZ_8b, 128, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
		gSP1Quadrangle(POLY_OPA_DISP++, 16, 18, 19, 17, 0);
	}
...
void FileChoose_ConfigModeDraw(GameState* pthisx)
{
	...
	if((pthis->configMode != CM_NAME_ENTRY) && (pthis->configMode != CM_START_NAME_ENTRY))
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->state.gfxCtx, "../z_file_choose.c", 2282), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	if((pthis->configMode >= CM_ROTATE_TO_NAME_ENTRY) && (pthis->configMode <= CM_NAME_ENTRY_TO_MAIN))
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->state.gfxCtx, "../z_file_choose.c", 2316), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	if((pthis->configMode >= CM_MAIN_TO_OPTIONS) && (pthis->configMode <= CM_OPTIONS_TO_MAIN))
	{
		...
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->state.gfxCtx, "../z_file_choose.c", 2337), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void FileChoose_SelectModeDraw(GameState* pthisx)
{
	...
	Matrix_Scale(0.78f, 0.78f, 0.78f, MTXMODE_APPLY);
	Matrix_RotateX(pthis->windowRot / 100.0f, MTXMODE_APPLY);
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->state.gfxCtx, "../z_file_choose.c", 2810), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void FileChoose_InitContext(GameState* pthisx)
{
	...
	pthis->windowAlpha = pthis->titleAlpha[0] = pthis->titleAlpha[1] = pthis->fileButtonAlpha[0] = pthis->fileButtonAlpha[1] = pthis->fileButtonAlpha[2] = pthis->nameBoxAlpha[0] = pthis->nameBoxAlpha[1] = pthis->nameBoxAlpha[2] = pthis->nameAlpha[0] =
	    pthis->nameAlpha[1] = pthis->nameAlpha[2] = pthis->connectorAlpha[0] = pthis->connectorAlpha[1] = pthis->connectorAlpha[2] = pthis->fileInfoAlpha[0] = pthis->fileInfoAlpha[1] = pthis->fileInfoAlpha[2] =
