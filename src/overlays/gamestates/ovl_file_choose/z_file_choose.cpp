...
namespace oot::gamestate
{
	...
	void FileChoose_SetView(FileChoose* pthis, f32 eyeX, f32 eyeY, f32 eyeZ)
	{
		Vec3f eye;
		Vec3f lookAt;
		Vec3f up;
	...
	{
		FileChoose* pthis = (FileChoose*)pthisx;
	...
	void FileChoose_FadeInMenuElements(GameState* pthisx)
	{
		...
		pthis->actionButtonAlpha[FS_BTN_ACTION_COPY] = pthis->actionButtonAlpha[FS_BTN_ACTION_ERASE] = pthis->optionButtonAlpha = pthis->windowAlpha;
	}
	...
	void FileChoose_StartFadeIn(GameState* pthisx)
	{
		...
		FileChoose_FadeInMenuElements(pthis);
		sScreenFillAlpha -= 40;
		pthis->windowPosX -= 20;
	...
	static void (*gConfigModeUpdateFuncs[])(GameState*) = {
	    FileChoose_StartFadeIn,    FileChoose_FinishFadeIn,	      FileChoose_UpdateMainMenu,     FileChoose_SetupCopySource,   FileChoose_SelectCopySource,	  FileChoose_SetupCopyDest1,	   FileChoose_SetupCopyDest2,
	    FileChoose_SelectCopyDest, FileChoose_ExitToCopySource1,  FileChoose_ExitToCopySource2,  FileChoose_SetupCopyConfirm1, FileChoose_SetupCopyConfirm2,  FileChoose_CopyConfirm,	   FileChoose_ReturnToCopyDest,
	    FileChoose_CopyAnim1,      FileChoose_CopyAnim2,	      FileChoose_CopyAnim3,	     FileChoose_CopyAnim4,	   FileChoose_CopyAnim5,	  FileChoose_ExitCopyToMain,	   FileChoose_SetupEraseSelect,
	    FileChoose_EraseSelect,    FileChoose_SetupEraseConfirm1, FileChoose_SetupEraseConfirm2, FileChoose_EraseConfirm,	   FileChoose_ExitToEraseSelect1, FileChoose_ExitToEraseSelect2,   FileChoose_EraseAnim1,
	    FileChoose_EraseAnim2,     FileChoose_EraseAnim3,	      FileChoose_ExitEraseToMain,    FileChoose_UnusedCM31,	   FileChoose_RotateToNameEntry,  FileChoose_UpdateKeyboardCursor, FileChoose_StartNameEntry,
	    FileChoose_RotateToMain,   FileChoose_RotateToOptions,    FileChoose_UpdateOptionsMenu,  FileChoose_StartOptions,	   FileChoose_RotateToMain,	  FileChoose_UnusedCMDelay,
	};
	...
	void FileChoose_ConfigModeUpdate(GameState* pthisx)
	{
		...
		gConfigModeUpdateFuncs[pthis->configMode](pthis);
	}
	...
	void FileChoose_SetWindowVtx(GameState* pthisx)
	{
		...
		pthis->windowVtx = (Vtx*)Graph_Alloc(pthis->gfxCtx, sizeof(Vtx) * 80);
		...
		for(x = 0, i = 0; i < 4; i++)
		{
			...
			for(j = 0, tmp3 = 0x50; j < 5; j++, x += 4, tmp3 -= 0x20)
			{
				...
				pthis->windowVtx[x].v.ob[2] = pthis->windowVtx[x + 1].v.ob[2] = pthis->windowVtx[x + 2].v.ob[2] = pthis->windowVtx[x + 3].v.ob[2] = 0;

				pthis->windowVtx[x].v.flag = pthis->windowVtx[x + 1].v.flag = pthis->windowVtx[x + 2].v.flag = pthis->windowVtx[x + 3].v.flag = 0;

				pthis->windowVtx[x].v.tc[0] = pthis->windowVtx[x].v.tc[1] = pthis->windowVtx[x + 1].v.tc[1] = pthis->windowVtx[x + 2].v.tc[0] = 0;
				...
				pthis->windowVtx[x].v.cn[0] = pthis->windowVtx[x + 2].v.cn[0] = pthis->windowVtx[x].v.cn[1] = pthis->windowVtx[x + 2].v.cn[1] = pthis->windowVtx[x].v.cn[2] = pthis->windowVtx[x + 2].v.cn[2] =
				    pthis->windowVtx[x + 1].v.cn[0] = pthis->windowVtx[x + 3].v.cn[0] = pthis->windowVtx[x + 1].v.cn[1] = pthis->windowVtx[x + 3].v.cn[1] = pthis->windowVtx[x + 1].v.cn[2] = pthis->windowVtx[x + 3].v.cn[2] =
					pthis->windowVtx[x].v.cn[3] = pthis->windowVtx[x + 2].v.cn[3] = pthis->windowVtx[x + 1].v.cn[3] = pthis->windowVtx[x + 3].v.cn[3] = 255;
			}
		}
	}
	...
	void FileChoose_SetWindowContentVtx(GameState* pthisx)
	{
		...
		pthis->windowContentVtx = (Vtx*)Graph_Alloc(pthis->gfxCtx, 0x288 * sizeof(Vtx));

		for(phi_t2 = 0; phi_t2 < 0x288; phi_t2 += 4)
		{
			pthis->windowContentVtx[phi_t2].v.ob[0] = pthis->windowContentVtx[phi_t2 + 2].v.ob[0] = 0x12C;
			pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + 0x10;

			pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = 0;
			pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0x10;

			pthis->windowContentVtx[phi_t2].v.ob[2] = pthis->windowContentVtx[phi_t2 + 1].v.ob[2] = pthis->windowContentVtx[phi_t2 + 2].v.ob[2] = pthis->windowContentVtx[phi_t2 + 3].v.ob[2] = 0;

			pthis->windowContentVtx[phi_t2].v.flag = pthis->windowContentVtx[phi_t2 + 1].v.flag = pthis->windowContentVtx[phi_t2 + 2].v.flag = pthis->windowContentVtx[phi_t2 + 3].v.flag = 0;

			pthis->windowContentVtx[phi_t2].v.tc[0] = pthis->windowContentVtx[phi_t2].v.tc[1] = pthis->windowContentVtx[phi_t2 + 1].v.tc[1] = pthis->windowContentVtx[phi_t2 + 2].v.tc[0] = 0;

			pthis->windowContentVtx[phi_t2 + 1].v.tc[0] = pthis->windowContentVtx[phi_t2 + 2].v.tc[1] = pthis->windowContentVtx[phi_t2 + 3].v.tc[0] = pthis->windowContentVtx[phi_t2 + 3].v.tc[1] = 0x200;

			pthis->windowContentVtx[phi_t2].v.cn[0] = pthis->windowContentVtx[phi_t2 + 1].v.cn[0] = pthis->windowContentVtx[phi_t2 + 2].v.cn[0] = pthis->windowContentVtx[phi_t2 + 3].v.cn[0] = pthis->windowContentVtx[phi_t2].v.cn[1] =
			    pthis->windowContentVtx[phi_t2 + 1].v.cn[1] = pthis->windowContentVtx[phi_t2 + 2].v.cn[1] = pthis->windowContentVtx[phi_t2 + 3].v.cn[1] = pthis->windowContentVtx[phi_t2].v.cn[2] = pthis->windowContentVtx[phi_t2 + 1].v.cn[2] =
				pthis->windowContentVtx[phi_t2 + 2].v.cn[2] = pthis->windowContentVtx[phi_t2 + 3].v.cn[2] = pthis->windowContentVtx[phi_t2].v.cn[3] = pthis->windowContentVtx[phi_t2 + 1].v.cn[3] =
				    pthis->windowContentVtx[phi_t2 + 2].v.cn[3] = pthis->windowContentVtx[phi_t2 + 3].v.cn[3] = 0xFF;
		}
		...
		for(phi_a1 = 0, phi_t2 = 4; phi_a1 < 3; phi_a1++)
		{
			...
			for(phi_t5 = 0; phi_t5 < 5; phi_t5++, phi_t2 += 4)
			{
				...
				pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + sFileInfoBoxPartWidths[phi_t5];

				pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = pthis->fileNamesY[phi_a1] + 0x2C;

				pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0x38;

				pthis->windowContentVtx[phi_t2 + 1].v.tc[0] = pthis->windowContentVtx[phi_t2 + 3].v.tc[0] = sFileInfoBoxPartWidths[phi_t5] << 5;
		...
		for(phi_t5 = 0; phi_t5 < 3; phi_t5++, phi_t2 += 20, phi_ra -= 0x10)
		{
			...
			pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + 0x40;

			pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = pthis->buttonYOffsets[phi_t5] + phi_ra;
			pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0x10;
			...
			pthis->windowContentVtx[phi_t2 + 5].v.ob[0] = pthis->windowContentVtx[phi_t2 + 7].v.ob[0] = pthis->windowContentVtx[phi_t2 + 4].v.ob[0] + 0x6C;

			pthis->windowContentVtx[phi_t2 + 4].v.ob[1] = pthis->windowContentVtx[phi_t2 + 5].v.ob[1] = pthis->buttonYOffsets[phi_t5] + phi_ra;

			pthis->windowContentVtx[phi_t2 + 6].v.ob[1] = pthis->windowContentVtx[phi_t2 + 7].v.ob[1] = pthis->windowContentVtx[phi_t2 + 4].v.ob[1] - 0x10;
			...
			else if(((pthis->configMode == CM_COPY_ANIM_3) || (pthis->configMode == CM_COPY_ANIM_4)) && (phi_t5 == pthis->copyDestFileIndex))
			{
			...
			pthis->windowContentVtx[phi_t2 + 9].v.ob[0] = pthis->windowContentVtx[phi_t2 + 11].v.ob[0] = pthis->windowContentVtx[phi_t2 + 8].v.ob[0] + 0x2C;

			pthis->windowContentVtx[phi_t2 + 8].v.ob[1] = pthis->windowContentVtx[phi_t2 + 9].v.ob[1] = temp_t1;
			pthis->windowContentVtx[phi_t2 + 10].v.ob[1] = pthis->windowContentVtx[phi_t2 + 11].v.ob[1] = pthis->windowContentVtx[phi_t2 + 8].v.ob[1] - 0x10;
			...
			pthis->windowContentVtx[phi_t2 + 13].v.ob[0] = pthis->windowContentVtx[phi_t2 + 15].v.ob[0] = pthis->windowContentVtx[phi_t2 + 12].v.ob[0] + 0x18;

			pthis->windowContentVtx[phi_t2 + 12].v.ob[1] = pthis->windowContentVtx[phi_t2 + 13].v.ob[1] = pthis->buttonYOffsets[phi_t5] + phi_ra;

			pthis->windowContentVtx[phi_t2 + 14].v.ob[1] = pthis->windowContentVtx[phi_t2 + 15].v.ob[1] = pthis->windowContentVtx[phi_t2 + 12].v.ob[1] - 0x10;
			...
			pthis->windowContentVtx[phi_t2 + 17].v.ob[0] = pthis->windowContentVtx[phi_t2 + 19].v.ob[0] = pthis->windowContentVtx[phi_t2 + 16].v.ob[0] + 0x18;

			pthis->windowContentVtx[phi_t2 + 16].v.ob[1] = pthis->windowContentVtx[phi_t2 + 17].v.ob[1] = pthis->buttonYOffsets[phi_t5] + phi_ra;

			pthis->windowContentVtx[phi_t2 + 18].v.ob[1] = pthis->windowContentVtx[phi_t2 + 19].v.ob[1] = pthis->windowContentVtx[phi_t2 + 16].v.ob[1] - 0x10;
		...
		for(phi_t5 = 0; phi_t5 < 3; phi_t5++, phi_ra -= WREG(38))
		{
			if(SLOT_OCCUPIED(sramCtx, phi_t5))
			{
				...
				{
					pthis->windowContentVtx[phi_t2].v.ob[0] = pthis->windowContentVtx[phi_t2 + 2].v.ob[0] = WREG(41) + phi_t0 + 0x40;
					pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + WREG(42);
					pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = temp_t1 - 3;
					pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - WREG(43);
				}
				...
				{
					pthis->windowContentVtx[phi_t2].v.ob[0] = pthis->windowContentVtx[phi_t2 + 2].v.ob[0] = phi_t0;
					pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + D_80812820[phi_a1];
					pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = temp_t1;
					pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - D_80812828[phi_a1];
				...
				{
					pthis->windowContentVtx[phi_t2].v.ob[0] = pthis->windowContentVtx[phi_t2 + 2].v.ob[0] = phi_t0;
					pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + 0xA;
					pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = temp_t1;
					pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0xA;
		...
		for(phi_t5 = 0; phi_t5 < 2; phi_t5++, phi_t2 += 4, phi_ra -= 0x10)
		{
			pthis->windowContentVtx[phi_t2].v.ob[0] = pthis->windowContentVtx[phi_t2 + 2].v.ob[0] = phi_t0;
			pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + 0x40;
			pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = pthis->buttonYOffsets[phi_t5 + 3] + phi_ra;
			pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0x10;
			pthis->windowContentVtx[phi_t2 + 1].v.tc[0] = pthis->windowContentVtx[phi_t2 + 3].v.tc[0] = 0x800;
		}
		...
		pthis->windowContentVtx[phi_t2].v.ob[0] = pthis->windowContentVtx[phi_t2 + 2].v.ob[0] = phi_t0;
		pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + 0x40;
		pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = pthis->buttonYOffsets[5] - 0x34;
		pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0x10;
		pthis->windowContentVtx[phi_t2 + 1].v.tc[0] = pthis->windowContentVtx[phi_t2 + 3].v.tc[0] = 0x800;

		phi_t2 += 4;

		if(((pthis->menuMode == FS_MENU_MODE_CONFIG) && (pthis->configMode >= CM_MAIN_MENU)) || ((pthis->menuMode == FS_MENU_MODE_SELECT) && (pthis->selectMode == SM_CONFIRM_FILE)))
		{
			if(pthis->menuMode == FS_MENU_MODE_CONFIG)
			{
				if((pthis->configMode == CM_SELECT_COPY_SOURCE) || (pthis->configMode == CM_SELECT_COPY_DEST) || (pthis->configMode == CM_ERASE_SELECT))
				{
			...
			pthis->windowContentVtx[phi_t2 + 1].v.ob[0] = pthis->windowContentVtx[phi_t2 + 3].v.ob[0] = pthis->windowContentVtx[phi_t2].v.ob[0] + 0x48;
			pthis->windowContentVtx[phi_t2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 1].v.ob[1] = pthis->windowContentVtx[phi_t5].v.ob[1] + 4;
			pthis->windowContentVtx[phi_t2 + 2].v.ob[1] = pthis->windowContentVtx[phi_t2 + 3].v.ob[1] = pthis->windowContentVtx[phi_t2].v.ob[1] - 0x18;
		...
		pthis->windowContentVtx[phi_t2 + 5].v.ob[0] = pthis->windowContentVtx[phi_t2 + 7].v.ob[0] = pthis->windowContentVtx[phi_t2 + 4].v.ob[0] + 0x80;
		pthis->windowContentVtx[phi_t2 + 4].v.ob[1] = pthis->windowContentVtx[phi_t2 + 5].v.ob[1] = pthis->windowContentVtx[D_80812830[pthis->warningButtonIndex]].v.ob[1];
		pthis->windowContentVtx[phi_t2 + 6].v.ob[1] = pthis->windowContentVtx[phi_t2 + 7].v.ob[1] = pthis->windowContentVtx[phi_t2 + 4].v.ob[1] - 0x10;
	...
	static void* sQuestItemTextures[] = {
	    gFileSelKokiriEmeraldTex, gFileSelGoronRubyTex, gFileSelZoraSapphireTex, gFileSelForestMedallionTex, gFileSelFireMedallionTex, gFileSelWaterMedallionTex, gFileSelSpiritMedallionTex, gFileSelShadowMedallionTex, gFileSelLightMedallionTex,
	};
	...
	void FileChoose_DrawFileInfo(GameState* pthisx, s16 fileIndex, s16 isActive)
	{
		...
		OPEN_DISPS(pthis->gfxCtx, "../z_file_choose.c", 1709);
		...
		if(pthis->nameAlpha[fileIndex] != 0)
		{
			...
			gDPSetPrimColor(POLY_OPA_DISP++, 0x00, 0x00, sNamePrimColors[isActive][0], sNamePrimColors[isActive][1], sNamePrimColors[isActive][2], pthis->nameAlpha[fileIndex]);
		...
		if((fileIndex == pthis->selectedFileIndex) || (fileIndex == pthis->copyDestFileIndex))
		{
			...
			gDPSetPrimColor(POLY_OPA_DISP++, 0x00, 0x00, sHeartPrimColors[heartType][0], sHeartPrimColors[heartType][1], sHeartPrimColors[heartType][2], pthis->fileInfoAlpha[fileIndex]);
			gDPSetEnvColor(POLY_OPA_DISP++, sHeartEnvColors[heartType][0], sHeartEnvColors[heartType][1], sHeartEnvColors[heartType][2], 255);
			...
			for(vtxOffset = 0, j = 0; j < 9; j++, vtxOffset += 4)
			{
				if(pthis->questItems[fileIndex] & gBitFlags[sQuestItemFlags[j]])
				{
					...
					gDPPipeSync(POLY_OPA_DISP++);
					gDPSetPrimColor(POLY_OPA_DISP++, 0x00, 0x00, sQuestItemRed[j], sQuestItemGreen[j], sQuestItemBlue[j], pthis->fileInfoAlpha[fileIndex]);
					gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 0);

					if(j < 3)
					{
						gDPLoadTextureBlock(POLY_OPA_DISP++, sQuestItemTextures[j], G_IM_FMT_RGBA, G_IM_SIZ_32b, 16, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOLOD);
		...
		CLOSE_DISPS(pthis->gfxCtx, "../z_file_choose.c", 1797);
	}
	...
	void FileChoose_DrawWindowContents(GameState* pthisx)
	{
		...
		{
			gDPPipeSync(POLY_OPA_DISP++);
			gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, pthis->windowColor[0], pthis->windowColor[1], pthis->windowColor[2], pthis->fileInfoAlpha[fileIndex]);
			gSPVertex(POLY_OPA_DISP++, &pthis->windowContentVtx[temp], 20, 0);

			for(quadVtxIndex = 0, i = 0; i < 5; i++, quadVtxIndex += 4)
			{
				gDPLoadTextureBlock(
				    POLY_OPA_DISP++, sFileInfoBoxTextures[i], G_IM_FMT_IA, G_IM_SIZ_16b, sFileInfoBoxPartWidths[i], 56, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
		...
		for(i = 0; i < 3; i++, temp += 20)
		{
			...
			// draw file name box
			gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, sWindowContentColors[isActive][0], sWindowContentColors[isActive][1], sWindowContentColors[isActive][2], pthis->nameBoxAlpha[i]);
			...
			{
				gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, sWindowContentColors[isActive][0], sWindowContentColors[isActive][1], sWindowContentColors[isActive][2], pthis->nameAlpha[i]);
				gDPLoadTextureBlock(POLY_OPA_DISP++, gFileSelDISKButtonTex, G_IM_FMT_IA, G_IM_SIZ_16b, 44, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
				gSP1Quadrangle(POLY_OPA_DISP++, 8, 10, 11, 9, 0);
			}

			// draw connectors
			gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, sWindowContentColors[isActive][0], sWindowContentColors[isActive][1], sWindowContentColors[isActive][2], pthis->connectorAlpha[i]);
			gDPLoadTextureBlock(POLY_OPA_DISP++, gFileSelConnectorTex, G_IM_FMT_IA, G_IM_SIZ_8b, 24, 16, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
		...
		for(fileIndex = 0; fileIndex < 3; fileIndex++)
		{
			...
			FileChoose_DrawFileInfo(pthis, fileIndex, isActive);
		}
		...
		if(((pthis->menuMode == FS_MENU_MODE_CONFIG) && ((pthis->configMode == CM_MAIN_MENU) || (pthis->configMode == CM_SELECT_COPY_SOURCE) || (pthis->configMode == CM_SELECT_COPY_DEST) || (pthis->configMode == CM_COPY_CONFIRM) ||
		   ...
		   ((pthis->menuMode == FS_MENU_MODE_SELECT) && (pthis->selectMode == SM_CONFIRM_FILE)))
		{
			gDPPipeSync(POLY_OPA_DISP++);
			gDPSetCombineLERP(POLY_OPA_DISP++, 1, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, 1, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0);
			gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, pthis->highlightColor[0], pthis->highlightColor[1], pthis->highlightColor[2], pthis->highlightColor[3]);
			gDPLoadTextureBlock(POLY_OPA_DISP++, gFileSelBigButtonHighlightTex, G_IM_FMT_I, G_IM_SIZ_8b, 72, 24, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
			gSP1Quadrangle(POLY_OPA_DISP++, 12, 14, 15, 13, 0);
		}
		...
		CLOSE_DISPS(pthis->gfxCtx, "../z_file_choose.c", 2198);
	}
	...
	void FileChoose_ConfigModeDraw(GameState* pthisx)
	{
		...
		OPEN_DISPS(pthis->gfxCtx, "../z_file_choose.c", 2218);
		gDPPipeSync(POLY_OPA_DISP++);
		...
		SkyboxDraw_Draw(&pthis->skyboxCtx, pthis->gfxCtx, 1, pthis->envCtx.skyboxBlend, eyeX, eyeY, eyeZ);
		gDPSetTextureLUT(POLY_OPA_DISP++, G_TT_NONE);
		ZREG(11) += ZREG(10);
		Environment_UpdateSkybox(SKYBOX_NORMAL_SKY, &pthis->envCtx, &pthis->skyboxCtx);
		gDPPipeSync(POLY_OPA_DISP++);
		func_800949A8(pthis->gfxCtx);
		FileChoose_SetView(pthis, 0.0f, 0.0f, 64.0f);
		FileChoose_SetWindowVtx(pthis);
		FileChoose_SetWindowContentVtx(pthis);

		if((pthis->configMode != CM_NAME_ENTRY) && (pthis->configMode != CM_START_NAME_ENTRY))
		{
			gDPPipeSync(POLY_OPA_DISP++);
			gDPSetCombineMode(POLY_OPA_DISP++, G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM);
			gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, pthis->windowColor[0], pthis->windowColor[1], pthis->windowColor[2], pthis->windowAlpha);
			gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 0);
			...
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->gfxCtx, "../z_file_choose.c", 2282), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			...
			FileChoose_DrawWindowContents(pthis);
		}
		...
		if((pthis->configMode >= CM_ROTATE_TO_NAME_ENTRY) && (pthis->configMode <= CM_NAME_ENTRY_TO_MAIN))
		{
			...
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->gfxCtx, "../z_file_choose.c", 2316), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			...
			FileChoose_DrawNameEntry(pthis);
		}
		...
		if((pthis->configMode >= CM_MAIN_TO_OPTIONS) && (pthis->configMode <= CM_OPTIONS_TO_MAIN))
		{
			...
			gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->gfxCtx, "../z_file_choose.c", 2337), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
			...
			FileChoose_DrawOptions(pthis);
		}

		gDPPipeSync(POLY_OPA_DISP++);
		FileChoose_SetView(pthis, 0.0f, 0.0f, 64.0f);

		CLOSE_DISPS(pthis->gfxCtx, "../z_file_choose.c", 2352);
	}
	...
	void FileChoose_FadeMainToSelect(GameState* pthisx)
	{
		...
		for(i = 0; i < 3; i++)
		{
			if(i != pthis->buttonIndex)
			{
				...
				pthis->actionButtonAlpha[FS_BTN_ACTION_COPY] = pthis->actionButtonAlpha[FS_BTN_ACTION_ERASE] = pthis->optionButtonAlpha = pthis->fileButtonAlpha[i];
	...
	void FileChoose_FadeInFileInfo(GameState* pthisx)
	{
		...
		pthis->confirmButtonAlpha[FS_BTN_CONFIRM_YES] = pthis->confirmButtonAlpha[FS_BTN_CONFIRM_QUIT] = pthis->fileInfoAlpha[pthis->buttonIndex];
	}
	...
	void FileChoose_LoadGame(GameState* pthisx)
	{
		...
		if(oot::config().game().enablDebugLevelSelect() && pthis->buttonIndex == FS_BTN_SELECT_FILE_1)
		{
			...
			Graph_SetNextGameState(factory::Select(pthisx->gfxCtx));
			pthis->running = false;
		}
		else
		{
			...
			gSaveContext.open(pthis->buttonIndex);
			gSaveContext.gameMode = 0;
			Graph_SetNextGameState(factory::Global(pthisx->gfxCtx));
			pthis->running = false;
		}
		...
		if((gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KOKIRI) && (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_MASTER) && (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_BGS) && (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KNIFE))
		{
	...
	static void (*gSelectModeUpdateFuncs[])(GameState*) = {
	    FileChoose_FadeMainToSelect, FileChoose_MoveSelectedFileToTop, FileChoose_FadeInFileInfo, FileChoose_ConfirmFile, FileChoose_FadeOutFileInfo, FileChoose_MoveSelectedFileToSlot, FileChoose_FadeOut, FileChoose_LoadGame,
	};
	...
	void FileChoose_SelectModeUpdate(GameState* pthisx)
	{
		...
		gSelectModeUpdateFuncs[pthis->selectMode](pthis);
	}
	...
	void FileChoose_SelectModeDraw(GameState* pthisx)
	{
		...
		OPEN_DISPS(pthis->gfxCtx, "../z_file_choose.c", 2753);
		...
		Matrix_Scale(0.78f, 0.78f, 0.78f, MTXMODE_APPLY);
		Matrix_RotateX(pthis->windowRot / 100.0f, MTXMODE_APPLY);
		gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->gfxCtx, "../z_file_choose.c", 2810), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
		...
		CLOSE_DISPS(pthis->gfxCtx, "../z_file_choose.c", 2834);
	}
	...
	void FileChoose_InitContext(GameState* pthisx)
	{
		...
		pthis->buttonIndex = pthis->selectMode = pthis->selectedFileIndex = pthis->copyDestFileIndex = pthis->confirmButtonIndex = 0;
		...
		pthis->windowAlpha = pthis->titleAlpha[0] = pthis->titleAlpha[1] = pthis->fileButtonAlpha[0] = pthis->fileButtonAlpha[1] = pthis->fileButtonAlpha[2] = pthis->nameBoxAlpha[0] = pthis->nameBoxAlpha[1] = pthis->nameBoxAlpha[2] =
		    pthis->nameAlpha[0] = pthis->nameAlpha[1] = pthis->nameAlpha[2] = pthis->connectorAlpha[0] = pthis->connectorAlpha[1] = pthis->connectorAlpha[2] = pthis->fileInfoAlpha[0] = pthis->fileInfoAlpha[1] = pthis->fileInfoAlpha[2] =
			pthis->actionButtonAlpha[FS_BTN_ACTION_COPY] = pthis->actionButtonAlpha[FS_BTN_ACTION_ERASE] = pthis->confirmButtonAlpha[FS_BTN_CONFIRM_YES] = pthis->confirmButtonAlpha[FS_BTN_CONFIRM_QUIT] = pthis->optionButtonAlpha =
		...
		pthis->warningButtonIndex = pthis->buttonYOffsets[0] = pthis->buttonYOffsets[1] = pthis->buttonYOffsets[2] = pthis->buttonYOffsets[3] = pthis->buttonYOffsets[4] = pthis->buttonYOffsets[5] = pthis->fileNamesY[0] = pthis->fileNamesY[1] =
		    pthis->fileNamesY[2] = 0;
		...
		Skybox_Init(pthis, &pthis->skyboxCtx, SKYBOX_NORMAL_SKY);
	...
	Base* FileChoose::next()
	{
		return new FileChoose(gfxCtx);
	}

	void FileChoose::main()
	{
		...
		OPEN_DISPS(gfxCtx, "../z_file_choose.c", 2898);
		...
		CLOSE_DISPS(this->gfxCtx, "../z_file_choose.c", 3035);
	}
