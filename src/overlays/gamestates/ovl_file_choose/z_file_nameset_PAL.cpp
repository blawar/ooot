...
void FileChoose_SetNameEntryVtx(GameState* thisx)
{
	...
	{
		FileChoose_DrawCharacter(pthis->state.gfxCtx, font->fontBuf + pthis->fileNames[pthis->buttonIndex][phi_v0] * FONT_CHAR_TEX_SIZE, phi_s0);
	}
		...
		else
		{
			if(pthis->charPage <= FS_CHAR_PAGE_ENG)
			{
				if(pthis->kbdY != 5)
				{
					...
					FileChoose_DrawCharacter(pthis->state.gfxCtx, font->fontBuf + D_808123F0[pthis->charIndex] * FONT_CHAR_TEX_SIZE, 0);
...
void FileChoose_DrawOptionsImpl(GameState* thisx)
{
	...
	for(i = 0, vtx = 0; i < 4; i++, vtx += 4)
	{
		...
		gDPLoadTextureBlock(
		    POLY_OPA_DISP++, gOptionsMenuSettings[i].texture[gSaveContext.language], G_IM_FMT_IA, G_IM_SIZ_8b, gOptionsMenuSettings[i].width[gSaveContext.language], gOptionsMenuHeadersHeight, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP,
	...
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->state.gfxCtx, "../z_file_nameset_PAL.c", 1009), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	...
	gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(pthis->state.gfxCtx, "../z_file_nameset_PAL.c", 1033), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
