...
namespace oot::gamestate
{
	...
	void FileChoose_SetupCopySource(GameState* thisx)
	{
		...
		if(pthis->actionTimer == 0)
		{
			...
			pthis->actionButtonAlpha[FS_BTN_ACTION_COPY] = pthis->actionButtonAlpha[FS_BTN_ACTION_ERASE] = pthis->optionButtonAlpha = 0;
	...
	void FileChoose_SelectCopySource(GameState* thisx)
	{
		...
		if(((pthis->buttonIndex == FS_BTN_COPY_QUIT) && CHECK_BTN_ANY(input->press.button, BTN_A | BTN_START)) || CHECK_BTN_ALL(input->press.button, BTN_B))
		{
			pthis->actionTimer = 8;
	...
	void FileChoose_CopyConfirm(GameState* thisx)
	{
		...
		if(((pthis->buttonIndex != FS_BTN_CONFIRM_YES) && CHECK_BTN_ANY(input->press.button, BTN_A | BTN_START)) || CHECK_BTN_ALL(input->press.button, BTN_B))
		{
			pthis->actionTimer = 8;
			pthis->nextTitleLabel = FS_TITLE_COPY_TO;
	...
	void FileChoose_CopyAnim1(GameState* thisx)
	{
		...
		if(pthis->actionTimer == 0)
		{
			...
			pthis->actionTimer = 8;
			pthis->configMode++;
			osSyncPrintf("connect_alpha=%d  decision_alpha[%d]=%d\n", pthis->connectorAlpha[pthis->copyDestFileIndex], pthis->copyDestFileIndex, pthis->fileInfoAlpha[pthis->copyDestFileIndex]);
		}
	}
	...
	void FileChoose_ExitCopyToMain(GameState* thisx)
	{
		...
		pthis->optionButtonAlpha = pthis->actionButtonAlpha[FS_BTN_ACTION_ERASE] = pthis->actionButtonAlpha[FS_BTN_ACTION_COPY];
	}
	...
	void FileChoose_EraseAnim1(GameState* thisx)
	{
		...
		if(sEraseDelayTimer == 0)
		{
			...
			if(pthis->actionTimer == 0)
			{
				...
				pthis->fileInfoAlpha[pthis->selectedFileIndex] = pthis->nameBoxAlpha[pthis->selectedFileIndex] = pthis->confirmButtonAlpha[1];
