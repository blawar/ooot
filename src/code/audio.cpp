...
#define Audio_StartSeq(playerIdx, fadeTimer, seqId) Audio_QueueSeqCmd(0x00000000 | ((u8)playerIdx << 24) | ((u8)(fadeTimer) << 0x10) | (u16)seqId)
#define Audio_SeqSetIoPortGlobal(playerIdx, a, b) Audio_QueueSeqCmd(0x70000000 | ((u8)playerIdx << 0x18) | ((u8)a << 0x10) | (u8)(b))
#define Audio_SeqStartWithSetup(playerIdx, a, b, c) Audio_QueueSeqCmd(0xC0000000 | ((u8)playerIdx << 24) | ((u8)a << 16) | ((u8)b << 8) | ((u8)(c)))
...
#define Audio_SeqUpdateTempo(playerIdx, a, b, c) Audio_QueueSeqCmd(0xB0000000 | ((u8)playerIdx << 24) | ((u8)a << 16) | ((u8)b << 8) | ((u8)c))
#define Audio_SeqUpdateTempo2(playerIdx, a, b) Audio_QueueSeqCmd(0xB0004000 | ((u8)playerIdx << 24) | ((u8)a << 16) | ((u8)b))
#define Audio_SeqSetTransVolScale(playerIdx, a, b, c) Audio_QueueSeqCmd(0x60000000 | ((u8)playerIdx << 24) | ((u8)(a) << 16) | ((u8)b << 8) | ((u8)c))
...
#define Audio_SeqSetIoPortMasked(playerIdx, a, b, c) Audio_QueueSeqCmd(0x80000000 | ((u8)playerIdx << 24) | ((u8)a << 16) | ((u8)b << 8) | ((u8)c))
...
D_801306DC_s D_801306DC[20] = {
    {0xC0FF, 0xC0FE, {0, 2,   0, 0, 3, 0, 1, 2,	 9, 1, 3, 64, 1, 4,   0, 1, 5, 32, 2, 2,  4, 2, 3, 0, 2, 4,   1, 2, 5, 16, 3, 2,  10, 3, 3,  112, 3, 4,	  1, 3, 5, 48, 4, 2,  14,  4,
		      3, 127, 4, 4, 0, 4, 5, 16, 5, 2, 0, 5,  3, 127, 5, 4, 1, 5,  5, 16, 6, 2, 1, 6, 3, 127, 6, 4, 3, 6,  5, 16, 7,  2, 17, 7,	  3, 127, 7, 4, 1, 7,  5, 16, 0xFF}},
    {0xC0FB, 0xC0FA, {0, 2,  0, 0, 3, 0, 1, 2,	 4, 1, 3, 0, 1, 4,  1, 1, 5, 16, 3, 2,	 11, 3, 3, 112, 3, 4,  1, 3, 5,	 48, 4, 2,   14, 4, 3, 127, 4, 4,  0,	4,
		      5, 16, 5, 2, 0, 5, 3, 127, 5, 4, 1, 5, 5, 16, 6, 2, 1, 6,	 3, 127, 6,  4, 3, 6,	5, 16, 7, 2, 17, 7,  3, 127, 7,	 4, 1, 7,   5, 16, 0xFF}},
    ...
    {0xC07F, 0xC07E, {0, 2, 0, 0,  3, 0, 0, 4, 0, 1, 2, 10, 1, 3, 64, 1,  4, 0, 1, 5, 32, 2, 2, 11, 2, 3, 112, 2,  4, 1, 2, 5, 48, 3, 2, 12, 3, 3, 127, 3,  4,
		      0, 3, 5, 16, 4, 2, 6, 4, 3, 0, 4, 4,  0, 4, 5,  16, 5, 2, 0, 5, 3,  0, 5, 4,  0, 5, 5,   16, 6, 2, 1, 6, 3,  0, 6, 4,  0, 6, 5,	16, 0xFF}},
    {0xC003, 0xC000, {0, 2, 0, 0, 3, 0, 1, 2, 4, 1, 3, 0, 1, 4, 1, 1, 5, 16, 0xFF}},
    {0xC003, 0xC000, {0, 2, 0, 0, 3, 0, 1, 2, 4, 1, 3, 0, 1, 4, 1, 1, 5, 16, 0xFF}},
...
u8 sFrogsSongNotes[14] = {
    OCARINA_NOTE_A, OCARINA_NOTE_C_LEFT, OCARINA_NOTE_C_RIGHT, OCARINA_NOTE_C_DOWN, OCARINA_NOTE_C_LEFT,  OCARINA_NOTE_C_RIGHT, OCARINA_NOTE_C_DOWN,
    OCARINA_NOTE_A, OCARINA_NOTE_C_DOWN, OCARINA_NOTE_A,       OCARINA_NOTE_C_DOWN, OCARINA_NOTE_C_RIGHT, OCARINA_NOTE_C_LEFT,	OCARINA_NOTE_A,
};
		...
		for(i = sOcarinaSongNoteStartIdx; i < sOcarinaSongCnt; i++)
		{
			...
			if(sOcarinaAvailSongs & sh)
			{
				...
				if(inputChanged)
				{
					...
					if((D_8016BA50[i] >= D_8016BA70[i] - 0x12) && (D_8016BA50[i] >= D_8016BA70[i] + 0x12) && (sOcarinaSongs[i][sLearnSongPos[i]].unk_02 == 0) && (sLearnSongLastBtn == sLearnSongExpectedNote[i]))
					{
						D_80131878 = i + 1;
						sOcarinaInpEnabled = 0;
						D_80130F3C = 0;
				...
				else if(D_8016BA50[i] >= (D_8016BA70[i] - 0x12))
				{
					...
					while(prevNote->noteIdx == note->noteIdx || (note->noteIdx == OCARINA_NOTE_INVALID && note->unk_02 != 0))
					{
...
void func_800ED458(s32 arg0)
{
	...
	if((D_8016BA10 == 0) || ((D_8016BA10 & sOcarinaAllowedBtnMask) != (sCurOcarinaBtnPress & sOcarinaAllowedBtnMask)))
	{
		D_8016BA10 = 0;
...
void Audio_OcaPlayback(void)
{
	...
	if(sPlaybackState != 0)
	{
		...
		if(sNotePlaybackTimer == 0)
		{
			...
			if((sPlaybackSong[sPlaybackNotePos].volume == sPlaybackSong[sPlaybackNotePos - 1].volume && (sPlaybackSong[sPlaybackNotePos].vibrato == sPlaybackSong[sPlaybackNotePos - 1].vibrato) &&
			...
			if(sDisplayedNoteValue != sPlaybackSong[sPlaybackNotePos].noteIdx)
			{
				...
				if(sDisplayedNoteValue != 0xFF)
				{
					...
					Audio_PlaySoundGeneral(NA_SE_OC_OCARINA, &gAudioDefaultPos, 4, &sNormalizedNotePlaybackTone, &sNormalizedNotePlaybackVolume, &gReverbAdd2);
				}
				else
				{
					Audio_StopSfxById(NA_SE_OC_OCARINA);
	...
	if(sRecordingState == 2)
	{
		if(sStaffPlayingPos >= 8)
		{
			...
			for(i = 0; i < OCARINA_SONG_SCARECROW; i++)
			{
				...
				{
					for(k = 0; k < gOcarinaSongNotes[i].len && k + j < 8 && gOcarinaSongNotes[i].notesIdx[k] == gOcarinaSongNotes[OCARINA_SONG_SCARECROW].notesIdx[k + j]; k++)
					{
						;
					}
			...
			while(i < 8)
			{
				if(gOcarinaSongNotes[OCARINA_SONG_SCARECROW].notesIdx[0] != gOcarinaSongNotes[OCARINA_SONG_SCARECROW].notesIdx[i])
				{
					i = 9;
				}
				else
...
char sAudioDebugPageNames[15][23] = {
    "Non", "Sound Control", "Spec Info", "Heap Info", "Grp Track Info", "Sub Track Info", "Channel Info", "Interface Info", "SE Flag Swap", "Block Change BGM", "Natural Sound Control", "Ocarina Test", "SE Parameter Change", "Scroll Print", "Free Area",
};
...
void AudioDebug_ProcessInput_SndCont(void)
{
	...
	if(CHECK_BTN_ANY(sDebugPadPress, BTN_A))
	{
		switch(sAudioSndContSel)
		{
			...
			case 2:
			case 3:
				Audio_PlaySoundGeneral(((sAudioSndContWork[2] << 12) & 0xFFFF) + sAudioSndContWork[3] + SFX_FLAG, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
				break;
			case 4:
...
void AudioDebug_ProcessInput_ScrPrt(void)
{
	...
	D_801333F0 = sAudioScrPrtWork[3] + (sAudioScrPrtWork[4] * 2) + (sAudioScrPrtWork[5] * 4) + (sAudioScrPrtWork[6] * 8) + (sAudioScrPrtWork[7] * 0x10) + (sAudioScrPrtWork[8] * 0x20);
}
	...
	else
	{
		...
		if(CHECK_BTN_ANY(sDebugPadPress, BTN_DUP) || CHECK_BTN_ANY(sDebugPadPress, BTN_DDOWN))
		{
			...
			if(sAudioSfxSwapNibbleSel < 4)
			{
				...
				val = (val + step) & 0xF;
				gAudioSfxSwapSource[sAudioSfxSwapSel] = (gAudioSfxSwapSource[sAudioSfxSwapSel] & ((0xF << ((3 - sAudioSfxSwapNibbleSel) * 4)) ^ 0xFFFF)) + (val << ((3 - sAudioSfxSwapNibbleSel) * 4));
			}
			else
			{
				val = gAudioSfxSwapTarget[sAudioSfxSwapSel] >> ((7 - sAudioSfxSwapNibbleSel) * 4);
				val = (val + step) & 0xF;
				gAudioSfxSwapTarget[sAudioSfxSwapSel] = (gAudioSfxSwapTarget[sAudioSfxSwapSel] & ((0xF << ((7 - sAudioSfxSwapNibbleSel) * 4)) ^ 0xFFFF)) + (val << ((7 - sAudioSfxSwapNibbleSel) * 4));
			}
		}
...
void AudioDebug_ProcessInput_SfxParamChg(void)
{
	...
	if(CHECK_BTN_ANY(sDebugPadPress, BTN_CDOWN))
	{
		if(sAudioSfxParamChgSel == 2)
		{
			gSoundParams[sAudioSfxParamChgWork[0]][sAudioSfxParamChgWork[1]].params ^= (1 << (0xF - sAudioSfxParamChgBitSel));
		}
	}
...
void Audio_PlaySoundRiver(Vec3f* pos, f32 freqScale)
{
	...
	Audio_PlaySoundGeneral(NA_SE_EV_RIVER_STREAM - SFX_FLAG, pos, 4, &sRiverFreqScaleLerp.value, &D_801333E0, &gReverbAdd2);
}
...
void Audio_PlaySoundWaterfall(Vec3f* pos, f32 freqScale)
{
	...
	Audio_PlaySoundGeneral(NA_SE_EV_WATER_WALL_BIG - SFX_FLAG, pos, 4, &sWaterfallFreqScaleLerp.value, &sWaterfallFreqScaleLerp.value, &gReverbAdd2);
}
		...
		for(i = 0; i < 0x10; i++)
		{
			if(gAudioContext.seqPlayers[SEQ_PLAYER_BGM_MAIN].channels[i] != &gAudioContext.sequenceChannelNone)
			{
				if((u8)gAudioContext.seqPlayers[SEQ_PLAYER_BGM_MAIN].channels[i]->soundScriptIO[5] != 0xFF)
				{
					...
					// CHAN_UPD_REVERB
					Audio_QueueCmdS8(_SHIFTL(CHAN_UPD_REVERB, 24, 8) | _SHIFTL(SEQ_PLAYER_BGM_MAIN, 16, 8) | _SHIFTL(i, 8, 8) | _SHIFTL(0, 0, 8), (u8)phi_v0_2);
				}
			}
		}
...
{
	Audio_PlaySoundGeneral(sfxId, pos, 4, &gNoteFrequencies[semitones[sAudioIncreasingTranspose] + 39], &D_801333E0, &gReverbAdd2);
...
void Audio_SplitBgmChannels(s8 volSplit)
{
	...
	if((Audio_GetSequenceBeingPlayed(SEQ_PLAYER_FANFARE) == NA_BGM_DISABLED) && (Audio_GetSequenceBeingPlayed(SEQ_PLAYER_BGM_SUB) != NA_BGM_LONLON))
	{
...
{
	if((Audio_GetSequenceBeingPlayed(SEQ_PLAYER_BGM_MAIN) != NA_BGM_DISABLED) && (D_80130628 != NA_BGM_DISABLED) && (sSeqFlags[Audio_GetSequenceBeingPlayed(SEQ_PLAYER_BGM_MAIN) & 0xFF] & 8))
	{
...
void Audio_UpdateFanfare(void)
{
	...
	if(gFanfareDelay != 0)
	{
		...
		if(gFanfareDelay == 0)
		{
			...
			fanfare_seq = Audio_GetSequenceBeingPlayed(SEQ_PLAYER_FANFARE);
			bgm_sub_seq = Audio_GetSequenceBeingPlayed(SEQ_PLAYER_BGM_SUB);
			if(fanfare_seq == NA_BGM_DISABLED)
			{							 // Nothing playing?
				Audio_SetVolScale(SEQ_PLAYER_BGM_MAIN, 1, 0, 5); // Start fanfare
				...
				if(bgm_sub_seq != NA_BGM_LONLON)
				{
					Audio_SeqStartWithSetup(SEQ_PLAYER_FANFARE, 0x93, 0, 0);
				}
			}
			Audio_StartSeq(SEQ_PLAYER_FANFARE, 1, gFanfareLastSeqPlayed);
...
void func_800F6268(f32 dist, u16 seqid)
{
	s8 pad;
	s8 phi_v1;
	s16 bgm_seq;
	...
	if(D_8016B9F2 == 0)
	{
		bgm_seq = (s8)(Audio_GetSequenceBeingPlayed(SEQ_PLAYER_BGM_MAIN) & 0xFF);
		if(bgm_seq == (seqid & 0xFF))
		{
		...
		else if((bgm_seq == NA_BGM_NATURE_AMBIENCE) && ((seqid & 0xFF) == NA_BGM_LONLON))
		{
			bgm_seq = (s8)(Audio_GetSequenceBeingPlayed(SEQ_PLAYER_BGM_SUB) & 0xFF);
			if((bgm_seq != (seqid & 0xFF)) && (D_8016B9D8 < 10))
			{
...
void Audio_FadeOutMostSFX(u16 fade)
{
	...
	Audio_SeqDisablePlayer(SEQ_PLAYER_BGM_MAIN, (fade * 3) / 2);
	Audio_SeqDisablePlayer(SEQ_PLAYER_FANFARE, (fade * 3) / 2);
	for(i = 0; i < 0x10; i++)
	{
		skip = false;
		switch(i)
		...
		if(!skip)
		{
			Audio_SeqSetTransVolScale(SEQ_PLAYER_SFX, fade >> 1, i, 0);
		}
	}

	Audio_SeqDisablePlayer(SEQ_PLAYER_BGM_SUB, (fade * 3) / 2);
}

void Audio_FadeOutPlayers(u16 arg0)
{
...
void Audio_WaitUntilNotesPlayed(void)
{
	...
	Audio_ScheduleProcessCmds();
	while(true)
	{ // Wait until all notes are played
		if(Audio_UpdateNotes(0) == 0)
		{ // Are any notes being played?
			return;
		}
	}
}
...
void func_800F6FB4(u8 arg0)
{
	...
	if((gAudioPlayers[SEQ_PLAYER_BGM_MAIN].seqAndFlags == NA_BGM_DISABLED) || !(sSeqFlags[((u8)gAudioPlayers[SEQ_PLAYER_BGM_MAIN].seqAndFlags) & 0xFF] & 0x80))
	{
