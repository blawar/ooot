...
{
	s32 i;
	auto skeleton = (SkinLimb**)SEGMENTED_TO_VIRTUAL(skin->skeletonHeader->segment);
	auto temp_v1 = (Struct_800A5E28*)SEGMENTED_TO_VIRTUAL(((SkinLimb*)SEGMENTED_TO_VIRTUAL(skeleton[limbIndex]))->segment);
	auto temp_v0 = (Struct_800A598C*)SEGMENTED_TO_VIRTUAL(temp_v1->unk_4);
	...
	for(i = 0; i < 2; i++)
	{
		...
		{
			auto phi_s1 = (Struct_800A57C0*)SEGMENTED_TO_VIRTUAL(phi_s0->unk_8);
...
void func_800A663C(GlobalContext* globalCtx, PSkinAwb* skin, SkeletonHeader* skeletonHeader, AnimationHeader* animationHeader)
{
	...
	s32 i;
	SkinLimb** skeleton;
	auto virtSkelHeader = (SkeletonHeader*)SEGMENTED_TO_VIRTUAL(skeletonHeader);
	...
	skin->avbTbl = (SkinAvb*)ZeldaArena_MallocDebug(limbCount * sizeof(SkinAvb), "../z_skin_awb.c", 212);

	ASSERT(skin->avbTbl != NULL, "pskin_awb->avb_tbl != NULL", "../z_skin_awb.c", 214);
	for(i = 0; i < limbCount; i++)
	{
		SkinAvb* avbEntry = &skin->avbTbl[i];
		auto limb = (SkinLimb*)SEGMENTED_TO_VIRTUAL(skeleton[i]);
		...
		{
			auto temp_s1 = (Struct_800A5E28*)SEGMENTED_TO_VIRTUAL(((void)0, limb->segment));

			avbEntry->unk_0 = 0;
			avbEntry->buf[0] = (Vtx*)ZeldaArena_MallocDebug(temp_s1->unk_0 * sizeof(Vtx), "../z_skin_awb.c", 235);
			ASSERT(avbEntry->buf[0] != NULL, "psavb->buf[0] != NULL", "../z_skin_awb.c", 237);
			avbEntry->buf[1] = (Vtx*)ZeldaArena_MallocDebug(temp_s1->unk_0 * sizeof(Vtx), "../z_skin_awb.c", 240);
...
{
	s32 pad;
	auto limb = (SkinLimb*)SEGMENTED_TO_VIRTUAL(skeleton[limbIndex]);
	MtxF* mtx;
	s32 temp_ret;
	MtxF sp28;
...
s32 func_800A6AC4(PSkinAwb* skin, MtxF* arg1, Actor* actor, s32 arg3)
{
	...
	SkinMatrix_SetTranslateRotateYXZScale(
	    &skin->mtx, actor->scale.x, actor->scale.y, actor->scale.z, actor->shape.rot.x, actor->shape.rot.y, actor->shape.rot.z, actor->world.pos.x, actor->world.pos.y + (actor->shape.yOffset * actor->scale.y), actor->world.pos.z);

	temp_ret = func_800A698C(skin, (SkinLimb**)SEGMENTED_TO_VIRTUAL(skin->skeletonHeader->segment), arg1, 0xFF, 0);
	if(!temp_ret)
	{ // func_800A698C only returns false
		return temp_ret;
	}
