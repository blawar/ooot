...
const char* D_8012CF18[] = {GFXP_KATAKANA "Eﾓ-ﾄﾞ" GFXP_HIRAGANA "ｾﾞｯﾀｲ", GFXP_HIRAGANA "ｶﾞﾒﾝ" GFXP_KATAKANA "   ﾃﾞﾓ", GFXP_HIRAGANA "ｶﾞﾒﾝ   ﾌﾂｳ"};
const char* D_8012CF24[] = {GFXP_HIRAGANA "Pｼﾞｶﾝ  MAX", GFXP_KATAKANA "ﾘﾝｸ" GFXP_HIRAGANA "    ｷｵｸ", GFXP_KATAKANA "ﾘﾝｸ" GFXP_HIRAGANA "     ﾑｼ"};
...
const char* D_8012CF88[] = {GFXP_HIRAGANA "ｼﾃﾓｲｲﾃﾞｽｶ?", GFXP_HIRAGANA "ｹﾞﾝｻﾞｲﾍﾝｼｭｳﾁｭｳﾉ", GFXP_KATAKANA "ﾌｧｲﾙ" GFXP_HIRAGANA "ﾊﾊｷｻﾚﾏｽ"};
...
void func_800B44E0(DbCamera* dbCamera, Camera* cam)
{
	...
	if(!func_800BB2B4(&sDbCamAnim.positionPos, &sDbCamAnim.roll, &sDbCamAnim.fov, dbCamera->sub.position, &sDbCamAnim.keyframe, &sDbCamAnim.curFrame) &&
	   !func_800BB2B4(&sDbCamAnim.lookAtPos, &sDbCamAnim.roll, &sDbCamAnim.fov, dbCamera->sub.lookAt, &sDbCamAnim.keyframe, &sDbCamAnim.curFrame) && sDbCamAnim.unk_0A == 1)
	{
	...
	if(CHECK_BTN_ALL(sGlobalCtx->input[2].cur.button, BTN_R))
	{
		...
		else
		{
			...
			else if(CHECK_BTN_ALL(sGlobalCtx->input[2].press.button, BTN_R))
			{
				...
				{
					Audio_PlaySoundGeneral(NA_SE_IT_SWORD_PUTAWAY, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
...
s32 func_800B91B0(Camera* cam, DbCamera* dbCamera)
{
	...
	if(!func_800BB2B4(&sDbCamAnim.positionPos, &sDbCamAnim.roll, &sDbCamAnim.fov, sDbCameraCuts[D_8016110C].position, &sDbCamAnim.keyframe, &sDbCamAnim.curFrame) &&
	   !func_800BB2B4(&sDbCamAnim.lookAtPos, &sDbCamAnim.roll, &sDbCamAnim.fov, sDbCameraCuts[D_8016110C].lookAt, &sDbCamAnim.keyframe, &sDbCamAnim.curFrame))
	{
