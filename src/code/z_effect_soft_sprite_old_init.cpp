...
void EffectSs_DrawGEffect(GlobalContext* globalCtx, EffectSs* pthis, void* texture)
{
	...
	if(mtx != NULL)
	{
		...
		gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, pthis->rgPrimColorR, pthis->rgPrimColorG, pthis->rgPrimColorB, pthis->rgPrimColorA);
...
void EffectSsHahen_SpawnBurst(GlobalContext* globalCtx, Vec3f* pos, f32 burstScale, s16 unused, s16 scale, s16 randScaleRange, s16 count, s16 objId, s16 life, Gfx* dList)
{
	...
	for(i = 0; i < count; i++)
	{
		...
		EffectSsHahen_Spawn(globalCtx, pos, &velocity, &accel, unused, Rand_S16Offset(scale, randScaleRange), objId, life, dList);
	}
}
...
void EffectSsIcePiece_SpawnBurst(GlobalContext* globalCtx, Vec3f* refPos, f32 scale)
{
	...
	for(i = 0; i < ARRAY_COUNT(vecScales); i++)
	{
		...
		EffectSsIcePiece_Spawn(globalCtx, &pos, (Rand_ZeroFloat(1.0f) + 0.5f) * ((scale * 1.3f) * 100.0f), &velocity, &accel, 25);
	}
}
...
void EffectSsFireTail_SpawnFlame(GlobalContext* globalCtx, Actor* actor, Vec3f* pos, f32 arg3, s16 bodyPart, f32 colorIntensity)
{
	...
	EffectSsFireTail_Spawn(globalCtx, actor, pos, arg3, &actor->velocity, 15, &primColor, &envColor, (colorIntensity == 1.0f) ? 0 : 1, bodyPart, 1);
}
