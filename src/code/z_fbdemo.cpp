...
void TransitionUnk_InitGraphics(TransitionUnk* pthis)
{
	...
	for(colTex = 0, col = 0; col < pthis->col; colTex += 0x20, col++)
	{
		...
		for(rowTex = 0, row = 0, row2 = 0; row < pthis->row;)
		{
			...
			gDPLoadTextureTile(
			    gfx++, SEGMENT_ADDR(0xB, 0), G_IM_FMT_RGBA, G_IM_SIZ_16b, SCREEN_WIDTH, SCREEN_HEIGHT, rowTex, colTex, rowTex + 0x20, colTex + 0x20, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
			    G_TX_NOLOD);
...
void TransitionUnk_Update(TransitionUnk* pthis)
{
	...
	for(col = 0; col < pthis->col + 1; col++)
	{
		for(row = 0; row < pthis->row + 1; row++)
		{
			temp_f00 = (pthis->unk_0C + row + col * (pthis->row + 1))->unk_0 - (pthis->unk_0C + 5 + 4 * (pthis->row + 1))->unk_0;
			temp_f12 = (pthis->unk_0C + row + col * (pthis->row + 1))->unk_4 - (pthis->unk_0C + 5 + 4 * (pthis->row + 1))->unk_4;
