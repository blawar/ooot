...
#define Audio_StartSeq(playerIdx, fadeTimer, seqId) Audio_QueueSeqCmd(0x00000000 | ((playerIdx) << 24) | ((fadeTimer) << 16) | (seqId))
...
#define Audio_SetVolScaleNow(playerIdx, volFadeTimer, volScale) Audio_ProcessSeqCmd(0x40000000 | ((u8)playerIdx << 24) | ((u8)volFadeTimer << 16) | ((u8)(volScale * 127.0f)));
...
void Audio_PlaySequenceInternal(u8 playerIdx, u8 seqId, u8 seqFlags, u16 fadeTimer)
{
	...
	if(gAudioDisable == 0 || playerIdx == SEQ_PLAYER_SFX)
	{
		...
		{
			Audio_QueueCmdS32(CHAN_LOAD_UKN_82 << 24 | _SHIFTL(playerIdx, 16, 8) | _SHIFTL(seqId, 8, 8), (fadeTimer * (u16)gAudioContext.audioBufferParameters.updatesPerFrame) / 4);
		}
...
{
	Audio_QueueCmdS32(CHAN_LOAD_DISABLE_SEQUENCE << 24 | ((u8)playerIdx << 16), (arg1 * (u16)gAudioContext.audioBufferParameters.updatesPerFrame) / 4);
...
void Audio_ProcessSeqCmd(u32 cmd)
{
	...
	switch(op)
	{
		...
		case AUDIO_CMD_SEQUENCE_TRANS_VOLUME:
			...
			{
				gAudioPlayers[playerIdx].unk_08 = (gAudioPlayers[playerIdx].volCur - gAudioPlayers[playerIdx].volTarget) / (f32)duration;
		...
		case AUDIO_CMD_SEQUENCE_TRANS_FREQ_SCALE_ALL:
			...
			for(i = 0; i < 16; i++)
			{
				...
				gAudioPlayers[playerIdx].unk_50[i].unk_18 = (gAudioPlayers[playerIdx].unk_50[i].unk_10 - freqScale) / (f32)duration;
			}
		...
		case AUDIO_CMD_SEQUENCE_TRANS_FREQ_SCALE:
			...
			gAudioPlayers[playerIdx].unk_50[chanIdx].unk_18 = (gAudioPlayers[playerIdx].unk_50[chanIdx].unk_10 - freqScale) / (f32)duration;
		...
		case AUDIO_CMD_SEQUENCE_TRANS_VOL_SCALE:
			...
			{
				gAudioPlayers[playerIdx].unk_50[chanIdx].unk_08 = (gAudioPlayers[playerIdx].unk_50[chanIdx].unk_00 - gAudioPlayers[playerIdx].unk_50[chanIdx].unk_04) / (f32)duration;
		...
		case AUDIO_CMD_SET_IO_PORT_MASKED:
			...
			{
				Audio_QueueCmdS8(CHAN_UPD_SCRIPT_IO << 24 | _SHIFTL(playerIdx, 16, 8) | _SHIFTL(chanIdx, 8, 8) | _SHIFTL(port, 0, 8), val);
			}
			break;
		...
		if(gAudioPlayers[playerIdx].unk_252 != 0)
		{
			for(k = 0; k < 0x10; k++)
			{
				if(gAudioPlayers[playerIdx].unk_50[k].unk_0C != 0)
				{
					...
					Audio_QueueCmdF32(CHAN_UPD_VOL_SCALE << 24 | _SHIFTL(playerIdx, 16, 8) | _SHIFTL(k, 8, 8), gAudioPlayers[playerIdx].unk_50[k].unk_00);
				}
			}
		}
		...
		if(gAudioPlayers[playerIdx].unk_250 != 0)
		{
			for(k = 0; k < 0x10; k++)
			{
				if(gAudioPlayers[playerIdx].unk_50[k].unk_1C != 0)
				{
					...
					// CHAN_UPD_FREQ_SCALE
					Audio_QueueCmdF32(CHAN_UPD_FREQ_SCALE << 24 | _SHIFTL(playerIdx, 16, 8) | _SHIFTL(k, 8, 8), gAudioPlayers[playerIdx].unk_50[k].unk_10);
				}
			}
		}
