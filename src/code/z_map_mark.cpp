...
static MapMarkDataOverlay sMapMarkDataOvl = {
    NULL, _ovl_map_mark_dataSegmentRomStart, _ovl_map_mark_dataSegmentRomEnd, _ovl_map_mark_dataSegmentStart, _ovl_map_mark_dataSegmentEnd, gMapMarkDataTable,
};
	...
	while(true)
	{
		...
		for(i = 0; i < mapMarkIconData->count; i++)
		{
			if((mapMarkIconData->markType != MAP_MARK_CHEST) || !Flags_GetTreasure(globalCtx, markPoint->chestFlag))
			{
				...
				gDPPipeSync(OVERLAY_DISP++);
				gDPLoadTextureBlock(
				    OVERLAY_DISP++, markInfo->texture, markInfo->imageFormat, G_IM_SIZ_MARK, markInfo->textureWidth, markInfo->textureHeight, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
				    G_TX_NOLOD);

				rectLeft = ((GREG(94) + GFX_ALIGN_RIGHT(markPoint->x) + 204) << 2);
				rectTop = (GREG(95) + markPoint->y + 140) << 2;
				gSPTextureRectangle(OVERLAY_DISP++, rectLeft, rectTop, markInfo->rectWidth + rectLeft, rectTop + markInfo->rectHeight, G_TX_RENDERTILE, 0, 0, markInfo->dsdx, markInfo->dtdy);
			}
			markPoint++;
		}
		mapMarkIconData++;
