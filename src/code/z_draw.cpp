...
void GetItem_DrawMirrorShield(GlobalContext* globalCtx, s16 drawId) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x08,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (uintptr_t)(globalCtx->state.frames * 0) % 256,
                                (uintptr_t)(globalCtx->state.frames * 2) % 256, 64, 64, 1,
                                (uintptr_t)(globalCtx->state.frames * 0) % 128, (uintptr_t)(globalCtx->state.frames * 1) % 128, 32,
                                32));
