...
u8 D_80130520[] = {
    0x81, 0x00, 0x81, 0x01, 0x00, 0x00, 0x00, 0x81, 0x01, 0x01, 0x01, 0x42, 0x81, 0xC2, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00, 0x00, 0x00, 0x01, 0x42, 0x01, 0x01, 0x01, 0x81, 0x01, 0x01, 0x81, 0x81, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x81, 0x01, 0x01, 0x01, 0x81, 0x01, 0x01, 0x03, 0x03, 0x01, 0x00, 0x01, 0x01, 0x81, 0x03, 0x01, 0x00, 0x02, 0x00, 0x01, 0x01, 0x82, 0x00, 0x01, 0x01, 0x01, 0x01, 0x81, 0x00, 0x00, 0x01, 0x81, 0x81, 0x81, 0x81, 0x00, 0x00, 0x00,
};
...
s32 AudioSeq_SeqLayerProcessScriptStep5(SequenceLayer* layer, s32 sameSound)
{
	if(!layer->stopSomething && layer->sound != NULL && layer->sound->sample->codec == CODEC_S16_INMEMORY && layer->sound->sample->medium != MEDIUM_RAM)
	{
		layer->stopSomething = true;
		return -1;
	}
	...
	if(layer->continuousNotes == true && layer->note != NULL && layer->bit3 && sameSound == true && layer->note->playbackState.parentLayer == layer)
	{
		if(layer->sound == NULL)
		{
...
s32 AudioSeq_SeqLayerProcessScriptStep3(SequenceLayer* layer, s32 cmd)
{
	...
	{
		floatDelta = layer->velocitySquare * (f32)(gAudioContext.audioRandom % channel->velocityRandomVariance) / 100.0f;
...
void AudioSeq_SetInstrument(SequenceChannel* channel, u8 instId)
{
	...
	else if((channel->instOrWave = AudioSeq_GetInstrument(channel, instId, &channel->instrument, &channel->adsr)) == 0)
	{
		channel->hasInstrument = false;
		return;
	}
			...
			else
			{
				switch(command)
				{
					...
					case 0xB6:
						...
						break;
					case 0xB7:
						channel->unk_22 = (parameters[0] == 0) ? gAudioContext.audioRandom & 0xFFFF : gAudioContext.audioRandom % parameters[0];
						break;
					case 0xB8:
						scriptState->value = (parameters[0] == 0) ? gAudioContext.audioRandom & 0xFFFF : gAudioContext.audioRandom % parameters[0];
						break;
					case 0xBD:
					{
			...
			switch(command & 0xF0)
			{
				...
				case 0x10:
					if(lowBits < 8)
					{
						...
						if(AudioLoad_SlowLoadSample(channel->fontId, scriptState->value, &channel->soundScriptIO[lowBits]) == -1)
						{
							break;
						}
					}
					else
					{
						lowBits -= 8;
						channel->soundScriptIO[lowBits] = -1;
						if(AudioLoad_SlowLoadSample(channel->fontId, channel->unk_22 + 0x100, &channel->soundScriptIO[lowBits]) == -1)
						{
							break;
						}
					}
...
void AudioSeq_SequencePlayerProcessSequence(SequencePlayer* seqPlayer)
{
	...
	else
	{
		...
		while(true)
		{
			...
			if(command >= 0xF2)
			{
				s32 scriptHandled = AudioSeq_HandleScriptFlowControl(seqPlayer, seqScript, command, AudioSeq_GetScriptControlFlowArgument(&seqPlayer->scriptState, command));
					...
					case 0xDB:
						...
						switch(seqPlayer->state)
						{
							...
							case 0:
								...
								{
									seqPlayer->fadeVelocity = ((value / 127.0f) - seqPlayer->fadeVolume) / (s32)(seqPlayer->fadeTimer);
								}
								else
								{
...
void AudioSeq_ProcessSequences(s32 arg0)
{
	...
	gAudioContext.noteSubEuOffset = (gAudioContext.audioBufferParameters.updatesPerFrame - arg0 - 1) * gAudioContext.numNotes;
