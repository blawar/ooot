...
Gfx sWipeDList[] = {
	...
	gsDPLoadTextureBlock_4b(sWipe1Tex, G_IM_FMT_I, 64, 64, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_MIRROR | G_TX_WRAP, 6, 6, 11, G_TX_NOLOD),
	gsDPLoadMultiBlock_4b(sWipe1Tex, 0x0100, 1, G_IM_FMT_I, 64, 64, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_MIRROR | G_TX_WRAP, 6, 6, 11, 1),
...
void TransitionWipe_Start(void* pthisx)
{
	...
	if(pthis->direction != TRANS_WIPE_DIR_IN)
	{
		pthis->texY = (s32)(83.25f * (1 << 2));
	}
	else
	{
		pthis->texY = (s32)(153.0f * (1 << 2));
	}
...
void TransitionWipe_Update(void* pthisx, s32 updateRate)
{
	...
	if(pthis->direction != TRANS_WIPE_DIR_IN)
	{
		pthis->texY += (((void)0, gSaveContext.wipeSpeed) * 3) / updateRate;
		if(pthis->texY >= (s32)(153.0f * (1 << 2)))
		{
			pthis->texY = (s32)(153.0f * (1 << 2));
			pthis->isDone = true;
		}
...
void TransitionWipe_Draw(void* pthisx, Gfx** gfxP)
{
    ...
    gDPPipeSync(gfx++);
    texScroll = Gfx_BranchTexScroll(&gfx, pthis->texX, pthis->texY, 0, 0);
...
void TransitionWipe_SetType(void* pthisx, s32 type)
{
	...
	if(type == 1)
	{
		pthis->direction = TRANS_WIPE_DIR_OUT;
	}
	else
	{
		pthis->direction = TRANS_WIPE_DIR_IN;
	}
...
void TransitionWipe_SetAltColor(void* pthisx, Color_RGBA8 color)
{
	TransitionWipe* pthis = (TransitionWipe*)pthisx;

	pthis->altColor = color;
}
