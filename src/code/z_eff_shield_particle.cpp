...
void EffectShieldParticle_Init(void* thisx, void* initParamsx)
{
	...
	if((pthis != NULL) && (initParams != NULL))
	{
		...
		if(pthis->lightDecay == true)
		{
			...
			pthis->lightNode = LightContext_InsertLight(Effect_GetGlobalCtx(), &Effect_GetGlobalCtx()->lightCtx, &pthis->lightInfo);
		}
		else
		{
...
void EffectShieldParticle_Draw(void* thisx, GraphicsContext* gfxCtx)
{
	...
	if(pthis != NULL)
	{
		...
		gDPLoadTextureBlock(POLY_XLU_DISP++, gUnknownCircle6Tex, G_IM_FMT_I, G_IM_SIZ_8b, 32, 32, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 5, 5, G_TX_NOLOD, G_TX_NOLOD);

		gDPSetCombineLERP(POLY_XLU_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, PRIMITIVE, 0, TEXEL0, 0, 0, 0, 0, COMBINED, 0, 0, 0, COMBINED);
