...
void EffectBlure_AddVertex(EffectBlure* pthis, Vec3f* p1, Vec3f* p2)
{
	...
	if(pthis != NULL)
	{
		...
		elem = &pthis->elements[numElements];
		elem->state = 1;
		...
		else
		{
			...
			if(!(fabsf(scale) < 0.008f))
			{
				...
				func_800A7EC0(&spD0, pthis->addAngle, sp154.x, sp154.y, sp154.z);
...
void EffectBlure_InitElements(EffectBlure* pthis)
{
	EffectBlureElement* elem;
	s32 i;
...
{
	EffectBlure* pthis = (EffectBlure*)pthisx;
	EffectBlureInit1* initParams = (EffectBlureInit1*)initParamsx;

	if((pthis != NULL) && (initParams != NULL))
	{
		EffectBlure_InitElements(pthis);
		pthis->p1StartColor.r = initParams->p1StartColor[0];
		pthis->p2StartColor.r = initParams->p2StartColor[0];
		pthis->p1EndColor.r = initParams->p1EndColor[0];
		pthis->p2EndColor.r = initParams->p2EndColor[0];
		pthis->p1StartColor.g = initParams->p1StartColor[1];
		pthis->p2StartColor.g = initParams->p2StartColor[1];
		pthis->p1EndColor.g = initParams->p1EndColor[1];
		pthis->p2EndColor.g = initParams->p2EndColor[1];
		pthis->p1StartColor.b = initParams->p1StartColor[2];
		pthis->p2StartColor.b = initParams->p2StartColor[2];
		pthis->p1EndColor.b = initParams->p1EndColor[2];
		pthis->p2EndColor.b = initParams->p2EndColor[2];
		pthis->p1StartColor.a = initParams->p1StartColor[3];
		pthis->p2StartColor.a = initParams->p2StartColor[3];
		pthis->p1EndColor.a = initParams->p1EndColor[3];
		pthis->p2EndColor.a = initParams->p2EndColor[3];
		pthis->elemDuration = initParams->elemDuration;
		pthis->unkFlag = initParams->unkFlag;
		pthis->calcMode = initParams->calcMode;
		pthis->flags = 0;
		pthis->addAngleChange = 0;
		pthis->addAngle = 0;
		pthis->drawMode = 0;
		pthis->altPrimColor.r = 0;
		pthis->altPrimColor.g = 0;
		pthis->altPrimColor.b = 0;
		pthis->altPrimColor.a = 0;
		pthis->altEnvColor.r = 0;
		pthis->altEnvColor.g = 0;
		pthis->altEnvColor.b = 0;
		pthis->altEnvColor.a = 0;
		pthis->mode4Param = 1.0f;
	}
}
...
void EffectBlure_Init2(void* pthisx, void* initParamsx)
{
	...
	if((pthis != NULL) && (initParams != NULL))
	{
		...
		pthis->unkFlag = initParams->unkFlag;
		pthis->calcMode = initParams->calcMode;
		pthis->flags = initParams->flags;
		pthis->drawMode = initParams->drawMode;
		pthis->addAngleChange = initParams->addAngleChange;
		pthis->addAngle = 0;
		pthis->mode4Param = initParams->mode4Param;
		pthis->altPrimColor = initParams->altPrimColor;
		pthis->altEnvColor = initParams->altEnvColor;
	}
}
...
s32 EffectBlure_Update(void* pthisx)
{
	...
	while(true)
	{
		if(pthis->elements[0].state == 0)
		{
			for(i = 0; i < 15; i++)
			{
				pthis->elements[i] = pthis->elements[i + 1];
			}

			pthis->elements[i].state = 2;
			pthis->elements[i].p1.x = 0;
			pthis->elements[i].p1.y = 0;
			pthis->elements[i].p1.z = 0;
			pthis->elements[i].p2.x = 0;
			pthis->elements[i].p2.y = 0;
			pthis->elements[i].p2.z = 0;
			pthis->elements[i].flags = 0;
			pthis->elements[i].timer = 0;
	...
	{
		pthis->elements[i].timer++;
	}

	if(pthis->elemDuration < pthis->elements[0].timer)
	{
		for(i = 0; i < 15; i++)
		{
			pthis->elements[i] = pthis->elements[i + 1];
	...
	pthis->addAngle += pthis->addAngleChange;
	return 0;
}
...
void EffectBlure_UpdateFlags(EffectBlureElement* elem)
{
	...
	else
	{
		...
		if(Math3D_CosOut(&sp64, &sp4C, &sp34) || Math3D_CosOut(&sp58, &sp40, &sp30) || Math3D_CosOut(&sp4C, &sp40, &sp2C))
		{
			elem->flags &= ~3;
			elem->flags |= 0;
		}
...
void EffectBlure_GetComputedValues(EffectBlure* pthis, s32 index, f32 ratio, Vec3s* vec1, Vec3s* vec2, Color_RGBA8* color1, Color_RGBA8* color2)
{
	Vec3s sp30;
	f32 mode4Param;
	EffectBlureElement* elem = &pthis->elements[index];

	switch(pthis->calcMode)
	{
		case 1:
		...
		case 4:
			...
			mode4Param = pthis->mode4Param - 1.0f;
	...
	{
		color1->r = func_80027E84(pthis->p1StartColor.r, pthis->p1EndColor.r, ratio);
		color1->g = func_80027E84(pthis->p1StartColor.g, pthis->p1EndColor.g, ratio);
		color1->b = func_80027E84(pthis->p1StartColor.b, pthis->p1EndColor.b, ratio);
		color1->a = func_80027E84(pthis->p1StartColor.a, pthis->p1EndColor.a, ratio);
		color2->r = func_80027E84(pthis->p2StartColor.r, pthis->p2EndColor.r, ratio);
		color2->g = func_80027E84(pthis->p2StartColor.g, pthis->p2EndColor.g, ratio);
		color2->b = func_80027E84(pthis->p2StartColor.b, pthis->p2EndColor.b, ratio);
		color2->a = func_80027E84(pthis->p2StartColor.a, pthis->p2EndColor.a, ratio);
	}
}

void EffectBlure_SetupSmooth(EffectBlure* pthis, GraphicsContext* gfxCtx)
{
...
void EffectBlure_DrawElemNoInterpolation(EffectBlure* pthis, EffectBlureElement* elem, s32 index, GraphicsContext* gfxCtx)
{
	...
	Math_Vec3s_ToVec3f(&sp6C, &pthis->elements[0].p2);

	vtx = (Vtx*)Graph_Alloc(gfxCtx, sizeof(Vtx[4]));
	if(vtx == NULL)
	{
		// "Vertices cannot be secured."
		osSyncPrintf("z_eff_blure.c::SQ_NoInterpolate_disp() Vertices cannot be secured.\n");
	}
	else
	{
		vtx[0].v = baseVtx;
		...
		ratio = (f32)elem->timer / (f32)pthis->elemDuration;
		EffectBlure_GetComputedValues(pthis, index, ratio, &sp8C, &sp84, &sp7C, &sp78);
		...
		ratio = (f32)(elem + 1)->timer / (f32)pthis->elemDuration;
		EffectBlure_GetComputedValues(pthis, index + 1, ratio, &sp8C, &sp84, &sp7C, &sp78);
...
void EffectBlure_DrawElemHermiteInterpolation(EffectBlure* pthis, EffectBlureElement* elem, s32 index, GraphicsContext* gfxCtx)
{
	...
	Math_Vec3s_ToVec3f(&sp138, &pthis->elements[0].p2);

	ratio = (f32)elem->timer / (f32)pthis->elemDuration;
	EffectBlure_GetComputedValues(pthis, index, ratio, &sp1EC, &sp1E4, &sp1A4, &sp1A0);
	Math_Vec3s_ToVec3f(&sp1CC, &sp1EC);
	Math_Vec3s_ToVec3f(&sp1C0, &sp1E4);

	ratio = (f32)(elem + 1)->timer / (f32)pthis->elemDuration;
	EffectBlure_GetComputedValues(pthis, index + 1, ratio, &sp1EC, &sp1E4, &sp19C, &sp198);
	...
	else
	{
		...
		ratio = (f32)(elem - 1)->timer / (f32)pthis->elemDuration;
		EffectBlure_GetComputedValues(pthis, index - 1, ratio, &sp1EC, &sp1E4, &sp1DC, &sp1D8);
	...
	else
	{
		...
		ASSERT(index + 2 < pthis->numElements, "index + 2 < pthis2->now_edge_num", "../z_eff_blure.c", 1032);

		ratio = (f32)(elem + 2)->timer / (f32)pthis->elemDuration;
		EffectBlure_GetComputedValues(pthis, index + 2, ratio, &sp1EC, &sp1E4, &sp1DC, &sp1D8);
	...
	vtx = (Vtx*)Graph_Alloc(gfxCtx, sizeof(Vtx[16]));
	if(vtx == NULL)
	{
		// "Vertices cannot be secured."
		osSyncPrintf("z_eff_blure.c::SQ_HermiteInterpolate_disp() Vertices cannot be secured.\n");
	}
	else
	{
...
void EffectBlure_DrawSmooth(EffectBlure* pthis2, GraphicsContext* gfxCtx)
{
	EffectBlure* pthis = pthis2;
	EffectBlureElement* elem;
	s32 i;
	MtxF spDC;
	MtxF sp9C;
	...
	if(pthis->numElements < 2)
	{
		return;
	}

	pthis->elements[0].flags &= ~3;
	pthis->elements[0].flags |= 2;

	for(elem = &pthis->elements[1]; elem < pthis->elements + pthis->numElements - 1; elem++)
	{
		EffectBlure_UpdateFlags(elem);
	}

	pthis->elements[pthis->numElements - 1].flags &= ~3;
	pthis->elements[pthis->numElements - 1].flags |= 2;

	EffectBlure_SetupSmooth(pthis, gfxCtx);
	SkinMatrix_SetTranslate(&spDC, pthis->elements[0].p2.x, pthis->elements[0].p2.y, pthis->elements[0].p2.z);
	...
	for(i = 0, elem = &pthis->elements[0]; elem < pthis->elements + pthis->numElements - 1; i++, elem++)
	{
		if((elem->state == 0) || ((elem + 1)->state == 0))
		{
			continue;
		}
		if((((elem->flags & 3) == 0) && (((elem + 1)->flags & 3) == 0)) || (((elem->flags & 3) == 2) && (((elem + 1)->flags & 3) == 0)) || (((elem->flags & 3) == 0) && (((elem + 1)->flags & 3) == 2)) ||
		   (((elem->flags & 3) == 2) && (((elem + 1)->flags & 3) == 2)))
		{
			EffectBlure_DrawElemNoInterpolation(pthis, elem, i, gfxCtx);
		}
		else
		{
			EffectBlure_DrawElemHermiteInterpolation(pthis, elem, i, gfxCtx);
		}
	}
...
void EffectBlure_SetupSimple(GraphicsContext* gfxCtx, EffectBlure* pthis, Vtx* vtx)
{
...
void EffectBlure_SetupSimpleAlt(GraphicsContext* gfxCtx, EffectBlure* pthis, Vtx* vtx)
{
	...
	gDPLoadTextureBlock(POLY_XLU_DISP++, gUnknownEffBlureTex, G_IM_FMT_I, G_IM_SIZ_8b, 64, 32, 0, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_WRAP, 6, 5, G_TX_NOLOD, G_TX_NOLOD);

	gDPSetCombineLERP(POLY_XLU_DISP++, TEXEL0, PRIMITIVE, PRIM_LOD_FRAC, TEXEL0, TEXEL0, 0, PRIMITIVE, 0, PRIMITIVE, ENVIRONMENT, COMBINED, ENVIRONMENT, 0, 0, 0, COMBINED);
	...
	gDPSetEnvColor(POLY_XLU_DISP++, pthis->altEnvColor.r, pthis->altEnvColor.g, pthis->altEnvColor.b, pthis->altEnvColor.a);

	CLOSE_DISPS(gfxCtx, "../z_eff_blure.c", 1329);
}

void (*sSetupHandlers[])(GraphicsContext* gfxCtx, EffectBlure* pthis, Vtx* vtx) = {
...
void EffectBlure_DrawSimpleVertices(GraphicsContext* gfxCtx, EffectBlure* pthis, Vtx* vtx)
{
	Mtx* mtx;

	OPEN_DISPS(gfxCtx, "../z_eff_blure.c", 1356);

	sSetupHandlers[pthis->drawMode](gfxCtx, pthis, vtx);
	gDPPipeSync(POLY_XLU_DISP++);

	{
		...
		for(i = 0; i < pthis->numElements - 1; i++)
		{
			if(pthis->drawMode == 1)
			{
				alphaRatio = (f32)pthis->elements[i].timer / (f32)pthis->elemDuration;
				gDPSetPrimColor(POLY_XLU_DISP++, 0x00, 0x80, pthis->altPrimColor.r, pthis->altPrimColor.g, pthis->altPrimColor.b, pthis->altPrimColor.a * (1.0f - alphaRatio));
				gDPPipeSync(POLY_XLU_DISP++);
			}
			...
			if(pthis->flags & 4)
			{
				...
				if(fabsf(scale) > 0.0005f)
				{
					...
					if(mtx == NULL)
					{
						...
						osSyncPrintf("EffectBlureInfo2_disp_makeDisplayList() Forced termination because a matrix cannot be taken\n");
						break;
					}
...
void EffectBlure_DrawSimple(EffectBlure* pthis2, GraphicsContext* gfxCtx)
{
	EffectBlure* pthis = pthis2;
	Vtx* vtx;
	Vtx* vtxIter;
	...
	if(pthis->numElements >= 2)
	{
		vtxCount = pthis->numElements * 4;

		vtx = (Vtx*)Graph_Alloc(gfxCtx, vtxCount * sizeof(Vtx));
		if(vtx == NULL)
		{
		...
		if(pthis->numElements >= 2)
		{
			for(elem = pthis->elements; elem < pthis->elements + pthis->numElements - 2; elem++)
			{
				for(i = 0; i < 4; i++)
				{
		...
		for(i = 0; i < pthis->numElements; i++)
		{
			...
			ratio = (f32)elem->timer / (f32)pthis->elemDuration;
			EffectBlure_GetComputedValues(pthis, i, ratio, &sp74, &sp6C, &sp64, &sp60);
		...
		EffectBlure_DrawSimpleVertices(gfxCtx, pthis, vtx);
	}
}
...
void EffectBlure_Draw(void* pthisx, GraphicsContext* gfxCtx)
{
	...
	if(pthis->numElements != 0)
	{
		if(pthis->flags == 0)
		{
			...
			vtx = (Vtx*)Graph_Alloc(gfxCtx, sizeof(Vtx[32]));
			if(vtx == NULL)
			{
			...
			else
			{
				...
				for(i = 0; i < pthis->numElements; i++)
				{
					...
					if(elem->state == 1)
					{
						f32 ratio = (f32)elem->timer / (f32)pthis->elemDuration;
						...
						vtx[j].v.tc[0] = 0;
						vtx[j].v.tc[1] = 0;
						vtx[j].v.cn[0] = func_80027E84(pthis->p1StartColor.r, pthis->p1EndColor.r, ratio);
						vtx[j].v.cn[1] = func_80027E84(pthis->p1StartColor.g, pthis->p1EndColor.g, ratio);
						vtx[j].v.cn[2] = func_80027E84(pthis->p1StartColor.b, pthis->p1EndColor.b, ratio);
						vtx[j].v.cn[3] = func_80027E84(pthis->p1StartColor.a, pthis->p1EndColor.a, ratio);
						j++;

						vtx[j].v.flag = 0;
						vtx[j].v.tc[0] = 0;
						vtx[j].v.tc[1] = 0;
						vtx[j].v.cn[0] = func_80027E84(pthis->p2StartColor.r, pthis->p2EndColor.r, ratio);
						vtx[j].v.cn[1] = func_80027E84(pthis->p2StartColor.g, pthis->p2EndColor.g, ratio);
						vtx[j].v.cn[2] = func_80027E84(pthis->p2StartColor.b, pthis->p2EndColor.b, ratio);
						vtx[j].v.cn[3] = func_80027E84(pthis->p2StartColor.a, pthis->p2EndColor.a, ratio);
						j++;
					}
				}
		...
		else if(pthis->drawMode < 2)
		{
			EffectBlure_DrawSimple(pthis, gfxCtx);
		}
		else
		{
			EffectBlure_DrawSmooth(pthis, gfxCtx);
		}
	}
