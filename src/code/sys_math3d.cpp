...
s32 Math3D_PlaneVsLineSegClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA, f32 planeBB, f32 planeBC, f32 planeBDist, Vec3f* linePointA, Vec3f* linePointB, Vec3f* closestPoint)
{
	...
	if(!Math3D_PlaneVsPlaneNewLine(planeAA, planeAB, planeAC, planeADist, planeBA, planeBB, planeBC, planeBDist, &planeIntersectLine))
	{
	...
	if(!Math3D_LineVsLineClosestTwoPoints(&planeIntersectSeg.a, &planeIntersectSeg.b, linePointA, linePointB, closestPoint, &sp34))
	{
		return false;
	}
	return true;
...
s32 Math3D_LineVsLineClosestTwoPoints(Vec3f* lineAPointA, Vec3f* lineAPointB, Vec3f* lineBPointA, Vec3f* lineBPointB, Vec3f* lineAClosestToB, Vec3f* lineBClosestToA)
{
	...
	compBAAlongB = ((lineBx * (lineAPointA->x - lineBPointA->x)) + (lineBy * (lineAPointA->y - lineBPointA->y)) + (lineBz * (lineAPointA->z - lineBPointA->z))) * scaleB;
...
void Math3D_LineClosestToPoint(Linef* line, Vec3f* pos, Vec3f* closestPoint)
{
	...
	t = (((pos->x - line->a.x) * line->b.x) + ((pos->y - line->a.y) * line->b.y) + ((pos->z - line->a.z) * line->b.z)) / dirVectorSize;
...
s32 Math3D_PlaneVsPlaneVsLineClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA, f32 planeBB, f32 planeBC, f32 planeBDist, Vec3f* point, Vec3f* closestPoint)
{
	...
	if(!Math3D_PlaneVsPlaneNewLine(planeAA, planeAB, planeAC, planeADist, planeBA, planeBB, planeBC, planeBDist, (InfiniteLine*)&planeIntersect))
	{
		return false;
	}
...
s32 Math3D_CirSquareVsTriSquare(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2, f32 centerX, f32 centerY, f32 radius)
{
	...
	if((minX - radius) <= centerX && (maxX + radius) >= centerX && (minY - radius) <= centerY && (maxY + radius) >= centerY)
	{
		return true;
	}
	return false;
...
s32 Math3D_SphCubeVsTriCube(Vec3f* v0, Vec3f* v1, Vec3f* v2, Vec3f* center, f32 radius)
{
	...
	if((center->x >= (minX - radius)) && (center->x <= (maxX + radius)) && (center->y >= (minY - radius)) && (center->y <= (maxY + radius)) && (center->z >= (minZ - radius)) && (center->z <= (maxZ + radius)))
	{
		return true;
	}
...
{
	return (plane->normal.x * pointOnPlane->x) + (plane->normal.y * pointOnPlane->y) + (plane->normal.z * pointOnPlane->z) + plane->originDist;
}
...
s32 Math3D_TriChkPointParaYImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z, f32 x, f32 detMax, f32 chkDist, f32 ny)
{
	...
	if(((SQ(v0->z - z) + SQ(v0->x - x)) < chkDistSq) || ((SQ(v1->z - z) + SQ(v1->x - x)) < chkDistSq) || ((SQ(v2->z - z) + SQ(v2->x - x)) < chkDistSq))
	{
		return true;
	}
	...
	if(((detMax >= detv0v1) && (detMax >= detv1v2) && (detMax >= detv2v0)) || ((-detMax <= detv0v1) && (-detMax <= detv1v2) && (-detMax <= detv2v0)))
	{
		return true;
	}
...
s32 Math3D_TriChkPointParaXImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 y, f32 z, f32 detMax, f32 chkDist, f32 nx)
{
	...
	if(((SQ(v0->y - y) + SQ(v0->z - z)) < chkDistSq) || ((SQ(v1->y - y) + SQ(v1->z - z)) < chkDistSq) || ((SQ(v2->y - y) + SQ(v2->z - z)) < chkDistSq))
	{
		return true;
	}
	...
	if(((detv0v1 <= detMax) && (detv1v2 <= detMax) && (detv2v0 <= detMax)) || ((-detMax <= detv0v1) && (-detMax <= detv1v2) && (-detMax <= detv2v0)))
	{
		return true;
	}
...
s32 Math3D_TriChkPointParaZImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 x, f32 y, f32 detMax, f32 chkDist, f32 nz)
{
	...
	if(((SQ(x - v0->x) + SQ(y - v0->y)) < chkDistSq) || ((SQ(x - v1->x) + SQ(y - v1->y)) < chkDistSq) || ((SQ(x - v2->x) + SQ(y - v2->y)) < chkDistSq))
	{
...
s32 Math3D_TriLineIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* linePointA, Vec3f* linePointB, Vec3f* intersect, s32 fromFront)
{
	...
	if(((nx == 0.0f) || (Math3D_TriChkPointParaX(v0, v1, v2, nx, intersect->y, intersect->z))) && ((ny == 0.0f) || (Math3D_TriChkPointParaY(v0, v1, v2, ny, intersect->z, intersect->x))) &&
...
s32 Math3D_LineVsSph(Sphere16* sphere, Linef* line)
{
	...
	temp_f0_2 = ((((sphere->center.x - line->a.x) * lineDiff.x) + ((sphere->center.y - line->a.y) * lineDiff.y)) + ((sphere->center.z - line->a.z) * lineDiff.z)) / lineLenSq;
	...
	if(SQ(sphLinePerpendicularPoint.x - sphere->center.x) + SQ(sphLinePerpendicularPoint.y - sphere->center.y) + SQ(sphLinePerpendicularPoint.z - sphere->center.z) <= SQ((f32)sphere->radius))
	{
		return true;
	}
	return false;
...
s32 Math3D_TriVsSphIntersect(Sphere16* sphere, TriNorm* tri, Vec3f* intersectPoint)
{
	...
	planeDist = Math3D_UDistPlaneToPos(tri->plane.normal.x, tri->plane.normal.y, tri->plane.normal.z, tri->plane.originDist, &sphereCenter);
	if(radius < planeDist)
	{
	...
	if(Math3D_Planef(tri->plane.normal.x, tri->plane.normal.y, tri->plane.normal.z, tri->plane.originDist, &sphereCenter) > 0.0f)
	{
	...
	{
		if(Math3D_TriChkPointParaYDeterminate(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.z, sphPlanePos.x, 0.0f, tri->plane.normal.y))
		{
			Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
			return true;
		}
	...
	{
		if(Math3D_TriChkPointParaXDeterminate(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.y, sphPlanePos.z, 0.0f, tri->plane.normal.x))
		{
			Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
			return true;
		}
	}
	else if(Math3D_TriChkPointParaZDeterminate(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.x, sphPlanePos.y, 0.0f, tri->plane.normal.z))
	{
		Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
		return true;
	}
...
s32 Math3D_CylTriVsIntersect(Cylinder16* cyl, TriNorm* tri, Vec3f* intersect)
{
	...
	if(((tri->vtx[0].y < cylBottom) && (tri->vtx[1].y < cylBottom) && (tri->vtx[2].y < cylBottom)) || ((cylTop < tri->vtx[0].y) && (cylTop < tri->vtx[1].y) && (cylTop < tri->vtx[2].y)))
	{
	...
	if(Math3D_TriChkLineSegParaYIntersect(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], tri->plane.normal.x, tri->plane.normal.y, tri->plane.normal.z, tri->plane.originDist, cyl->pos.z, cyl->pos.x, &yIntersect, cylBottom, cylTop))
	{
		...
		Math3D_PointOnInfiniteLine(&cylIntersectCenter, &diffMidpointIntersect, radiusTodistFromCylYIntersectTov0v1, intersect);
		return true;
	}
	...
	if((Math3D_TriVsSphIntersect(&topSphere, tri, intersect)) || (Math3D_TriVsSphIntersect(&bottomSphere, tri, intersect)))
	{
		return true;
	}
	return false;
...
s32 Math3D_CylOutsideCylDist(Cylinder16* ca, Cylinder16* cb, f32* deadSpace, f32* xzDist)
{
	...
	*xzDist = sqrtf(SQ(caf.pos.x - cbf.pos.x) + SQ(caf.pos.z - cbf.pos.z));
	...
	if(((caf.pos.y + caf.yShift) + caf.height) < (cbf.pos.y + cbf.yShift) || (((cbf.pos.y + cbf.yShift) + cbf.height) < (caf.pos.y + caf.yShift)))
	{
		return false;
	}
...
s32 Math3D_TriVsTriIntersect(TriNorm* ta, TriNorm* tb, Vec3f* intersect)
{
	...
	if(Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2], tb->plane.normal.x, tb->plane.normal.y, tb->plane.normal.z, tb->plane.originDist, &ta->vtx[0], &ta->vtx[1], intersect, 0))
	{
		return true;
	}
	if(Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2], tb->plane.normal.x, tb->plane.normal.y, tb->plane.normal.z, tb->plane.originDist, &ta->vtx[1], &ta->vtx[2], intersect, 0))
	{
		return true;
	}
	if(Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2], tb->plane.normal.x, tb->plane.normal.y, tb->plane.normal.z, tb->plane.originDist, &ta->vtx[2], &ta->vtx[0], intersect, 0))
	{
		return true;
	}
