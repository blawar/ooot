...
void GameOver_FadeInLights(GlobalContext* globalCtx)
{
	...
	if((gameOverCtx->state >= GAMEOVER_DEATH_WAIT_GROUND && gameOverCtx->state < GAMEOVER_REVIVE_START) || (gameOverCtx->state >= GAMEOVER_REVIVE_RUMBLE && gameOverCtx->state < GAMEOVER_REVIVE_FADE_OUT))
	{
...
void GameOver_Update(GlobalContext* globalCtx)
{
	...
	switch(gameOverCtx->state)
	{
		case GAMEOVER_DEATH_START:
			...
			Rumble_Shake(0.0f, ((v90 > 0x64) ? 0xFF : (v90 * 0xFF) / 0x64), (CLAMP_MAX(v91 * 3, 0xFF)), ((v92 > 0x64) ? 0xFF : (v92 * 0xFF) / 0x64));
