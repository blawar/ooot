...
SpeedMeterTimeEntry sSpeedMeterTimeEntryArray[] = {
    ...
    {&gRSPAudioTotalTime_LastFrame, 0, 4, GPACK_RGBA5551(0, 0, 255, 1)},
    {&gRSPGFXTotalTime_LastFrame, 0, 6, GPACK_RGBA5551(255, 128, 128, 1)},
    {&gRDPTotalTime_LastFrame, 0, 8, GPACK_RGBA5551(0, 255, 0, 1)},
...
void SpeedMeter_DrawTimeEntries(SpeedMeter* pthis, GraphicsContext* gfxCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(sSpeedMeterTimeEntryArray); i++)
	{
		DrawRec(gfx++, sSpeedMeterTimeEntryPtr->color, baseX, lry + sSpeedMeterTimeEntryPtr->y, sSpeedMeterTimeEntryPtr->x, lry + sSpeedMeterTimeEntryPtr->y + 1);
		sSpeedMeterTimeEntryPtr++;
	}
	gDPPipeSync(gfx++);
...
void SpeedMeter_InitAllocEntry(SpeedMeterAllocEntry* pthis, u32 maxval, u32 val, u16 backColor, u16 foreColor, u32 ulx, u32 lrx, u32 uly, u32 lry)
{
	pthis->maxval = maxval;
	pthis->val = val;
...
void SpeedMeter_DrawAllocEntries(SpeedMeter* meter, GraphicsContext* gfxCtx, GameState* state)
{
	...
	if(SREG(0) > 2)
	{
		if(ZeldaArena_IsInitalized())
		{
			...
			SpeedMeter_InitAllocEntry(&entry, zeldaFree + zeldaAlloc, zeldaAlloc, GPACK_RGBA5551(0, 0, 255, 1), GPACK_RGBA5551(255, 255, 255, 1), ulx, lrx, y, y + 1);
	...
	if(SREG(0) > 1)
	{
		...
		SpeedMeter_InitAllocEntry(&entry, sysFree + sysAlloc - state->tha.size, sysAlloc - state->tha.size, GPACK_RGBA5551(0, 0, 255, 1), GPACK_RGBA5551(255, 128, 128, 1), ulx, lrx, y, y);
		SpeedMeter_DrawAllocEntry(&entry, gfxCtx);
		y++;
	}

	thga = (TwoHeadGfxArena*)&state->tha;
	SpeedMeter_InitAllocEntry(&entry, thga->size, thga->size - THA_GetSize((TwoHeadArena*)thga), GPACK_RGBA5551(0, 0, 255, 1), GPACK_RGBA5551(0, 255, 0, 1), ulx, lrx, y, y);
	SpeedMeter_DrawAllocEntry(&entry, gfxCtx);
	y++;

	thga = &gfxCtx->polyOpa;
	SpeedMeter_InitAllocEntry(&entry, thga->size, thga->size - THGA_GetSize(thga), GPACK_RGBA5551(0, 0, 255, 1), GPACK_RGBA5551(255, 0, 255, 1), ulx, lrx, y, y);
	SpeedMeter_DrawAllocEntry(&entry, gfxCtx);
	y++;

	thga = &gfxCtx->polyXlu;
	SpeedMeter_InitAllocEntry(&entry, thga->size, thga->size - THGA_GetSize(thga), GPACK_RGBA5551(0, 0, 255, 1), GPACK_RGBA5551(255, 255, 0, 1), ulx, lrx, y, y);
	SpeedMeter_DrawAllocEntry(&entry, gfxCtx);
	y++;

	thga = &gfxCtx->overlay;
	SpeedMeter_InitAllocEntry(&entry, thga->size, thga->size - THGA_GetSize(thga), GPACK_RGBA5551(0, 0, 255, 1), GPACK_RGBA5551(255, 0, 0, 1), ulx, lrx, y, y);
	SpeedMeter_DrawAllocEntry(&entry, gfxCtx);
	y++;
}
