#define INTERNAL_SRC_CODE_OSMALLOC_C
...
void __osMallocAddBlock(Arena* arena, void* start, s32 size) {
    ...
    if (start != NULL) {
        ...
        diff = POINTER_SUB(firstNode, start);
        size2 = (size - diff) & ~0xF;

        if (size2 > (s32)sizeof(ArenaNode)) {
            z_memset(firstNode, BLOCK_UNINIT_MAGIC, size2); // memset
            firstNode->next = NULL;
            firstNode->prev = NULL;
...
void ArenaImpl_RemoveAllBlocks(Arena* arena) {
    ...
    while (iter != NULL) {
        ...
        z_memset(iter, BLOCK_UNINIT_MAGIC, iter->size + sizeof(ArenaNode)); // memset
        iter = next;
    }
    ...
    while (iter != NULL) {
        if (iter->isFree && iter->size >= size) {
            ...
            if (arena->flag & FILL_ALLOCBLOCK) {
                z_memset(alloc, BLOCK_ALLOC_MAGIC, size);
            }
...
void* __osMallocRDebug(Arena* arena, u32 size, const char* file, s32 line) {
    ...
    while (iter != NULL) {
        if (iter->isFree && iter->size >= size) {
            ...
            if (arena->flag & FILL_ALLOCBLOCK) {
                z_memset(allocR, BLOCK_ALLOC_MAGIC, size);
            }
...
void __osFree_NoLock(Arena* arena, void* ptr) {
    ...
    if (node == NULL || node->magic != NODE_MAGIC) {
        ...
        osSyncPrintf(VT_COL(RED, WHITE) "__osFree: Unauthorized release (%08x)\n" VT_RST, ptr);
        return;
    }
    if (node->isFree) {
        osSyncPrintf(VT_COL(RED, WHITE) "__osFree: Double release (%08x)\n" VT_RST, ptr); // "__osFree: Double release (%08x)"
        return;
    }
    if (arena != node->arena && arena != NULL) {
        // "__osFree:Tried to release in a different way than when it was secured (%08x:%08x)"
        osSyncPrintf(VT_COL(RED, WHITE) "__osFree:Tried to release in a different way than when it was secured (%08x:%08x)\n" VT_RST, arena,
                     node->arena);
        return;
    }
    ...
    if (arena->flag & FILL_FREEBLOCK) {
        z_memset((uintptr_t)node + sizeof(ArenaNode), BLOCK_FREE_MAGIC, node->size);
    }
    ...
    if ((uintptr_t)next == (uintptr_t)node + sizeof(ArenaNode) + node->size && next->isFree) {
        ...
        if (arena->flag & FILL_FREEBLOCK) {
            z_memset(next, BLOCK_FREE_MAGIC, sizeof(ArenaNode));
        }
        node->next = newNext;
        next = newNext;
    }
    ...
    if (prev != NULL && prev->isFree && (uintptr_t)node == (uintptr_t)prev + sizeof(ArenaNode) + prev->size) {
        ...
        if (arena->flag & FILL_FREEBLOCK) {
            z_memset(node, BLOCK_FREE_MAGIC, sizeof(ArenaNode));
        }
    }
}
...
void __osFree_NoLockDebug(Arena* arena, void* ptr, const char* file, s32 line) {
    ...
    if (node == NULL || node->magic != NODE_MAGIC) {
        // "__osFree: Unauthorized release (%08x)"
        osSyncPrintf(VT_COL(RED, WHITE) "__osFree: Unauthorized release (%08x) [%s:%d ]\n" VT_RST, ptr, file, line);
        return;
    }
    if (node->isFree) {
        // "__osFree: Double release (%08x)"
        osSyncPrintf(VT_COL(RED, WHITE) "__osFree: Double release (%08x) [%s:%d ]\n" VT_RST, ptr, file, line);
        return;
    }
    if (arena != node->arena && arena != NULL) {
        // "__osFree:Tried to release in a different way than when it was secured (%08x:%08x)"
