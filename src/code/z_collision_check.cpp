...
void Collider_DrawPoly(GraphicsContext* gfxCtx, Vec3f* vA, Vec3f* vB, Vec3f* vC, u8 r, u8 g, u8 b)
{
	...
	gDPPipeSync(POLY_OPA_DISP++);
	gDPSetCombineLERP(POLY_OPA_DISP++, SHADE, 0, PRIMITIVE, 0, SHADE, 0, PRIMITIVE, 0, 0, 0, 0, COMBINED, 0, 0, 0, COMBINED);
...
s32 Collider_InitInfo(GlobalContext* globalCtx, ColliderInfo* info)
{
	static ColliderInfo init = {
	    {0, 0, 0}, {0xFFCFFFFF, 0, 0, {0, 0, 0}}, ELEMTYPE_UNK0, TOUCH_NONE, BUMP_NONE, OCELEM_NONE, NULL, NULL, NULL, NULL,
	};
...
s32 Collider_SetJntSphToActor(GlobalContext* globalCtx, ColliderJntSph* dest, ColliderJntSphInitToActor* src)
{
	...
	dest->elements = (ColliderJntSphElement*)ZeldaArena_MallocDebug(src->count * sizeof(ColliderJntSphElement), "../z_collision_check.c", 1443);
	...
	for(destElem = dest->elements, srcElem = src->elements; destElem < dest->elements + dest->count; destElem++, srcElem++)
	{
...
s32 Collider_SetJntSphAllocType1(GlobalContext* globalCtx, ColliderJntSph* dest, Actor* actor, ColliderJntSphInitType1* src)
{
	...
	dest->count = src->count;
	dest->elements = (ColliderJntSphElement*)ZeldaArena_MallocDebug(src->count * sizeof(ColliderJntSphElement), "../z_collision_check.c", 1490);
...
s32 Collider_SetJntSphAlloc(GlobalContext* globalCtx, ColliderJntSph* dest, Actor* actor, ColliderJntSphInit* src)
{
	...
	dest->count = src->count;
	dest->elements = (ColliderJntSphElement*)ZeldaArena_MallocDebug(src->count * sizeof(ColliderJntSphElement), "../z_collision_check.c", 1551);
...
s32 Collider_SetTrisAllocType1(GlobalContext* globalCtx, ColliderTris* dest, Actor* actor, ColliderTrisInitType1* src)
{
	...
	Collider_SetBaseType1(globalCtx, &dest->base, actor, &src->base);
	dest->count = src->count;
	dest->elements = (ColliderTrisElement*)ZeldaArena_MallocDebug(dest->count * sizeof(ColliderTrisElement), "../z_collision_check.c", 2156);
	if(dest->elements == NULL)
	{
		dest->count = 0;
		osSyncPrintf(VT_FGCOL(RED));
...
s32 Collider_SetTrisAlloc(GlobalContext* globalCtx, ColliderTris* dest, Actor* actor, ColliderTrisInit* src)
{
	...
	Collider_SetBase(globalCtx, &dest->base, actor, &src->base);
	dest->count = src->count;
	dest->elements = (ColliderTrisElement*)ZeldaArena_MallocDebug(dest->count * sizeof(ColliderTrisElement), "../z_collision_check.c", 2207);
...
void CollisionCheck_HitSolid(GlobalContext* globalCtx, ColliderInfo* info, Collider* collider, Vec3f* hitPos)
{
	...
	if(flags == TOUCH_SFX_NORMAL && collider->colType != COLTYPE_METAL)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_IT_SHIELD_BOUND, &collider->actor->projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
	}
	...
	else if(flags == TOUCH_SFX_WOOD)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_IT_REFLECTION_WOOD, &collider->actor->projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
	}
}
...
s32 CollisionCheck_SwordHitAudio(Collider* at, ColliderInfo* acInfo)
{
	if(at->actor != NULL && at->actor->category == ACTORCAT_PLAYER)
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_IT_SWORD_STRIKE, &at->actor->projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
		else if(acInfo->elemType == ELEMTYPE_UNK1)
		{
			Audio_PlaySoundGeneral(NA_SE_IT_SWORD_STRIKE_HARD, &at->actor->projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
		else if(acInfo->elemType == ELEMTYPE_UNK2)
		{
			Audio_PlaySoundGeneral(NA_SE_PL_WALK_GROUND - SFX_FLAG, &at->actor->projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
...
static HitInfo sHitInfo[] = {
    {BLOOD_BLUE, HIT_WHITE}, {BLOOD_NONE, HIT_DUST}, {BLOOD_GREEN, HIT_DUST}, {BLOOD_NONE, HIT_WHITE}, {BLOOD_WATER, HIT_NONE}, {BLOOD_NONE, HIT_RED},	 {BLOOD_GREEN, HIT_WHITE},
    {BLOOD_RED, HIT_WHITE},  {BLOOD_BLUE, HIT_RED},  {BLOOD_NONE, HIT_SOLID}, {BLOOD_NONE, HIT_NONE},  {BLOOD_NONE, HIT_SOLID}, {BLOOD_NONE, HIT_SOLID}, {BLOOD_NONE, HIT_WOOD},
};
...
void CollisionCheck_HitEffects(GlobalContext* globalCtx, Collider* at, ColliderInfo* atInfo, Collider* ac, ColliderInfo* acInfo, Vec3f* hitPos)
{
	...
	else
	{
		...
		{
			Audio_PlaySoundGeneral(NA_SE_IT_SHIELD_BOUND, &ac->actor->projectedPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
		}
	}
}
...
s32 CollisionCheck_SetATvsAC(GlobalContext* globalCtx, Collider* at, ColliderInfo* atInfo, Vec3f* atPos, Collider* ac, ColliderInfo* acInfo, Vec3f* acPos, Vec3f* hitPos)
{
	...
	if(!(atInfo->toucherFlags & TOUCH_AT_HITMARK) && ac->colType != COLTYPE_METAL && ac->colType != COLTYPE_WOOD && ac->colType != COLTYPE_HARD)
	{
	...
	if(at->count > 0 && at->elements != NULL && ac->count > 0 && ac->elements != NULL)
	{
		for(atItem = at->elements; atItem < at->elements + at->count; atItem++)
		{
			...
			for(acElem = ac->elements; acElem < ac->elements + ac->count; acElem++)
			{
				...
				if(Math3D_SphVsSphOverlapCenter(&atItem->dim.worldSphere, &acElem->dim.worldSphere, &overlapSize, &centerDist) == 1)
				{
					f32 acToHit;
					Vec3f hitPos;
					Vec3f atPos;
					...
					CollisionCheck_SetATvsAC(globalCtx, &at->base, &atItem->info, &atPos, &ac->base, &acElem->info, &acPos, &hitPos);
	...
	if(at->count > 0 && at->elements != NULL && ac->dim.radius > 0 && ac->dim.height > 0)
	{
		...
		for(atItem = at->elements; atItem < at->elements + at->count; atItem++)
		{
			...
			if(Math3D_SphVsCylOverlapCenterDist(&atItem->dim.worldSphere, &ac->dim, &overlapSize, &centerDist))
			{
				...
				CollisionCheck_SetATvsAC(globalCtx, &at->base, &atItem->info, &atPos, &ac->base, &ac->info, &acPos, &hitPos);
				return;
			}
		}
	}
		...
		for(atSph = at->elements; atSph < at->elements + at->count; atSph++)
		{
			...
			for(acTri = ac->elements; acTri < ac->elements + ac->count; acTri++)
			{
				...
				if(Math3D_TriVsSphIntersect(&atSph->dim.worldSphere, &acTri->dim, &hitPos) == 1)
				{
					...
					CollisionCheck_SetATvsAC(globalCtx, &at->base, &atSph->info, &atPos, &ac->base, &acTri->info, &acPos, &hitPos);
					return;
				}
			}
		}
...
void CollisionCheck_AC_JntSphVsQuad(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* colAT, Collider* colAC)
{
	...
	if(at->count > 0 && at->elements != NULL)
	{
		...
		for(atItem = at->elements; atItem < at->elements + at->count; atItem++)
		{
			...
			if(Math3D_TriVsSphIntersect(&atItem->dim.worldSphere, &D_8015E230, &hitPos) == 1 || Math3D_TriVsSphIntersect(&atItem->dim.worldSphere, &D_8015E268, &hitPos) == 1)
			{
				Vec3f atPos;
				Vec3f acPos;
...
void CollisionCheck_AC_QuadVsJntSph(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* colAT, Collider* colAC)
{
	...
	if(ac->count > 0 && ac->elements != NULL)
	{
		...
		for(acElem = ac->elements; acElem < ac->elements + ac->count; acElem++)
		{
			...
			if(Math3D_TriVsSphIntersect(&acElem->dim.worldSphere, &D_8015E2A0, &hitPos) == 1 || Math3D_TriVsSphIntersect(&acElem->dim.worldSphere, &D_8015E2D8, &hitPos) == 1)
			{
				if(Collider_QuadSetNearestAC(globalCtx, at, &hitPos))
				{
					...
					atPos.x = (at->dim.quad[0].x + (at->dim.quad[1].x + (at->dim.quad[3].x + at->dim.quad[2].x))) / 4.0f;
					atPos.y = (at->dim.quad[0].y + (at->dim.quad[1].y + (at->dim.quad[3].y + at->dim.quad[2].y))) / 4.0f;
					atPos.z = (at->dim.quad[0].z + (at->dim.quad[1].z + (at->dim.quad[3].z + at->dim.quad[2].z))) / 4.0f;
					CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos, &ac->base, &acElem->info, &acPos, &hitPos);
					if(!(ac->base.ocFlags2 & OC2_FIRST_ONLY))
					{
						return;
	...
	if(ac->dim.radius > 0 && ac->dim.height > 0 && at->count > 0 && at->elements != NULL)
	{
		...
		for(atItem = at->elements; atItem < at->elements + at->count; atItem++)
		{
			...
			if(Math3D_CylTriVsIntersect(&ac->dim, &atItem->dim, &D_8015E310) == 1)
			{
				...
				atPos.z = (atItem->dim.vtx[0].z + atItem->dim.vtx[1].z + atItem->dim.vtx[2].z) * (1.0f / 3);
				Math_Vec3s_ToVec3f(&acPos, &ac->dim.pos);
				CollisionCheck_SetATvsAC(globalCtx, &at->base, &atItem->info, &atPos, &ac->base, &ac->info, &acPos, &D_8015E310);
				return;
			}
		}
	}
...
void CollisionCheck_AC_CylVsQuad(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* colAT, Collider* colAC)
{
	...
	if(at->dim.height > 0 && at->dim.radius > 0)
	{
		...
		if(Math3D_CylTriVsIntersect(&at->dim, &D_8015E320, &D_8015E390) == 1)
		{
			...
			CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos1, &ac->base, &ac->info, &acPos1, &D_8015E390);
		}
		else if(Math3D_CylTriVsIntersect(&at->dim, &D_8015E358, &D_8015E390) == 1)
		{
			...
			CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos2, &ac->base, &ac->info, &acPos2, &D_8015E390);
		}
	}
}
	...
	if(ac->dim.height > 0 && ac->dim.radius > 0)
	{
		...
		if(Math3D_CylTriVsIntersect(&ac->dim, &D_8015E3A0, &D_8015E410) == 1)
		{
			if(Collider_QuadSetNearestAC(globalCtx, at, &D_8015E410))
			{
				...
				CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos1, &ac->base, &ac->info, &acPos1, &D_8015E410);
				return;
			}
		}
		if(Math3D_CylTriVsIntersect(&ac->dim, &D_8015E3D8, &D_8015E410) == 1)
		{
			if(Collider_QuadSetNearestAC(globalCtx, at, &D_8015E410))
			{
				...
				CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos2, &ac->base, &ac->info, &acPos2, &D_8015E410);
			}
		}
	}
}
		...
		for(acElem = ac->elements; acElem < ac->elements + ac->count; acElem++)
		{
			...
			for(atItem = at->elements; atItem < at->elements + at->count; atItem++)
			{
				...
				if(Math3D_TriVsTriIntersect(&atItem->dim, &acElem->dim, &D_8015E420) == 1)
				{
					...
					acPos.y = (acElem->dim.vtx[0].y + acElem->dim.vtx[1].y + acElem->dim.vtx[2].y) * (1.0f / 3);
					acPos.z = (acElem->dim.vtx[0].z + acElem->dim.vtx[1].z + acElem->dim.vtx[2].z) * (1.0f / 3);
					CollisionCheck_SetATvsAC(globalCtx, &at->base, &atItem->info, &atPos, &ac->base, &acElem->info, &acPos, &D_8015E420);
					return;
				}
			}
		}
...
void CollisionCheck_AC_TrisVsQuad(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* colAT, Collider* colAC)
{
	...
	if(at->count > 0 && at->elements != NULL)
	{
		...
		for(atItem = at->elements; atItem < at->elements + at->count; atItem++)
		{
			...
			if(Math3D_TriVsTriIntersect(&D_8015E440, &atItem->dim, &D_8015E430) == 1 || Math3D_TriVsTriIntersect(&D_8015E478, &atItem->dim, &D_8015E430) == 1)
			{
				Vec3f atPos;
				Vec3f acPos;
				...
				acPos.y = (ac->dim.quad[0].y + (ac->dim.quad[1].y + (ac->dim.quad[3].y + ac->dim.quad[2].y))) / 4.0f;
				acPos.z = (ac->dim.quad[0].z + (ac->dim.quad[1].z + (ac->dim.quad[3].z + ac->dim.quad[2].z))) / 4.0f;
				CollisionCheck_SetATvsAC(globalCtx, &at->base, &atItem->info, &atPos, &ac->base, &ac->info, &acPos, &D_8015E430);
				return;
			}
		}
	}
...
void CollisionCheck_AC_QuadVsTris(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* colAT, Collider* colAC)
{
	...
	if(ac->count > 0 && ac->elements != NULL)
	{
		...
		for(acElem = ac->elements; acElem < ac->elements + ac->count; acElem++)
		{
			...
			if(Math3D_TriVsTriIntersect(&D_8015E4C0, &acElem->dim, &D_8015E4B0) == 1 || Math3D_TriVsTriIntersect(&D_8015E4F8, &acElem->dim, &D_8015E4B0) == 1)
			{
				if(Collider_QuadSetNearestAC(globalCtx, at, &D_8015E4B0))
				{
					...
					atPos.y = (at->dim.quad[0].y + (at->dim.quad[1].y + (at->dim.quad[3].y + at->dim.quad[2].y))) / 4.0f;
					atPos.z = (at->dim.quad[0].z + (at->dim.quad[1].z + (at->dim.quad[3].z + at->dim.quad[2].z))) / 4.0f;
					CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos, &ac->base, &acElem->info, &acPos, &D_8015E4B0);
					return;
				}
			}
		}
		...
		for(j = 0; j < 2; j++)
		{
			if(Math3D_TriVsTriIntersect(&D_8015E5A8[j], &D_8015E530[i], &D_8015E598) == 1)
			{
				if(Collider_QuadSetNearestAC(globalCtx, at, &D_8015E598))
				{
					...
					acPos.y = (ac->dim.quad[0].y + (ac->dim.quad[1].y + (ac->dim.quad[3].y + ac->dim.quad[2].y))) / 4.0f;
					acPos.z = (ac->dim.quad[0].z + (ac->dim.quad[1].z + (ac->dim.quad[3].z + ac->dim.quad[2].z))) / 4.0f;
					CollisionCheck_SetATvsAC(globalCtx, &at->base, &at->info, &atPos, &ac->base, &ac->info, &acPos, &D_8015E598);
					return;
				}
			}
		}
...
void CollisionCheck_SetJntSphHitFX(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* collider)
{
	...
	for(element = jntSph->elements; element < jntSph->elements + jntSph->count; element++)
	{
		if((element->info.bumperFlags & BUMP_DRAW_HITMARK) && (element->info.acHitInfo != NULL) && !(element->info.acHitInfo->toucherFlags & TOUCH_DREW_HITMARK))
		{
			Math_Vec3s_ToVec3f(&hitPos, &element->info.bumper.hitPos);
			CollisionCheck_HitEffects(globalCtx, element->info.acHit, element->info.acHitInfo, &jntSph->base, &element->info, &hitPos);
...
void CollisionCheck_SetCylHitFX(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* collider)
{
	...
	if((cylinder->info.bumperFlags & BUMP_DRAW_HITMARK) && (cylinder->info.acHitInfo != NULL) && !(cylinder->info.acHitInfo->toucherFlags & TOUCH_DREW_HITMARK))
	{
		...
		CollisionCheck_HitEffects(globalCtx, cylinder->info.acHit, cylinder->info.acHitInfo, &cylinder->base, &cylinder->info, &hitPos);
...
void CollisionCheck_SetTrisHitFX(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* collider)
{
	...
	for(element = tris->elements; element < tris->elements + tris->count; element++)
	{
		...
		{
			Math_Vec3s_ToVec3f(&hitPos, &element->info.bumper.hitPos);
			CollisionCheck_HitEffects(globalCtx, element->info.acHit, element->info.acHitInfo, &tris->base, &element->info, &hitPos);
			element->info.acHitInfo->toucherFlags |= TOUCH_DREW_HITMARK;
			return;
		}
	}
...
static ColChkVsFunc sACVsFuncs[4][4] = {
    {CollisionCheck_AC_JntSphVsJntSph, CollisionCheck_AC_JntSphVsCyl, CollisionCheck_AC_JntSphVsTris, CollisionCheck_AC_JntSphVsQuad},
    {CollisionCheck_AC_CylVsJntSph, CollisionCheck_AC_CylVsCyl, CollisionCheck_AC_CylVsTris, CollisionCheck_AC_CylVsQuad},
    {CollisionCheck_AC_TrisVsJntSph, CollisionCheck_AC_TrisVsCyl, CollisionCheck_AC_TrisVsTris, CollisionCheck_AC_TrisVsQuad},
    {CollisionCheck_AC_QuadVsJntSph, CollisionCheck_AC_QuadVsCyl, CollisionCheck_AC_QuadVsTris, CollisionCheck_AC_QuadVsQuad},
};
...
void CollisionCheck_SetOCvsOC(Collider* left, ColliderInfo* leftInfo, Vec3f* leftPos, Collider* right, ColliderInfo* rightInfo, Vec3f* rightPos, f32 overlap)
{
	...
	if(rightMassType == MASSTYPE_IMMOVABLE)
	{
		...
		{ // leftMassType == MASS_HEAVY | MASS_NORMAL
			leftDispRatio = 0;
			rightDispRatio = 1;
		}
	}
	else if(rightMassType == MASSTYPE_HEAVY)
	{
		...
		{ // leftMassType == MASS_NORMAL
			leftDispRatio = 0;
			rightDispRatio = 1;
		}
	}
	else
	{ // rightMassType == MASS_NORMAL
		...
		{ // leftMassType == MASS_HEAVY | MASS_IMMOVABLE
			leftDispRatio = 1;
			rightDispRatio = 0;
		}
	}
		...
		for(leftElem = left->elements; leftElem < left->elements + left->count; leftElem++)
		{
			...
			for(rightElem = right->elements; rightElem < right->elements + right->count; rightElem++)
			{
				...
				if(Math3D_SphVsSphOverlap(&leftElem->dim.worldSphere, &rightElem->dim.worldSphere, &overlap) == 1)
				{
					...
					CollisionCheck_SetOCvsOC(&left->base, &leftElem->info, &leftPos, &right->base, &rightElem->info, &rightPos, overlap);
				}
			}
		}
	}
		...
		if((right->base.ocFlags1 & OC1_ON) && (right->info.ocElemFlags & OCELEM_ON))
		{
			for(leftElem = left->elements; leftElem < left->elements + left->count; leftElem++)
			{
				...
				if(Math3D_SphVsCylOverlapDist(&leftElem->dim.worldSphere, &right->dim, &overlap) == 1)
				{
					...
					CollisionCheck_SetOCvsOC(&left->base, &leftElem->info, &leftPos, &right->base, &right->info, &rightPos, overlap);
				}
			}
		}
	}
	...
	if((left->base.ocFlags1 & OC1_ON) && (right->base.ocFlags1 & OC1_ON))
	{
		if((left->info.ocElemFlags & OCELEM_ON) && (right->info.ocElemFlags & OCELEM_ON))
		{
			if(Math3D_CylOutsideCyl(&left->dim, &right->dim, &deadSpace) == 1)
			{
				...
				Math_Vec3s_ToVec3f(&rightPos, &right->dim.pos);
				CollisionCheck_SetOCvsOC(&left->base, &left->info, &leftPos, &right->base, &right->info, &rightPos, deadSpace);
			}
		}
	}
}
...
{
	if(!(left->ocFlags1 & right->ocFlags2 & OC1_TYPE_ALL) || !(left->ocFlags2 & right->ocFlags1 & OC1_TYPE_ALL) || ((left->ocFlags2 & OC2_UNK1) && (right->ocFlags2 & OC2_UNK2)) || ((right->ocFlags2 & OC2_UNK1) && (left->ocFlags2 & OC2_UNK2)))
	{
		return 1;
	}
...
void CollisionCheck_OC(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx)
{
	...
	for(left = colChkCtx->colOC; left < colChkCtx->colOC + colChkCtx->colOCCount; left++)
	{
		...
		{
			if(*right == NULL || CollisionCheck_SkipOC(*right) == 1 || CollisionCheck_Incompatible(*left, *right) == 1)
			{
				continue;
			}
...
void Collider_UpdateSpheres(s32 limb, ColliderJntSph* collider)
{
	...
	for(i = 0; i < collider->count; i++)
	{
		if(limb == collider->elements[i].dim.limb)
		{
			...
			collider->elements[i].dim.worldSphere.radius = collider->elements[i].dim.modelSphere.radius * collider->elements[i].dim.scale;
		}
	}
}
...
void CollisionCheck_SpawnShieldParticles(GlobalContext* globalCtx, Vec3f* v)
{
	static EffectShieldParticleInit initMetal = {
	    16, {0, 0, 0}, {0, 200, 255, 255}, {255, 255, 255, 255}, {255, 255, 128, 255}, {255, 255, 0, 255}, {255, 64, 0, 200}, {255, 0, 0, 255}, 2.1f, 35.0f, 30.0f, 8, {0, 0, 0, {0, 128, 255}, false, 300}, true,
	};
	s32 effectIndex;
...
void CollisionCheck_SpawnShieldParticlesWood(GlobalContext* globalCtx, Vec3f* v, Vec3f* actorPos)
{
	static EffectShieldParticleInit initWood = {
	    16, {0, 0, 0}, {0, 200, 255, 255}, {255, 255, 255, 255}, {255, 255, 128, 255}, {255, 255, 0, 255}, {255, 64, 0, 200}, {255, 0, 0, 255}, 2.1f, 35.0f, 30.0f, 8, {0, 0, 0, {0, 128, 255}, false, 300}, false,
	};
	s32 effectIndex;
