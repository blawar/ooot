...
{
	sCurrentMatrix = (MtxF*)GameState_Alloc(gameState, 20 * sizeof(MtxF), "../sys_matrix.c", 153);
...
template <class T> static inline void _func_800D1694(f32 x, f32 y, f32 z, T* vec)
{
	MtxF* cmf = sCurrentMatrix;
 ...
 */
template <class T> static inline void _Matrix_MtxFToYXZRotS(MtxF* mf, T* rotDest, s32 flag)
{
	f32 temp;
	f32 temp2;
	f32 temp3;
 ...
 * The flag value doesn't matter for a rotation matrix. Not 0 does extra calculation.
 */
template <class T> static inline void _Matrix_MtxFToZYXRotS(MtxF* mf, T* rotDest, s32 flag)
{
	f32 temp;
	f32 temp2;
	f32 temp3;
