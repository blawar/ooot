...
s32 EffectSs_FindSlot(s32 priority, s32* pIndex)
{
	...
	while(true)
	{
		...
		if((priority <= sEffectSsInfo.table[i].priority) && !((priority == sEffectSsInfo.table[i].priority) && (sEffectSsInfo.table[i].flags & 1)))
		{
			break;
		}
...
void EffectSs_Spawn(GlobalContext* globalCtx, s32 type, s32 priority, void* initParams)
{
	...
	else
	{
		if(overlayEntry->loadedRamAddr == NULL)
		{
			...
			osSyncPrintf("EFFECT SS OVL:SegRom %08x %08x, Seg %08x %08x, RamStart %08x, type: %d\n", overlayEntry->vromStart, overlayEntry->vromEnd, overlayEntry->vramStart, overlayEntry->vramEnd, overlayEntry->loadedRamAddr, type);
			osSyncPrintf(VT_RST);
		}
...
void EffectSs_DrawAll(GlobalContext* globalCtx)
{
	...
	for(i = 0; i < sEffectSsInfo.tableSize; i++)
	{
		if(sEffectSsInfo.table[i].life > -1)
		{
			if((sEffectSsInfo.table[i].pos.x > 32000.0f) || (sEffectSsInfo.table[i].pos.x < -32000.0f) || (sEffectSsInfo.table[i].pos.y > 32000.0f) || (sEffectSsInfo.table[i].pos.y < -32000.0f) || (sEffectSsInfo.table[i].pos.z > 32000.0f) ||
