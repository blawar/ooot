...
s32 func_800AAA9C(View* view)
{
	...
	projection = (Mtx*)Graph_Alloc(gfxCtx, sizeof(Mtx));
	...
	if(QREG(88) & 1)
	{
		...
		osSyncPrintf("fovy %f near %f far %f scale %f aspect %f normal %08x\n", view->fovy, view->zNear, view->zFar, view->scale, aspect, view->normal);
	...
	viewing = (Mtx*)Graph_Alloc(gfxCtx, sizeof(Mtx));
	...
	guLookAt(viewing, view->eye.x, view->eye.y, view->eye.z, view->lookAt.x, view->lookAt.y, view->lookAt.z, view->up.x, view->up.y, view->up.z);
...
s32 func_800AB0A8(View* view)
{
	...
	guOrtho(projection, -(f32)gScreenWidth * 0.5f, (f32)gScreenWidth * 0.5f, -(f32)gScreenHeight * 0.5f, (f32)gScreenHeight * 0.5f, view->zNear, view->zFar, view->scale);
...
s32 func_800AB2C4(View* view)
{
	...
	gDPSetScissor(OVERLAY_DISP++, G_SC_NON_INTERLACE, view->viewport.leftX, view->viewport.topY, view->viewport.rightX, view->viewport.bottomY);
	gSPViewport(OVERLAY_DISP++, vp);
...
s32 func_800AB944(View* view)
{
	...
	func_800ABE74(view->eye.x, view->eye.y, view->eye.z);
	guLookAt(view->viewingPtr, view->eye.x, view->eye.y, view->eye.z, view->lookAt.x, view->lookAt.y, view->lookAt.z, view->up.x, view->up.y, view->up.z);
...
s32 func_800AB9EC(View* view, s32 arg1, Gfx** gfxp)
{
	...
	if(arg1 & 2)
	{
		...
		gDPPipeSync(gfx++);
		gDPSetScissor(gfx++, G_SC_NON_INTERLACE, view->viewport.leftX, view->viewport.topY, view->viewport.rightX, view->viewport.bottomY);
		gSPViewport(gfx++, vp);
	}
	...
	else if(arg1 & 6)
	{
		...
		guPerspective(projection, &view->normal, view->fovy, (f32)width / (f32)height, view->zNear, view->zFar, view->scale);
