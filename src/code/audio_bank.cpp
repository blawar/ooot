...
u8 sBankSizes[ARRAY_COUNT(gSoundBanks)] = {
    ARRAY_COUNT(gSoundBankPlayer), ARRAY_COUNT(gSoundBankItem), ARRAY_COUNT(gSoundBankEnvironment), ARRAY_COUNT(gSoundBankEnemy), ARRAY_COUNT(gSoundBankSystem), ARRAY_COUNT(gSoundBankOcarina), ARRAY_COUNT(gSoundBankVoice),
};
...
void Audio_ProcessSoundRequest(void)
{
	...
	while(index != 0xFF && index != 0)
	{
		...
		{
			if((gSoundParams[SFX_BANK_SHIFT(req->sfxId)][SFX_INDEX(req->sfxId)].params & 0x20) && gSoundParams[SFX_BANK_SHIFT(req->sfxId)][SFX_INDEX(req->sfxId)].importance == gSoundBanks[bankId][index].sfxImportance)
			{
				return;
			}
		...
		else if(gSoundBanks[bankId][entryIndex].state != SFX_STATE_EMPTY)
		{
			...
			else
			{
				if(entry->dist > 0x7FFFFFD0)
				{
					...
					osSyncPrintf(D_80133344, entry->sfxId, entry->posX, entry->posZ, *entry->posX, *entry->posY, *entry->posZ);
				}
	...
	for(i = 0; i < gChannelsPerBank[gSfxChannelLayout][bankId]; i++)
	{
		...
		if(entryIndex != 0xFF)
		{
			...
			if(entry->state == SFX_STATE_READY)
			{
				...
				Audio_QueueCmdS8(CHAN_UPD_SCRIPT_IO << 24 | SEQ_PLAYER_SFX << 16 | ((sCurSfxPlayerChannelIdx & 0xFF) << 8) | 4, entry->sfxId & 0xFF);
				if(gIsLargeSoundBank[bankId])
				{
					Audio_QueueCmdS8(CHAN_UPD_SCRIPT_IO << 24 | SEQ_PLAYER_SFX << 16 | ((sCurSfxPlayerChannelIdx & 0xFF) << 8) | 5, (entry->sfxId & 0x100) >> 8);
				}
				if(entry->sfxId & 0xC00)
				{
