...
void Cutscene_DrawDebugInfo(GlobalContext* globalCtx, Gfx** dlist, CutsceneContext* csCtx)
{
	...
	GfxPrint_Printf(&printer, "%06d", csCtx->frames.whole());
...
void Cinema_Update(GlobalContext* globalCtx, CutsceneContext* csCtx)
{
	...
	if(CHECK_BTN_ALL(input->press.button, BTN_DUP) && (csCtx->state == CS_STATE_IDLE) && (gSaveContext.sceneSetupIndex >= 4) && !gDbgCamEnabled)
	{
		D_8015FCC8 = 1;
		gSaveContext.cutsceneIndex = 0xFFFD;
		gSaveContext.cutsceneTrigger = 1;
...
void func_80064824(GlobalContext* globalCtx, CutsceneContext* csCtx, CsCmdBase* cmd)
{
	...
	switch(cmd->base)
	{
		...
		case 15:
			if(sp3F != 0)
			{
				TitleCard_InitPlaceName(globalCtx, &globalCtx->actorCtx.titleCtx, player->giObjectSegment, 160, 120, 144, 24, 20);
			}
			break;
		case 16:
			if(sp3F != 0)
	...
	if((csCtx->frames == cmd->startFrame) || (temp != 0) || ((csCtx->frames > 20) && CHECK_BTN_ALL(globalCtx->input[0].press.button, BTN_START) && (gSaveContext.fileNum != 0xFEDC)))
	{
		...
		switch(cmd->base)
		{
			...
			case 94: // Shadow Temple with a door to Water Temple
				...
				break;
			case 95: // doesnt work
				if((gSaveContext.eventChkInf[4] & 0x100) && (gSaveContext.eventChkInf[4] & 0x200) && (gSaveContext.eventChkInf[4] & 0x400))
				{
					globalCtx->nextEntranceIndex = 0x0053;
					globalCtx->sceneLoadFlag = 0x14;
					gSaveContext.cutsceneIndex = 0xFFF3;
			...
			case 113: // Riding Epona with Title Screen
				if(Flags_GetEventChkInf(0xBB) && Flags_GetEventChkInf(0xBC) && Flags_GetEventChkInf(0xBD) && Flags_GetEventChkInf(0xBE) && Flags_GetEventChkInf(0xBF) && Flags_GetEventChkInf(0xAD))
				{
		...
		switch(cmd->base)
		{
			...
			case 5:
				...
				if(cmd->base == 1)
				{
					...
					{
						Audio_PlaySoundGeneral(NA_SE_SY_WHITE_OUT_S, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
					}
					else if((temp == 0.0f) && ((gSaveContext.entranceIndex == 0x0053) || (gSaveContext.entranceIndex == 0x0138) || (gSaveContext.entranceIndex == 0x0371)))
					{
						Audio_PlaySoundGeneral(NA_SE_EV_WHITE_OUT, &gAudioDefaultPos, 4, &D_801333E0, &D_801333E0, &gReverbAdd2);
					}
...
s32 Cutscene_Command_CameraPositions(GlobalContext* globalCtx, CutsceneContext* csCtx, u8* cmd, u8 relativeToLink)
{
	...
	if((cmdBase->startFrame < csCtx->frames) && (csCtx->frames < cmdBase->endFrame) && ((csCtx->unk_18 < cmdBase->startFrame) || (csCtx->unk_18 >= 0xF000)))
	{
		csCtx->unk_1B = 1;
		...
		if(csCtx->unk_1A != 0)
		{
			...
			if(D_8015FCC8 != 0)
			{
				Gameplay_CameraChangeSetting(globalCtx, csCtx->cutsceneCamera, CAM_SET_CS_0);
				Gameplay_ChangeCameraStatus(globalCtx, gActiveCameraIndex, CAM_STAT_WAIT);
				Gameplay_ChangeCameraStatus(globalCtx, csCtx->cutsceneCamera, CAM_STAT_ACTIVE);
				Camera_ResetAnim(Gameplay_GetCamera(globalCtx, csCtx->cutsceneCamera));
				Camera_SetCSParams(Gameplay_GetCamera(globalCtx, csCtx->cutsceneCamera), csCtx->cameraFocus, csCtx->cameraPosition, GET_PLAYER(globalCtx), relativeToLink);
			}
		}
	}
...
s32 Cutscene_Command_CameraFocus(GlobalContext* globalCtx, CutsceneContext* csCtx, u8* cmd, u8 relativeToLink)
{
	...
	if((cmdBase->startFrame < csCtx->frames) && (csCtx->frames < cmdBase->endFrame) && ((D_8015FCC0 < cmdBase->startFrame) || (D_8015FCC0 >= 0xF000)))
	{
		csCtx->unk_1A = 1;
...
s32 Cutscene_Command_07(GlobalContext* globalCtx, CutsceneContext* csCtx, u8* cmd, u8 unused)
{
	...
	if((cmdBase->startFrame < csCtx->frames) && (csCtx->frames < cmdBase->endFrame) && ((D_8015FCC2 < cmdBase->startFrame) || (D_8015FCC2 >= 0xF000)))
	{
		csCtx->unk_1B = 1;
		csCtx->cameraPosition = (CutsceneCameraPoint*)cmd;
		if(csCtx->unk_1A != 0)
		{
			...
			if(D_8015FCC8 != 0)
			{
				sp2C = Gameplay_GetCamera(globalCtx, csCtx->cutsceneCamera);
				sp2C->player = NULL;
				Gameplay_ChangeCameraStatus(globalCtx, MAIN_CAM, CAM_STAT_WAIT);
				Gameplay_ChangeCameraStatus(globalCtx, csCtx->cutsceneCamera, CAM_STAT_ACTIVE);
				Gameplay_CameraChangeSetting(globalCtx, csCtx->cutsceneCamera, CAM_SET_FREE0);
				...
				Gameplay_CameraSetAtEye(globalCtx, csCtx->cutsceneCamera, &sp3C, &sp30);
				Gameplay_CameraSetFov(globalCtx, csCtx->cutsceneCamera, csCtx->cameraPosition->viewAngle);
			}
		}
	}
...
s32 Cutscene_Command_08(GlobalContext* globalCtx, CutsceneContext* csCtx, u8* cmd, u8 unused)
{
	...
	if((cmdBase->startFrame < csCtx->frames) && (csCtx->frames < cmdBase->endFrame) && ((D_8015FCC4 < cmdBase->startFrame) || (D_8015FCC4 >= 0xF000)))
	{
		csCtx->unk_1A = 1;
		csCtx->cameraFocus = (CutsceneCameraPoint*)cmd;
		if(csCtx->unk_1B != 0)
...
void Cutscene_Command_Textbox(GlobalContext* globalCtx, CutsceneContext* csCtx, CsCmdTextbox* cmd)
{
	...
	if((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame))
	{
		...
		if(csCtx->frames >= cmd->endFrame)
		{
			...
			if((dialogState != TEXT_STATE_CLOSING) && (dialogState != TEXT_STATE_NONE) && (dialogState != TEXT_STATE_SONG_DEMO_DONE) && (dialogState != TEXT_STATE_8))
			{
				csCtx->frames--;
	...
	if(func_8006472C(globalCtx, csCtx, 0.0f))
	{
		...
		if(D_8015FCC8 != 0)
		{
			switch(gSaveContext.entranceIndex)
			{
				...
				case 0x0292:
				case 0x0476:
					Gameplay_CopyCamera(globalCtx, gActiveCameraIndex, csCtx->cutsceneCamera);
			}

			Gameplay_ChangeCameraStatus(globalCtx, gActiveCameraIndex, CAM_STAT_ACTIVE);
			Gameplay_ClearCamera(globalCtx, csCtx->cutsceneCamera);
...
void Demo_Update(GlobalContext* globalCtx, CutsceneContext* csCtx)
{
	...
	if((gSaveContext.cutsceneIndex >= 0xFFF0) && (csCtx->state == CS_STATE_IDLE))
	{
		...
		if(csCtx->state == CS_STATE_SKIPPABLE_INIT)
		{
			...
			if(D_8015FCC8 != 0)
			{
				csCtx->cutsceneCamera = Gameplay_CreateSubCamera(globalCtx);
			}
...
void Cutscene_HandleEntranceTriggers(GlobalContext* globalCtx)
{
	...
	for(i = 0; i < ARRAY_COUNT(sEntranceCutsceneTable); i++)
	{
		...
		if((gSaveContext.entranceIndex == entranceCutscene->entrance) && (!Flags_GetEventChkInf(entranceCutscene->flag) || (entranceCutscene->flag == 0x18)) && (gSaveContext.cutsceneIndex < 0xFFF0) && ((u8)gSaveContext.linkAge == requiredAge) &&
...
void Cutscene_HandleConditionalTriggers(GlobalContext* globalCtx)
{
	...
	if((gSaveContext.gameMode == 0) && (gSaveContext.respawnFlag <= 0) && (gSaveContext.cutsceneIndex < 0xFFF0))
	{
		...
		else if((gSaveContext.entranceIndex == 0x00DB) && LINK_IS_ADULT && (gSaveContext.eventChkInf[4] & 0x0100) && (gSaveContext.eventChkInf[4] & 0x0200) && (gSaveContext.eventChkInf[4] & 0x0400) && !Flags_GetEventChkInf(0xAA))
		{
		...
		else if(CHECK_QUEST_ITEM(QUEST_MEDALLION_SPIRIT) && CHECK_QUEST_ITEM(QUEST_MEDALLION_SHADOW) && LINK_IS_ADULT && !Flags_GetEventChkInf(0xC4) && (gEntranceTable[((void)0, gSaveContext.entranceIndex)].scene == SCENE_TOKINOMA))
		{
			Flags_SetEventChkInf(0xC4);
			gSaveContext.entranceIndex = 0x0053;
			gSaveContext.cutsceneIndex = 0xFFF8;
		}
		else if(!Flags_GetEventChkInf(0xC7) && (gEntranceTable[((void)0, gSaveContext.entranceIndex)].scene == SCENE_GANON_DEMO))
		{
