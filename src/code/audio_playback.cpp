...
void Audio_NoteInit(Note* note)
{
	...
	{
		Audio_AdsrInit(&note->playbackState.adsr, note->playbackState.parentLayer->channel->adsr.envelope, &note->playbackState.adsrVolScaleUnused);
	}
	else
	{
		Audio_AdsrInit(&note->playbackState.adsr, note->playbackState.parentLayer->adsr.envelope, &note->playbackState.adsrVolScaleUnused);
	}
...
void Audio_ProcessNotes(void)
{
	...
	for(i = 0; i < gAudioContext.numNotes; i++)
	{
		...
		if(playbackState->parentLayer != NO_LAYER)
		{
			...
			else if(!playbackState->parentLayer->enabled && playbackState->unk_04 == 0 && playbackState->priority >= 1)
			{
				// do nothing
			}
			...
			else if(playbackState->parentLayer->channel->seqPlayer->muted && (playbackState->parentLayer->channel->muteBehavior & 0x40))
			{
				// do nothing
			}
			else
...
void Audio_SeqLayerDecayRelease(SequenceLayer* layer, s32 target)
{
	...
	{
		if(note->playbackState.parentLayer == NO_LAYER && note->playbackState.wantedParentLayer == NO_LAYER && note->playbackState.prevParentLayer == layer && target != ADSR_STATE_DECAY)
		{
	...
	if(note->playbackState.adsr.action.s.state != ADSR_STATE_DECAY)
	{
		...
		else
		{
			...
			note->playbackState.adsr.sustain = ((f32)(s32)(layer->channel->adsr.sustain) * note->playbackState.adsr.current) / 256.0f;
		}
	}
...
Note* Audio_AllocNote(SequenceLayer* layer)
{
	...
	{
		ret = layer->note;
		if(ret != NULL && ret->playbackState.prevParentLayer == layer && ret->playbackState.wantedParentLayer == NO_LAYER)
		{
	...
	if(policy & 2)
	{
		if(!(ret = Audio_AllocNoteFromDisabled(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromDecaying(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromActive(&layer->channel->notePool, layer)))
		{
			goto null_return;
		}
		return ret;
	...
	if(policy & 4)
	{
		if(!(ret = Audio_AllocNoteFromDisabled(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromDisabled(&layer->channel->seqPlayer->notePool, layer)) && !(ret = Audio_AllocNoteFromDecaying(&layer->channel->notePool, layer)) &&
		   !(ret = Audio_AllocNoteFromDecaying(&layer->channel->seqPlayer->notePool, layer)) && !(ret = Audio_AllocNoteFromActive(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromActive(&layer->channel->seqPlayer->notePool, layer)))
		{
			goto null_return;
		}
		return ret;
	...
	if(policy & 8)
	{
		if(!(ret = Audio_AllocNoteFromDisabled(&gAudioContext.noteFreeLists, layer)) && !(ret = Audio_AllocNoteFromDecaying(&gAudioContext.noteFreeLists, layer)) && !(ret = Audio_AllocNoteFromActive(&gAudioContext.noteFreeLists, layer)))
		{
			goto null_return;
		}
		return ret;
	...
	if(!(ret = Audio_AllocNoteFromDisabled(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromDisabled(&layer->channel->seqPlayer->notePool, layer)) && !(ret = Audio_AllocNoteFromDisabled(&gAudioContext.noteFreeLists, layer)) &&
	   !(ret = Audio_AllocNoteFromDecaying(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromDecaying(&layer->channel->seqPlayer->notePool, layer)) && !(ret = Audio_AllocNoteFromDecaying(&gAudioContext.noteFreeLists, layer)) &&
	   !(ret = Audio_AllocNoteFromActive(&layer->channel->notePool, layer)) && !(ret = Audio_AllocNoteFromActive(&layer->channel->seqPlayer->notePool, layer)) && !(ret = Audio_AllocNoteFromActive(&gAudioContext.noteFreeLists, layer)))
	{
		goto null_return;
	}
	return ret;
