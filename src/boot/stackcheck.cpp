...
void StackCheck_Init(StackEntry* entry, void* stackTop, void* stackBottom, u32 initValue, s32 minSpace,
    ...
    if (entry == NULL) {
        ...
        while (iter) {
            if (iter == entry) {
                osSyncPrintf(VT_COL(RED, WHITE) "stackcheck_init: %08x is already listed\n" VT_RST, entry);
                return;
            }
            iter = iter->next;
        }
...
void StackCheck_Cleanup(StackEntry* entry) {
    ...
    if (inconsistency) {
        osSyncPrintf(VT_COL(RED, WHITE) "stackcheck_cleanup: %08x does not conform to list\n" VT_RST, entry);
    }
}
